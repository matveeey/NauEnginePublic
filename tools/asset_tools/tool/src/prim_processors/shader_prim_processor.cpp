...
#include "shader_prim_processor.h"
...
namespace nau
{
    ...
    {
        bool ShaderPrimProcessor::canProcess(const nau::UsdMetaInfo& metaInfo) const
        {
            return metaInfo.type == getType();
        }

        nau::Result<AssetMetaInfo> ShaderPrimProcessor::process(PXR_NS::UsdStageRefPtr stage, const std::string& outputPath, const std::string& projectRootPath, const nau::UsdMetaInfo& metaInfo, int folderIndex)
        {
            FileSystem fs;

            auto extraData = reinterpret_cast<ExtraInfoShader*>(metaInfo.extraInfo.get());
