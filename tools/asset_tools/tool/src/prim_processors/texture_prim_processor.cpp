...
#include "texture_prim_processor.h"
...
namespace nau
{
    ...
    {
        bool TexturePrimProcessor::canProcess(const nau::UsdMetaInfo& metaInfo) const
        {
            return metaInfo.type == getType();
        }

        nau::Result<AssetMetaInfo> TexturePrimProcessor::process(PXR_NS::UsdStageRefPtr stage, const std::string& outputPath, const std::string& projectRootPath, const nau::UsdMetaInfo& metaInfo, int folderIndex)
        {
            auto extraData = reinterpret_cast<ExtraInfoTexture*>(metaInfo.extraInfo.get());
