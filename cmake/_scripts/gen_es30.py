...
#     genCode += indent2+'  return;\n'
    ...
    if esFunction.hasComponents:
      preForIndent = indent2
      genCode += indent2 + 'auto comp = components.begin(), compE = components.end(); NAU_ASSERT(comp!=compE);\n' + indent2 + 'do\n'
...
#  print ("Warning: currently ECS codegen supports only one event handler, because Events are untyped!")
  ...
  for index in range(0, len(esFunction.eventHandlers)):
    ...
    if (eventType != 'ecs::Event'):
      if len(esFunction.eventHandlers) != 1:
        ...
        genCode += '  NAU_FAST_ASSERT(evt.is<{eventType}>());\n'.format(**locals())
    else:
    ...
    if len(esFunction.eventHandlers[index].condition):
      ...
      genCode += '''  {indent}auto comp = components.begin(), compE = components.end(); NAU_ASSERT(comp!=compE); do
  {indent}{{
    {indent}if ( !({condition}) )
      {indent}continue;
  ...
  if len(esFunction.eventHandlers) != 1:
    genCode += '  }} else {{NAU_ASSERT(0, "Unexpected event type <\{\}> in {funcName}", evt.getName());}}\n'.format(funcName=funcName) if (not hasGenericEvent) else '}\n'
  genCode += '}\n'
  return genCode
...
def gen_ecs_query_simd(esFunction):
  ...
  loop_expr = 'auto comp = components.begin(), compE = components.end(); NAU_ASSERT(comp != compE); do' if not esFunction.is_eid_query else 'constexpr size_t comp = 0;'
...
def gen_es(allParsedFunctions, event_suffix, input_file_name, path_to_meta_file):
  global event_handler_suffix
...
#
  ...
  for i in range(0, len(allESFunctions)):
    ...
    metaESdata += f'system;{allESFunctions[i].funcName};'
    metaESdata += f'{"+".join(allESFunctions[i].eventList + allESFunctions[i].stagesTypes)};'
    metaESdata += f'{"+".join(allESFunctions[i].annotatedTracked)};'
    metaESdata += f'{"+".join(allESFunctions[i].annotatedBefore)};'
    metaESdata += f'{"+".join(allESFunctions[i].annotatedAfter)};'
    for param in  allESFunctions[i].rw_params + \
               allESFunctions[i].ro_params + \
               allESFunctions[i].rq_params + \
               allESFunctions[i].no_params:
      metaESdata += f'{param["type"]};' + \
                    f'{param["name"]};' + \
                    f'{param["param_type"]};' + \
                    f'{param["optional"] if param["optional"] else ""};'
    metaESdata += "\n"
  ...
  for i in range(0, len(allQueryFunctions)):
    ...
    metaESdata += f'query;{allQueryFunctions[i].funcName};;;;;'
    for param in  allQueryFunctions[i].rw_params + \
                  allQueryFunctions[i].ro_params + \
                  allQueryFunctions[i].rq_params + \
                  allQueryFunctions[i].no_params:
      metaESdata += f'{param["type"]};' + \
                f'{param["name"]};' + \
                f'{param["param_type"]};' + \
                f'{param["optional"] if param["optional"] else ""};'
  ...
  if False:
    if(os.path.isfile(path_to_meta_file)):
      f = open(path_to_meta_file, "a")
    else:
      f = open(path_to_meta_file, "w")
      f.write(f'ES type(system/query); ES name; Events to trigger ES; Tracked by ES components; ES before systems for proper ordering; ES after systems for proper ordering;' )
      for i in range(50):
        f.write(f'Component type;Component name;Component access type;Default value;')
      f.write("\n")
