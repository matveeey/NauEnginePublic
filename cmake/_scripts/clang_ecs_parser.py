...
clang_dir = None
if ((clang_executable != None) and
    sys.platform.startswith('win') and
    (len(Path(clang_executable).resolve().parents) > 0)):
    clang_dir = Path(clang_executable).resolve().parents[0]
    pass
...
def parse_ecs_functions(input_filename, search_filename, clang_args, should_parse_name, skipFunctionBodies, compiler_errors, all_gets):
    ...
    if clang_dir != None:
      clang.cindex.conf.set_library_path(clang_dir)
    ...
    index = Index.create()
    sysroot = '-isysroot/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX14.2.sdk' if sys.platform == 'darwin' else ''
    clang_args = ['-x', 'c++', input_filename, sysroot] + clang_args
    # print clang_args
