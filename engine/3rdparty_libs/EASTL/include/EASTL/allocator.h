...
namespace eastl
{
	...
	public:
		...
		void* realloc(void* p, size_t n, int flags = 0);
	...
	namespace eastl
	{
		...
		inline void* allocator::realloc(void* p, size_t n, int flags)
		{
			void* outPtr = nullptr;
			#if EASTL_DLL
				outPtr = allocate(n, EASTL_SYSTEM_ALLOCATOR_MIN_ALIGNMENT, 0, flags);
			#elif (EASTL_DEBUGPARAMS_LEVEL <= 0)
				outPtr = ::new((char*)0, flags, 0, (char*)0, 0) char[n];
			#elif (EASTL_DEBUGPARAMS_LEVEL == 1)
				outPtr = ::new(pName, flags, 0, (char*)0, 0) char[n];
			#else
				outPtr = ::new(pName, flags, 0, __FILE__, __LINE__) char[n];
			#endif
...
namespace eastl
{
	...
	inline void* allocate_memory(Allocator& a, size_t n, size_t alignment, size_t alignmentOffset)
	{
		...
		if (alignment <= EASTL_ALLOCATOR_MIN_ALIGNMENT)
		{
			...
			//EASTL_ASSERT((reinterpret_cast<size_t>(result)& ~(alignment - 1)) == reinterpret_cast<size_t>(result)); // TODO: uncomment this line after alignment would be implemented for our allocators
		}
		else
		{
