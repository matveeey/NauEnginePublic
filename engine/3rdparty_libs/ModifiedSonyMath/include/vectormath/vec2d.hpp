...
// Compute the dot product of two 2-D points
//
inline float dot(const Point2 & vec0, const Point2 & vec1);
...
public:
    ...
    //
    inline Point2() { } //-V730
    inline Point2(const Point2&) = default;
    inline Point2(Point2&&) = default;
    ...
    //
    template <class T>
        requires std::is_constructible_v<float, T>
    inline Point2(T&& x, T&& y): mX(float(x)), mY(float(y)) {};
    ...
    // Multiply a 2-D point to a 2-D vector
    //
    inline const Point2 operator * (const float & scale) const;
...
inline const Point2 Point2::operator * (const float & scale) const
{
    return Point2((mX * scale), (mY * scale));
}
