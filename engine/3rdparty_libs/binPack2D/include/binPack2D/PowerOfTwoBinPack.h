...
class NAU_BINPACK_EXPORT PowerOfTwoBinPack
{
public:
  ...
  {
    int logSize = nau::math::get_bigger_log2(width);
    Rect rect = insertLogSize(logSize);//width = 1<<logSize;
    NAU_ASSERT((rect.x&((1<<logSize)-1)) == 0 && (rect.y&((1<<logSize)-1)) == 0, "{}x{} {{}", rect.x, rect.y, rect.width);
