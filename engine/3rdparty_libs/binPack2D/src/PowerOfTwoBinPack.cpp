#include <binPack2d/PowerOfTwoBinPack.h>
...
{
  rbp_assert(nau::math::is_pow_of2(maxBucket));
  int bigger = nau::math::get_bigger_pow2(maxBucket);
  if (bigger>maxBucket)
    maxBucket = bigger>>1;
  int logSize = nau::math::get_log2i(maxBucket);
  sizeBucket.resize(0);
...
void PowerOfTwoBinPack::freeUsedRect(const Rect &rect)
{
  ...
  rbp_assert(nau::math::is_pow_of2(rect.width));
  int logSize = nau::math::get_log2i(rect.width);
  rbp_assert((rect.x&((1<<logSize)-1)) == 0 && (rect.y&((1<<logSize)-1)) == 0);
