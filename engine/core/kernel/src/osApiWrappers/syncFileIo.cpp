...
namespace nau::hal
{
#if !defined(NAU_PLATFORM_WIN32) && !defined(_TARGET_XBOX)
    #define _fileno(f) fileno(f)
    #define _filelength(f) filelength(f)
#endif  // !NAU_PLATFORM_WIN32 && !_TARGET_XBOX
    ...
    dag::CriticalSection RomFileReader::critSec;

#if CHECK_FILE_CASE && NAU_PLATFORM_WIN32 && DAGOR_DBGLEVEL > 0
    #include <windows.h>
        ...
        if(h != INVALID_HANDLE_VALUE)
            if(GetFinalPathNameByHandleA(h, realname, DAGOR_MAX_PATH, 0))
            {
                ...
                if(!strstr(realname, fn))
                    NAU_FAILURE("trying to open {} as {}", realname, fn);
            }
    }
#else
    static void check_file_case(FILE* /*fp*/, const char* /*fn*/)
        ...
        else
        {
            switch(flags & DF_RDWR)
            {
                ...
                default:
                    {
                        NAU_LOG_ERROR(
                            "cannot open '{}', expected DF_READ or DF_WRITE or DF_RDWR or DF_APPEND "
                            "in flags",
                            filename);
                        return NULL;
                    }
    ...
    int df_close(file_ptr_t fp)
    {
        ...
        if(result != 0)
        {
            NAU_LOG_ERROR("fclose({:#x}) returns error {}. Probably invalid pointer, IO error (disk full?) or file is already closed", _fp, errno);
            G_UNUSED(_fp);
            return result;
        }
    ...
    */
    ...
    int df_stat(const char* path, DagorStat* buf)
    {
        ...
        */
        ...
        else if(iterate_base_paths_simplify(path, [&st](const char* fn)
        {
            return STAT(fn, &st) == 0;
        }))
            return sys_stat2dagor_stat(&st, buf);
