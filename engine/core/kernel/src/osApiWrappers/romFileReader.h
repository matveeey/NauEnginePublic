...
namespace nau::hal
{
    ...
    public:
        ...
        int seekRel(int ofs_rel)
        {
            return seekTo(pos + ofs_rel);
        }
        int seekEnd(int ofs_end)
        {
            return seekTo(size + ofs_end);
        }
    ...
    public:
        ...
        static int getBaseOfs(eastl::span<const char> p)
        {
            int ofs = ptrdiff_t(p.data());
            return ofs > 0 ? ofs : 0;
        }
        /* TODO: VROM integration
        static RomFileReader *open(eastl::span<const char> p, VirtualRomFsPack *pack, VirtualRomFsData *vrom_src)
        {
          int i;
          {
        ...
        */
...
#else
        static RomFileReader* openAsset(const char*)
        {
            return nullptr;
        }
        static int getAssetSize(const char*)
        {
            return -1;
        }
#endif
        ...
        const VirtualRomFsData* getSrcVrom() const
        {
            return srcVrom;
        }
    ...
    inline file_ptr_t to_file_ptr_t(RomFileReader* r)
    {
        return (file_ptr_t)(r ? uintptr_t(r) | 0x1 : 0);
    }
