...
#if NAU_PLATFORM_APPLE | NAU_PLATFORM_LINUX | _TARGET_PC_LINUX | _TARGET_ANDROID | _TARGET_C3
    #include <pthread.h>
    #include <sched.h>
#endif
...
namespace nau::hal
{
    ...
    const char* get_platform_string_id()
    {
        return get_platform_string_by_id(get_platform_id());
    }
    /*
    // Removed, shouldn't be here
    ...
    */
...
#elif(NAU_PLATFORM_WIN32 && !defined(STARFORCE_PROTECT)) || _TARGET_XBOX
    ...
    {
        NAU_ASSERT(dllName != NULL && dllName[0] != 0);
    ...
    bool detect_os_compatibility_mode(char* os_real_name, size_t os_real_name_size)
    {
        ...
        {
            NAU_ASSERT(os_real_name_size > 0);
            os_real_name[0] = 0;
        }
        ...
        if(get_dll_version("kernel32.dll", dllVersionMajor, dllVersionMinor))
        {
            ...
            if(get_version_ex(&osvi))
            {
                if(dllVersionMajor != osvi.dwMajorVersion || dllVersionMinor != osvi.dwMinorVersion)
                {
                    if(os_real_name != NULL)
                    {
                        int sprintf_result_len_ = snprintf(os_real_name, os_real_name_size, "Windows %s v%d.%d",
                                                           osvi.wProductType == VER_NT_WORKSTATION ? "Workstation" : "Server", (int)dllVersionMajor, (int)dllVersionMinor);
                        NAU_ASSERT((unsigned)sprintf_result_len_ < (unsigned)os_real_name_size);
...
#else
    bool detect_os_compatibility_mode(char*, size_t)
    {
        return false;
    }
#endif
    ...
    NAU_RENDER_EXPORT void init_main_thread_id()
    {
    ...
    bool is_main_thread()
    {
        return get_current_thread_id() == get_main_thread_id();
    }
