...
#include <nau/dag_ioSys/dag_msgIo.h>
...
namespace nau::iosys
{
    ...
    ThreadSafeMsgIo::ThreadSafeMsgIo(int buf_sz)
    {
        ...
        buffer = nau::getDefaultAllocator()->allocate(buf_sz);
    ...
    ThreadSafeMsgIo::~ThreadSafeMsgIo()
    {
        ...
        if(buffer)
        {
            ...
            nau::getDefaultAllocator()->deallocate(buffer);
        }
    }
    ...
    void SimpleBlockSave::beginBlock()
    {
        if((unsigned)blkUsed >= BLOCK_MAX)
            NAU_THROW(SaveException("too much nested blocks", tell()));
    ...
    void SimpleBlockSave::endBlock(unsigned block_flags)
    {
        ...
        NAU_ASSERT(block_flags == 0, "block_flags={:08x}", block_flags);  // 2 bits max
        if(blkUsed <= 0)
            NAU_THROW(SaveException("block not begun", tell()));
        // G_ANALYSIS_ASSUME((unsigned)blkUsed > 0);
        int o = tell();
        NAU_ASSERT(o >= blkOfs[blkUsed - 1] && !((o - blkOfs[blkUsed - 1]) & 0xFFFF0000), "o={:08x} blkOfs[]={:08x} len={:08x}", o,
    ...
    int SimpleBlockSave::getBlockLevel()
    {
        return blkUsed;
    }
    ...
    void SimpleBlockSave::write(const void* ptr, int size)
    {
        ...
        if(sz > limSize)
            NAU_THROW(SaveException("cant write full packet", tell()));
    ...
    void SimpleBlockSave::seekto(int abs_ofs)
    {
        if(abs_ofs < 0 || abs_ofs > limSize)
            NAU_THROW(SaveException("seek out of range", tell()));
    ...
    void SimpleBlockSave::seektoend(int)
    {
        NAU_THROW(SaveException("seek to end not supported", tell()));
    }
    ...
    int SimpleBlockLoad::beginBlock(unsigned* out_block_flags)
    {
        if(blkUsed >= BLOCK_MAX)
            NAU_THROW(LoadException("too much nested blocks", tell()));
        // G_ANALYSIS_ASSUME((unsigned)blkUsed < BLOCK_MAX);
    ...
    int SimpleBlockLoad::getBlockLevel()
    {
        return blkUsed;
    }
    ...
    {
        if(abs_ofs < 0 || abs_ofs > limSize)
            NAU_THROW(LoadException("seek out of range", tell()));
    ...
    void SimpleBlockLoad::seekrel(int rel_ofs)
    {
        SimpleBlockLoad::seekto(SimpleBlockLoad::tell() + rel_ofs);
    }
}  // namespace nau::iosys