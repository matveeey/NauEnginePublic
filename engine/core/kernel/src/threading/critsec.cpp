...
//TODO: move to platform dirrectory and split for different OS.
#include "critsec.h"
...
namespace dag
{
    void create_critical_section(void* p, const char* name)
    {
        ...
        static_assert(sizeof(csimpl::type) <= CRITICAL_SECTION_OBJECT_SIZE);
#if HAVE_WINAPI
        ...
        if(InitializeCriticalSectionEx(cc, CS_SPIN_COUNT, initFlags))
            return;
        NAU_FAILURE("InitializeCriticalSectionEx failed with {}", ::GetLastError());
    #else  // winxp compat
    ...
    {
        // ScopeLockProfiler<da_profiler::DescCritsec> lp; TODO Profiler support?
        // NAU_UNUSED(lp);
