...
namespace nau
{
    ...
    static bool namemap_loadCompact(DataBlockShared& nameMap, eastl::vector<int>& conv, class iosys::IGenLoad& crd)
    {
        ...
        if(num < 0 || num > (2 << 20) || (buckets_cnt & 0x3FFF) < 1 || !nau::math::is_pow_of2(buckets_cnt & 0x3FFF))
            return false;
        ...
        for(int i = 0; i < snum; i++)
        {
            ...
            nau::string tempStr = str;
            conv[oldNameId] = len ? nameMap.addNameId(reinterpret_cast<const char*>(tempStr.c_str()), len) : -1;
            bucketsNames[hash]++;
        }
    ...
    static bool string_loadCompact(iosys::IGenLoad& crd, DBNameMap& stringMap)
    {
        stringMap.reset();
        const bool load_le = true;
        ...
        stringMap.allocator.page_shift = nau::math::get_bigger_log2(page->used);
        stringMap.allocator.start_page_shift = stringMap.allocator.page_shift;
        stringMap.allocator.max_page_shift = std::max(stringMap.allocator.page_shift, stringMap.allocator.max_page_shift);
    ...
    bool DataBlock::doLoadFromStreamBBF3(iosys::IGenLoad& crd)
    {
        eastl::vector<int> conv;
        if(!namemap_loadCompact(*shared, conv, crd))
        {
            issue_error_load_failed(reinterpret_cast<const char*>(crd.getTargetName()), "incompatible namemap params in BBF\\3 format");
            return false;
        }
        DBNameMap stringMap;
        if(!string_loadCompact(crd, stringMap))
        {
            issue_error_load_failed(reinterpret_cast<const char*>(crd.getTargetName()), "bad strings table in BBF\\3 format");
            return false;
        }
        return loadFromStreamBBF3(crd, conv.begin(), conv.end(), stringMap);
    }

    bool DataBlock::loadFromStreamBBF3(iosys::IGenLoad& crd, const int* cnv, const int* cnvEnd, const DBNameMap& strings)
    {
        const bool load_le = true;
        ...
        if(paramsNum)
        {
            ...
            for(int i = 0; i < paramsNum; ++i, ++params)
            {
                ...
                if(newNameId < 0)
                {
                    issue_error_load_failed(reinterpret_cast<const char*>(crd.getTargetName()), "empty param name in BBF\\3 format");
                    return false;
                }
                p.nameId = newNameId;
                p.type = (hdr >> 24) & 0x7F;
            ...
            for(i = 0; i < paramsCount; ++i)
            {
                ...
                switch(p.type)
                {
                    case TYPE_STRING:
                        ...
                        {
                            nau::string s = strings.getStringDataUnsafe(s_idx);
                            p.v = insertNewString(reinterpret_cast<const char*>(s.c_str()), s.size());
                        }
                        else
                            return false;
                        break;
                    case TYPE_INT:
                        p.v = readInt32(crd, load_le);
                        break;
                    case TYPE_REAL:
                        p.v = memcpy_cast<uint32_t, float>(readReal32(crd, load_le));
                        break;
                    case TYPE_POINT3:
                    case TYPE_IPOINT3:
                    case TYPE_POINT2:
                    ...
                    case TYPE_E3DCOLOR:
                        {
                            ...
                            p.v = memcpy_cast<uint32_t, nau::math::E3DCOLOR>(nau::math::E3DCOLOR((c >> 24) & 0xFF, (c >> 16) & 0xFF, (c >> 8) & 0xFF, c & 0xFF));
                        }
                        break;
                    case TYPE_INT64:
                        p.v = complexParamsData;
            ...
            NAU_ASSERT(complexParamsData == complexParamsSize());
        }
