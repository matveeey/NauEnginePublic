...
namespace nau
{
    ...
    void DataBlock::createData()
    {
        ...
        if(shared)
        {
            ...
            void* ptr = shared->allocateData();
            data = new (ptr) DataBlockOwned;
        }
    }

    bool DataBlock::topMost() const
    {
        return shared && (nameIdAndFlags & IS_TOPMOST);
    }
    ...
    DataBlock::DataBlock(nau::IMemAllocator::Ptr allocator) :
        shared(new DataBlockShared)
    {
        if(!allocator)
        {
            allocator = nau::getDefaultAllocator();
        }
        nameIdAndFlags |= IS_TOPMOST;
    }
    DataBlock::DataBlock(const char* filename, nau::IMemAllocator::Ptr a) :
        DataBlock(a)
    {
        load(filename);
    }
    DataBlock::DataBlock(const DataBlock& b) :
        DataBlock()
    {
        if(!b.hasNoNameId())
    ...
    DataBlock::DataBlock(DataBlockShared* s, const char* nm) :
        shared(s)
    {
        createData();
    ...
    const DataBlock DataBlock::emptyBlock(nau::IMemAllocator::Ptr{});

    const DataBlock::Param* DataBlock::getParamsPtr() const
    {
    ...
    __forceinline T DataBlock::get(uint32_t param_idx, const T& def) const
    {
        ...
        const uint32_t& pv = getParamV(p);
        if(p.type != TypeOf<T>::type) [[unlikely]]
        {
    ...
    int DataBlock::insertParamAt(uint32_t at, uint32_t name_id, const string_t& v)
    {
        ...
        p.v = insertNewString(v ? v : "", v ? eastl::CharStrlen(v) : 0);
    ...
    T DataBlock::getParamString(int pv) const
    {
        ...
        return return_string<T>(rw ? reinterpret_cast<const char*>(shared->rw.allocator.getDataRawUnsafe(pv)) : roDataAt(pv));
    }

    uint32_t DataBlock::allocateNewString(string_t v, size_t vlen)
    {
        return shared->rw.allocator.addDataRaw(reinterpret_cast<const char*>(v), vlen + 1);
    }
    ...
    bool DataBlock::set(uint32_t param_idx, const int& v)
    {
        NAU_ASSERT(isOwned());
        Param& p = getParam<true>(param_idx);
        if(NAU_LIKELY(p.type == TYPE_INT))
            p.v = v;
        else if(NAU_LIKELY(p.type == TYPE_INT64))
    ...
    bool DataBlock::set(uint32_t param_idx, const int64_t& v)
    {
        NAU_ASSERT(isOwned());
        Param& p = getParam<true>(param_idx);
        if(NAU_LIKELY(p.type == TYPE_INT64))
            getRW<int64_t>(getUsedSize() + getParamV(p)) = v;
        else if(NAU_LIKELY(p.type == TYPE_INT))
        {
            if(NAU_LIKELY(int(v) == v))
                p.v = v;
            else
            {
    ...
    bool DataBlock::set(uint32_t param_idx, const string_t& v)
    {
        ...
        uint32_t& pv = getParamV(p);
        const size_t vlen = v ? eastl::CharStrlen(v) : 0;
        ...
        if(is_string_id_in_namemap(pv) || vlen > eastl::CharStrlen(curStr))  // if string id is namemap id or string cant fit 'inplace'
        {
            // add new string
...
#define TYPE_FUNCTION_CR(CppType, ApiName) TYPE_FUNCTION_3(CppType, const CppType&, ApiName)
    ...
    TYPE_FUNCTION_CR(nau::math::vec2, Point2)
    TYPE_FUNCTION_CR(nau::math::vec3, Point3)
    TYPE_FUNCTION_CR(nau::math::vec4, Point4)
    TYPE_FUNCTION_CR(nau::math::ivec2, IPoint2)
    TYPE_FUNCTION_CR(nau::math::ivec3, IPoint3)
    TYPE_FUNCTION_CR(nau::math::mat4, Tm)
#undef TYPE_FUNCTION
...
#undef TYPE_FUNCTION_3
    ...
    DataBlock* DataBlock::addNewBlock(const char* name)
    {
        ...
        NAU_ASSERT_RETURN(bCnt < maxBlocksCount, getBlock(bCnt - 1), "blockCount={} reaches limit of {} max blocks", bCnt, maxBlocksCount);
    ...
    void DataBlock::setFrom(const DataBlock* from, const char* src_fname)
    {
        ...
        if(src_fname)
        {
            NAU_ASSERT(topMost() && "setFrom() may set src filename only to root BLK");
    ...
    void DataBlock::appendParamsFrom(const DataBlock* blk)
    {
        blk->addParamsTo(*this);
    }
    ...
    DataBlock* DataBlock::getBlock(uint32_t i)
    {
        return isBlocksOwned() ? getBlockRW(i) : getBlockRO(i);
    }
    ...
    const char* DataBlock::roDataAt(uint32_t at) const
    {
        return reinterpret_cast<const char*>(shared->getUnsafe(at));
    }
    ...
    int DataBlock::addNameId(const char* name)
    {
        return name && shared ? shared->addNameId(name) : -1;
    }
    int DataBlock::getNameId(const char* name) const
    {
        return name && shared ? shared->getNameId(name) : -1;
    }
                ...
                if(sz > INPLACE_PARAM_SIZE)
                {
                    if(cParam->type == TYPE_STRING)
                    {
                        if(!is_string_id_in_namemap(cParam->v))  // otherwise it is interned already
                        {
                            ...
                            cParam->v = allocateNewString(paramData, eastl::CharStrlen(paramData));  // intern string
                        }
                    }
                    else
                    {
    ...
    struct DbUtils
    {
        ...
        __forceinline static int find(int name_id, int start_after, const DataBlock::Param* __restrict params, const DataBlock::Param* __restrict e)
        {
            start_after = Vectormath::max(start_after + 1, int(0));
    ...
    void DataBlock::shrink()
    {
        ...
        if(isOwned())
        {
            ...
            for(int i = 0, e = paramCount(); i < e; ++i, ++oldParams)
            {
                ...
                // NAU_CORE_DEBUG_LF("pid={} used={} ({}), params = {}", i, getUsedSize(), data->data.size(), paramCount());
                eastl::swap(newData, data->data);
                eastl::swap(newParamsCount, paramsCount);
            }
            eastl::swap(newData, data->data);
            eastl::swap(newParamsCount, paramsCount);
            // NAU_CORE_DEBUG_LF("2 getUsedSize()={} blocks = {}", getUsedSize(), blocks);
    ...
    size_t DataBlock::memUsed() const
    {
        return memUsed_() + (shared ? shared->roDataSize() + sizeof(DataBlockShared) : 0);
    }
    ...
    {
        NAU_ASSERT_RETURN(topMost() || hasNoNameId(), , "trying to reset child datablock: nameId={} topMost={}", getNameId(), topMost());
    ...
    {
        NAU_ASSERT(hasNoNameId() && "setSharedNameMapAndClearData() must be called for root datablock", );
        reset();
    ...
    bool DataBlock::operator==(const DataBlock& rhs) const
    {
        ...
        if(!!my_name != !!rhs_name || (my_name && strcmp(reinterpret_cast<const char*>(my_name), reinterpret_cast<const char*>(rhs_name)) != 0))  //-V575
            return false;
        ...
        {
            if(param1->type != param2->type || strcmp(reinterpret_cast<const char*>(getName(param1->nameId)), reinterpret_cast<const char*>(rhs.getName(param2->nameId))) != 0)  //-V575
                return false;

            bool eq = false;
            if(param1->type == TYPE_STRING)
                eq = (strcmp(reinterpret_cast<const char*>(getParamData(*param1)), reinterpret_cast<const char*>(rhs.getParamData(*param2))) == 0);
            else
...
#define INSTANCIATE_GET_BY_NAME_ID(CLS)                                                     \
    ...
    INSTANCIATE_GET_BY_NAME_ID(nau::math::E3DCOLOR);
    INSTANCIATE_GET_BY_NAME_ID(float);
    INSTANCIATE_GET_BY_NAME_ID(DataBlock::string_t);
    INSTANCIATE_GET_BY_NAME_ID(nau::math::vec2);
    INSTANCIATE_GET_BY_NAME_ID(nau::math::vec3);
    INSTANCIATE_GET_BY_NAME_ID(nau::math::vec4);
    INSTANCIATE_GET_BY_NAME_ID(nau::math::ivec2);
    INSTANCIATE_GET_BY_NAME_ID(nau::math::ivec3);
    INSTANCIATE_GET_BY_NAME_ID(nau::math::mat4);
