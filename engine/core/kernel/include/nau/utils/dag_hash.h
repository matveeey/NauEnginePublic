...
template <int HashBits>
constexpr HashVal<HashBits> str_hash_fnv1(const char8_t *s, HashVal<HashBits> result = FNV1Params<HashBits>::offset_basis)
{
  HashVal<HashBits> c = 0;
  while ((c = *s++) != 0)
    result = (result * FNV1Params<HashBits>::prime) ^ c;
...
template <int HashBits>
constexpr HashVal<HashBits> mem_hash_fnv1(const char8_t *b, size_t len, HashVal<HashBits> result = FNV1Params<HashBits>::offset_basis)
{
  for (size_t i = 0; i < len; ++i)
  {
    HashVal<HashBits> c = b[i];
...
constexpr HashVal<32> mem_hash_fnv1(const char8_t* b, size_t len){ return mem_hash_fnv1<32>(b, len);
}
...
constexpr HashVal<32> operator"" _h(const char8_t *str, size_t len) { return mem_hash_fnv1<32>(str, len); }
