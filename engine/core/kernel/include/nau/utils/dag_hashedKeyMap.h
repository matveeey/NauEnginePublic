...
struct HashedKeyMap
{
  ...
  {
    NAU_FAST_ASSERT(key != EmptyKey);
    if(used >= mask * load_factor_nom / load_factor_denom) [[unlikely]]
  ...
  {
    return oa_hashmap_util::max(4, nau::math::get_bigger_pow2(sz * load_factor_denom / load_factor_nom));
  }
