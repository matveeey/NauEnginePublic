...
#define BOUNDS_CHECK_ASSERT_RETURN  NAU_ASSERT_RETURN
#define BOUNDS_CHECK_ASSERTF_RETURN NAU_ASSERT_RETURN
#define BOUNDS_CHECK_ASSERT         NAU_ASSERT
#define BOUNDS_CHECK_ASSERTF        NAU_ASSERT
#define DAG_VALIDATE_IDX(idx)                                                                            \
  if (EASTL_UNLIKELY((unsigned)idx >= (unsigned)size()))                                                 \
    NAU_ASSERT(0, "RelocatableFixedVector<T,{}{}>[{}] size={} this={} data={} sizeof(T)={}", static_size, \
      canOverflow ? ", canOverflow" : "", idx, size(), (void*)this, (void*)data(), sizeof(T));
    #else
...
template <typename T, size_t inplace_count, bool allow_overflow = true, typename Allocator = /* MidmemAlloc TODO Allocators.*/ EASTLAllocatorType, typename Counter = uint32_t,
...
public:
  ...
  void reserve(size_t v)
  {
    IF_CONSTEXPR (!canOverflow)
      ...
      {
        BOUNDS_CHECK_ASSERTF(v <= base_type::inplaceCapacity(), "reserve({}) inplaceCapacity={}", v, base_type::inplaceCapacity());
...
protected:
  ...
  void allocateAndCopy(size_type count, const value_type *p) // no Free! internal function to avoid copy-paste!
  {
    IF_CONSTEXPR (!canOverflow)
      ...
      {
        BOUNDS_CHECK_ASSERTF(count <= base_type::inplaceCapacity(), "allocateAndCopy(count={}) inplaceCapacity={}", count,
...
void RelocatableFixedVector<T, N, O, A, C, Z>::swap(RelocatableFixedVector &a)
{
  ...
  // NAU_FAST_ASSERT(this_sz <= sizeof(tmp_stor) && a_sz <= sizeof(tmp_stor));
  CHECK_RELOCATABLE();
  if (a_sz <= this_sz)
  {
...
inline void RelocatableFixedVector<T, N, O, A, C, Z>::resize(size_type newCount)
{
  ...
  IF_CONSTEXPR (!canOverflow)
    ...
    {
      BOUNDS_CHECK_ASSERTF(newCount <= base_type::inplaceCapacity(), "resize({}) inplaceCapacity={}", newCount,
        base_type::inplaceCapacity());
...
inline void *RelocatableFixedVector<T, N, O, A, C, Z>::push_back_uninitialized(size_t add_count)
{
  ...
  NAU_ASSERT_RETURN(canOverflow || newCount <= base_type::inplaceCapacity(), nullptr,
    "push_back(num={}) while size()={} and inplaceCapacity={}", add_count, base_type::used(), base_type::inplaceCapacity());
  IF_CONSTEXPR (!canOverflow)
  {
#if !NAU_DEBUG // otherwise it was checked in BOUNDS_CHECK_ASSERTF_RETURN
    if (newCount > base_type::inplaceCapacity())
      return nullptr;
#endif
  }
...
inline void *RelocatableFixedVector<T, N, O, A, C, Z>::insert_uninitialized(const_iterator at, size_t add_count)
{
  ...
  BOUNDS_CHECK_ASSERTF_RETURN(size_type(idx) <= size(), (iterator)at, "idx={} size()={}", idx, size());
  BOUNDS_CHECK_ASSERTF_RETURN(size_type(base_type::used() + add_count) >= add_count, (void *)at, "size()={} add_count={}",
    base_type::used(), add_count);
  BOUNDS_CHECK_ASSERTF_RETURN(canOverflow || newCount <= base_type::inplaceCapacity(), nullptr,
    "insert(num={}) while size()={} inplaceCapacity={}", add_count, base_type::used(), base_type::inplaceCapacity());
  IF_CONSTEXPR (!canOverflow)
  {
#if !NAU_DEBUG // otherwise it was checked in BOUNDS_CHECK_ASSERTF_RETURN
    if (newCount > base_type::inplaceCapacity())
  ...
  const value_type *first, const value_type *last)
{
  ...
  {
    BOUNDS_CHECK_ASSERTF(first >= cend() || last <= cbegin(), "insert({}, {}, {}) overlaps with begin={} .. end={}", at, first, last,
      data(), data() + capacity());
    return nullptr;
  }
  ...
  const_iterator till)
{
  ...
  // bounds check
  BOUNDS_CHECK_ASSERTF_RETURN(delCount && size() >= delCount && idx + delCount <= size(), begin(), "idx={} delCount={} size()={}", idx,
    delCount, size());
  const bool wasInplace = base_type::isInplace();
  const bool willBeInplace = base_type::is_inplace(newCount);
