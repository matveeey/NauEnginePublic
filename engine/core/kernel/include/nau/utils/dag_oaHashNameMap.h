...
struct OAHashNameMap
{
  ...
  eastl::vector<uint32_t> strings;
  dag::StringTableAllocator allocator;
  ...
  int addNameId(const char *name, size_t name_len, hash_t hash)
  {
    ...
    if (noCollisions()) [[likely]]
    {
      ...
      if (it != -1 && !string_equal(name, name_len, it)) [[unlikely]]
      {
        noCollisions() = 0;
        it = -1;
      }
...
struct FixedCapacityOAHashNameMap
{
  ...
  bool init(char *data, uint32_t strings_size, uint32_t cnt, uint32_t total_size, bool zeroed = false)
  {
    ...
    const uint32_t buckets = oa_hashmap_util::max(4, nau::math::get_bigger_pow2(cnt * 4 / 3));
...
using FastNameMapEx = FastNameMap;

} //namespace dag

#include <nau/utils/dag_iterateStatus.h>
...
{
  eastl::vector<uint32_t> sorted_ids;
