...
namespace nau::iosys
{
    ...
    class NAU_KERNEL_EXPORT IGenSave
    {
    public:
        /// Exception class to throw on write error.
        struct SaveException : NauException
        {
            int fileOffset;

            SaveException(const char* desc, int ofs = 0) :
                NauException(1, desc),
                fileOffset(ofs)
            {
            }
        ...
        inline void writeShortString(char* s)
        {
            writeShortString((const char*)s);
        }
        ...
        {
            NAU_STATIC_ASSERT(BYTENUM > 0 && BYTENUM <= 4);
    ...
    public:
        ...
        struct LoadException : public NauException
        {
            int fileOffset;

            LoadException(const char* desc, int ofs = 0) :
                NauException(2, desc),
                fileOffset(ofs)
            {
            }
        ...
        /// Returns some name as stream description (e.g., filename for file stream) //TODO: change to char8_t
        virtual const char* getTargetName() = 0;

        /// Returns size of target data stream if available, or -1 when size is not known
        virtual int64_t getTargetDataSize()
        {
            return -1;
        }
        ...
        virtual eastl::span<const char> getTargetRomData() const
        {
            return {};
        }
        ...
        inline bool readExact(void* ptr, int size)
        {
            ...
            NAU_ASSERT(rd == size, "{}: readExact({:p},{})={}", getTargetName(), ptr, size, rd);
            return rd == size;
        }
        ...
        template <class T>
        inline void readVectorData(T& vector)
        {
            const auto dataSize = vector.size() * sizeof(typename T::value_type);
            if(dataSize)
            {
                read(vector.data(), dataSize);
            }
        }
