...
namespace nau::iosys
{
    ...
    class NAU_KERNEL_EXPORT ZlibLoadCB : public IGenLoad
    {
    public:
        ...
        void open(IGenLoad& in_crd, int in_size, bool raw_inflate = false);
        void close();
    ...
    class NAU_KERNEL_EXPORT BufferedZlibLoadCB : public ZlibLoadCB
    {
    public:
        BufferedZlibLoadCB(IGenLoad& in_crd, int in_size) :
            ZlibLoadCB(in_crd, in_size),
            curPos(0),
            totalOut(0)
        {
        }
    ...
    class NAU_KERNEL_EXPORT ZlibSaveCB : public IGenSave
    {
    public:
        ZlibSaveCB(IGenSave& in_save_cb, int compression_level, bool raw_inflate = false);
        ~ZlibSaveCB();
        ...
        virtual void endBlock(unsigned /*block_flags*/)
        {
            issueFatal();
        }
        virtual int getBlockLevel()
        ...
        virtual const char* getTargetName()
        {
            return saveCb ? saveCb->getTargetName() : NULL;
        }
        virtual void flush()
        { /*noop*/
        }
    ...
    NAU_KERNEL_EXPORT int zlib_compress_data(IGenSave& dest, int compression_level, IGenLoad& src, int sz);
}  // namespace nau::iosys