...
namespace nau::hal
{
    ...
    template <typename Cb>
    static inline const char* iterate_base_paths_fast_s(const char* fname, char* fnbuf, int fnbuf_sz, bool force_root_iter, bool force_simplify, Cb cb)
    {
    ...
    static inline bool is_path_unc_win(const char* fn)
    {
        return (strncmp(fn, "\\\\", 2) == 0);
    }

    static inline bool is_path_unc_unix(const char* fn)
    {
        return (strncmp(fn, "//", 2) == 0);
    }

    static inline bool is_path_unc(const char* fn)
    {
        return is_path_unc_win(fn) || is_path_unc_unix(fn);
    }
    ...
    static inline bool is_path_abs(const char* fn)
    {
        ...
        if(is_path_unc(fn))
            return true;
#if _TARGET_C1 | _TARGET_C2 | NAU_PLATFORM_APPLE | NAU_PLATFORM_LINUX | _TARGET_ANDROID
        if(fn[0] == '/')
            return true;
