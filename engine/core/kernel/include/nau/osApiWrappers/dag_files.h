...
namespace nau::hal
{
    ...
    // DECLARE_DSA_OVERLOADS_FAMILY(static inline int df_printf, extern "C" NAU_KERNEL_EXPORT int df_printf, return df_printf);
    ...
    extern "C"
    {
        ...
        NAU_KERNEL_EXPORT file_ptr_t df_open(const char* fname, int flags);

        // close file that opened with df_open
        // returns -1 on error
        NAU_KERNEL_EXPORT int df_close(file_ptr_t fp);

        NAU_KERNEL_EXPORT void df_flush(file_ptr_t fp);

        // read from file
        // returns the number of bytes read or -1 on error
        NAU_KERNEL_EXPORT int df_read(file_ptr_t fp, void* buf, int len);

        // write to file
        // returns the number of bytes written or -1 on error
        NAU_KERNEL_EXPORT int df_write(file_ptr_t fp, const void* buf, int len);

        // returns 0 on error
        // 2K characters max
        NAU_KERNEL_EXPORT int df_vprintf(file_ptr_t fp, const char* fmt, va_list lst);
        NAU_KERNEL_EXPORT int df_cprintf(file_ptr_t fp, const char* fmt, ...);

        // analog of fgets
        NAU_KERNEL_EXPORT char* df_gets(char* buf, int n, file_ptr_t fp);

        // move the read/write pointer
        // returns the current pointer or -1 on error
        NAU_KERNEL_EXPORT int df_seek_to(file_ptr_t fp, int offset_abs);
        NAU_KERNEL_EXPORT int df_seek_rel(file_ptr_t fp, int offset_rel);
        NAU_KERNEL_EXPORT int df_seek_end(file_ptr_t fp, int offset_from_end);

        // returns the current pointer or -1 on error
        NAU_KERNEL_EXPORT int df_tell(file_ptr_t fp);

        // returns file length or -1 on error
        NAU_KERNEL_EXPORT int df_length(file_ptr_t fp);

        // returns pointer to vromfs if 'fp' was opened as ROM-file from vromfs; otherwise returns nullptr
        NAU_KERNEL_EXPORT const VirtualRomFsData* df_get_vromfs_for_file_ptr(file_ptr_t fp);
        ...
        NAU_KERNEL_EXPORT const char* df_get_vromfs_file_data_for_file_ptr(file_ptr_t fp, int& out_data_sz);
        ...
        NAU_KERNEL_EXPORT int df_stat(const char* path, DagorStat* buf);
        NAU_KERNEL_EXPORT int df_fstat(file_ptr_t fp, DagorStat* buf);

        // creates read-only file mapping
        NAU_KERNEL_EXPORT const void* df_mmap(file_ptr_t fp, int* out_file_len = NULL, int length = 0, int offset = 0);
        // deletes file mapping
        NAU_KERNEL_EXPORT void df_unmap(const void* start, int length);
        ...
        NAU_KERNEL_EXPORT const char* df_get_real_name(const char* file_name);
        ...
        NAU_KERNEL_EXPORT const char* df_get_real_folder_name(const char* folder_name);

        // returns pointer to string containing full path to specified file_name; returns NULL if file not exists
        // finds and adds base-path where file is located; works for both real and vromfs files
        // NOTE: uses static buffer, don't store this pointer for long
        NAU_KERNEL_EXPORT const char* df_get_abs_fname(const char* file_name);
        ...
        NAU_KERNEL_EXPORT file_ptr_t df_mkstemp(char* templ);
