...
namespace nau::hal
{
    ...
    extern "C"
    {
        ...
        NAU_KERNEL_EXPORT bool dd_add_base_path(const char* base_path, bool insert_first = false);

        //! removes base_path from list, returns pointer removed from list (pointer to local storage must not be free'd)
        NAU_KERNEL_EXPORT const char* dd_remove_base_path(const char* base_path);

        //! clears list of base pathes
        NAU_KERNEL_EXPORT void dd_clear_base_paths();

        //! dumps list of base pathes to output
        NAU_KERNEL_EXPORT void dd_dump_base_paths();

        //! iterates over base pathes, returns NULL when finished
        NAU_KERNEL_EXPORT const char* df_next_base_path(int* index_ptr);
        ...
        NAU_KERNEL_EXPORT void dd_set_named_mount_path(const char* mount_name, const char* path_to);

        //! returns path assigned for named mount (when mount exists) or nullptr (when mount_name not known)
        NAU_KERNEL_EXPORT const char* dd_get_named_mount_path(const char* mount_name, int mount_name_len = -1);

        //! dumps list of named mounts (with assigned pathes) to output
        NAU_KERNEL_EXPORT void dd_dump_named_mounts();
        ...
        NAU_KERNEL_EXPORT const char* dd_resolve_named_mount_in_path(const char* fpath, const char*& mnt_path);
        ...
        NAU_KERNEL_EXPORT bool dd_check_named_mount_in_path_valid(const char* fpath);
