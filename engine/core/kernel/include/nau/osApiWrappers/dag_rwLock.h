...
namespace nau::hal
{
    ...
    NAU_KERNEL_EXPORT void os_rwlock_init(os_rwlock_t&);
    NAU_KERNEL_EXPORT void os_rwlock_destroy(os_rwlock_t&);
    NAU_KERNEL_EXPORT void os_rwlock_acquire_read_lock(os_rwlock_t&);
    NAU_KERNEL_EXPORT bool os_rwlock_try_acquire_read_lock(os_rwlock_t&);
    NAU_KERNEL_EXPORT void os_rwlock_release_read_lock(os_rwlock_t&);
    NAU_KERNEL_EXPORT void os_rwlock_acquire_write_lock(os_rwlock_t&);
    NAU_KERNEL_EXPORT bool os_rwlock_try_acquire_write_lock(os_rwlock_t&);
    NAU_KERNEL_EXPORT void os_rwlock_release_write_lock(os_rwlock_t&);
    ...
    public:
        ...
        {
            os_rwlock_init(lock);
            NAU_ASSERT(dbgrtls != /*TLS_OUT_OF_INDEXES*/ ~0u);
        }
        ~OSReadWriteLock()
        {
    ...
    public:
        ...
        bool tryLockWrite()
        {
            return os_rwlock_try_acquire_write_lock(lock);
        }
        void unlockWrite()
        {
    ...
    class NAU_KERNEL_EXPORT OSReentrantReadWriteLock
    {
        os_rwlock_t lock;
#if NAU_PLATFORM_WIN32 | _TARGET_XBOX
        ...
        unsigned long getWriteThreadId() const
        {
            return wtid;
        }
        void setWriteThreadId(unsigned long tid = 0)
        {
            wtid = tid;
        }
        ...
        {
            rtls = TlsAlloc();
            NAU_ASSERT(rtls != /*TLS_OUT_OF_INDEXES*/ ~0u);
        }
        void rtlsDestroy()
        {
            NAU_ASSERT(TlsFree(rtls), "{}", GetLastError());
        }
        size_t rtlsGet(size_t = 0)
        {
            return (size_t)TlsGetValue(rtls);
        }
        void rtlsSet(size_t v)
        {
            TlsSetValue(rtls, (void*)v);
        }
#else
        ...
        void setWriteThreadId()
        {
            __atomic_store_n(&wtid, 0, __ATOMIC_RELAXED);
        }
    #if _TARGET_APPLE  // Apple's read locks are also not reentrant in presense of writer
        pthread_key_t rtls;
        void rtlsCreate()
        {
            G_VERIFY(pthread_key_create(&rtls, NULL) == 0);
        }
        void rtlsDestroy()
        {
            G_VERIFY(pthread_key_delete(rtls) == 0);
        }
        size_t rtlsGet(size_t = 0)
        {
            return (size_t)pthread_getspecific(rtls);
        }
        void rtlsSet(size_t v)
        {
        ...
        constexpr size_t rtlsGet(size_t v = 0)
        {
            return v;
        }
        void rtlsSet(size_t)
    ...
    public:
        ...
        void unlockWrite()
        {
            NAU_ASSERT(wcount > 0 && getWriteThreadId() == get_current_thread_id());  // if triggered then it's attempt to unlock unowned
                                                                                      // lock
            if(--wcount == 0)
            {
        ...
        void unlockRead()
        {
            ...
            NAU_ASSERT(rcount != 0);  // if triggered then it's attempt to unlock not locked for read (by this thread) lock (or lock/unlock
                                      // inbalance)
            if(--rcount == 0)
                os_rwlock_release_read_lock(lock);
            rtlsSet(rcount);
    ...
    {
        ScopedLockWriteTemplate(Lock& lock /*, ::da_profiler::desc_id_t profile_token = ::da_profiler::DescWriteLock*/) :
            savedLock(&lock)
        {
