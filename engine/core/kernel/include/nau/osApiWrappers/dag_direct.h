// Copyright 2024 N-GINN LLC. All rights reserved.
// Copyright (C) 2024  Gaijin Games KFT.  All rights reserved
#pragma once

#include <nau/kernel/kernel_config.h>
#include <nau/utils/dag_baseDef.h>
#include <nau/utils/dag_stdint.h>
#include <stdarg.h>
#include <stdbool.h>

namespace nau::hal
{
#ifdef __cplusplus
    extern "C"
    {
#endif

        // Compare filenames taking into consideration '*' and '?' symbols
        // Returns 0, if filenames are equal, 1 otherwise
        NAU_KERNEL_EXPORT bool dd_fname_equal(const char* f1, const char* fn2);

        // remove leading and trailing spaces, remove quotes and extra slashes,
        // remove extra up-dirs (e.g. some\dir\..\tex => some\tex)
        NAU_KERNEL_EXPORT void dd_simplify_fname_c(char* fn);

        // returns pointer to extension inside filename or NULL if no extension
        // first character in returned string will be dot (.)
        NAU_KERNEL_EXPORT const char* dd_get_fname_ext(const char* pathname);

        // returns pointer to filena with extension inside pathname
        // or NULL for empty pathname
        NAU_KERNEL_EXPORT const char* dd_get_fname(const char* pathname);

        // returns pointer to buf containing location of filename
        // size of buffer must be sufficiently large (260 at max for Win32)
        NAU_KERNEL_EXPORT char* dd_get_fname_location(char* buf, const char* pathname);

        // returns pointer to inside buf containing location of filename, without path and extenstion
        // size of buffer must be sufficiently large (260 at max for Win32)
        NAU_KERNEL_EXPORT const char* dd_get_fname_without_path_and_ext(char* buf, int buf_size, const char* path);

        // append slash, if there is no slash
        // NOTE: target buffer must have enough space for this slash
        NAU_KERNEL_EXPORT void dd_append_slash_c(char* pathname);

        enum
        {
            DA_FILE = 0x0,
            DA_SUBDIR = 0x10,

            DA_READONLY = 0x01,
            DA_HIDDEN = 0x02,
            DA_SYSTEM = 0x04,
        };

        // structure to be used for dd_find_first/dd_find_next
        struct alefind_t
        {
            int grp, fattr;
            void* data;
            char fmask[DAGOR_MAX_PATH - sizeof(int) * 4 - sizeof(void*) - sizeof(int64_t) * 3];
            char name[DAGOR_MAX_PATH];  //< filename
            // times are set to -1 if underlying implementation or fs is not supports them
            int64_t atime;  //< time of most recent access (disabled by default on Windows since Vista)
            int64_t mtime;  //< time of most recent content modification
            int64_t ctime;  //< platform dependent: time of most recent metadata change on Unix, time of creation on Windows
            int size;       //< file size
            int attr;       //< file atributes
        };

        // rename file
        // return 0 on error
        // silenly rewrites destination if it exists (POSIX behavior)
        NAU_KERNEL_EXPORT bool dd_rename(const char* oldname, const char* newname);

        // Erase file
        // Returns 0 if error occured
        NAU_KERNEL_EXPORT bool dd_erase(const char* filename);

        // make dir, returns 0 on error
        NAU_KERNEL_EXPORT bool dd_mkdir(const char* path);

        // make all dirs of the path, returns 0 on error
        NAU_KERNEL_EXPORT bool dd_mkpath(const char* path);

        // remove dir, returns 0 on error
        NAU_KERNEL_EXPORT bool dd_rmdir(const char* path);

        // Find first file
        // Returns 0 if error occured
        NAU_KERNEL_EXPORT int dd_find_first(const char* mask, char attr, alefind_t*);

        // Find next file
        // Returns 0 if error occured
        NAU_KERNEL_EXPORT int dd_find_next(alefind_t*);

        // Finalize file search
        // Returns 0 if error occured
        NAU_KERNEL_EXPORT int dd_find_close(alefind_t*);

        NAU_KERNEL_EXPORT bool dd_file_exist(const char* fn);

        NAU_KERNEL_EXPORT bool dd_dir_exist(const char* fn);

        inline bool dd_file_exists(const char* fn)
        {
            return dd_file_exist(fn);
        }
        inline bool dd_dir_exists(const char* fn)
        {
            return dd_dir_exist(fn);
        }

#ifdef __cplusplus
    }
#endif
}  // namespace nau::hal