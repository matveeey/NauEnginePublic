...
Properties::Properties(Data* data, ssize_t* dataIdx, const eastl::string& name, const char* id, const char* parentID, Properties* parent)
...
Properties* Properties::createNonRefCounted(const std::string& url)
{
    if (url.size() == 0)
    {
        NAU_LOG_ERROR("Attempting to create a Properties object from an empty URL!");
        return nullptr;
    }
...
void Properties::readProperties()
{
    ...
    while (true)
    {
        ...
        if (rc == NULL)
        {
            NAU_LOG_ERROR("Error reading line from file.");
            return;
        }
        ...
        else if (strncmp(line, "//", 2) != 0)
        {
            ...
            if (rc != NULL)
            {
                ...
                if (name == NULL)
                {
                    NAU_LOG_ERROR("Error parsing properties file: attribute without name.");
                    return;
                }
                ...
                if (value == NULL)
                {
                    NAU_LOG_ERROR("Error parsing properties file: attribute with name ('{}') but no value.", name);
                    return;
                }
            ...
            else
            {
                ...
                if (name == NULL)
                {
                    NAU_LOG_ERROR("Error parsing properties file: failed to determine a valid token for line '{}'.", line);
                    return;
                }
                else if (name[0] == '}')
                {
                ...
                if (value != NULL && value[0] == '{')
                {
                    ...
                    if (rccc && rccc == lineEnd)
                    {
                        ...
                        {
                            NAU_LOG_ERROR("Failed to seek back to before a '}' character in properties file.");
                            return;
                        }
                        while (readChar() != '}')
                        {
                    ...
                    if (rccc && rccc == lineEnd)
                    {
                        if (seekFromCurrent(1) == false)
                        {
                            NAU_LOG_ERROR("Failed to seek to immediately after a '}' character in properties file.");
                            return;
                        }
                    }
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            // Back up from fgetc()
                            if (seekFromCurrent(-1) == false)
                                NAU_LOG_ERROR("Failed to seek backwards a single character after testing if the next line starts with '{'.");
...
void Properties::skipWhiteSpace()
{
    ...
    if (c != EOF)
    {
        if (seekFromCurrent(-1) == false)
        {
            NAU_LOG_ERROR("Failed to seek backwards one character after skipping whitespace.");
        }
    }
}
...
int Properties::getInt(const char* name) const
{
    ...
    if (valueString)
    {
        ...
        if (scanned != 1)
        {
            NAU_LOG_ERROR("Error attempting to parse property '{}' as an integer.", name);
            return 0;
        }
        return value;
    }
...
float Properties::getFloat(const char* name) const
{
    ...
    if (valueString)
    {
        ...
        if (scanned != 1)
        {
            NAU_LOG_ERROR("Error attempting to parse property '{}' as a float.", name);
            return 0.0f;
        }
        return value;
    }
...
long Properties::getLong(const char* name) const
{
    ...
    if (valueString)
    {
        ...
        if (scanned != 1)
        {
            NAU_LOG_ERROR("Error attempting to parse property '{}' as a long integer.", name);
            return 0L;
        }
        return value;
    }
...
bool Properties::getMat4(const char* name, Mat4* out) const
{
    ...
    if (valueString)
    {
        ...
        if (scanned != 16)
        {
            NAU_LOG_ERROR("Error attempting to parse property '{}' as a matrix.", name);
            out->setIdentity();
            return false;
        }
...
void calculateNamespacePath(const std::string& urlString, std::string& fileString, std::vector<std::string>& namespacePath)
{
    ...
    if (loc != eastl::string::npos)
    {
        ...
        while ((loc = namespacePathString.find('/')) != eastl::string::npos)
        {
