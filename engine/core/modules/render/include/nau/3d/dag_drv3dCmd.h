...
enum
{
  ...
  DRV3D_COMMAND_IS_DEFRAG_REQUESTED,
...
struct CompilePipelineSet
{
  ...
  const nau::DataBlock *featureSet;
  // A block with a set of blocks describing input layouts.
  const nau::DataBlock *inputLayoutSet;
  ...
  const nau::DataBlock *renderStateSet;
  ...
  const nau::DataBlock *outputFormatSet;
  ...
  const nau::DataBlock *graphicsPipelineSet;
  ...
  const nau::DataBlock *meshPipelineSet;
  // A block with a set of blocks describing compute pipelines.
  const nau::DataBlock *computePipelineSet;
};
...
NAU_RENDER_EXPORT EventQuery* create_event_query();

NAU_RENDER_EXPORT void release_event_query(EventQuery* query);

// Returns true if successfully issued.
NAU_RENDER_EXPORT bool issue_event_query(EventQuery* query);

// Returns false when query is issued but not yet signaled (waiting).
// Returns true otherwise - signaled, not issued, or bad query.
NAU_RENDER_EXPORT bool get_event_query_status(EventQuery* query, bool force_flush);
