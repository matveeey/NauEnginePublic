...
#include <binPack2D/GuillotineBinPack.h>
#include <binPack2D/SkylineBinPack.h>
#include <EASTL/hash_map.h>
#include <EASTL/fixed_vector.h>
#include "nau/3d/dag_drv3d.h"
#include "nau/3d/dag_textureIDHolder.h"
...
public:
  ...
  void init(const IPoint2 &sz, int resv, int _margin, const char *tex_name, int tex_fmt = TEXFMT_R8 | TEXCF_UPDATE_DESTINATION)
  {
    ...
    if (texSz.x && texSz.y)
    {
      ...
      if (pure_fmt == TEXFMT_DXT1 || pure_fmt == TEXFMT_DXT5 || pure_fmt == TEXFMT_BC7)
      {
        ...
        {
          NAU_LOG_ERROR("bad margin={} for block compression, changing to 0", margin);
          margin = 0;
        }
      }
      else
  ...
  const ItemData *addItem(int bmin_x, int bmin_y, int bmax_x, int bmax_y, unsigned hash, int idx = -1, bool allow_discard = true)
  {
    ...
    else
    {
        ...
        idx = itemData.size();
        itemData.push_back_uninitialized();
    }
    ...
    NAU_ASSERT(cornerResv != 4 || (!(g.x0 % 4) && !(g.y0 % 4)), "rect={},{}, {}x{}", r.x, r.y, r.width, r.height);
  ...
  const ItemData *addDirectItemOnly(unsigned hash, int x0, int y0, int w, int h, bool sched_for_load = false)
  {
    ...
    {
        //idx = append_items(itemData, 1); // TODO: potentional problem
        idx = itemData.size();
        itemData.push_back_uninitialized();
    }
...
protected:
  HashToIdx hash2idx;
  eastl::vector<ItemData> itemData;
  eastl::vector<int> itemUu;
  ...
  void reArrangeAndAlloc(rbp::Rect &r, int need_w, int need_h)
  {
    ...
    {
      NAU_LOG_WARNING("{}: {} items will use {}%, rearrange is meaningless", get_managed_texture_name(tex.getId()), rect_num,
    ...
    if (!tmp_tex)
    {
      NAU_LOG_ERROR("{}: failed to alloc temp texture {}x{} to rearrange items", name, texSz.x, texSz.y);
      return;
    }
    ...
    NAU_LOG_WARNING("{}: rearrange atlas ({} items will use {}%)", name, src_rects.size(), used_area * 100 / (texSz.x * texSz.y));
...
#if DAGOR_DBGLEVEL > 0
    if (!upd_ok)
      NAU_LOG_ERROR("{}: failed to update corner quad {}x{} (tex {}x{} cflg={:#x})", name, cornerResv, cornerResv, texSz.x, texSz.y, ti.cflg);
#endif
...
#if DAGOR_DBGLEVEL > 0
      if (!upd_ok)
        NAU_LOG_ERROR("{}: failed to copy image[{}] ({},{}) {}x{} to new place ({},{}) (tex {}x{} cflg={:#x}, marginLtOfs={} margin={})", name,
    ...
    NAU_LOG_WARNING("-> allocated {} rects, new_rect={},{} {}x{}", new_rects.size(), r.x, r.y, r.width, r.height);

    upd_ok = t->updateSubRegion(tmp_tex, 0, 0, 0, 0, texSz.x, texSz.y, 1, 0, 0, 0, 0);
#if DAGOR_DBGLEVEL > 0
    if (!upd_ok)
      NAU_LOG_ERROR("{}: failed to swap/copy image (tex {}x{} cflg={:#x}, marginLtOfs={} margin={})", name, texSz.x, texSz.y, ti.cflg,
        marginLtOfs, margin);
#endif
...
public:
  ...
  void init(const nau::math::IVector2 &sz, int tex_fmt = TEXFMT_R8)
  {
    sysTexSz = sz;
    texFmt = tex_fmt;
    if (texFmt == TEXFMT_R8)
  ...
  void alloc()
  {
    NAU_ASSERT(sysTexCount < MAX_GEN_SYS_TEX_CNT);
    curSysTex = sysTexCount++;
    SysTexRec &r = sysTex[curSysTex];
    r.tex = (Texture *)d3d::create_tex(NULL, sysTexSz.getX(), sysTexSz.getY(),
      texFmt | TEXCF_SYSMEM | TEXCF_DYNAMIC | TEXCF_READABLE | TEXCF_LINEAR_LAYOUT, 1, "upd-sysTex");
    r.pack.Init(sysTexSz.getX(), sysTexSz.getY(), false);
  ...
  Texture *add(int w, int h, CopyRec &out_rec)
  {
    ...
    for (int attempts = sysTexCount; attempts > 0; attempts--)
    {
      ...
      {
        rec->pack.Init(sysTexSz.getX(), sysTexSz.getY(), false);
  ...
  const nau::math::IVector2 &getSz() const { return sysTexSz; }

protected:
  SysTexRec sysTex[MAX_GEN_SYS_TEX_CNT];
  int curSysTex = -1, sysTexCount = 0;
  nau::math::IVector2 sysTexSz = nau::math::IVector2(1, 1);
...
public:
  ...
  struct FencedGPUResource
  {
    ...
    bool lock()
    {
      ...
      {
        NAU_LOG_ERROR("DynamicAtlasFencedTexUpdater can't lock texture");
        return false;
      }
    ...
    void init(const nau::math::IVector2 &sz, int tex_fmt = TEXFMT_R8)
    {
      tex = d3d::create_tex(NULL, sz.getX(), sz.getY(), tex_fmt | TEXCF_SYSMEM | TEXCF_DYNAMIC | TEXCF_READABLE | TEXCF_LINEAR_LAYOUT, 1,
        "upd-sysTex");
  ...
  void init(const nau::math::IVector2 &sz, int tex_fmt = TEXFMT_R8)
  {
    ...
    sysTextures.push_back_uninitialized();
  ...
  bool tryLock()
  {
    ...
    pack.Init(sysTexSz.getX(), sysTexSz.getY(), false);
    return true;
  }
...
protected:
  eastl::fixed_vector<FencedGPUResource, MAX_FRAME_QUEUE, false> sysTextures; // StaticTab
