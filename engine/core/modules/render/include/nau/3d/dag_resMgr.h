...
struct D3dResManagerData
{
  ...
  //! validates D3DRESID and reports stale generation to NAU_LOG_ERROR (by default)
...
protected:
  ...
  NAU_RENDER_EXPORT static int indexCount, maxTotalIndexCount;
  NAU_RENDER_EXPORT static uint16_t *__restrict generation;
  NAU_RENDER_EXPORT static int *__restrict refCount;
...
NAU_RENDER_EXPORT D3DRESID register_managed_res(const char* name, D3dResource* res);
...
NAU_RENDER_EXPORT void release_managed_res(D3DRESID id);
...
NAU_RENDER_EXPORT void release_managed_res_verified(D3DRESID& id, D3dResource* check_res);

// helpers for textures (derived from D3dResource)
NAU_RENDER_EXPORT BaseTexture* acquire_managed_tex(D3DRESID id);
...
NAU_RENDER_EXPORT const char *get_managed_res_name(D3DRESID id);

// Returns managed resource ID by name, or BAD_D3DRESID if name is invalid.
NAU_RENDER_EXPORT D3DRESID get_managed_res_id(const char *res_name);
...
NAU_RENDER_EXPORT void enable_res_mgr_mt(bool enable, int max_res_entry_count);
