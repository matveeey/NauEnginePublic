...
//#include "nau/shaders/dag_shaders.h"
...
#endif
//#include "nau/shaders/dag_shaderVar.h"
...
struct Helper<Sbuffer> : public Helper<D3dResource>
{
  ...
  static const char* getName(Sbuffer *res) { return res ? reinterpret_cast<const char*>(res->getBufName()) : nullptr; }
  ...
  // TODO: create something as replacement
...
struct Helper<BaseTexture>
{
  ...
  static const char *getName(BaseTexture *res) { return res ? reinterpret_cast<const char*>(res->getTexName()) : nullptr; }
...
public:
  ...
  {
    NAU_ASSERT(isBuf(PrivateDataGetter::getResource(this)));
...
#define ENABLE_MOVING_BEHAVIOR(Class)                                                                                  \
  ...
  {                                                                                                                    \
    NAU_ASSERT(Class::isValidForMoveAssignment(*this, other), "Invalid for move assignment. Call close() before this"); \
...
public:
  ...
  static void release(typename Helper<ResType>::resid_t &res_id, ResType *res)
  {
    ...
    NAU_ASSERT(res_id == Helper<ResType>::BAD_ID || Helper<ResType>::getManagedRefCount(res_id) == 1,
      "resource_name = '{}' res_id=0x{} refCount={}", res ? reinterpret_cast<const char*>(res->getResName()) : "", (void*)&res_id,
...
public:
  ...
  ConcreteResHolder(ManagedResType &&resource, const char *shader_var_name = nullptr)
  {
    ...
    //this->setVarId(get_shader_variable_id(shader_var_name)); // TODO: fix it
  ...
  ~ConcreteResHolder()
  {
    ...
    //if (this->mShaderVarId != -1) // TODO: fix it
...
template class NAU_RENDER_EXPORT resptr_detail::ManagedResView<ManagedTex>;
template class NAU_RENDER_EXPORT resptr_detail::ManagedResView<ManagedBuf>;
...
template class NAU_RENDER_EXPORT resptr_detail::ResPtr<BaseTexture>;
template class NAU_RENDER_EXPORT resptr_detail::ResPtr<Sbuffer>;
...
{
  return resptr_detail::ResPtrFactory(d3d::create_tex(img, w, h, flg, levels, reinterpret_cast<const char8_t*>(name)));
}

static inline ResPtr<CubeTexture> create_cubetex(int size, int flg, int levels, const char *name)
{
  return resptr_detail::ResPtrFactory(d3d::create_cubetex(size, flg, levels, reinterpret_cast<const char8_t*>(name)));
}

static inline ResPtr<VolTexture> create_voltex(int w, int h, int d, int flg, int levels, const char *name)
{
  return resptr_detail::ResPtrFactory(d3d::create_voltex(w, h, d, flg, levels, reinterpret_cast<const char8_t*>(name)));
}

static inline ResPtr<ArrayTexture> create_array_tex(int w, int h, int d, int flg, int levels, const char *name)
{
  return resptr_detail::ResPtrFactory(d3d::create_array_tex(w, h, d, flg, levels, reinterpret_cast<const char8_t*>(name)));
}

static inline ResPtr<ArrayTexture> create_cube_array_tex(int side, int d, int flg, int levels, const char *name)
{
  return resptr_detail::ResPtrFactory(d3d::create_cube_array_tex(side, d, flg, levels, reinterpret_cast<const char8_t*>(name)));
}
...
{
  return resptr_detail::ResPtrFactory(d3d::alias_tex(base_tex, img, w, h, flg, levels, reinterpret_cast<const char8_t*>(name)));
}

static inline ResPtr<Vbuffer> create_vb(int sz, int f, const char *name)
{
  return resptr_detail::ResPtrFactory(d3d::create_vb(sz, f, reinterpret_cast<const char8_t*>(name)));
}

static inline ResPtr<Ibuffer> create_ib(int size_bytes, int flags, const char *name)
{
  return resptr_detail::ResPtrFactory(d3d::create_ib(size_bytes, flags, reinterpret_cast<const char8_t*>(name)));
}

static inline ResPtr<Sbuffer> create_sbuffer(int struct_size, int elements, unsigned flags, unsigned texfmt, const char *name)
{
  return resptr_detail::ResPtrFactory(d3d::create_sbuffer(struct_size, elements, flags, texfmt, reinterpret_cast<const char8_t*>(name)));
}
...
//}
static inline SharedTex add_managed_array_texture(const char *name, eastl::span<const char *> tex_slice_nm)
{
  return SharedTex(::add_managed_array_texture(name, tex_slice_nm));
}
static inline SharedTexHolder add_managed_array_texture(const char *name, eastl::span<const char *> tex_slice_nm,
  const char *varname)
{
...
{
  return resptr_detail::ResPtrFactory(d3d::place_texture_in_resource_heap(heap, desc, offset, alloc_info, reinterpret_cast<const char8_t*>(name)));
}

static inline BufPtr place_buffer_in_resource_heap(ResourceHeap *heap, const ResourceDescription &desc, size_t offset,
  const ResourceAllocationProperties &alloc_info, const char *name)
{
  return resptr_detail::ResPtrFactory(d3d::place_buffere_in_resource_heap(heap, desc, offset, alloc_info, reinterpret_cast<const char8_t*>(name)));
}
