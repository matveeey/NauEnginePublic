...
struct D3dInterfaceTable
{
  ...
  BaseTexture *(*create_tex_0)(TexImage32 *img, int w, int h, int flg, int levels, const char8_t *stat_name);
  BaseTexture *(*create_cubetex_0)(int size, int flg, int levels, const char8_t *stat_name);
  BaseTexture *(*create_voltex)(int w, int h, int d, int flg, int levels, const char8_t *stat_name);
  BaseTexture *(*create_array_tex)(int w, int h, int d, int flg, int levels, const char8_t *stat_name);
  BaseTexture *(*create_cube_array_tex)(int side, int d, int flg, int levels, const char8_t *stat_name);

  BaseTexture *(*create_ddsx_tex)(nau::iosys::IGenLoad &crd, int flg, int q, int lev, const char8_t *stat_name);
  BaseTexture *(*alloc_ddsx_tex)(const ddsx::Header &hdr, int flg, int q, int lev, const char8_t *stat_name, int stub_tex_idx);

  BaseTexture *(*alias_tex_0)(BaseTexture *baseTexture, TexImage32 *img, int w, int h, int flg, int levels, const char8_t *stat_name);
  BaseTexture *(*alias_cubetex_0)(BaseTexture *baseTexture, int size, int flg, int levels, const char8_t *stat_name);
  BaseTexture *(*alias_voltex_0)(BaseTexture *baseTexture, int w, int h, int d, int flg, int levels, const char8_t *stat_name);
  BaseTexture *(*alias_array_tex_0)(BaseTexture *baseTexture, int w, int h, int d, int flg, int levels, const char8_t *stat_name);
  BaseTexture *(*alias_cube_array_tex_0)(BaseTexture *baseTexture, int side, int d, int flg, int levels, const char8_t *stat_name);

  bool (*set_tex_usage_hint)(int w, int h, int mips, const char *format, unsigned int tex_num);
  void (*discard_managed_textures)();
  bool (*stretch_rect_0)(BaseTexture *src, BaseTexture *dst, nau::math::RectInt *rsrc, nau::math::RectInt *rdst);
  ...
  Sbuffer *(*create_vb_0)(int size_bytes, int flags, const char8_t *name);
  Sbuffer *(*create_ib)(int size_bytes, int flags, const char8_t *stat_name);
  Sbuffer *(*create_sbuffer)(int struct_size, int elements, unsigned flags, unsigned texfmt, const char8_t *);
  ...
  bool (*settm_0)(int which, const nau::math::Matrix4 *tm);
  bool (*settm_1)(int which, const nau::math::Matrix4 &tm);
  //void (*settm_2)(int which, const nau::math::Matrix4 &tm);
  bool (*gettm_0)(int which, nau::math::Matrix4 *out_tm);
  bool (*gettm_1)(int which, nau::math::Matrix4 &out_tm);
  //void (*gettm_2)(int which, nau::math::Matrix4 &out_tm);
  const nau::math::Matrix4 &(*gettm_cref)(int which);
  void (*getm2vtm)(nau::math::Matrix4 &out_m2v);
  void (*setglobtm)(nau::math::Matrix4 &m);
  void (*getglobtm)(nau::math::Matrix4 &);
  bool (*setpersp)(const Driver3dPerspective &, nau::math::Matrix4 *);
  bool (*getpersp)(Driver3dPerspective &);
  bool (*validatepersp)(const Driver3dPerspective &);
  bool (*calcproj_0)(const Driver3dPerspective &, nau::math::Matrix4 &);
  bool (*calcproj_1)(const Driver3dPerspective &, nau::math::Matrix4 &);
  void (*calcglobtm_0)(const nau::math::Matrix4 &, const nau::math::Matrix4 &, nau::math::Matrix4 &);
  void (*calcglobtm_1)(const nau::math::Matrix4 &, const Driver3dPerspective &, nau::math::Matrix4 &);
  void (*calcglobtm_2)(const nau::math::Matrix4 &, const nau::math::Matrix4 &, nau::math::Matrix4 &);
  void (*calcglobtm_3)(const nau::math::Matrix4 &, const Driver3dPerspective &, nau::math::Matrix4 &);

  void (*getglobtm_vec)(nau::math::Matrix4 &);
  void (*setglobtm_vec)(const nau::math::Matrix4 &);

  bool (*setscissor)(int x, int y, int w, int h);
  bool (*setscissors)(eastl::span<const ScissorRect> scissorRects);

  bool (*setview)(int x, int y, int w, int h, float minz, float maxz);
  bool (*setviews)(eastl::span<const Viewport> viewports);
  bool (*getview)(int &x, int &y, int &w, int &h, float &minz, float &maxz);
  bool (*clearview)(int what, nau::math::E3DCOLOR, float z, uint32_t stencil);
...
#include "rayTrace/rayTrace3di.inl.h"
  ...
  Sbuffer *create_vb(int size_bytes, int flags, const char8_t *name = u8"") { return create_vb_0(size_bytes, flags, name); }
  BaseTexture *create_tex(TexImage32 *img, int w, int h, int flg, int levels, const char8_t *stat_name = NULL)
  {
    return create_tex_0(img, w, h, flg, levels, stat_name);
  }
  bool stretch_rect(BaseTexture *src, BaseTexture *dst, nau::math::RectInt *rsrc = NULL, nau::math::RectInt *rdst = NULL)
  {
  ...
  BaseTexture *create_cubetex(int size, int flg, int levels, const char8_t *stat_name = NULL)
  {
  ...
  bool settm(int which, const nau::math::Matrix4 *tm) { return settm_0(which, tm); }
  bool settm(int which, const nau::math::Matrix4 &tm) { return settm_1(which, tm); }
  //void settm(int which, const nau::math::Matrix4 &tm) { settm_2(which, tm); }
  bool gettm(int which, nau::math::Matrix4 *out_tm) { return gettm_0(which, out_tm); }
  bool gettm(int which, nau::math::Matrix4 &out_tm) { return gettm_1(which, out_tm); }
  //void gettm(int which, nau::math::Matrix4 &out_tm) { gettm_2(which, out_tm); }
