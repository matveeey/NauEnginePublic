...
struct DeviceDriverCapabilitiesBase
{
   ...
   * \someNYI
   * \platformtable{hasQuadTessellation,c,a,c,a,c,c,c,c,c,c,r}
   */
  ...
  /**
   *\capbrief supports raytracing.
   * \someNYI
   * \platformtable{hasRaytracing,c,c,c,a,c,c,c,r,c,r,r}
   */
  bool hasRaytracing : 1;
  /**
   * \capbrief supports extended raytracing features, like inline raytracing (aka RayQuery).
   * \todo This may needs to be split into several caps, as it bundles a few features that may not be fully supported by all platforms.
   * \someNYI
   * \platformtable{hasRaytracingT11,c,c,c,a,r,r,c,r,r,r,r}
   */
  bool hasRaytracingT11 : 1;
  /**
   ...
   * \someNYI
   * \platformtable{hasResourceHeaps,c,a,c,a,c,c,c,c,c,c,r}
   */
   ...
   * \platformtable{hasNativeRenderPassSubPasses,c,a,c,a,c,c,c,c,r,c,r}
   */
...
struct DeviceDriverCapabilitiesXboxOne : DeviceDriverCapabilitiesBase
{
  ...
  //! \briefconstcap{false, DeviceDriverCapabilitiesBase::hasRaytracing}
  static constexpr bool hasRaytracing = false;
  //! \briefconstcap{false, DeviceDriverCapabilitiesBase::hasRaytracingT11}
  static constexpr bool hasRaytracingT11 = false;
...
struct DeviceDriverCapabilitiesScarlett : DeviceDriverCapabilitiesXboxOne
{
  ...
  //! \briefconstcap{true, DeviceDriverCapabilitiesBase::hasRaytracing}
  static constexpr bool hasRaytracing = true;
  //! \briefconstcap{true, DeviceDriverCapabilitiesBase::hasRaytracingT11}
  //! \warning Inline Raytracing is not directly supported, there is a software implementation available, see GDK documentation for
  //! details.
  static constexpr bool hasRaytracingT11 = true;
...
struct DeviceDriverIssuesBase
{
  ...
  /**
   * Some device drivers have some issues with shader byte code generated by our DXC shader compiler stack.
   * \note
   * Known on the following devices:
   * - \adreno with certain driver versions
   * \note
   * - \constissue{DeviceDriverIssuesNoIssues::hasLimitedDXCShaders}
   * - \runtimeissue{DeviceDriverIssuesAndroid, \android}
   */
  bool hasLimitedDXCShaders : 1;
  /**
  ...
  /**
   * Some device drivers may hang during rendering with multisampled rastering and blending enabled.
   * \note
   * Known for certain device driver combinations.
   * \note
   * - \constissue{DeviceDriverIssuesNoIssues::hasMultisampledAndBlendingHang}
   * - \runtimeissue{DeviceDriverIssuesAndroid, \android}
   */
...
/**
 * \brief Issues structure specific for \android. As \android may suffer from all of the currently known issues, there are no
 * optimizations.
 */
 ...
 */
struct DeviceDriverIssuesNoIssues : DeviceDriverIssuesBase
{
  /**
  ...
  /**
   * \brief Is constant true on \xbone, \scarlett, \ps4, \ps5, \ios, \tvos, \nswitch, \mac, \linux and \win32
   * \baseissue{DeviceDriverIssuesBase::hasLimitedDXCShaders}
   **/
  static constexpr bool hasLimitedDXCShaders = false;
  /**
   * \brief Is constant true on \xbone, \scarlett, \ps4, \ps5, \ios, \tvos, \nswitch, \mac, \linux and \win32
...
NAU_RENDER_EXPORT struct Driver3dDesc // -V730
{
  ...
  int minWarpSize; // if compute shaders are available. It is size of WARP, which is GPU SIMD size. Real HW value can be bigger;
  ...
  int raytraceTopAccelerationInstanceElementSize;
  // required for calculating size and offsets for shader binding table buffers
  // only valid when caps.hasRaytracing is set
  int raytraceShaderGroupSize;
  // how many rays can be invoked in each trace thread
  // only valid when caps.hasRaytracing is set
  int raytraceMaxRecursion;
