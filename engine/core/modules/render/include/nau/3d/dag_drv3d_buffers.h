...
{
  return d3d::create_sbuffer(CBUFFER_REGISTER_SIZE, registers_count, SBCF_CB_PERSISTENT, 0, reinterpret_cast<const char8_t*>(name)); // TODO: change all nau::string to eastl::string and all char8_t to char in render module
}
...
{
  return d3d::create_sbuffer(CBUFFER_REGISTER_SIZE, registers_count, SBCF_CB_ONE_FRAME, 0, reinterpret_cast<const char8_t*>(name));
}
...
inline Sbuffer *create_ua_sr_byte_address(uint32_t size_in_dwords, const char *name, Init buffer_init = Init::No)
{
  return d3d::create_sbuffer(BYTE_ADDRESS_ELEMENT_SIZE, size_in_dwords,
    SBCF_UA_SR_BYTE_ADDRESS | (buffer_init == Init::Zero ? SBCF_ZEROMEM : 0), 0, reinterpret_cast<const char8_t*>(name));
}
  ...
  Init buffer_init = Init::No)
{
  return d3d::create_sbuffer(structure_size, elements_count, SBCF_UA_SR_STRUCTURED | (buffer_init == Init::Zero ? SBCF_ZEROMEM : 0), 0,
      reinterpret_cast<const char8_t*>(name));
}
...
{
  return d3d::create_sbuffer(BYTE_ADDRESS_ELEMENT_SIZE, size_in_dwords, SBCF_UA_BYTE_ADDRESS, 0, reinterpret_cast<const char8_t*>(name));
}
...
{
  return d3d::create_sbuffer(structure_size, elements_count, SBCF_UA_STRUCTURED, 0, reinterpret_cast<const char8_t*>(name));
}
...
{
  return d3d::create_sbuffer(BYTE_ADDRESS_ELEMENT_SIZE, size_in_dwords,
    SBCF_UA_BYTE_ADDRESS_READBACK | (buffer_init == Init::Zero ? SBCF_ZEROMEM : 0), 0, reinterpret_cast<const char8_t*>(name));
}
  ...
  Init buffer_init = Init::No)
{
  return d3d::create_sbuffer(structure_size, elements_count,
    SBCF_UA_STRUCTURED_READBACK | (buffer_init == Init::Zero ? SBCF_ZEROMEM : 0), 0, reinterpret_cast<const char8_t*>(name));
}
...
inline uint32_t dword_count_per_call(Indirect indirect_type)
{
  ...
  NAU_ASSERT(false); // impossible situation
  return 0;
}
...
inline Sbuffer *create_ua_indirect(Indirect indirect_type, uint32_t records_count, const char *name)
{
  ...
  return d3d::create_sbuffer(BYTE_ADDRESS_ELEMENT_SIZE, dwordsCount, SBCF_UA_INDIRECT, 0, reinterpret_cast<const char8_t*>(name));
}
...
{
  const uint32_t dwordsCount = records_count * dword_count_per_call(indirect_type);
  return d3d::create_sbuffer(BYTE_ADDRESS_ELEMENT_SIZE, dwordsCount, SBCF_INDIRECT, 0, reinterpret_cast<const char8_t*>(name));
}
...
{
  return d3d::create_sbuffer(1, size_in_bytes, SBCF_STAGING_BUFFER, 0, reinterpret_cast<const char8_t*>(name));
}
...
inline Sbuffer *create_persistent_sr_tbuf(uint32_t elements_count, uint32_t format, const char *name, Init buffer_init = Init::No)
{
  return d3d::create_sbuffer(get_tex_format_desc(format).bytesPerElement, elements_count,
    SBCF_BIND_SHADER_RES | SBCF_CPU_ACCESS_WRITE | (buffer_init == Init::Zero ? SBCF_ZEROMEM : 0), format, reinterpret_cast<const char8_t*>(name));
}
...
{
  return d3d::create_sbuffer(BYTE_ADDRESS_ELEMENT_SIZE, size_in_dwords,
    SBCF_BIND_SHADER_RES | SBCF_CPU_ACCESS_WRITE | SBCF_MISC_ALLOW_RAW | (buffer_init == Init::Zero ? SBCF_ZEROMEM : 0), 0, reinterpret_cast<const char8_t*>(name));
}
...
{
  return d3d::create_sbuffer(structure_size, elements_count,
    SBCF_BIND_SHADER_RES | SBCF_CPU_ACCESS_WRITE | SBCF_MISC_STRUCTURED | (buffer_init == Init::Zero ? SBCF_ZEROMEM : 0), 0, reinterpret_cast<const char8_t*>(name));
}
...
{
  return d3d::create_sbuffer(get_tex_format_desc(format).bytesPerElement, elements_count,
    SBCF_BIND_SHADER_RES | SBCF_CPU_ACCESS_WRITE | SBCF_DYNAMIC | SBCF_FRAMEMEM, format, reinterpret_cast<const char8_t*>(name));
}
...
{
  return d3d::create_sbuffer(BYTE_ADDRESS_ELEMENT_SIZE, size_in_dwords,
    SBCF_BIND_SHADER_RES | SBCF_CPU_ACCESS_WRITE | SBCF_MISC_ALLOW_RAW | SBCF_DYNAMIC | SBCF_FRAMEMEM, 0, reinterpret_cast<const char8_t*>(name));
}
...
{
  return d3d::create_sbuffer(structure_size, elements_count,
    SBCF_BIND_SHADER_RES | SBCF_CPU_ACCESS_WRITE | SBCF_MISC_STRUCTURED | SBCF_DYNAMIC | SBCF_FRAMEMEM, 0, reinterpret_cast<const char8_t*>(name));
}
