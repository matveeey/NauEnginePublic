...
public:
  ...
  ~FencedRingBuffer() { NAU_ASSERT(usedSize == 0, "Ring buffer must be released"); }
  ...
  {
    NAU_ASSERT(head >= allocated_minus_actual_used && currentFrameSize >= allocated_minus_actual_used);
  ...
  void releaseCompletedFrames(IsFinishedFence finished)
  {
    ...
    while (!completedFrameHeads.empty() && finished(completedFrameHeads.front().fence))
    {
      ...
      NAU_ASSERT(oldestFrameHead.size <= usedSize);
