...
public:
  ...
  virtual int texbordercolor(nau::math::E3DCOLOR) = 0;
  ...
  const char8_t* getTexName() const { return getResName(); }
  virtual void setResApiName(const char8_t * /*name*/) const {}
  ...
  virtual bool addDirtyRect(const nau::math::RectInt &) { return false; };
  ...
  virtual bool downSize(int width, int height, int depth, int mips, unsigned start_src_level, unsigned level_offset)
  {
    ...
    unsigned int sourceLevel = std::max<unsigned int>(level_offset, start_src_level);
    unsigned int  sourceLevelEnd = std::min(uint32_t(selfInfo.mipLevels), mips + level_offset);
    ...
    for (; sourceLevel < sourceLevelEnd; sourceLevel++)
    {
      ...
      {
        rep->updateSubRegion(this, calcSubResIdx(sourceLevel, s, selfInfo.mipLevels), 0, 0, 0, std::max<int>(selfInfo.w >> sourceLevel, 1),
                               std::max<int>(selfInfo.h >> sourceLevel, 1), std::max<int>(selfInfo.d >> sourceLevel, selfInfo.a),
  ...
  virtual bool upSize(int width, int height, int depth, int mips, unsigned start_src_level, unsigned level_offset)
  {
    ...
    unsigned destinationLevelEnd = std::min<unsigned>(selfInfo.mipLevels + level_offset, mips);
    ...
    for (; destinationLevel < destinationLevelEnd; destinationLevel++)
    {
      for (int s = 0; s < selfInfo.a; s++)
      {
        rep->updateSubRegion(this, calcSubResIdx(destinationLevel - level_offset, s, selfInfo.mipLevels), 0, 0, 0,
          std::max<int>(width >> destinationLevel, 1), std::max<int>(height >> destinationLevel, 1),
          std::max<int>(depth >> destinationLevel, selfInfo.a), calcSubResIdx(destinationLevel, s, mips), 0, 0, 0);
      }
    }
...
NAU_RENDER_EXPORT uint32_t auto_mip_levels_count(uint32_t w, uint32_t h, uint32_t min_size);
...
NAU_RENDER_EXPORT const TextureFormatDesc &get_tex_format_desc(uint32_t fmt);
NAU_RENDER_EXPORT const char *get_tex_format_name(uint32_t fmt);
...
extern bool (*d3d_load_ddsx_tex_contents)(BaseTexture *tex, TEXTUREID tid, TEXTUREID paired_tid, const ddsx::Header &hdr,
                                          nau::iosys::IGenLoad &crd, int q_id, int start_lev, unsigned tex_ld_lev);
/// load texture content from DDSx stream using DDSx header to specified slice of previously allocated array texture
extern bool (*d3d_load_ddsx_to_slice)(BaseTexture *tex, int slice, const ddsx::Header &hdr, nau::iosys::IGenLoad &crd, int q_id, int start_lev,
  unsigned tex_ld_lev);
