...
//   bool differs(StLtData &lt, float thres)
// interpolates between 2 lights:
//   void interpolate(const StLtData &lt1, const StLtData &lt2, float a)
// interpolate in triangle:
//   void interpolate(const StLtData &lt0, const StLtData &lt1, const StLtData &lt2, float a10, float 20 )
//
...
public:
  ...
  bool getLight(const nau::math::Point3 &pos, LtData &lt);
  float getAltitude(const nau::math::Point3 &p);
...
protected:
  ...
  __forceinline float __getAltitude(const nau::math::Point3 &p);
...
template <class LtData, class StLtData, int NUM_HT_LEV>
float Lt2DMeshServer<LtData, StLtData, NUM_HT_LEV>::getAltitudeL(int level)
{
...
template <class LtData, class StLtData, int NUM_HT_LEV>
__forceinline float Lt2DMeshServer<LtData, StLtData, NUM_HT_LEV>::__getAltitude(const nau::math::Point3 &p)
{
  Point2 p2d;
  p2d.x = p.x;
  p2d.y = p.z;
  ...
  for (int i = 0; i < fn; i++)
  {
    ...
    float r2 = (p.x - f.c.x) * (p.x - f.c.x) + (p.z - f.c.z) * (p.z - f.c.z);
    if (r2 > f.cr2)
      continue;
...
//  /// /*-debug*/debug_ctx ( "lev=%d vn=%d fn=%d", lev, vn, fn);
//
//  crd.read(h_level, lev * sizeof(float));
...
template <class LtData, class StLtData, int NUM_HT_LEV>
bool Lt2DMeshServer<LtData, StLtData, NUM_HT_LEV>::getLight(const nau::math::Point3 &pos, LtData &lt)
{
  float dh, ah = 0.f;
...
template <class LtData, class StLtData, int NUM_HT_LEV>
float Lt2DMeshServer<LtData, StLtData, NUM_HT_LEV>::getAltitude(const nau::math::Point3 &p)
{
  float h = __getAltitude(p);
  if (__face_n == -1)
    return h;
  return p.y - h;
}
