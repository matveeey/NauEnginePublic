...
NAU_RENDER_EXPORT void init(int force_core_count = -1, bool reserve_jobmgr_cores = true);
//! terminate cpu jobs management system, optionally cancelling all pending jobs
NAU_RENDER_EXPORT void term(bool cancel_jobs, int timeout_msec = 10000);


//! returns cpujobs init status
NAU_RENDER_EXPORT int is_inited();


//! return number of logical (SMT/HT) CPU cores
NAU_RENDER_EXPORT int get_logical_core_count();
inline int get_core_count() { return get_logical_core_count(); } // compat alias

//! return number of physical CPU cores
NAU_RENDER_EXPORT int get_physical_core_count();

//! return number of logical CPU cores per Last Level Cache (typically L3).
NAU_RENDER_EXPORT int get_core_count_per_llc();

//! start job manager on specified Core. require reserve_jobmgr_cores = true passed in init
NAU_RENDER_EXPORT bool start_job_manager(int core_id, int stack_sz = 64 << 10, const char *thread_name = NULL,
...
NAU_RENDER_EXPORT bool stop_job_manager(int core_id, bool finish_jobs);
...
NAU_RENDER_EXPORT int create_virtual_job_manager(int stack_sz = 64 << 10, uint64_t affinity_mask = ~uint64_t(0), const char *thread_name = NULL,
  int thread_priority = DEFAULT_THREAD_PRIORITY,
...
//! no jobs can be added after manager is stopped;
//! processing of current job queue will be cancelled if finish_jobs==false
NAU_RENDER_EXPORT void destroy_virtual_job_manager(int vmgr_id, bool finish_jobs);


//! add job to job manager on specified Core or by virtual manager ID; returns false on error
NAU_RENDER_EXPORT bool add_job(int core_or_vmgr_id, IJob *job, bool prepend = false, bool need_release = true);

//! removes all queued jobs except for current; optionally calls release_done_jobs()
NAU_RENDER_EXPORT void reset_job_queue(int core_or_vmgr_id, bool auto_release_jobs = true);

//! removes tagged queued jobs except for current; optionally calls release_done_jobs()
NAU_RENDER_EXPORT void remove_jobs_by_tag(int core_or_vmgr_id, unsigned tag, bool auto_release_jobs = true);

//! returns true if job manager has active jobs
NAU_RENDER_EXPORT bool is_job_manager_busy(int core_or_vmgr_id);

//! returns true if job is found in manager's main queue
NAU_RENDER_EXPORT bool is_job_running(int core_or_vmgr_id, IJob *job);

//! returns true if job is found in manager's done queue
NAU_RENDER_EXPORT bool is_job_done(int core_or_vmgr_id, IJob *job);

//! returns thread id which belongs to specified job manager; use get_current_thread_id() to compare
NAU_RENDER_EXPORT int64_t get_job_manager_thread_id(int vmgr_id);


//! returns true when current thread is executing in context of any job manager
NAU_RENDER_EXPORT bool is_in_job();

//! returns true if job manager is to be cancelled soon
NAU_RENDER_EXPORT bool is_exit_requested(int core_or_vmgr_id);
...
NAU_RENDER_EXPORT void release_done_jobs();
} // namespace cpujobs
