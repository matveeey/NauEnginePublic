...
NAU_RENDER_EXPORT unsigned stackhlp_fill_stack(void **stack, unsigned max_size, int skip_frames = 0);

//! fills stack with specified call stack start addresses and returns number of stack frames;
//! no more than max_size addresses stored, the tail entry (if any space left) filled with 0xFFFFFFFF
NAU_RENDER_EXPORT unsigned stackhlp_fill_stack_exact(void **stack, unsigned max_size, const void *ctx_ptr);

//! returns pointer out_buf filled with decoded call stack
NAU_RENDER_EXPORT const char *stackhlp_get_call_stack(char *out_buf, int out_buf_sz, const void *const *stack, unsigned max_size);

//! returns string filled with decoded call stack
NAU_RENDER_EXPORT nau::string stackhlp_get_call_stack_str(const void *const *stack, unsigned max_size);

NAU_RENDER_EXPORT void *stackhlp_get_bp();
...
// value.
using CallStackCaptureCallback = CallStackResolverCallbackAndSizePair (*)(stackhelp::CallStackInfo stack, void *context);
...
struct CallStackContext
{
  ...
  CallStackResolverCallbackAndSizePair operator()(stackhelp::CallStackInfo stack) const
  {
    if (capture)
    {
...
NAU_RENDER_EXPORT CallStackContext set_extended_call_stack_capture_context(CallStackContext new_context);
// Invokes current set thread local extended call stack context call back.
NAU_RENDER_EXPORT CallStackResolverCallbackAndSizePair capture_extended_call_stack(CallStackInfo stack);
...
NAU_RENDER_EXPORT const char *get_call_stack(char *buf, unsigned max_buf, stackhelp::CallStackInfo stack,
...
NAU_RENDER_EXPORT nau::string get_call_stack_str(stackhelp::CallStackInfo stack, stackhelp::ext::CallStackInfo ext_stack);
} // namespace ext
} // namespace stackhelp
