...
struct FixedMoveOnlyFunctionBase
{
  ...
  template <typename F>
  static void relocateImpl(void *from, void *to) // TODO: check if its work
  {
    if (to != nullptr)
      new (to) F{(F &&) * static_cast<F *>(from)}; // _NEW_INPLACE
...
public:
  ...
  {
    NAU_ASSERT(call != nullptr, "Attempt to call an empty FixedMoveOnlyFunction!");
