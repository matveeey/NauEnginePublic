...
static inline const char *data_to_str_hex(S &dest, const void *src_data, size_t src_data_len)
{
  ...
  {
    int sprintf_result_len_ = snprintf((&dest[i * 2]), 3, "%02x", ((unsigned char *)src_data)[i]);
    NAU_ASSERT((unsigned)sprintf_result_len_ < (unsigned)3);
...
static inline int parse_string_list(eastl::vector<nau::string> &list, const char *str, const char *prefix = "", const char *suffix = "")
{
  list.clear();
  while (const char *p = strchr(str, '|'))
  {
    if (p - str > 0)
    {
        nau::string tString = nau::string::format("%s%.*s%s", prefix, p - str, str, suffix);
        list.push_back(tString);//.printf(0, "%s%.*s%s", prefix, p - str, str, suffix);
    }
    str = p + 1;
  }
  if (*str)
  {
      list.push_back(nau::string::format("%s%s%s", prefix, str, suffix));//.printf(0, "%s%s%s", prefix, str, suffix);
  }
  return list.size();
}

static inline nau::string urlencode(const nau::string &src)
{
  nau::string ret;
  char sym[8] = {0};
