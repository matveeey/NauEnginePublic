...
//   NAU_ASSERT(read_dump->InnerLayout[5].InnerInnerLayout[6].indices[7] == 42);
// ```
...
struct Context
{
  ...
  {
    NAU_ASSERT(!current_props.stream_offset && !current_props.wrapper_address && props_stack.empty());
    NAU_ASSERT(!worker && deferred_actions.empty());
    worker = &wk;
    worker->begin();
    enable();
  }
  ...
  {
    NAU_ASSERT(expected_offset >= actual_offset);
...
static void writeOffset(uint64_t offset)
{
  ...
  if (offset)
  {
    ...
    NAU_ASSERT(delta64 >= eastl::numeric_limits<int32_t>::min() && delta64 <= eastl::numeric_limits<int32_t>::max());
...
{
  NAU_ASSERT(context().offset_to_lists.empty());
...
protected:
  ...
  {
    NAU_ASSERT(mData.size() <= eastl::numeric_limits<int32_t>::max());
    return mData.size();
  }
...
class AddressMaster : public DeferredAction
{
  ...
  void deferredRead() override
  {
    ...
    NAU_ASSERT(delta % traits::sizeOf<UserDataType>() == 0);
...
struct Context : master::Context<StreamerType, WrapperAddressType>
{
  ...
  {
    NAU_ASSERT(address_to_offset.empty() && offset_to_address.empty() && mOffset == 0);
  ...
  ValueType remap(const eastl::map<KeyType, ValueType> &map, KeyType key)
  {
    ...
    --found;
    NAU_ASSERT(key >= found->first);
    uint64_t delta = key - found->first;
    NAU_ASSERT(delta % sizeof(Type) == 0);
...
public:
  ...
  {
    NAU_ASSERT(no < size(), "index is {}, size() is {}", no, size());
...
const EnableHash<LayoutClass, MAPPER> *map(const uint8_t *dump, const EnableHash<LayoutClass, MAPPER> *)
{
  ...
  {
    NAU_LOG_WARNING("The layout format does not match the dump format");
    return nullptr;
  }
  return mapped_data;
}
...
bool streamReadLayout(LayoutType &layout, IReader &reader)
{
  ...
  NAU_ASSERT(is_layout_valid, "The layout format does not match the dump format");
