...
struct TextureMetaData
{
  ...
  NAU_RENDER_EXPORT const char *encode(const char *fpath, nau::string *storage = NULL) const;
  ...
  NAU_RENDER_EXPORT const char *decode(const char *fstring, nau::string *storage = NULL);
  ...
  NAU_RENDER_EXPORT bool decodeData(const char *fstring, bool dec_bt_name = true);

  //! read metadata from DataBlock (single standard for texture asset metadata)
  NAU_RENDER_EXPORT void read(const nau::DataBlock &blk, const char *spec_target_str);

  //! write metadata to DataBlock (single standard for texture asset metadata)
  NAU_RENDER_EXPORT void write(nau::DataBlock &blk);


  //! returns converts d3d constant for logical tex filtering mode
  NAU_RENDER_EXPORT int d3dTexFilter() const;

  //! returns converts d3d constant for logical mip filtering mode
  NAU_RENDER_EXPORT int d3dMipFilter() const;


  //! converts logical addressing mode to d3d constant
  NAU_RENDER_EXPORT static int d3dTexAddr(unsigned addr);

  //! returns file path for given file string [uses highly temporary buffer when storage==NULL]
  NAU_RENDER_EXPORT static const char *decodeFileName(const char *fstring, nau::string *storage = NULL);
  ...
  NAU_RENDER_EXPORT static const char *decodeBaseTexName(const char *fstring);
};
