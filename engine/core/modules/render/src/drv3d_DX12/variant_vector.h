...
struct ExtendedVariant
{
  ...
  ExtendedVariant(T& inCmd, eastl::span<P0> inP0) :
      cmd(inCmd),
      p0(inP0)
  {
...
struct ExtendedVariant2
{
  ...
  ExtendedVariant2(T& inCmd, eastl::span<P0> inP0, eastl::span<P1> inP1) :
    cmd(inCmd),
    p0(inP0),
    p1(inP1)
  {
...
protected:
  ...
  static void copyConstructAt(DataStoreType *pos, const T &value)
  {
    ...
    static_assert(UsedTypeHandler::size != 0);
    MemoryWriter m{pos};
  ...
  static void moveConstructAt(DataStoreType *pos, T &&value)
  {
    ...
    using UsedTypeHandler = TypeHandler<DT, true>;
    // Prevents creation of meta types
    static_assert(UsedTypeHandler::is_valid);
    MemoryWriter m{pos};
    m.write<IndexType>(TypeIndexOf<DT, ThisTypesPack>::value);
...
private:
  ...
  void vectorGrow(size_t new_size)
  {
    ...
    size_t newSize = Vectormath::max(new_size, size * 2);
...
public:
  ...
  void resize(size_t ring_size, size_t appendix_numerator = 1, size_t appendix_denominator = 1)
  {
    ...
    {
      NAU_ASSERT(appendix_numerator > 0, "A memory appendix has to be allocated to handle wrapping "
                                        "correctly!");
      NAU_ASSERT(appendix_denominator > 0, "Denominator has to be greater than zero or division by "
                                          "zero!");
      NAU_ASSERT(allocationCount == freeCount && committedCount == freeCount, "You can not resize a "
                                                                             "ring buffer with "
                                                                             "active objects in "
                                                                             "it");
  ...
  size_t addressToIndex(const void *ptr) const
  {
    ...
    NAU_ASSERT(p >= store.get() && p < store.get() + size);
...
private:
  ...
  __forceinline size_t allocateSpace(size_t sz)
  {
    ...
    for (;;)
    {
      if ((calculateFreeSize(currentAllocationCount, freePos) >= sz)) //DAGOR_LIKELY
      {
        auto nextAllocationCount = currentAllocationCount + sz;
