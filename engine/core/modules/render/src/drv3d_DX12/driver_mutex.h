...
public:
  void lock()
  {
    ...
    NAU_ASSERT(true == enabled, "DX12: Trying to lock the driver context without enabling "
  ...
  {
#if DX12_ENABLE_PEDANTIC_MT_VALIDATION
    NAU_ASSERT(true == enabled, "DX12: Trying to unlock the driver context without enabling "
                               "multithreading first");
#endif
    --recursionCount;
    mutex.unlock();
