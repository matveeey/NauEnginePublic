...
public:
  ...
  void free(D3D12_CPU_DESCRIPTOR_HANDLE slot)
  {
    ...
    NAU_ASSERT("DX12: DescriptorHeap::free unable to find heap for descriptor");
  }
...
public:
  ...
  {
    NAU_ASSERT(static_cast<bool>(index));
...
public:
  ...
  {
    NAU_ASSERT(static_cast<bool>(range));
  ...
  {
    NAU_ASSERT(size < SAMPLER_HEAP_SIZE, "Can not reserve {} descriptors, because its more than a single "
                                        "heap can provide");
