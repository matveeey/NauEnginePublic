...
BEGIN_BITFIELD_TYPE(ImageViewState, uint64_t)
  ...
  D3D12_SHADER_RESOURCE_VIEW_DESC asSRVDesc(D3D12_RESOURCE_DIMENSION dim, bool is_multisampled) const
  {
    ...
    NAU_ASSERT(!fmt.isDepth() || !is_multisampled || d3d::get_driver_desc().caps.hasReadMultisampledDepth);
    ...
    switch (dim)
    {
      ...
      case D3D12_RESOURCE_DIMENSION_UNKNOWN: NAU_FAILURE("Usage error!"); return {};
  ...
  D3D12_DEPTH_STENCIL_VIEW_DESC asDSVDesc(D3D12_RESOURCE_DIMENSION dim, bool is_multisampled) const
  {
    ...
    switch (dim)
    {
      ...
      case D3D12_RESOURCE_DIMENSION_TEXTURE2D:
        ...
        break;
      case D3D12_RESOURCE_DIMENSION_TEXTURE3D: NAU_FAILURE("DX12: Volume depth stencil view not supported"); break;
    }
    return result;
  }
  ...
  {
    setMipBase(index);
    static_assert(1 == MIPMAP_RANGE_OFFSET);
    mipmapRange = 0;
  }
  ...
  {
    setArrayBase(index);
    static_assert(1 == ARRAY_RANGE_OFFSET);
    arrayRange = 0;
  }
