...
{
  // NAU_ASSERT(value < PRIM_TRIFAN, "primitive topology was {}", value);
#if _TARGET_XBOX
...
{
  static_assert(D3D12_SHADING_RATE_COMBINER_PASSTHROUGH == static_cast<uint32_t>(VariableRateShadingCombiner::VRS_PASSTHROUGH));
  static_assert(D3D12_SHADING_RATE_COMBINER_OVERRIDE == static_cast<uint32_t>(VariableRateShadingCombiner::VRS_OVERRIDE));
  static_assert(D3D12_SHADING_RATE_COMBINER_MIN == static_cast<uint32_t>(VariableRateShadingCombiner::VRS_MIN));
  static_assert(D3D12_SHADING_RATE_COMBINER_MAX == static_cast<uint32_t>(VariableRateShadingCombiner::VRS_MAX));
  static_assert(D3D12_SHADING_RATE_COMBINER_SUM == static_cast<uint32_t>(VariableRateShadingCombiner::VRS_SUM));
...
{
  NAU_ASSERT_RETURN(x <= 4 && y <= 4, D3D12_SHADING_RATE_1X1, "Variable Shading Rate can not exceed 4");
  NAU_ASSERT_RETURN(x != 3 && y != 3, D3D12_SHADING_RATE_1X1, "Variable Shading Rate can not be 3");
  NAU_ASSERT_RETURN(abs(int(x / 2) - int(y / 2)) < 2, D3D12_SHADING_RATE_1X1,
    "Variable Shading Rate invalid combination of x=%u and y=%u shading rates", x, y);
  static_assert(D3D12_SHADING_RATE_X_AXIS_SHIFT == 2);
  static_assert(D3D12_SHADING_RATE_VALID_MASK == 3);
