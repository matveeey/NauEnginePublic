...
inline void shorten_name_for_pix(nau::string &name)
{
...
public:
  ...
  explicit BufferGlobalId(uint32_t index) : value{index} { NAU_ASSERT((index == (index & index_mask))); }
...
struct BufferState
{
  ...
  ValueRange<uint64_t> usageRange() const 
  {
      return make_value_range<uint64_t>(offset, totalSize());
  }

  uint64_t currentOffset() const
  { 
      return offset + currentDiscardIndex * size;
  }
...
struct PipelineCreateInfoData : PipelineCreateInfoDataBase
{
  ...
  T &append(Args &&...args)
  {
    NAU_ASSERT(writeSize + sizeof(T) <= MAX_SIZE);
...
class BasePipeline
{
  ...
  struct BaseVariantKey
  {
    ...
    {
      NAU_ASSERT(il.get() < 0x80000);
      inputLayout = il.get();
    }

    void setStaticRenderState(StaticRenderStateID srs)
    {
      NAU_ASSERT(srs.get() < 0x80000);
...
public:
    ...
    program(GraphicsProgramID::Null()), signature(s), vsModule(vsm), psModule(psm)
  {
    ...
    if (cnt > 0)
    {
      ...
      NAU_LOG_VERBOSE("DX12: Graphics pipeline create found {} variants in cache, loading...", cnt);
  ...
  {
    NAU_ASSERT(!program, "Program ID must be set only once!");
    program = programId;
  }
    ...
    bool give_name, backend::PipelineNameGenerator &name_generator)
  {
...
#endif
      NAU_LOG_DEBUG("DX12: Rootsignature for {} was null, skipping creating compute pipeline", name);
      return true;
    }
    ...
    if (!pipeline)
    {
      ...
      if (FAILED(result) && cacheTarget.CachedBlobSizeInBytes > 0)
      {
        ...
        NAU_LOG_DEBUG("DX12: Retrying to create compute pipeline without cache");
        ...
        if (is_recoverable_error(result))
        {
          switch (on_error)
          {
            ...
            case RecoverablePipelineCompileBehavior::ASSERT_FAIL:
              NAU_ASSERT(!"CreatePipelineState failed, "
...
public:
  ...
  void registerStaticRenderState(StaticRenderStateID ident, const RenderStateSystem::StaticState &state)
  {
    ...
    {
      NAU_FAILURE("DX12: registerStaticRenderState was called with ident of {} but table has {} entries", ident.get(),
        staticRenderStateTable.size());
    }
    else if (staticRenderStateTable[ident.get()] != state)
    {
      NAU_LOG_DEBUG("DX12: need to patch render state {}", ident.get());
      ...
      if (end(staticRenderStateTable) == ref)
      {
        ...
        NAU_LOG_DEBUG("DX12: moving existing render state to {}", moveID.get());
      ...
      else
      {
        ...
        NAU_LOG_DEBUG("DX12: swapping existing render state with {}", swapID.get());
  ...
  eastl::vector<eastl::variant<eastl::monostate, ShaderID, ProgramID>> pixelShaderComputeProgramIDMap[max_scripted_shaders_bin_groups];
  ...
  void removeShaderGroup(uint32_t group, ProgramRecorder &&prog, GraphicsProgramRecorder &&g_prog)
  {
    ...
    {
      NAU_LOG_WARNING("Graphics pipelines are preloaded, but never used");
    }
  ...
  void compileGraphicsPipelineSet(ID3D12Device2 *device, PipelineCache &pipeline_cache, FramebufferLayoutManager &fbs,
    const DynamicArray<InputLayoutID> &input_layouts, const DynamicArray<StaticRenderStateID> &static_render_states,
    const DynamicArray<FramebufferLayout> &framebuffer_layouts, const DynamicArray<GraphicsPipelinePreloadInfo> &graphics_pipelines)
  {
    for (auto &pipeline : graphics_pipelines)
    {
      NAU_LOG_DEBUG("DX12: precomiling graphics pipeline...");

      char hashString[1 + 2 * sizeof(dxil::HashValue)];
      pipeline.base.vs.convertToString(hashString, countof(hashString));
      NAU_LOG_DEBUG("DX12: Looking for VS {}...", hashString);
      auto vsID = findVertexShader(pipeline.base.vs);
      if (ShaderID::Null() == vsID)
      {
        NAU_LOG_DEBUG("DX12: ...shader not found");
        continue;
      }
      pipeline.base.ps.convertToString(hashString, countof(hashString));
      NAU_LOG_DEBUG("DX12: Looking for PS {}...", hashString);
      ...
      NAU_LOG_DEBUG("DX12: looking for existing pipeline...");
      ...
      for (auto &preloadInfo : preloadedGraphicsPipelines)
      {
        ...
        NAU_LOG_DEBUG("DX12: ...found in preloadedGraphicsPipelines");
      ...
      if (!pipelineBase)
      {
        ...
        if (!pipelineBase)
        {
          ...
          NAU_LOG_DEBUG("DX12: ...none found, creating new...");
        }
        else
        {
          NAU_LOG_DEBUG("DX12: ...found loaded pipeline");
        }
      }
      if (!pipelineBase)
      {
      ...
      NAU_LOG_DEBUG("DX12: ...loading variants...");
      for (auto &variant : pipeline.variants)
      {
        ...
        {
          NAU_LOG_DEBUG("DX12: ...invalid state index, ignoring pipeline variant...");
          continue;
        }
        if (StaticRenderStateID::Null() == static_render_states[variant.staticRenderStateIndex])
        {
          NAU_LOG_DEBUG("DX12: ...unsupported render state, ignoring pipeline variant...");
          continue;
        }
        NAU_LOG_DEBUG("DX12: ...resolving input layout and output layout...");
        auto inputLayout = remapInputLayout(input_layouts[variant.inputLayoutIndex], pipelineBase->getVertexShaderInputMask());
        NAU_LOG_DEBUG("DX12: ...resolving frame buffer layout...");
        auto fbLayout = fbs.getLayoutID(framebuffer_layouts[variant.framebufferLayoutIndex]);
        NAU_LOG_DEBUG("DX12: ...compiling variant...");
      ...
      NAU_LOG_DEBUG("DX12: ...completed");
    }
  }

  void compileMeshPipelineSet(ID3D12Device2 *device, PipelineCache &pipeline_cache, FramebufferLayoutManager &fbs,
    const DynamicArray<StaticRenderStateID> &static_render_states, const DynamicArray<FramebufferLayout> &framebuffer_layouts,
    const DynamicArray<MeshPipelinePreloadInfo> &mesh_pipelines)
  {
    for (auto &pipeline : mesh_pipelines)
    {
      NAU_LOG_DEBUG("DX12: precomiling mesh pipeline...");
  ...
  void compileComputePipelineSet(ID3D12Device2 *device, PipelineCache &pipeline_cache,
    const DynamicArray<ComputePipelinePreloadInfo> &compute_pipelines)
  {
    for (auto &pipeline : compute_pipelines)
    {
      NAU_LOG_DEBUG("DX12: precomiling compute pipeline...");
      char hashString[1 + 2 * sizeof(dxil::HashValue)];
      pipeline.base.hash.convertToString(hashString, countof(hashString));
      NAU_LOG_DEBUG("DX12: Looking for CS {}...", hashString);
      bool found = false;
      enumerateShaderFromHash(pipeline.base.hash, [device, &pipeline_cache, &found, this](auto gi, auto si, auto vs_count) {
        ...
        {
          NAU_LOG_DEBUG("DX12: ...already loaded...");
          // already loaded, so we are done
          found = true;
          return false;
        }
        NAU_LOG_DEBUG("DX12: ...loading...");
      ...
      {
        NAU_LOG_DEBUG("DX12: ...not found");
      }
    }
  }

  void compilePipelineSet(ID3D12Device2 *device, PipelineCache &pipeline_cache, FramebufferLayoutManager &fbs,
    DynamicArray<InputLayoutID> &&input_layouts, DynamicArray<StaticRenderStateID> &&static_render_states,
  ...
  {
    compileGraphicsPipelineSet(device, pipeline_cache, fbs, input_layouts, static_render_states, framebuffer_layouts,
      graphics_pipelines);
...
struct NullResourceTable
{
  ...
  D3D12_CPU_DESCRIPTOR_HANDLE get(D3D_SHADER_INPUT_TYPE type, D3D_SRV_DIMENSION dim) const
  {
    ...
    {
      NAU_FAILURE("unexpected descriptor lookup");
    }
    ...
    else if (D3D_SIT_TEXTURE == type)
    {
      switch (dim)
      {
        ...
        case D3D_SRV_DIMENSION_TEXTURE2DMSARRAY: NAU_FAILURE("unexpected descriptor lookup"); break;
    ...
    else if (D3D_SIT_UAV_RWTYPED == type)
    {
      switch (dim)
      {
        ...
        case D3D_SRV_DIMENSION_TEXTURECUBEARRAY: NAU_FAILURE("unexpected descriptor lookup"); break;
    ...
    {
      NAU_FAILURE("Use of invalid resource type D3D_SIT_UAV_APPEND_STRUCTURED");
    }
    ...
    {
      NAU_FAILURE("ray trace acceleration structure was not set!");
    }
    return {0};
  }
};
...
struct PipelineStageStateBase
{
  ...
  uint32_t cauclateBRegisterDescriptorCount(uint32_t b_register_mask) const { return nau::math::__popcount(b_register_mask); }
  uint32_t cauclateTRegisterDescriptorCount(uint32_t t_register_mask) const { return nau::math::__popcount(t_register_mask); }
  uint32_t cauclateURegisterDescriptorCount(uint32_t u_register_mask) const { return nau::math::__popcount(u_register_mask); }
