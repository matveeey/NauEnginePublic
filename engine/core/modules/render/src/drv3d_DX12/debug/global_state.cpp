...
void debug::GlobalState::setup(const nau::DataBlock *settings, Direct3D12Enviroment &d3d_env)
{
  ...
  {
    NAU_LOG_DEBUG("DX12: Debugging is turned off");
    return;
  }
  ...
  {
    NAU_LOG_DEBUG("DX12: DXGIGetDebugInterface1 for IDXGIDebug...");
    HRESULT hr = DXGIGetDebugInterface1(0, COM_ARGS(&dxgiDebug));
    NAU_LOG_DEBUG("DX12: DXGIGetDebugInterface1 returns {:#08x}", hr);
  }
    ...
    if (config.anyValidation())
    {
      ...
      if (SUCCEEDED(D3D12GetDebugInterface(COM_ARGS(&debug0))))
      {
        ...
        if (SUCCEEDED(debug0.As(&debug1)))
        {
          ...
          {
            NAU_LOG_DEBUG("DX12: GPU based validation enabled");
            NAU_LOG_DEBUG("DX12: NONE: GPU based validation may induce TDRs, you might need to adjust "
      ...
      {
        NAU_ASSERT(false, "DX12: User requested debug layer to be enabled, but it seems not to be installed.");
        NAU_LOG_DEBUG("DX12: No validation layer available");
  ...
  {
    NAU_LOG_DEBUG("DX12: d3d12.dll does not export D3D12GetDebugInterface");
    config.enableCPUValidation = false;
    config.enableGPUValidation = false;
  }
}
