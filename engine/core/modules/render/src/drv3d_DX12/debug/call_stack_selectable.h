...
public:
  void configure(const nau::DataBlock *api_config)
  {
    ...
    if (eastl::end(modes) != ref)
    {
      ...
      NAU_LOG_DEBUG("DX12: debug::call_stack::selectable using mode '{}'", ref->name);
    }
    else
    {
      NAU_LOG_DEBUG("DX12: debug::call_stack::selectable invalid mode '{}' from {}/debug/call_stack/mode:t=, using default {}", modeName,
...
public:
  ...
  nau::string_view resolve(const CommandData &data)
  {
    switch (data.activeMember)
    {
