...
#include <drv3d_commonCode/validationLayer.h>
  ...
  LPCSTR description, void *context)
{
  ...
  switch (severity)
  {
    ...
    case D3D12_MESSAGE_SEVERITY_ERROR: NAU_LOG_ERROR("DX12: [#{}] {}", (int)id, description); break;
    case D3D12_MESSAGE_SEVERITY_WARNING: NAU_LOG_VERBOSE("DX12: [#{}] {}", (int)id, description); break;
    case D3D12_MESSAGE_SEVERITY_INFO:
    case D3D12_MESSAGE_SEVERITY_MESSAGE: NAU_LOG_VERBOSE("DX12: [#{}] {}", (int)id, description); break;
  }
}
...
bool debug::pc::DeviceState::setup(debug::GlobalState &global, ID3D12Device *device, const Direct3D12Enviroment &d3d_env)
{
  ...
  NAU_LOG_DEBUG("DX12: Trying to acquire debug message log...");
  if (SUCCEEDED(device->QueryInterface(COM_ARGS(&debugQueue))))
  {
    // TODO: make this setup configurable
    NAU_LOG_DEBUG("DX12: Success, reporting queued debug messages on frame end and reset errors");
    ...
    denyId.push_back(D3D12_MESSAGE_ID_CLEARRENDERTARGETVIEW_MISMATCHINGCLEARVALUE);
    denyId.push_back(D3D12_MESSAGE_ID_CREATE_COMMANDLIST12);
    denyId.push_back(D3D12_MESSAGE_ID_DESTROY_COMMANDLIST12);
  ...
  else
  {
    ...
    {
      NAU_ASSERT(false, "DX12: User requested debug layer to be enabled, but it is not available.");
    }
    NAU_LOG_DEBUG("DX12: Failed, no debug messages are reported to the log...");
  }
...
void debug::pc::DeviceState::onDeviceRemoved(D3DDevice *device, HRESULT remove_reason)
{
  ...
  {
    NAU_LOG_DEBUG("DX12: Removed reason was DXGI_ERROR_INVALID_CALL, no postmortem data collected. "
...
void debug::pc::DeviceState::sendGPUCrashDump(const char *type, const void *data, uintptr_t size)
{
  ...
  {
    NAU_LOG_ERROR("DX12: Tried to send unrecognized GPU crash dump type <{}>", type);
  }
}
