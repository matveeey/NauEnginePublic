...
public:
  ...
  CommandListTraceRecorder(ID3D12Device3 *device)
  {
    ...
    if (!traceMemory)
    {
      NAU_LOG_DEBUG("DX12: VirtualAlloc of {} bytes failed, can not record GPU trace", D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT);
      return;
    }

    if (FAILED(device->OpenExistingHeapFromAddress(traceMemory.get(), COM_ARGS(&traceMemoryHeap))))
    {
      NAU_LOG_DEBUG("DX12: OpenExistingHeapFromAddress failed, can not record GPU trace");
      traceMemory.reset();
      return;
    }
    ...
    {
      NAU_LOG_DEBUG("DX12: CreatePlacedResource failed, can not record GPU trace");
    ...
    NAU_LOG_DEBUG("DX12: New GPU trace recording buffer, memory at {}, heap {}, buffer {} at GPU address {}", (void*)traceMemory.get(),
      (void*)traceMemoryHeap.Get(), (void*)traceBuffer.Get(), traceBufferAddress);
  }
