  ...
  const Direct3D12Enviroment &d3d_env)
{
  ...
  {
    NAU_LOG_DEBUG("DX12: ...DRED is not allowed per configuration...");
    return false;
  }
  ...
  if (!config.ignoreDREDAvailability)
  {
    if (!is_dred_avilable())
    {
      NAU_LOG_DEBUG("DX12: ...after registry inspection, it seems DRED is not available, DRED is ignored...");
      return false;
    }
  }
  else
  {
    NAU_LOG_DEBUG("DX12: ...skipped DRED availability inspection of the registry, assuming it is available...");
  }
  ...
  {
    NAU_LOG_DEBUG("DX12: ...unable to query DRED settings interface, assuming it is unavailable...");
    return false;
  }
...
{
  NAU_LOG_DEBUG("DX12: Acquiring breadcrumb information from DRED...");
  D3D12_DRED_AUTO_BREADCRUMBS_OUTPUT breacrumbInfo = {};
  if (FAILED(dred->GetAutoBreadcrumbsOutput(&breacrumbInfo)))
  {
    NAU_LOG_DEBUG("DX12: ...failed, no breadcrumb data available");
    return;
  }
  ...
  call_stack::Reporter &reporter)
{
  if (!node)
  {
    NAU_LOG_DEBUG("No breadcrumb nodes found");
  }
  ...
  for (; node; node = node->pNext)
  {
    ...
    {
      NAU_LOG_DEBUG("Command Buffer was executed without error");
      NAU_LOG_DEBUG("Command Buffer: {:p}", (void*)node->pCommandList);
      report_alternate_name("Name: %s", node->pCommandListDebugNameA, node->pCommandListDebugNameW);
      NAU_LOG_DEBUG("Command Queue: {:p}", (void*)node->pCommandQueue);
    ...
    {
      NAU_LOG_DEBUG("Command Buffer execution was likely not started yet");
      NAU_LOG_DEBUG("Command Buffer: {:p}", (void*)node->pCommandList);
      report_alternate_name("Name: %s", node->pCommandListDebugNameA, node->pCommandListDebugNameW);
      NAU_LOG_DEBUG("Command Queue: {:p}", (void*)node->pCommandQueue);
      report_alternate_name("Name: %s", node->pCommandQueueDebugNameA, node->pCommandQueueDebugNameW);
      NAU_LOG_DEBUG("Breadcrumb count: {}", node->BreadcrumbCount);
      NAU_LOG_DEBUG("Last breadcrumb value: {}", *node->pLastBreadcrumbValue);
    ...
    else
    {
      ...
      NAU_LOG_DEBUG("Command Buffer execution incomplete");
      NAU_LOG_DEBUG("Command Buffer: {:p}", (void*)node->pCommandList);
      report_alternate_name("Name: %s", node->pCommandListDebugNameA, node->pCommandListDebugNameW);
      NAU_LOG_DEBUG("Command Queue: {:p}", (void*)node->pCommandQueue);
      report_alternate_name("Name: %s", node->pCommandQueueDebugNameA, node->pCommandQueueDebugNameW);
