...
{
public:
  void configure() { NAU_LOG_DEBUG("DX12: ...no frame capturing tool is active..."); }
  void beginCapture() {}
  void endCapture() {}
  void onPresent() {}
...
public:
  ...
  {
    NAU_LOG_DEBUG("DX12: Looking for RenderDoc capture attachment...");
    auto iface = try_connect_interface();
    if (!iface)
    {
      NAU_LOG_DEBUG("DX12: ...nothing found");
      return false;
    }

    NAU_LOG_DEBUG("DX12: ...found, using RenderDoc interface for frame capturing");
...
public:
  ...
  {
    NAU_LOG_DEBUG("DX12: Looking for legacy PIX capture attachment...");
    ...
    NAU_LOG_DEBUG("DX12: ...found, using legacy PIX interface for frame capturing");
...
public:
  ...
  static bool connect(const Configuration &, Direct3D12Enviroment &, T &target)
  {
    NAU_LOG_DEBUG("DX12: Loading PIX runtime library...");
    auto runtime = try_load_runtime_interface();
    if (!runtime)
    {
      NAU_LOG_DEBUG("DX12: ...failed, using fallback event and marker reporting...");
    }
    NAU_LOG_DEBUG("DX12: Looking for PIX capture attachment...");
    ...
    NAU_LOG_DEBUG("DX12: ...found, using PIX interface for frame capturing");
  ...
  static bool load(const Configuration &, Direct3D12Enviroment &, T &target)
  {
    ...
    NAU_LOG_DEBUG("DX12: Loading PIX capture interface...");
    ...
    NAU_LOG_DEBUG("DX12: ...success, using PIX interface for frame capturing");
    target = PIX{eastl::move(runtime), eastl::move(capture)};
    return true;
  }
};
...
public:
  ...
  static bool connect(const Configuration &, Direct3D12Enviroment &, T &target)
  {
    NAU_LOG_DEBUG("DX12: Looking for Nvidia NSight...");
    if (!try_connect_interface())
    {
      NAU_LOG_DEBUG("DX12: ...nothing found");
      return false;
    }
    NAU_LOG_DEBUG("DX12: ...found, using fallback event and marker reporting...");
    target = NSight{};
    return true;
  }
};
