...
ImageCreateResult TextureImageFactory::createTexture(DXGIAdapter *adapter, ID3D12Device *device, const ImageInfo &ii, const char8_t *name)
{
  ...
  if (ii.memoryClass != DeviceMemoryClass::RESERVED_RESOURCE)
  {
    ...
    if (!memory)
    {
      ...
      if (!texture)
      {
        NAU_LOG_WARNING("DX12: Unable to allocate committed resource");
        return result;
      }
      ...
      result.image = newImageObject(ResourceMemory{}, eastl::move(texture), ii.type, ii.memoryLayout, ii.format, ii.size, ii.mips,
        ii.arrays, subResIdBase, nau::math::get_log2i_of_pow2(ii.sampleDesc.Count));
    ...
    else
    {
      ...
      if(ii.format.isDepth())
      {
          clearValuePtr = &clearValue;
          clearValue.Format = ii.format.asLinearDxGiFormat();
      ...
      {
          clearValuePtr = &clearValue;
          clearValue.Format = ii.format.asDxGiFormat();
      ...
      errorCode =
        ...
        DX12_CHECK_RESULT_NO_OOM_CHECK(
          device->CreatePlacedResource(memory.getHeap(), memory.getOffset(), &desc, result.state, clearValuePtr, COM_ARGS(&texture)));
#endif
      ...
      result.image = newImageObject(memory, eastl::move(texture), ii.type, ii.memoryLayout, ii.format, ii.size, ii.mips, ii.arrays,
        subResIdBase, nau::math::get_log2i_of_pow2(ii.sampleDesc.Count));
...
Image *TextureImageFactory::adoptTexture(ID3D12Resource *texture, const char8_t *name)
{
  ComPtr<ID3D12Resource> texture_ref{texture};

  // TODO: this works only 2D
  D3D12_RESOURCE_DESC desc = texture->GetDesc();
  NAU_ASSERT(D3D12_RESOURCE_DIMENSION_BUFFER != desc.Dimension);
  NAU_ASSERT(D3D12_RESOURCE_DIMENSION_TEXTURE3D != desc.Dimension);
  ...
  for (auto texture : textures)
  {
    ...
    texture->getDebugName([this, texture](auto &name) {
      recordTextureFreed(texture->getMipLevelRange(), texture->getArrayLayers(), texture->getBaseExtent(),
        !texture->isAliased() ? texture->getMemory().size() : 0, texture->getFormat(), (const char*)name.c_str());
    });
  }
...
void AliasHeapProvider::freeUserHeap(ID3D12Device *device, ::ResourceHeap *ptr)
{
  ...
  auto aliasHeapsAccess = aliasHeaps.access();
  NAU_ASSERT(index < aliasHeapsAccess->size(), "DX12: Tried to free non existing resource heap {:p}", (void*)ptr);
  if (index >= aliasHeapsAccess->size())
  {
    NAU_LOG_ERROR("DX12: Tried to free non existing resource heap {:p}", (void*)ptr);
    return;
  }
  auto &heap = (*aliasHeapsAccess)[index];
  if (!heap)
  {
    NAU_LOG_ERROR("DX12: Tried to free already freed resource heap {:p}", (void*)ptr);
    return;
  }
  ...
  NAU_ASSERT(heap.images.empty() && heap.buffers.empty(), "DX12: Resources of a resource heap should be destroyed before the heap is "
                                                         "destroyed");
#ifdef _TARGET_XBOX
...
ResourceAllocationProperties AliasHeapProvider::getResourceAllocationProperties(ID3D12Device *device, const ResourceDescription &desc)
{
  ...
  if (is_valid_allocation_info(allocInfo))
  {
    static_assert(sizeof(ResourceAllocationProperties::heapGroup) >= sizeof(ResourceHeapProperties::raw));
...
ImageCreateResult AliasHeapProvider::placeTextureInHeap(ID3D12Device *device, ::ResourceHeap *heap, const ResourceDescription &desc,
  size_t offset, const ResourceAllocationProperties &alloc_info, const char8_t *name)
{
  ImageCreateResult result{};
  auto dxDesc = as_desc(desc);
  ...
  switch (desc.asBasicTexRes.activation)
  {
    ...
    case ResourceActivationAction::DISCARD_AS_UAV:
      NAU_ASSERT(TEXCF_UNORDERED & desc.asBasicRes.cFlags);
    ...
    case ResourceActivationAction::DISCARD_AS_RTV_DSV:
      NAU_ASSERT(TEXCF_RTARGET & desc.asBasicRes.cFlags);
      if (fmt.isColor())
      {
  ...
  result.image = newImageObject(memory, eastl::move(texture), dxDesc.Dimension, dxDesc.Layout, fmt, ext, mipMapCount, arrayLayerCount,
    subResIdBase, nau::math::get_log2i_of_pow2(dxDesc.SampleDesc.Count));
...
ResourceHeapGroupProperties AliasHeapProvider::getResourceHeapGroupProperties(::ResourceHeapGroup *heap_group)
{
  ...
  if (isUMASystem())
  {
    ...
    result.maxHeapSize = Vectormath::max(getHostLocalPhysicalLimit(), getDeviceLocalPhysicalLimit());
  }
  else
  {
  ...
  result.optimalMaxHeapSize = Vectormath::min(result.maxHeapSize, optimal_max_heap_size);
  return result;
}
#else
ResourceHeapGroupProperties AliasHeapProvider::getResourceHeapGroupProperties(::ResourceHeapGroup *heap_group)
...
bool AliasHeapProvider::detachTextures(eastl::span<Image *> textures)
{
  ...
  for (auto texture : textures)
  {
    ...
    if (heapId.isAlias)
    {
      NAU_ASSERT(heapId.index < aliasHeapsAccess->size());
      if (heapId.index < aliasHeapsAccess->size())
      {
        ...
        NAU_ASSERT(ref != end(heap.images));
...
bool AliasHeapProvider::detachBuffer(const BufferState &buf)
{
  ...
  if (heapID.isAlias)
  {
    ...
    if (heapID.index < aliasHeapsAccess->size())
    {
      ...
      NAU_ASSERT(ref != end(heap.buffers));
...
ImageCreateResult AliasHeapProvider::aliasTexture(ID3D12Device *device, const ImageInfo &ii, Image *base, const char8_t *name)
{
  ImageCreateResult result{};
  auto desc = ii.asDesc();
  ...
  if (!baseMemory)
  {
    base->getDebugName([name](auto &base_name) {
      NAU_LOG_WARNING("DX12: Can not alias textures '{}' with '{}'. No heap associated with '{}'. Possibly a committed resource.", base_name,
        name, base_name);
    });
    return result;
  }
  ...
  if (baseMemoryProperties != memoryProperties)
  {
    base->getDebugName([name, baseMemoryProperties, memoryProperties](auto &base_name) {
      NAU_LOG_WARNING("DX12: Can not alias textures '{}' with '{}' as the memory properties {} and {} are "
              "incompatible",
    ...
    "DX12: OOM during %s for <%s>", "aliasTexture", name);
  {
    ...
    {
      NAU_LOG_WARNING("DX12: Tried to create aliasing texture with insufficient memory from base, base "
    ...
    {
      NAU_LOG_WARNING("DX12: Tried to create aliasing texture on insufficient memory region of heap, "
...
void DebugViewBase::storeViewSettings()
{
  ...
  if (!blk)
  {
    NAU_LOG_DEBUG("DX12: ResourceMemoryHeap::storeViewSettings 'imgui_get_blk' returned null");
    return;
  }
  ...
  if (!dx12Block)
  {
    NAU_LOG_DEBUG("DX12: ResourceMemoryHeap::storeViewSettings unable to write dx12 block");
    return;
  }
  ...
  if (!memoryBlock)
  {
    NAU_LOG_DEBUG("DX12: ResourceMemoryHeap::storeViewSettings unable to write memory block");
    return;
  }
...
void DebugViewBase::restoreViewSettings()
{
  ...
  if (!blk)
  {
    NAU_LOG_DEBUG("DX12: ResourceMemoryHeap::restoreViewSettings 'imgui_get_blk' returned null");
    return;
  }
  ...
  if (!dx12Block)
  {
    NAU_LOG_DEBUG("DX12: ResourceMemoryHeap::restoreViewSettings unable to read dx12 block");
    return;
  }
  ...
  if (!memoryBlock)
  {
    NAU_LOG_DEBUG("DX12: ResourceMemoryHeap::restoreViewSettings unable to read memory block");
    return;
  }
  ...
  if (filter)
  {
    ...
    {
      NAU_LOG_ERROR("DX12: Restore of dx12/memory/event-object-name-filter with malicious string");
    }
    else
    {
...
void MetricsVisualizer::drawMetricsEvnetsViewFilterControls()
{
  ...
  if (begin_sub_section("DX12-Live-Metrics-Event-Filter-Controls", "Events view filters", child_height))
  {
    ...
    ImGui::TextUnformatted("Frame range filter Vectormath::min / max. Setting a slider to max will pin it to "
...
void DebugView::drawESRamInfoView()
{
        ...
        ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_Resizable | ImGuiTableFlags_ScrollY))
  {
    ...
    dag::CSAutoLock lock{esramMutex};
...
struct ResourceHeapVisitor
{
  ...
  {
    str = nau::string::format(128, "Heap Group %08X (%s, %s%s) with %d heaps\n", ident, is_cpu_visible ? "CPU visible" : "dedicated GPU",
  ...
  void visitHeap(ByteUnits total_size, ByteUnits free_size, uint32_t fragmentation_percent)
  {
    ...
    freeSize += free_size;
    str = nau::string::format(128, "Size %6.2f %7s Free %6.2f %7s, %3u%% fragmentation\n", total_size.units(), total_size.name(), free_size.units(),
  ...
  void visitHeapFreeRange(ValueRange<uint64_t> range)
  {
    ...
    str = nau::string::format(128, "DX12: Free segment at %016llX with %6.2f %7s\n", range.front(), sizeBytes.units(), sizeBytes.name());
  }
};
} // namespace

void ResourceMemoryHeap::generateResourceAndMemoryReport(uint32_t *num_textures, uint64_t *total_mem, nau::string *out_text)
{
  if (num_textures)
    *num_textures = 0;
  if (total_mem)
  ...
  if (out_text)
  {
    ...
    *out_text = nau::string::format(128, "%u resource heaps, with %6.2f %7s in total and %6.2f %7s free\n", totalHeapCount,
    ...
    out_text->append(nau::string::format(128, "%6.2f %7s push ring buffers\n", pushRingSize.units(), pushRingSize.name()));
    out_text->append(nau::string::format(128, "%6.2f %7s upload ring buffers\n", uploadRingSize.units(), uploadRingSize.name()));
    out_text->append(nau::string::format(128, "%6.2f %7s temporary upload buffers\n", tempUploadSize.units(), tempUploadSize.name()));
    out_text->append(nau::string::format(128, "%6.2f %7s persistent upload buffers\n", persistUploadSize.units(), persistUploadSize.name()));
    out_text->append(nau::string::format(128, "%6.2f %7s persistent read back buffers\n", persistReadBackSize.units(), persistReadBackSize.name()));
    out_text->append(nau::string::format(128, "%6.2f %7s persistent bidirectional buffers\n", persistBidirSize.units(), persistBidirSize.name()));
      ...
      visitBufferObjects([out_text, &bufferTotalSize, &bufferSysCopySize, &sbufferCount](auto buffer) //
        {
          ...
          out_text->append(nau::string::format(128, "%6.2f %7s %s '%s' buffer%s\n", size.units(), size.name(),
            buffer->getFlags() & SBCF_DYNAMIC ? "dynamic" : "static", buffer->getBufName(),
            buffer->hasSystemCopy() ? "(has copy)" : ""));
        });
      out_text->append(nau::string::format(256, "%u buffers, with %6.2f %7s buffer memory and %6.2f %7s system memory\n", sbufferCount,
    ...
    visitImageObjects([out_text, &totalImageResourcesSize, &imageCount, &imageAliases](auto image) //
      {
        ...
        {
          ++imageAliases;
          image->getDebugName([&out_text](auto &name) { out_text->append(nau::string::format(128, "%14s '%s'\n", "aliased", name)); });
        }
        else
        {
          ...
          image->getDebugName(
            [&out_text, size](auto &name) { out_text->append(nau::string::format(128, "%6.2f %7s '%s'\n", size.units(), size.name(), name)); });
        }
        ++imageCount;
      });
    out_text->append(nau::string::format(128, "%u textures, with %6.2f %7s plus %u aliased textures\n", imageCount, totalImageResourcesSize.units(),
        ...
        if (auto img = tex->getDeviceImage())
        {
          ...
          switch (tex->resType)
          {
            case RES3D_TEX:
              ...
              if (out_text)
              {
                out_text->append(nau::string::format(0,
                  "%6.2f %7s q%d  2d  %4dx%-4d, m%2d, aniso=%-2d filter=%7s mip=%s, %s "
                  "- '%s'",
                  imageBytes.units(), imageBytes.name(), (int)ql, tex->width, tex->height, tex->level_count(), tex->samplerState.getAniso(),
                  filter_type_to_string(tex->samplerState.getMinFilter()), filter_type_to_string(tex->samplerState.getMip()),
            ...
            case RES3D_CUBETEX:
              ...
              if (out_text)
              {
                out_text->append(nau::string::format(0, "%6.2f %7s q%d cube %4d, m%2d, %s - '%s'", imageBytes.units(), imageBytes.name(), (int)ql, tex->width,
            ...
            case RES3D_VOLTEX:
              ...
              if (out_text)
              {
                out_text->append(nau::string::format(0,
                  ...
                  "%s - '%s'",
                  imageBytes.units(), imageBytes.name(), (int)ql, tex->width, tex->height, tex->depth, tex->level_count(),
                  tex->samplerState.getAniso(), filter_type_to_string(tex->samplerState.getMinFilter()),
  ...
  if (out_text)
  {
    out_text->append(nau::string::format(256,
...
void OutOfMemoryRepoter::reportOOMInformation()
{
  if (!didReportOOM)
  {
    ...
    static_assert((eastl::is_base_of<OutOfMemoryRepoter, ResourceMemoryHeap>::value));
    ...
    NAU_LOG_DEBUG("GPU MEMORY STATISTICS:\n======================");
    NAU_LOG_DEBUG(texStat.c_str());
    NAU_LOG_DEBUG("======================\nEND GPU MEMORY STATISTICS");
  }
  else
  {
    NAU_LOG_DEBUG("GPU MEMORY STATISTICS: not reported as this is not the first time.");
  }
}
