...
public:
  ...
  FormatStore getDepthStencilFormat() const { return FormatStore::fromCreateFlags(TEXFMT_DEPTH32); }
...
{
    PresentationMode presentMode = PresentationMode::VSYNCED;
public:
    uint16_t currentColorTargetIndex = 0;
    uint16_t previousColorTargetIndex = 0;
...
public:
    ...
    {
        return numFramesPresentedByFrontend < numPresentSentToBackend;
    }

    void signalPresentSentToBackend()
    {
        numPresentSentToBackend++;
    }
    ...
    bool setup(Device& device, DXGIFactory* factory, SwapchainInitParameter* queue, SwapchainCreateInfo&& sci);

    void onFrameBegin();
    void present(Device& device);

    BaseTex* getColorTexture();
    Image* getColorImage();
