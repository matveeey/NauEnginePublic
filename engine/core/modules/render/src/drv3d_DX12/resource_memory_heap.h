...
#if REPORT_HEAP_INFO
#define HEAP_LOG(...) NAU_LOG_DEBUG(__VA_ARGS__)
#else
#define HEAP_LOG(...)
#endif
...
public:
  ImageCreateResult createTexture(DXGIAdapter *adapter, ID3D12Device *device, const ImageInfo &ii, const char8_t *name);
  Image *adoptTexture(ID3D12Resource *texture, const char8_t *name);
...
protected:
  ...
  ContainerMutexWrapper<eastl::vector<AliasHeap>, nau::threading::SpinLock> aliasHeaps;
...
public:
  ...
  ImageCreateResult aliasTexture(ID3D12Device *device, const ImageInfo &ii, Image *base, const char8_t *name);
...
protected:
  ...
  struct TempScratchBufferState
  {
    ...
    bool ensureSize(DXGIAdapter *adapter, ID3D12Device *device, size_t size, size_t alignment, ScratchBufferProvider *heap)
    {
      ...
      auto nextSize = Vectormath::max<size_t>(scartch_buffer_min_size, buffer.bufferMemory.size() * 2);
      // out of space
      if (buffer)
      {
  ...
  ContainerMutexWrapper<TempScratchBufferState, nau::threading::SpinLock> tempScratchBufferState;
...
class SamplerDescriptorProvider : public ScratchBufferProvider
{
  ...
  ContainerMutexWrapper<State, nau::threading::SpinLock> samplers;
...
protected:
  ...
  {
    lock_(recodingPendingFrameCompletionMutex);
...
public:
  ...
  {
    uint32_t retries = 0;
    NAU_LOG_DEBUG("DX12: Begin waiting for temporary memory uses to be completed...");
    while (!allTemporaryUsesCompleted([this](auto invoked) {
      NAU_LOG_DEBUG("DX12: Checking for temporary memory uses to be completed...");
      visitAllFrameFinilazierData(invoked);
    }))
    {
      if (++retries > 100)
      {
        NAU_LOG_DEBUG("DX12: Waiting for too long, exiting, risking crashes...");
        break;
      }
      NAU_LOG_DEBUG("DX12: Wating 10ms to complete temporary memory uses...");
      nau::hal::sleep_msec(10);
    }
    NAU_LOG_DEBUG("DX12: Finished waiting for temporary memory uses...");
  }
