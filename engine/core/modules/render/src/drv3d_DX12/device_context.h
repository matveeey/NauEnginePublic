...
struct GraphicsState
{
  enum
  {
    SCISSOR_RECT_DIRTY = 0,
    SCISSOR_ENABLED = 1,
    SCISSOR_ENABLED_DIRTY = 2,
    VIEWPORT_DIRTY = 3,
    USE_WIREFRAME = 4,

    INDEX_BUFFER_DIRTY = 5,
    VERTEX_BUFFER_0_DIRTY = 6,
    VERTEX_BUFFER_1_DIRTY = 7,
    VERTEX_BUFFER_2_DIRTY = 8,
    VERTEX_BUFFER_3_DIRTY = 9,
    VERTEX_BUFFER_4_DIRTY = 10,
    VERTEX_BUFFER_5_DIRTY = 11,
    VERTEX_BUFFER_6_DIRTY = 12,
    VERTEX_BUFFER_7_DIRTY = 13,
    // state
    INDEX_BUFFER_STATE_DIRTY = 14,
    VERTEX_BUFFER_STATE_0_DIRTY = 15,
    VERTEX_BUFFER_STATE_1_DIRTY = 16,
    VERTEX_BUFFER_STATE_2_DIRTY = 17,
    VERTEX_BUFFER_STATE_3_DIRTY = 18,
    VERTEX_BUFFER_STATE_4_DIRTY = 19,
    VERTEX_BUFFER_STATE_5_DIRTY = 20,
    VERTEX_BUFFER_STATE_6_DIRTY = 21,
    VERTEX_BUFFER_STATE_7_DIRTY = 22,

    PREDICATION_BUFFER_STATE_DIRTY = 23,
  ...
  void onFlush()
  {
    static_assert(8 == MAX_VERTEX_INPUT_STREAMS);
...
struct CommandStreamSet
{
  ...
  CommandListResultType allocateList(ID3D12Device *device)
  {
    ...
    else
    {
      ...
      {
        NAU_LOG_ERROR("DX12: Unable to allocate new command list");
        NAU_ASSERT(false);
...
class DeviceContext : protected ResourceUsageHistoryDataSetDebugger, public debug::call_stack::Generator //-V553
{
  ...
  dag::CriticalSection newSwapchainCreation;
...
#undef DX12_CONTEXT_COMMAND_PARAM_ARRAY
  ...
  struct ContextState : public debug::DeviceContextState
  {
    ...
    void onFrameStateInvalidate(D3D12_CPU_DESCRIPTOR_HANDLE null_ct)
    {
      ...
      NAU_ASSERT(renderTargetSplitStarts.empty());
  ...
  public:
    ...
    void setBlendConstantFactor(nau::math::E3DCOLOR constant);
    ...
    void clearRenderTargets(ViewportState vp, uint32_t clear_mask, const nau::math::E3DCOLOR *clear_color, float clear_depth,
  ...
  struct Backend
  {
    ...
    eastl::map<SWAPID, backend::AdditionalSwapchain> addSwapchains{};
  ...
  // dag::CriticalSection stubSwapGuard;
#if _TARGET_XBOX
  ...
  {
      return ExecutionMode::CONCURRENT != executionMode;
  }
  ...
  dag::CriticalSection &getFrontGuard();
#if FIXED_EXECUTION_MODE
  void initMode();
#else
...
public:
  ...
  void setBlendConstant(nau::math::E3DCOLOR color);
  void setDepthBoundsRange(float from, float to);
  ...
  void popEvent();
  void updateViewports(eastl::span<const ViewportState> viewports);
  ...
  void destroyBuffer(BufferState buffer, const char8_t *name);
  BufferState discardBuffer(BufferState to_discared, DeviceMemoryClass memory_class, FormatStore format, uint32_t struct_size,
    bool raw_view, bool struct_view, D3D12_RESOURCE_FLAGS flags, uint32_t cflags, const char8_t *name);
  void destroyImage(Image *img);
  void present(OutputMode mode);
  void changePresentMode(PresentationMode mode);
  void changeSwapchainExtents(Extent2D size);
  void changeAdditionalSwapchainExtents(Extent2D size, SWAPID swapID);
#if _TARGET_PC_WIN
  ...
  void setGamma(float power);
  /*dag::CriticalSection &getStubSwapGuard()
  {
  ...
  void beginSwapchainCreation() { newSwapchainCreation.lock(); }
  void endSwapchainCreation() { newSwapchainCreation.unlock(); }
#if !_TARGET_XBOXONE
  void setVariableRateShading(D3D12_SHADING_RATE rate, D3D12_SHADING_RATE_COMBINER vs_combiner,
    D3D12_SHADING_RATE_COMBINER ps_combiner);
  void setVariableRateShadingTexture(Image *texture);
  ...
  BaseTex* getSwapchainColorTexture(SWAPID id);
  eastl::vector<BaseTex*> getSwapchainsColorTextures();
  ...
  bool wasCurrentFramePresentSubmitted() const
  {
    ...
    if (hasWorkerThread())
    {
        bool addPresented = true;
        for (auto& [key, addSwapchain] : back.addSwapchains)
        {
            addPresented &= addSwapchain.wasCurrentFramePresentSubmitted();
        }

        return addPresented && back.swapchain.wasCurrentFramePresentSubmitted();
    }
#endif
    return false;
  }
  ...
  bool swapchainPresentFromMainThread()
  {
    ...
    if (hasWorkerThread())
    {
        ...
        for (auto& [key, addSwapchain] : back.addSwapchains)
        {
            addPresented &= addSwapchain.swapchainPresentFromMainThread();
        }

      return addPresented && back.swapchain.swapchainPresentFromMainThread(*this);
    }
#endif
    return true;
  }
  ...
  void removeShaderGroup(uint32_t group);
  void loadComputeShaderFromDump(ProgramID program);
  void compilePipelineSet(const nau::DataBlock *feature_sets, DynamicArray<InputLayoutID> &&input_layouts,
...
class SwapchainCreationLock
{
  DeviceContext &ctx;

public:
  SwapchainCreationLock(DeviceContext &c) : ctx{c} { ctx.beginSwapchainCreation(); }
  ~SwapchainCreationLock() { ctx.endSwapchainCreation(); }
  SwapchainCreationLock(const SwapchainCreationLock &) = delete;
  SwapchainCreationLock &operator=(const SwapchainCreationLock &) = delete;
  SwapchainCreationLock(SwapchainCreationLock &&) = delete;
  SwapchainCreationLock &operator=(SwapchainCreationLock &&) = delete;
};
} // namespace drv3d_dx12
