...
private:
  ...
  Extent3D extent{};
  typedef eastl::vector<ViewInfo> old_views_t;
...
#endif
  ContainerMutexWrapper<nau::string, nau::threading::SpinLock> debugName;
  uint8_t msaaLevel = 0;
...
public:
  ...
  void setDebugName(const char8_t *name) { *debugName.access() = name; }
  ...
  SubresourceRange getSubresourceRangeForBarrier(uint32_t offset, uint32_t count)
  {
    ...
    NAU_ASSERT_RETURN(offset < limit.count(), SubresourceRange::make_invalid(),
      "DX12: Invalid subresouce index %u in barrier for texture <%s>, texture has only only %u subresources",
      offset, *debugName.access(), limit.count()); // *debugName.access()
    if (!count)
    {
      count = limit.count() - offset;
    }
    NAU_ASSERT_RETURN(offset + count <= limit.count(), SubresourceRange::make_invalid(),
      "DX12: Invalid subresouce range from %u to %u in barrier for texture <%s>, texture has only %u subresources",
      offset, offset + count, *debugName.access(), limit.count());
  ...
  {
    Extent2D me = {Vectormath::max(extent.width >> level.index(), 1u), Vectormath::max(extent.height >> level.index(), 1u)};
    return me;
  }

  uint32_t getMipHeight(MipMapIndex level) const { return Vectormath::max(extent.height >> level.index(), 1u); }

  D3D_SRV_DIMENSION getSRVDimension() const
  {
    switch (imageType)
    {
      default: NAU_FAILURE("Unreachable switch case"); return D3D_SRV_DIMENSION_TEXTURE2D;
