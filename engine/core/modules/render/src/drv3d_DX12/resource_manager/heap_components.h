...
#include "drv3d_commonCode/value_range.h"
#include "drv3d_commonCode/free_list_utils.h"
...
protected:
  void setup(const SetupInfo &info)
  {
    ...
    NAU_LOG_DEBUG("DX12: Checking resource heap properties...");
    ...
    if (DX12_DEBUG_FAIL(info.device->CheckFeatureSupport(D3D12_FEATURE_ARCHITECTURE, &archInfo, sizeof(archInfo))))
    {
      ...
      NAU_LOG_DEBUG("DX12: Assuming NUMA memory architecture...");
    }
    else
    {
      features.set(Features::IS_UMA, FALSE != archInfo.UMA);
      NAU_LOG_DEBUG("DX12: {} memory architecture...", archInfo.UMA ? "UMA" : "NUMA");
    }
    ...
    if (DX12_DEBUG_FAIL(info.device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS, &featureInfo, sizeof(featureInfo))))
    {
      ...
      NAU_LOG_DEBUG("DX12: Assuming resource heap tier 1...");
    }
    else
    {
      features.set(Features::CAN_MIX_RESOURCES, D3D12_RESOURCE_HEAP_TIER_2 <= featureInfo.ResourceHeapTier);
      NAU_LOG_DEBUG("DX12: Resource heap tier {}...", static_cast<uint32_t>(featureInfo.ResourceHeapTier));
    }
  }
...
protected:
  ...
  uint64_t getAvailablePoolBudget(uint32_t pool_type) const
  {
    ...
    else if (host_local_memory_pool == pool_type)
    {
      ...
      return Vectormath::min(totalBudget - pool.CurrentUsage,
...
protected:
  ...
  struct BasicResourceHeap
  {
    ...
    {
      NAU_ASSERT(isValidRange(selected));
      ...
      NAU_ASSERT(freeRanges.empty() || freeRanges.back().stop <= totalSize);
      updateFragmentation();
  ...
  nau::threading::SpinLock heapGroupMutex;
...
protected:
  ...
  void listHeaps()
  {
    ...
    struct ResourceHeapVisitor
    {
      ...
      {
        NAU_LOG_DEBUG("DX12: Heap Group {:08X} ({}, {}{}) with {} heaps", ident, is_cpu_visible ? "CPU visible" : "dedicated GPU",
      ...
      void visitHeap(ByteUnits total_size, ByteUnits free_size, uint32_t fragmentation_percent)
      {
        ...
        freeSize += free_size;
        NAU_LOG_DEBUG("DX12: Size {:6.2f} {:7s} Free {:6.2f} {:7s}, {:3u}% fragmentation", total_size.units(), total_size.name(), free_size.units(),
      ...
      void visitHeapFreeRange(ValueRange<uint64_t> range)
      {
        ...
        NAU_LOG_DEBUG("DX12: Free segment at {:016x} with {:6.2f} {:7s}", range.front(), sizeBytes.units(), sizeBytes.name());
      }
    };
    visitHeaps(ResourceHeapVisitor{totalSize, freeSize, totalHeapCount});
    NAU_LOG_DEBUG("DX12: {} resource heaps, with {:6.2f} {:7s} in total and {:6.2f} {:7s} free", totalHeapCount, totalSize.units(), totalSize.name(),
