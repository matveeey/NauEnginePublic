...
#if 0
#define CACHE_INFO_VERBOSE(...) NAU_LOG_DEBUG(__VA_ARGS__)
#else
...
void PipelineCache::init(const SetupParameters &params)
{
  ...
  NAU_LOG_DEBUG("PipelineCache features:");
  NAU_LOG_DEBUG("Per PSO cache: {}", (deviceFeatures & D3D12_SHADER_CACHE_SUPPORT_SINGLE_PSO) != 0);
  NAU_LOG_DEBUG("Pipeline cache library: {}", (deviceFeatures & D3D12_SHADER_CACHE_SUPPORT_LIBRARY) != 0);
  NAU_LOG_DEBUG("Automatic OS in memory cache: {}", (deviceFeatures & D3D12_SHADER_CACHE_SUPPORT_AUTOMATIC_INPROC_CACHE) != 0);
  NAU_LOG_DEBUG("Automatic OS on disk cache: {}", (deviceFeatures & D3D12_SHADER_CACHE_SUPPORT_AUTOMATIC_DISK_CACHE) != 0);
...
void PipelineCache::shutdown(const ShutdownParameters &params)
{
  ...
  nau::iosys::FullFileSaveCB cacheFile{params.fileName};  //{params.fileName, DF_WRITE | DF_CREATE | DF_IGNORE_MISSING};
  if (!cacheFile.fileHandle)
  {
    NAU_LOG_ERROR("DX12: Failed to open {} to serialize pipeline cache, missing cache folder or "
           "insufficient privileges might be the cause for this error",
      params.fileName);
    return;
  }

  NAU_LOG_DEBUG("DX12: Serializing pipeline caches to {}", params.fileName);
  ...
  auto compSize = 42; //zstd_compress(compressedMemory.data(), compressedMemory.size(), mem.buf.data(), mem.buf.size(), 20);
  ...
  NAU_LOG_DEBUG("D12: Writing pipeline cache with {} bytes of size",
...
{
  NAU_LOG_DEBUG("DX12: onBindumpLoad...");
  ...
  {
    NAU_LOG_DEBUG("DX12: Full match!");
    return;
  }

  NAU_LOG_DEBUG("DX12: No match, resetting cache");
  // not matching
...
bool PipelineCache::loadFromFile(const SetupParameters &params)
{
  ...
  nau::iosys::FullFileLoadCB cacheFile{params.fileName, nau::hal::DF_IGNORE_MISSING | nau::hal::DF_READ};
  if (!cacheFile.fileHandle)
  {
    NAU_LOG_WARNING("DX12: Failed to open {} to load pipeline cache", params.fileName);
    return false;
  }
  ...
  {
    NAU_LOG_ERROR("DX12: Pipeline cache file is invalid");
    return false;
  }
  ...
  if (fileHeader.version != CACHE_FILE_VERSION)
  {
    ...
    NAU_LOG_DEBUG("DX12: Pipeline cache file is outdated");
    return false;
  }
  ...
  if ((0 != (fileHeader.flags & CFF_ROOT_SIGNATURES_USES_CBV_DESCRIPTOR_RANGES)) != params.rootSignaturesUsesCBVDescriptorRanges)
  {
    ...
    NAU_LOG_DEBUG("DX12: Pipeline incompatible root signature layout mode (CBV root descriptors vs "
           "descriptor ranges)");
    return false;
  }
      ...
      (fileHeader.subSystemID != params.subSystemID) || (fileHeader.revisionID != params.revisionID))
  {
    ...
    NAU_LOG_WARNING("DX12: Pipeline cache is incompatible, restoration aborted");
    return false;
  }
  ...
  {
    NAU_LOG_ERROR("DX12: Pipeline cache file is corrupted");
    return false;
  }
  ...
  if (fileHeader.compressedContentSize)
  {
    ...
    {
      NAU_LOG_ERROR("DX12: Error while reading data");
      return false;
    }
    auto outSize = 42; //zstd_decompress(data.buf.data(), data.buf.size(), compressedData.data(), compressedData.size());
    if (outSize != data.buf.size())
    {
      NAU_LOG_ERROR("DX12: Error while decompressing data");
      return false;
    }
  }
  else
  ...
  if (fileHeader.pointerSize != EXPECTED_POINTER_SIZE)
  {
    ...
    NAU_LOG_WARNING("DX12: Detected mismatch of pointer size of running executable ({}) and loaded cache "
            "({}), drivers might fail to load cache library/blob so they will be ignored.",
  ...
  {
    shouldLoadBlobsOrLibrary = false;
    NAU_LOG_WARNING("DX12: Detected driver version mismatch, drivers might fail to load cache library/blob "
  ...
  for (uint32_t cbi = 0; cbi < fileHeader.computeCaches; ++cbi)
  {
    ...
    {
      NAU_LOG_ERROR("DX12: Error while decoding pipeline cache");
      computeBlobs.clear();
      return false;
    }
    uint32_t sz = 0;
  ...
  for (uint32_t i = 0; i < fileHeader.graphicsStaticStateCaches; ++i)
  {
    ...
    {
      NAU_LOG_ERROR("DX12: Error while decoding static render state cache");
      computeBlobs.clear();
      graphicsCache.clear();
      computeSignatures.clear();
      graphicsSignatures.clear();
  ...
  for (uint32_t i = 0; i < fileHeader.inputLayoutCaches; ++i)
  {
    ...
    {
      NAU_LOG_ERROR("DX12: Error while decoding input layout cache");
      computeBlobs.clear();
      graphicsCache.clear();
      computeSignatures.clear();
      graphicsSignatures.clear();
  ...
  for (uint32_t i = 0; i < fileHeader.framebufferLayoutCaches; ++i)
  {
    ...
    {
      NAU_LOG_ERROR("DX12: Error while decoding framebuffer layout cache");
      computeBlobs.clear();
      graphicsCache.clear();
      computeSignatures.clear();
      graphicsSignatures.clear();
  ...
  NAU_LOG_DEBUG("DX12: Pipeline cache load statistics:");
  NAU_LOG_DEBUG("DX12: Restored {} compute pipelines with {} bytes from cache", computeBlobsLoaded, computeBlobsSize);
  NAU_LOG_DEBUG("DX12: Restored {} compute pipeline signatures with {} bytes from cache", fileHeader.computeSignatureCaches,
    computeSignatureBlobSize);
  NAU_LOG_DEBUG("DX12: Restored {} graphics pipeline variants of {} ({}) base pipelines", graphicsVairantsFound, graphicsWithAnyVariant,
    fileHeader.graphicsBaseCaches);
  NAU_LOG_DEBUG("DX12: Restored {} graphics pipelines with {} bytes from cache", graphicsVariantBlobsLoaded, graphicsVariantBlobsSize);
  NAU_LOG_DEBUG("DX12: Restored {} graphics pipeline signatures with {} bytes from cache", fileHeader.graphicsSignatureCaches,
    graphicsSignatureBlobSize);
  NAU_LOG_DEBUG("DX12: Restored {} graphics mesh pipeline signatures with {} bytes from cache", fileHeader.graphicsMeshSignatureCaches,
    graphicsMeshSignatureBlobSize);
  NAU_LOG_DEBUG("DX12: Restored {} graphics pipeline static render states", staticRenderStates.size());
  NAU_LOG_DEBUG("DX12: Restored {} graphics pipeline input layouts", inputLayouts.size());
  NAU_LOG_DEBUG("DX12: Restored {} graphics pipeline framebuffer layouts", framebufferLayouts.size());
  NAU_LOG_DEBUG("DX12: Restored pipeline library with {} bytes from cache", initialPipelineBlob.size());
