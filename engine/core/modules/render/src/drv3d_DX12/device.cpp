...
#include <drv3d_commonCode/nvLowLatency.h>
...
raytraceGeometryDescriptionToGeometryDescTriangles(const RaytraceGeometryDescription::TrianglesInfo &info)
{
  ...
  if (ibuf)
  {
    ...
    {
      NAU_ASSERT(DXGI_FORMAT_R16_UINT == desc.Triangles.IndexFormat); // -V547
...
void Device::setupNullViews()
{
...

  {
    ...
    nullBuffer = createDedicatedBuffer(max_const_buffer_width, const_element_width, 1, DeviceMemoryClass::DEVICE_RESIDENT_BUFFER,
      D3D12_RESOURCE_FLAG_NONE, SBCF_BIND_CONSTANT, u8"Null Buffer");
...
Image *Device::createImageNoContextLock(const ImageInfo &ii, const char8_t *name)
{
...
bool Device::init(DXGIFactory *factory, AdapterInfo &&adapterInfo, D3D_FEATURE_LEVEL feature_level,
  ...
  const nau::DataBlock *dxCfg, bool stereo_render)
{
  NAU_LOG_DEBUG("DX12: IDXGIAdapter1::QueryInterface IDXGIAdapter4...");
  if (FAILED(adapterInfo.adapter.As(&adapter)))
  {
    NAU_LOG_DEBUG("DX12: Failed...");
    return false;
  }
  NAU_LOG_DEBUG("DX12: D3D12CreateDevice...");
  ...
  if (validationLayerAvailable)
  {
    ...
    NAU_LOG_DEBUG("DX12: Detected enabled validation layer, enabling object naming");
    config.features.set(DeviceFeaturesConfig::NAME_OBJECTS);
#else
    NAU_LOG_WARNING("DX12: Detected enabled validation layer, but this build can not name objects!");
#endif
  }
  startDeviceErrorObserver(device.get());

  NAU_LOG_DEBUG("DX12: Checking shader model...");
  ...
  {
    NAU_LOG_DEBUG("DX12: Failed, does not support shader model 6.0 or higher");
    // on debug builds we allow dxbc shaders to work with debug tools, dxbc has better support
#if DAGOR_DBGLEVEL > 0
    NAU_LOG_DEBUG("DX12: Debug build, allowing target without DXIL support");
#else
    shutdown({});
    return false;
#endif
  ...
  NAU_LOG_DEBUG("DX12: Creating queues...");
  ...
  NAU_LOG_DEBUG("DX12: Creating swapchain...");
...

  {
    ...
    desc.setMinFilter(D3D12_FILTER_TYPE_LINEAR);
    desc.setMagFilter(D3D12_FILTER_TYPE_LINEAR);
  ...
  {
    execMode = DeviceContext::ExecutionMode::IMMEDIATE;
  }
...
void Device::shutdown(const DeviceCapsAndShaderModel &features)
{
  ...
  context.destroyBuffer(eastl::move(nullBuffer), u8"nullBuffer");
...
void Device::adjustCaps(Driver3dDesc &capabilities)
{
  ...
  NAU_LOG_DEBUG("DX12: GPU has support for Shader Model {}.{}", (unsigned int)capabilities.shaderModel.major, (unsigned int)capabilities.shaderModel.minor);
  ...
  // We don't support the limited Vectormath::max 128 SRVs per stage tier 1 supports.
  ...
  NAU_LOG_DEBUG("GPU has tier {} for view instancing.", op3.ViewInstancingTier);
  ...
  capabilities.maxtexw = Vectormath::min(capabilities.maxtexw, D3D12_REQ_TEXTURE2D_U_OR_V_DIMENSION);
  capabilities.maxtexh = Vectormath::min(capabilities.maxtexh, D3D12_REQ_TEXTURE2D_U_OR_V_DIMENSION);
  capabilities.maxcubesize = Vectormath::min(capabilities.maxcubesize, D3D12_REQ_TEXTURECUBE_DIMENSION);
  capabilities.maxvolsize = Vectormath::min(capabilities.maxvolsize, D3D12_REQ_TEXTURE3D_U_V_OR_W_DIMENSION);
  capabilities.maxtexaspect = Vectormath::max(capabilities.maxtexaspect, 0);
  capabilities.maxtexcoord = Vectormath::min(capabilities.maxtexcoord, D3D12_VS_INPUT_REGISTER_COUNT);
  capabilities.maxsimtex = Vectormath::min(capabilities.maxsimtex, (int)dxil::MAX_T_REGISTERS);
  capabilities.maxvertexsamplers = Vectormath::min(capabilities.maxvertexsamplers, D3D12_COMMONSHADER_SAMPLER_SLOT_COUNT);
  capabilities.maxclipplanes = Vectormath::min(capabilities.maxclipplanes, 0);
  capabilities.maxstreams = Vectormath::min(capabilities.maxstreams, D3D12_IA_VERTEX_INPUT_RESOURCE_SLOT_COUNT);
  capabilities.maxstreamstr = Vectormath::min(capabilities.maxstreamstr, 0x10000);
  capabilities.maxvpconsts = Vectormath::min(capabilities.maxvpconsts, D3D12_REQ_CONSTANT_BUFFER_ELEMENT_COUNT);
  // do not overflow into negative numbers
  capabilities.maxprims = Vectormath::min(capabilities.maxprims, (int)Vectormath::min(0x7FFFFFFFull, 1llu << D3D12_REQ_DRAW_VERTEX_COUNT_2_TO_EXP));
  capabilities.maxvertind = Vectormath::min(capabilities.maxvertind, (int)Vectormath::min(0x7FFFFFFFull, 1llu << D3D12_REQ_DRAWINDEXED_INDEX_COUNT_2_TO_EXP));
  capabilities.maxSimRT = Vectormath::min(capabilities.maxSimRT, D3D12_SIMULTANEOUS_RENDER_TARGET_COUNT);
#if !_TARGET_XBOXONE
...
Image *Device::createImage(const ImageInfo &ii, Image *base_image, const char8_t *name)
{
  if (!checkResourceCreateState(u8"createImage", name))
  {
    return nullptr;
  }
...
Texture *Device::wrapD3DTex(ID3D12Resource *tex_res, ResourceBarrier current_state, const char8_t *name, int flg)
{
  if (!checkResourceCreateState(u8"wrapD3DTex", name))
  {
    return nullptr;
  }
  ...
  tex->tex.image = image;
  NAU_ASSERT(1 << tex->tex.image->getMsaaLevel() == get_sample_count(tex->cflg));
...
{
  if (!checkResourceCreateState(u8"createBuffer", name))
  {
    return {};
  }
...
BufferState Device::createDedicatedBuffer(uint32_t size, uint32_t structure_size, uint32_t discard_count,
  DeviceMemoryClass memory_class, D3D12_RESOURCE_FLAGS flags, uint32_t cflags, const char8_t *name)
{
  if (!checkResourceCreateState(u8"createBuffer", name))
  {
    return {};
  ...
  uint32_t struct_size)
{
  ...
  {
    NAU_FAILURE("DX12: Invalid input to Device::addBufferView");
    return;
  }
}
...
void Device::setTexName(Image *img, const char8_t *name)
{
  if (!img || !name)
    return;
  ...
  RaytraceBuildFlags flags)
{
  ...
  NAU_ASSERT(bottomLevelPrebuildInfo.ResultDataMaxSizeInBytes > 0);

  resources.ensureRaytraceScratchBufferSize(getDXGIAdapter(), device.get(),
    Vectormath::max(bottomLevelPrebuildInfo.ScratchDataSizeInBytes, bottomLevelPrebuildInfo.UpdateScratchDataSizeInBytes));
...
RaytraceAccelerationStructure *Device::createRaytraceAccelerationStructure(uint32_t elements, RaytraceBuildFlags flags)
{
  ...
  NAU_ASSERT(topLevelPrebuildInfo.ResultDataMaxSizeInBytes > 0);

  resources.ensureRaytraceScratchBufferSize(getDXGIAdapter(), device.get(),
    Vectormath::max(topLevelPrebuildInfo.ScratchDataSizeInBytes, topLevelPrebuildInfo.UpdateScratchDataSizeInBytes));
...
void Device::enumerateDisplayModes(eastl::vector<nau::string> &list)
{
...
void Device::enumerateDisplayModesFromOutput(IDXGIOutput *dxgi_output, eastl::vector<nau::string> &list)
{
  ...
  if (SUCCEEDED(dxgi_output->GetDisplayModeList(DXGI_FORMAT_B8G8R8A8_UNORM, 0, &numModes, nullptr)))
  {
    ...
    if (SUCCEEDED(dxgi_output->GetDisplayModeList(DXGI_FORMAT_B8G8R8A8_UNORM, 0, &numModes, displayModes.data())))
    {
      ...
      eastl::transform(begin(displayModes), end(displayModes), list.data(),
        [](const DXGI_MODE_DESC &mode) //
        {
          nau::string retStr = nau::string::format(64, "{} x {}", mode.Width, mode.Height);
          return retStr;
        });
    }
  }
...
void Device::enumerateActiveMonitors(eastl::vector<nau::string> &result)
{
  ...
  const Direct3D12Enviroment &d3d_env, HWND wnd, SwapchainCreateInfo &&sci)
{
  ...
  NAU_LOG_DEBUG("DX12: Entering recovering state...");
...
void Device::registerDeviceResetEventHandler(DeviceResetEventHandler *handler)
{
  ...
  if (it == eastl::end(deviceResetEventHandlers))
    ...
    NAU_ASSERT(false, "Device reset event handler is already registered.");
}
...
void Device::unregisterDeviceResetEventHandler(DeviceResetEventHandler *handler)
{
  ...
  if (it != eastl::end(deviceResetEventHandlers))
    ...
    NAU_ASSERT(false, "Could not find device reset event handler to unregister.");
}
...
int64_t FrameInfo::beginFrame(Device &device, DeviceQueueGroup &queue_group, PipelineManager &pipe_man)
{
  ...
  NAU_LOG_DEBUG("DX12: Frame {} used {} resource descriptors", progress, resourceViewHeaps.getTotalUsedDescriptors());
  NAU_LOG_DEBUG("DX12: Frame {} used {} sampler descriptors", progress, samplerHeaps.getTotalUsedDescriptors());
#endif
...
namespace
{
void load_config_into_memory_setup(ResourceMemoryHeap::SetupInfo &setup, const nau::DataBlock *config)
{
...
bool is_threaded_command_execution(const char *name)
{
  ...
  if (0 == strcmp(random_execution_name, name))
  {
    if (0 == (grnd() % 2))
    {
      NAU_LOG_DEBUG("random execution mode: concurrent mode selected");
      return true;
    }
    else
    {
      NAU_LOG_DEBUG("random execution mode: immediate mode selected");
      return false;
    }
  }
#endif
  ...
  {
    NAU_LOG_WARNING("DX12: config 'exectionMode' can only be {}, {}, or {}, but found {}", immediate_exection_name, threaded_exection_name,
...
Device::Config drv3d_dx12::get_device_config(const nau::DataBlock *cfg)
{
  ...
  {
    NAU_LOG_DEBUG("DX12: concurrent execution mode selected");
  }
  else
  {
    NAU_LOG_DEBUG("DX12: immediate execution mode selected");
  }
...
#if _TARGET_PC_WIN
Device::Config drv3d_dx12::update_config_for_vendor(Device::Config config, const nau::DataBlock *cfg, Device::AdapterInfo &adapterInfo)
{
...
bool Device::getGpuMemUsageStats(uint64_t additionalVramUsageInBytes, uint32_t *out_mem_size, uint32_t *out_free_mem_kb,
...
SWAPID Device::createAdditionalSwachain(void* hwnd, DXGIFactory* factory)
{
    context.finish();

    SwapchainCreationLock ctxLock{context};

    SWAPID swapID = context.freeSwapchainID++;

    auto& bswapch = context.back.addSwapchains[swapID];
    
    SwapchainCreateInfo info;
    info.enableHdr = false;
    info.forceHdr = false;
    info.presentMode = PresentationMode::VSYNCED;
    info.resolutionX = 631;
    info.resolutionY = 611;
    info.window = static_cast<HWND>(hwnd);
    info.windowed = true;
    
    if(!bswapch.setup(*this, factory, queues[DeviceQueueType::GRAPHICS].getHandle(), eastl::move(info)))
    {
        NAU_FAILURE("Can't create swapchain");
        return false;
    }

    //bswapch.onFrameBegin(this->getDevice());
    //bswapch.signalPresentSentToBackend();

    return swapID;
}

void drv3d_dx12::Device::removeAdditionalSwapchain(SWAPID swapID)
{
    context.finish();
    ...
    auto& bswapch = context.back.addSwapchains[swapID];
    bswapch.prepareForShutdown(*this);
    bswapch.shutdown();

    context.back.addSwapchains.erase(swapID);
}
