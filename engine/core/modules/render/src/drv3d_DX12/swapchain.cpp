...
void frontend::Swapchain::setup(SetupInfo setup)
{
  ...
  {
    swapchainDepthStencilTex.reset(new BaseTex{RES3D_TEX, TEXFMT_DEPTH32 | TEXCF_RTARGET});
  }
  swapchainDepthStencilTex->setParams(extent.width, extent.height, 1, 1, u8"swapchain depth stencil target");
...
void frontend::Swapchain::waitForFrameStart()
{
  ...
  if (waitableObject)
  {
    ...
    if (WAIT_OBJECT_0 != result)
    {
      ...
      NAU_LOG_WARNING("DX12: Wait for low latency waitable object timed out after {}ms, with result {:#08x}", MAX_WAIT_OBJECT_TIMEOUT_MS,
        result);
    }
  }
#endif
...
BaseTex *frontend::Swapchain::getDepthStencilTexture(Device &device, Extent2D ext)
{
  if (swapchainDepthStencilTex->tex.image)
  {
    ...
    // adjust request to Vectormath::max of any
    ext.width = Vectormath::max(ext.width, depthExt.width);
    ext.height = Vectormath::max(ext.height, depthExt.height);
  ...
  swapchainDepthStencilTex->tex.image = device.createImageNoContextLock(ii, u8"swapchain depth stencil target");
  swapchainDepthStencilTex->setParams(ext.width, ext.height, 1, 1, u8"swapchain depth stencil target");
  return swapchainDepthStencilTex.get();
}
...
#endif
  {
    ...
    if (info.state.isSRV())
    {
      ...
      for (auto &buffer : colorTargets)
      {
        ...
        NAU_ASSERT(buffer.viewTable.size() == swapchainViewSet.size());
      }
    }
    ...
    {
      NAU_FAILURE("DX12: Invalid view type for swapchain");
    }
  }
#if _TARGET_XBOX
    ...
    if (!autoGameDvr)
    {
      if (info.state.isRTV())
      {
        ...
        for (auto &buffer : secondaryColorTargets)
        {
          ...
          device->CreateRenderTargetView(buffer.buffer.Get(), &desc, descriptor);
          buffer.viewTable.push_back(descriptor);
          NAU_ASSERT(buffer.viewTable.size() == secondarySwapchainViewSet.size());
        }
      }
      else
      {
  ...
  SwapchainCreateInfo &&sci)
{
  ...
  if ((sci.enableHdr && isHdrAvailable) || sci.forceHdr)
  {
    ...
    {
      NAU_LOG_WARNING("DX12: 16 bit hdr format isn't supported!");
      colorFormat = FormatStore::fromCreateFlags(TEXFMT_DEFAULT);
    }
  }
  else
...
{
    //NAU_LOG_DEBUG("Swapchain {} onFrameBegin", SwapChainID);
...
void backend::Swapchain::swapchainPresentOnPC()
{
  ...
  DX12_DEBUG_FAIL(swapchain->Present1(interval, flags, &params));
}
...
bool backend::Swapchain::swapchainPresentFromMainThread(DeviceContext &ctx)
{
  ...
  //onFrameBegin(ctx.device.getDevice());
...
bool backend::Swapchain::setGamma(float power)
{
  ...
  {
    NAU_LOG_DEBUG("DX12: setGamma: NumGammaControlPoints {}, no effect", gcc.NumGammaControlPoints);
    return true;
  }
...
void backend::Swapchain::prepareForShutdown(Device &device)
{
  auto prepareViewForShutDown = [&](const ImageViewInfo &view) {
    ...
    {
      NAU_FAILURE("DX12: Found a ViewState which is neither SRV, UAV, RTV or DSV");
    }
  };
  if (colorTarget)
  {
...
void backend::Swapchain::bufferResize(Device &device, const Extent2D &extent, FormatStore color_format)
{
  ...
  DXGI_SWAP_CHAIN_DESC currentDesc;
  swapchain->GetDesc(&currentDesc);
  NAU_LOG_DEBUG("DX12: ResizeBuffers({}, {}, {}, {}, {:#08x})", currentDesc.BufferCount, extent.width, extent.height,
  ...
  for (size_t i = 0; i < colorTargets.size(); ++i)
  {
    ...
    if (colorTargets[i].buffer.Get() != nullptr)
    {
      ...
      swprintf_s(stringBuf, L"Swapchain texture#%d", i);
    ...
    if (colorTargets[i].buffer.Get() == nullptr)
    {
      ...
      NAU_LOG_DEBUG("DX12: swapImage for index {} was null in Swapchain::getSwapchainBuffers, we assume we "
...
void backend::Swapchain::changeFullscreenExclusiveMode(bool is_exclusive)
{
  ...
  NAU_LOG_DEBUG("DX12: Swapchain full screen exclusive mode changed. is_exclusive: {}", is_exclusive ? "true" : "false");
  isInExclusiveFullscreenModeEnabled = isInExclusiveFullscreenMode();
}
...
void drv3d_dx12::backend::AdditionalSwapchain::swapchainPresentOnPC()
{
    //SCOPED_LATENCY_MARKER(lowlatency::get_current_render_frame(), PRESENT_START, PRESENT_END);
    UINT interval = 1;
    ...
    //NAU_LOG("On frame before presented {}", swapchain->GetCurrentBackBufferIndex());

    DX12_DEBUG_FAIL(swapchain->Present1(interval, flags, &params));

    //onFrameBegin();
    //NAU_LOG("On frame presented {}", swapchain->GetCurrentBackBufferIndex());
}

bool backend::AdditionalSwapchain::swapchainPresentFromMainThread()
{
  // wait for backend finish the frame
  while (numPresentSentToBackend != numFramesCompletedByBackend)
  {
...
void drv3d_dx12::backend::AdditionalSwapchain::registerSwapchainView(D3DDevice* device, Image* image, ImageViewInfo info)
{
    G_UNUSED(image);
    ...
    {
      auto desc = info.state.asRTVDesc(D3D12_RESOURCE_DIMENSION_TEXTURE2D, false); //image->isMultisampled());
      for (auto& buffer : colorTargets)
      {
          auto descriptor = swapchainBufferRTVHeap.allocate(device);
          device->CreateRenderTargetView(buffer.buffer.Get(), &desc, descriptor);
          ...
          BaseTex* dxTex = static_cast<BaseTex*>(buffer.swapchainColorTex);
          info.handle = descriptor;
          dxTex->tex.image->addView(info);
      }
    }
    else
    {
...
bool drv3d_dx12::backend::AdditionalSwapchain::setup(Device& device, DXGIFactory* factory, SwapchainInitParameter* queue, SwapchainCreateInfo&& sci)
{
    swapchainBufferSRVHeap.init(device.device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV));
    swapchainBufferRTVHeap.init(device.device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV));
...
void drv3d_dx12::backend::AdditionalSwapchain::onFrameBegin()
{
    //NAU_LOG("On frame before begin {}", currentColorTargetIndex);
    previousColorTargetIndex = currentColorTargetIndex;
    currentColorTargetIndex = swapchain->GetCurrentBackBufferIndex();

    //NAU_LOG("On frame begin {}", currentColorTargetIndex);
}

void drv3d_dx12::backend::AdditionalSwapchain::present(Device& device)
{
  if (device.getContext().hasWorkerThread())
  {
    numFramesCompletedByBackend++;
...
BaseTex* drv3d_dx12::backend::AdditionalSwapchain::getColorTexture()
{
    NAU_ASSERT(colorTargets.size() > currentColorTargetIndex);
    return colorTargets[currentColorTargetIndex].swapchainColorTex;
}

Image* drv3d_dx12::backend::AdditionalSwapchain::getColorImage()
{
    return colorTargets[currentColorTargetIndex].swapchainColorTex->tex.image;
}

void drv3d_dx12::backend::AdditionalSwapchain::waitForFrameStart()
{
    if (waitableObject)
    {
        // In some cases the object times out even when the GPU is idle and waits for work,
        // this seems to happen when frames on the CPU take very long (during loading for example),
        // and the GPU finishes all outstanding frames before we wait here.
        auto result = WaitForSingleObjectEx(waitableObject, MAX_WAIT_OBJECT_TIMEOUT_MS, FALSE);
        if (WAIT_OBJECT_0 != result)
        {
            // don't report it as error, we can go on, so just warn about it.
            NAU_LOG_WARNING("DX12: Wait for low latency waitable object timed out after {}ms, with result {}", MAX_WAIT_OBJECT_TIMEOUT_MS,
                result);
        }
    }
}

void drv3d_dx12::backend::AdditionalSwapchain::prepareForShutdown(Device& device)
{
    auto prepareViewForShutDown = [&](const ImageViewInfo &view) {
    if (!view.state.isValid())
      return;
    ...
    {
        prepareViewForShutDown(colorTarget.swapchainColorTex->tex.image->getRecentView());
        colorTarget.swapchainColorTex->tex.image->removeAllViews();
        device.resources.destroyTextureOnFrameCompletion(colorTarget.swapchainColorTex->tex.image);
        int refCount = colorTarget.swapchainColorTex->tex.image->image.Reset();
        colorTarget.swapchainColorTex->tex.image->replaceResource(nullptr);
    }
...
void drv3d_dx12::backend::AdditionalSwapchain::shutdown()
{
    colorTargets.clear();
    output.Reset();
    if (swapchain)
...
void drv3d_dx12::backend::AdditionalSwapchain::bufferResize(Device& device, const Extent2D& extent, FormatStore color_format)
{
    // have to always resize the buffer, even if size has not changed
    DXGI_SWAP_CHAIN_DESC currentDesc;
    swapchain->GetDesc(&currentDesc);

    NAU_LOG_DEBUG("DX12: ResizeBuffers for Additional Swapchain({}, {}, {}, {}, {})", currentDesc.BufferCount, extent.width, extent.height,
        static_cast<uint32_t>(currentDesc.BufferDesc.Format), static_cast<uint32_t>(currentDesc.Flags));

    HRESULT res = swapchain->ResizeBuffers(currentDesc.BufferCount, extent.width, extent.height, 
        currentDesc.BufferDesc.Format, currentDesc.Flags);
    DX12_DEBUG_RESULT(res);
    ...
    for (size_t i = 0; i < colorTargets.size(); ++i)
    {
        ...
        colorTargets[i].swapchainColorTex =
            static_cast<BaseTex*>(device.wrapD3DTex(colorTargets[i].buffer.Get(), RB_NONE, u8"additional backbuffer", TEXCF_RTARGET));
        
        BaseTex* dxTex = colorTargets[i].swapchainColorTex;
        dxTex->tex.image->updateExtents(ext);
        dxTex->tex.image->updateFormat(color_format);
        dxTex->tex.image->setGPUChangeable(true);

        dxTex->tex.image->swapChainIndex = i;

        device.getContext().back.sharedContextState.resourceStates.setTextureState(
            device.getContext().back.sharedContextState.initialResourceStateSet, dxTex->tex.image->getGlobalSubResourceIdBase(), 1, D3D12_RESOURCE_STATE_PRESENT);
    ...
    ImageViewState targetView;
    bool useSrgb = false; //toggleBits.test(ToggleState::USE_BACKBUFFER_SRGB);
    auto baseFormat = FormatStore::fromCreateFlags(TEXFMT_DEFAULT); //colorTargets[0].swapchainColorTex->getSwapchainColorFormat();
    ...
    ImageViewInfo info = {};
    info.state = targetView;

    registerSwapchainView(device.getDevice(), nullptr, info);

    targetView.setFormat(baseFormat.getSRGBVariant());
    info.state = targetView;
    registerSwapchainView(device.getDevice(), nullptr, info);
}
