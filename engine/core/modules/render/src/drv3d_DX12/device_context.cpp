...
#define DX12_LOCK_FRONT() dag::CSAutoLock lock(getFrontGuard())
...
{
  NAU_ASSERT(mip_levels <= MAX_MIPMAPS);
  ...
  for (uint32_t j = 0; j < mip_levels; ++j)
  {
    ...
    NAU_ASSERT(copy.layout.Offset % D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT == 0);
...
#endif
      SwapchainCreationLock ctxLock{*this};
...
void DeviceContext::manageLatchedState()
{
  ...
  if (workcycle_internal::application_active)
  {
    ...
    {
      NAU_LOG_DEBUG("DX12: Frame interval was {}us", frameTimeUsec);
      NAU_LOG_DEBUG("DX12: Frame latency was {}us", frameLatency);
      NAU_LOG_DEBUG("DX12: Frame front to back wait was {}us", frameFrontBackWait);
      NAU_LOG_DEBUG("DX12: Frame back to front wait was {}us", frameBackFrontWait);
      NAU_LOG_DEBUG("DX12: Frame GPU wait was {}us", frameGPUWait);
      NAU_LOG_DEBUG("DX12: Frame Swapchain swap was {}us", swapSwapWait);
      NAU_LOG_DEBUG("DX12: Frame Get Swapchain buffer was {}us", getSwapWait);
      NAU_LOG_DEBUG("DX12: Frame Wait For Swapchain waitable object was {}us", frontSwapchainWait);
    }
...
void DeviceContext::makeReadyForFrame(OutputMode mode)
{
  ...
  {
      for (auto& [key, addSwapchain] : back.addSwapchains)
      {
          addSwapchain.onFrameBegin();
      }
...
dag::CriticalSection &DeviceContext::getFrontGuard() { return mutex; }
...
bool DeviceContext::enableImmediateFlush()
{
  if (isImmediateMode())
  {
    ...
    NAU_ASSERT(!isImmediateFlushSuppressed);
    return true;
  }
  else
    NAU_LOG_ERROR("DX12: The immediate flush doesn't work in non immedite execution mode!");
  return false;
}
...
{
  NAU_ASSERT(mip_map_range.isValidRange());
...
void DeviceContext::compilePipelineSet(const nau::DataBlock *feature_sets, DynamicArray<InputLayoutID> &&input_layouts,
  DynamicArray<StaticRenderStateID> &&static_render_states, const nau::DataBlock *output_formats_set,
  const nau::DataBlock *graphics_pipeline_set, const nau::DataBlock *mesh_pipeline_set, const nau::DataBlock *compute_pipeline_set,
  const char *default_format)
{
  ...
  if (graphics_pipeline_set && graphics_pipeline_set->blockCount() > 0)
  {
    ...
    pipeline::DataBlockDecodeEnumarator<pipeline::GraphicsPipelineDecoder> decoder{
      *graphics_pipeline_set, 0, featureSetSupported.data(), (uint32_t)featureSetSupported.size()};
    for (; !decoder.completed(); decoder.next())
    {
  ...
  if (mesh_pipeline_set && mesh_pipeline_set->blockCount() > 0)
  {
    ...
    pipeline::DataBlockDecodeEnumarator<pipeline::MeshPipelineDecoder> decoder{
      *mesh_pipeline_set, 0, featureSetSupported.data(), (uint32_t)featureSetSupported.size()};
    for (; !decoder.completed(); decoder.next())
    {
  ...
  if (compute_pipeline_set && compute_pipeline_set->blockCount() > 0)
  {
    ...
    pipeline::DataBlockDecodeEnumarator<pipeline::ComputePipelineDecoder> decoder{
      *compute_pipeline_set, 0, featureSetSupported.data(), (uint32_t)featureSetSupported.size()};
    for (; !decoder.completed(); decoder.next())
    {
...
void DeviceContext::tidyFrame(FrontendFrameLatchedData &frame)
{
  ...
  front.frontendFinishedWorkItem.store(Vectormath::max(frame.progress, front.frontendFinishedWorkItem.load(std::memory_order_relaxed)),
  ...
  front.completedFrameProgress = Vectormath::max(frame.progress, front.completedFrameProgress);
#if DX12_RECORD_TIMING_DATA
  front.completedFrameIndex = Vectormath::max(frame.frameIndex, front.completedFrameIndex);
#endif
...
#undef DX12_CONTEXT_COMMAND_PARAM_ARRAY
    ...
    NAU_LOG_DEBUG("DX12: Command Stream visited commands statistics:");
    NAU_LOG_DEBUG("{:3s}, [{:3s}] {:<45s}, {:10s}, {:6s}, {:9s}", "#", "ID", "name", "count", "%", "frame avg");
    for (auto at = eastl::begin(commandArray), ed = eastl::end(commandArray) - 1; at != ed; ++at)
    {
      ...
      NAU_LOG_DEBUG("{:3u}, [{:3u}] {:<45s}, {:8.2f} {:1s}, {:5.2f}%, {:9.2f}", 1 + (at - eastl::begin(commandArray)), at->cmdIndex,
        cmdNames[at->cmdIndex], at->visitCount.units(), at->visitCount.name(), perc, perFrame);
    }
    NAU_LOG_DEBUG("{:.2f} {} total visits", totalCount.units(), totalCount.name());
  }
}
...
void DeviceContext::clearRenderTargets(ViewportState vp, uint32_t clear_mask, const nau::math::E3DCOLOR *clear_color, float clear_depth,
...
void DeviceContext::setBlendConstant(nau::math::E3DCOLOR color)
{
...
void DeviceContext::setScissorRects(eastl::span<const D3D12_RECT> rects)
{
...
{
  NAU_ASSERT(HostDeviceSharedMemoryRegion::Source::TEMPORARY == update.source,
...
void DeviceContext::updateViewports(eastl::span<const ViewportState> viewports)
{
...
void DeviceContext::destroyBuffer(BufferState buffer, const char8_t *name)
{
  if (buffer)
  {
...
BufferState DeviceContext::discardBuffer(BufferState to_discared, DeviceMemoryClass memory_class, FormatStore format,
  uint32_t struct_size, bool raw_view, bool struct_view, D3D12_RESOURCE_FLAGS flags, uint32_t cflags, const char8_t *name)
{
  BufferState result;
  {
    DX12_LOCK_FRONT();
...
// TODO Tracy    NAU_CPU_SCOPED_TAG(nau::PerfTag::Render);
      ...
      (frameInterval > capInfo.thresholdUS))
  {
    ...
    NAU_LOG_DEBUG("DX12: Frame {}: interval was {} us, threshold is {} us; capturing next frame", front.frameIndex, frameInterval,
      capInfo.thresholdUS);
    eastl::wstring name =
  ...
  if (hasWorkerThread() && OutputMode::PRESENT == mode)
  {
      for (auto& [key, addSwapchain] : back.addSwapchains)
      {
          addSwapchain.signalPresentSentToBackend();
      }
  ...
  while (!discardBytes.compare_exchange_strong(cnt, 0))
    ;
  NAU_LOG_DEBUG("DX12: Discarded {}Bytes / {}KiBytes / {}MiBytes", cnt, cnt / 1024, cnt / 1024 / 1024);
#endif
  ...
  if (OutputMode::PRESENT == mode)
  {
    ...
    for (auto& [key, addSwapchain] : back.addSwapchains)
    {
        addSwapchain.waitForFrameStart();
    }
  }
...
void DeviceContext::changePresentMode(PresentationMode mode)
{
  ...
  NAU_LOG_DEBUG("DX12: Swapchain present mode changed to {}, sending mode change to backend...", (uint32_t)mode);
...
void DeviceContext::changeSwapchainExtents(Extent2D size)
{
  ...
  NAU_LOG_DEBUG("DX12: Swapchain extent changed to {} / {}, sending mode change to backend...", size.width, size.height);
...
void drv3d_dx12::DeviceContext::changeAdditionalSwapchainExtents(Extent2D size, SWAPID swapID)
{
    DX12_LOCK_FRONT();
    finishInternal();
    //resizeSwapchain(size);

    NAU_ASSERT(back.addSwapchains.count(swapID));
    back.addSwapchains[swapID].prepareForShutdown(device);
    // finish all outstanding frames

    //NAU_LOG("back.sharedContextState.frames {}", back.sharedContextState.frames.size());
    //back.sharedContextState.frames.walkAll([=](auto &frame) //
    //  { frame.beginFrame(device, device.queues, device.pipeMan); });

    back.addSwapchains[swapID].bufferResize(device, size, front.swapchain.getColorFormat()); // TODO: replace front.swapchain

    //makeReadyForFrame(OutputMode::PRESENT);
    back.addSwapchains[swapID].onFrameBegin();
}
...
HRESULT DeviceContext::getSwapchainDesc(DXGI_SWAP_CHAIN_DESC* out_desc)
{
    return back.swapchain.getDesc(out_desc);
}

IDXGIOutput* DeviceContext::getSwapchainOutput()
{
    return back.swapchain.getOutput();
}
#endif
...
void DeviceContext::generateMipmaps(Image *img)
{
  ...
  for (auto a : arrays)
  {
    ...
    {
      blit.dstOffsets[1].x = Vectormath::max(1, blit.srcOffsets[1].x >> 1);
      blit.dstOffsets[1].y = Vectormath::max(1, blit.srcOffsets[1].y >> 1);
...
void DeviceContext::suspendExecution()
{
  ...
  NAU_LOG_DEBUG("DX12: DeviceContext::suspendExecution called...");
...
#endif
  {
    NAU_LOG_DEBUG("DX12: Sending suspend request to worker thread...");
    ...
    NAU_LOG_DEBUG("DX12: Worker is now in suspend...");
  }

  NAU_LOG_DEBUG("DX12: Sending suspend command to API...");
  ...
  NAU_LOG_DEBUG("DX12: API suspended...");
}
void DeviceContext::resumeExecution()
{
  NAU_LOG_DEBUG("DX12: Sending resume command to API...");
  // If we end up here, we got the resume signal
  device.queues.leaveSuspendedState();
  NAU_LOG_DEBUG("DX12: API resumed...");

#if _TARGET_XBOX
  NAU_LOG_DEBUG("DX12: Restoring swapchain configuration...");
...
#endif
  {
    NAU_LOG_DEBUG("DX12: Sending worker resume request to worker thread...");
...
static void check_dlss_state(DlssState state, const char *setting_name, int quality)
{
  switch (state) // -V785
  {
    case DlssState::SUPPORTED:
      NAU_LOG_DEBUG("DX12: DLSS is supported on this hardware, but it was not enabled. video/{}: {}", setting_name, quality);
      break;
    case DlssState::READY: NAU_LOG_DEBUG("DX12: DLSS is supported and is ready to go. video/{}: {}", setting_name, quality); break;
    case DlssState::NOT_SUPPORTED_INCOMPATIBLE_HARDWARE: NAU_LOG_DEBUG("DX12: DLSS is not supported on this hardware."); break;
    case DlssState::NOT_SUPPORTED_OUTDATED_VGA_DRIVER:
      NAU_LOG_DEBUG("DX12: DLSS is not supported on because of outdated VGA drivers.");
      break;
    case DlssState::NOT_SUPPORTED_32BIT: NAU_LOG_DEBUG("DX12: DLSS is not supported in 32bit builds."); break;
    case DlssState::DISABLED: NAU_LOG_DEBUG("DX12 drv: DLSS is disabled for drv3d_DX12 lib."); break;
    case DlssState::NGX_INIT_ERROR_NO_APP_ID:
      NAU_LOG_DEBUG("DX12: Couldn't initialize NGX because there is no Nvidia AppID set in game specific settings.blk");
      break;
    default: NAU_LOG_ERROR("DX12: Unexpected DLSS state after initialization: {}", int(state)); break;
  }
}
#endif
...
{
  NAU_ASSERT(offset % D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT == 0);
...
void DeviceContext::mapTileToResource(BaseTex *tex, ResourceHeap *heap, const TileMapping *mapping, size_t mapping_count)
{
  ...
  if (heap)
  {
    ...
    NAU_ASSERT(memory, "DX12: getUserHeapMemory for user heap {:p} returned empty memory range", (void*)heap);
    if (!memory)
    {
      return;
    }
...
{
  NAU_ASSERT_RETURN(nullptr != tex, , "DX12: Seems that a deleted texture was deactivated.");
  DX12_LOCK_FRONT();
...
struct BufferWriter
{
  ...
  void write(eastl::string_view text)
  {
    ...
    auto copyCount = Vectormath::min(text.length(), spaceLeft);
    memcpy(&buffer[offset], text.data(), copyCount);
    offset += copyCount;
  }

  void write(nau::string_view text)
  {
    size_t spaceLeft = size - offset - 1;
    auto copyCount = Vectormath::min(text.length(), spaceLeft);
    memcpy(&buffer[offset], text.data(), copyCount);
  ...
  size_t finish()
  {
    size_t len = Vectormath::min(offset, size - 1);
    buffer[len] = '\0';
    return len;
  }
};
...
void DeviceContext::ExecutionContext::setBlendConstantFactor(nau::math::E3DCOLOR constant)
{
  const float values[] = //
...
void DeviceContext::ExecutionContext::setInputLayout(InputLayoutID ident)
{
    contextState.graphicsState.inputLayoutIdent = ident;
}
...
bool DeviceContext::ExecutionContext::checkDrawCallHasOutput(eastl::span<const char> info)
{
  ...
  auto hasDepthStencil = contextState.graphicsState.framebufferState.framebufferLayout.depthBitStates.hasDepth != 0;
...
int64_t DeviceContext::ExecutionContext::flush(bool present, uint64_t progress, OutputMode mode)
{
  ...
  {
      for (auto& [key, addSwapchain] : self.back.addSwapchains)
      {
          contextState.resourceStates.useTextureAsSwapImageOutput(contextState.graphicsCommandListBarrierBatch,
              contextState.graphicsCommandListSplitBarrierTracker, addSwapchain.getColorImage());
      }
  ...
  {
    NAU_ASSERT(!contextState.postUploadBarrierBatch.hasBarriers(), "No texture uploads but barriers "
                                                                  "to flush some?");
  }
...
{ // TODO
  ...
  G_UNUSED(group_count);
  G_UNUSED(groups);
  NAU_ASSERT(false, "ExecutionContext::addRaytracePipeline called on API without support");
}
...
{ // TODO
  ...
  G_UNUSED(size);
  G_UNUSED(ptr);
  NAU_ASSERT(false, "ExecutionContext::copyRaytraceShaderGroupHandlesToMemory called on API without support");
}
  ...
  uint32_t latency_frame, OutputMode mode)
{
  ...
  /*  uint32_t deltaAlloc = device->memory.getAllocationCounter() - device->lastAllocationCount;
    ...
    if ((deltaAlloc + deltaFree) > 10)
    {
      ...
      NAU_LOG_DEBUG("Allocation count delta: {}", deltaAlloc);
      NAU_LOG_DEBUG("Free count delta: {}", deltaFree);
  ...
  uint32_t size)
{
  ...
  // never happen.
  NAU_ASSERT(src.resourceId != dst.resourceId, "It is invalid to copy with the source and the "
...
void DeviceContext::ExecutionContext::copyImage(Image *src, Image *dst, const ImageCopy &copy)
{
  ...
  else
  {
    ...
    for (auto p : Vectormath::min(srcPlaneCount, dstPlaneCount))
    {
      G_UNUSED(p);
...
void DeviceContext::ExecutionContext::resolveMultiSampleImage(Image *src, Image *dst)
{
  ...
  NAU_ASSERT(src->getFormat().asDxGiFormat() == dst->getFormat().asDxGiFormat());
  NAU_ASSERT(!src->getFormat().isDepth() || !src->isMultisampled() || d3d::get_driver_desc().caps.hasRenderPassDepthResolve);
...
void DeviceContext::ExecutionContext::clearRenderTargets(ViewportState vp, uint32_t clear_mask, const nau::math::E3DCOLOR *clear_color,
  float clear_depth, uint8_t clear_stencil)
{
  ...
  if (!ccMask && (null_clear_flag == dscMask))
  {
    ...
    NAU_LOG_WARNING("DX12: Clear was requested but no corresponding targets where set, active color target mask "
            "0x{:02X}, depth was set {}, clear request mask 0x{:02X}",
  ...
  else
  {
    ...
    for (auto i : nau::math::LsbVisitor{ccMask})
    {
      float cc[4];
...
void DeviceContext::ExecutionContext::flushRenderTargets()
{
  ...
  uint32_t cnt = 32 - nau::math::__clz(framebufferState.framebufferLayout.colorTargetMask);
...
void DeviceContext::ExecutionContext::flushGraphicsResourceBindings()
{
  ...
  bool needsFullFlush = frame.resourceViewHeaps.reserveSpace(devicePtr, totalBRegisterDescriptorCount, totalTRegisterDescriptorCount,
    totalURegisterDescriptorCount, bindlessCount, bindlessRev);
  ...
  vsStageState.pushConstantBuffers(devicePtr, frame.resourceViewHeaps, self.back.constBufferStreamDescriptors, nullConstBufferView,
    signature.vsCombinedBRegisterMask, contextState.cmdBuffer, STAGE_VS, constBufferMode);
  ...
  psStageState.pushConstantBuffers(devicePtr, frame.resourceViewHeaps, self.back.constBufferStreamDescriptors, nullConstBufferView,
    psHeader.resourceUsageTable.bRegisterUseMask, contextState.cmdBuffer, STAGE_PS, constBufferMode);
...
void DeviceContext::ExecutionContext::flushIndexBuffer()
{
  ...
  if (contextState.graphicsState.statusBits.test(GraphicsState::INDEX_BUFFER_DIRTY))
  {
    ...
    {
      NAU_ASSERT(buffer.size <= eastl::numeric_limits<uint32_t>::max());
...
void DeviceContext::ExecutionContext::flushVertexBuffers()
{
  constexpr uint32_t vertexBufferDirtyMask = 1u << GraphicsState::VERTEX_BUFFER_0_DIRTY | 1u << GraphicsState::VERTEX_BUFFER_1_DIRTY |
                                             ...
                                             1u << GraphicsState::VERTEX_BUFFER_4_DIRTY | 1u << GraphicsState::VERTEX_BUFFER_5_DIRTY |
                                             1u << GraphicsState::VERTEX_BUFFER_6_DIRTY | 1u << GraphicsState::VERTEX_BUFFER_7_DIRTY;
  ...
  constexpr uint32_t vertexBufferStateDirtyMask =
    ...
    1u << GraphicsState::VERTEX_BUFFER_STATE_4_DIRTY | 1u << GraphicsState::VERTEX_BUFFER_STATE_5_DIRTY |
    1u << GraphicsState::VERTEX_BUFFER_STATE_6_DIRTY | 1u << GraphicsState::VERTEX_BUFFER_STATE_7_DIRTY;
  ...
  if (vertexBufferBits)
  {
    first = nau::math::__ctz_unsafe(vertexBufferBits);
    last = 32 - nau::math::__clz_unsafe(vertexBufferBits);
    count = last - first;
  }

  // now create the view table
  D3D12_VERTEX_BUFFER_VIEW views[MAX_VERTEX_INPUT_STREAMS];
  ...
  BufferImageCopyListRef::RangeType regions, DeviceQueueType queue)
{
  ...
  {
    NAU_FAILURE("DX12: Texture read back requested on invalid queue {}", static_cast<uint32_t>(queue));
  }
}
  ...
  size_t offset)
{
  ...
  if ((infoD.Width < (dstOffset + buffer.size)) || (infoS.Width < (srcOffset + buffer.size)))
  {
    ...
    NAU_LOG_ERROR("DX12: Read Back Copy out of range (skipping):");
    cpu_memory.buffer->GetPrivateData(WKPDID_D3DDebugObjectNameW, &cnt, wcbuf);
    eastl::copy(wcbuf, wcbuf + cnt / sizeof(wchar_t), cbuf);
    NAU_LOG_ERROR("DX12: Dest '{}' Size {}, copy range {} - {}", cbuf, infoD.Width, dstOffset, dstOffset + buffer.size);

    buffer.buffer->GetPrivateData(WKPDID_D3DDebugObjectNameW, &cnt, wcbuf);
    eastl::copy(wcbuf, wcbuf + cnt / sizeof(wchar_t), cbuf);
    NAU_LOG_ERROR("DX12: Source '{}' Size {}, copy range {} - {}", cbuf, infoS.Width, srcOffset, srcOffset + buffer.size);
    return;
  }
#endif
  ...
  HostDeviceSharedMemoryRegion source, size_t source_offset)
{
  ...
  auto copyBufferRegion = [&](CopyCommandList<VersionedPtr<D3DCopyCommandList>> &activeUploadCommandList) {
    ...
    if ((infoD.Width < (dstOffset + target.size)) || (infoS.Width < (srcOffset + target.size)))
    {
      ...
      char cbuf[MAX_OBJECT_NAME_LENGTH];
      UINT cnt = sizeof(wcbuf);
      NAU_LOG_ERROR("DX12: Upload Copy out of range (skipping):");
      target.buffer->GetPrivateData(WKPDID_D3DDebugObjectNameW, &cnt, wcbuf);
      eastl::copy(wcbuf, wcbuf + cnt / sizeof(wchar_t), cbuf);
      NAU_LOG_ERROR("DX12: Dest '{}' Size {}, copy range {} - {}", cbuf, infoD.Width, dstOffset, dstOffset + target.size);

      source.buffer->GetPrivateData(WKPDID_D3DDebugObjectNameW, &cnt, wcbuf);
      eastl::copy(wcbuf, wcbuf + cnt / sizeof(wchar_t), cbuf);
      NAU_LOG_ERROR("DX12: Source '{}' Size {}, copy range {} - {}", cbuf, infoS.Width, srcOffset, srcOffset + target.size);
      return;
    }
#endif
...
void DeviceContext::ExecutionContext::bufferBarrier(BufferResourceReference buffer, ResourceBarrier barrier, GpuPipeline)
{
  ...
  if (buffer)
  {
    ...
    NAU_LOG_DEBUG("DX12: Resource barrier for buffer {} - {:p}, with {}, during {}", get_resource_name(buffer.buffer, cbuf), buffer.buffer,
  ...
  ResourceBarrier barrier, GpuPipeline, bool force_barrier)
{
  ...
  if (RB_NONE != (RB_ALIAS_FROM & barrier))
  {
    ...
    if (tex)
    {
      ...
      NAU_ASSERT(nullptr == contextState.lastAliasBegin, "DX12: Missing end aliasing barrier?");
  ...
  if (RB_NONE != (RB_ALIAS_TO & barrier))
  {
    ...
    NAU_ASSERT(validAliasingBarrier, "DX12: Missing begin aliasing barrier?");
    ...
    if (RB_NONE != (RB_FLAG_DONT_PRESERVE_CONTENT & barrier))
    {
      if (!tex)
      {
        NAU_LOG_ERROR("DX12: RB_ALIAS_TO_AND_DISCARD used with null resource!");
        return;
      }
  ...
  if (tex)
  {
    ...
    make_resource_barrier_string_from_state(maskNameBuffer, countof(maskNameBuffer), state, barrier);
    NAU_LOG_DEBUG("DX12: Resource barrier for texture {} - {:p} [{} - {}], with {}, during {}", static_cast<uint32_t>(barrier),
...
{
  NAU_LOG_DEBUG("DX12: Engine requested to suspend...");
  NAU_LOG_DEBUG("DX12: Entered suspended state, notifying engine and waiting for resume "
         "request...");
  ...
  NAU_LOG_DEBUG("DX12: Engine requested to resume...");
}
#endif
...
void DeviceContext::ExecutionContext::writeDebugMessage(StringIndexRef::RangeType message, int severity)
{
  if (severity < 1)
  {
    NAU_LOG_DEBUG(message.data());
  }
  else if (severity < 2)
  {
    NAU_LOG_WARNING(message.data());
  }
  else
  {
    NAU_LOG_ERROR(message.data());
  }
}
...
{
    if (image->swapChainID != DEFAULT_SWAPID)
    {
        //if (self.back.addSwapchain.isInitialized)
        {
            NAU_ASSERT(self.back.addSwapchains.count(image->swapChainID));
            self.back.addSwapchains[image->swapChainID].registerSwapchainView(device.device.get(), image, view);
        }
    }
    else
    {
...
void DeviceContext::ExecutionContext::onSwapchainSwapCompleted()
{
    for (auto& [key, addSwapchain] : self.back.addSwapchains)
    {
        addSwapchain.onFrameBegin();
    ...
    self.back.swapchain.onFrameBegin(device.device.get());
}
#endif
  ...
  uint32_t src_mip_map_offset, uint32_t dst_mip_map_offset)
{
  ...
  for (auto level : mip_map_range)
  {
    ...
    const auto copyExtent = max_extent(dstExtent, srcExtent);
    ...
    if (dstExtent != srcExtent)
    {
      src->getDebugName([src, srcLevel, dst, dstLevel, srcExtent, dstExtent](auto &name) {
        NAU_LOG_ERROR("DX12: resizeImageMipMapTransfer of <{}> from {:p}[{}] to {:p}[{}] mip-map level sizes "
               "do not match {}x{}x{} vs {}x{}x{}, using Vectormath::min",
          name, (void*)src, srcLevel.index(), (void*)dst, dstLevel.index(), srcExtent.width, srcExtent.height, srcExtent.depth, dstExtent.width,
