...
#include "driver.h"
#include "drv3d_commonCode/validation.h"
#include "constants.h"
...
class StatefulCommandBuffer
{
  ...
  bool validateTopology()
  {
    ...
    {
      NAU_LOG_ERROR("Invalid primitive topology {} for draw call encountered. Geometry stage active {}, "
             "Tessellation stage active {}.",
  ...
  bool canDraw()
  {
      return nullptr != graphicsState.pipeline;
  }
  
  bool canDispatch()
  {
      return nullptr != computeState.pipeline;
  }
    ...
    if (globalState.dirtyState.test(GlobalState::DirtySet::DESCRIPTOR_HEAPS))
    {
      ...
      else
      {
        if (need_sampler_heap == NeedSamplerHeap::Yes && globalState.descriptorHeaps[0] == nullptr)
          NAU_LOG_WARNING("The sampler heap is not set.");
        if (globalState.descriptorHeaps[1] == nullptr)
          NAU_LOG_WARNING("The resource heap is not set.");
...
public:
  ...
  {
    NAU_ASSERT(cmd.is<ID3D12GraphicsCommandList4>(), "Trying to execute raytrace commands on unsupported command list version");
  ...
  void setConstantBuffer(uint32_t stage, uint32_t slot, D3D12_GPU_VIRTUAL_ADDRESS cbv)
  {
    ...
    {
      NAU_ASSERT(!"DX12: Invalid shader stage");
    }
  }
  ...
  void setViewports(eastl::span<const D3D12_VIEWPORT> viewports)
  {
