  ...
  const void *dlss_params, const NVSDK_NGX_Dimensions &render_dimensions)
{
  ...
  NAU_ASSERT_RETURN(dlssParams.inColor != nullptr, NVSDK_NGX_Result_FAIL_InvalidParameter);
  NAU_ASSERT_RETURN(dlssParams.inDepth != nullptr, NVSDK_NGX_Result_FAIL_InvalidParameter);
  NAU_ASSERT_RETURN(dlssParams.inMotionVectors != nullptr, NVSDK_NGX_Result_FAIL_InvalidParameter);
  NAU_ASSERT_RETURN(dlssParams.outColor != nullptr, NVSDK_NGX_Result_FAIL_InvalidParameter);
  NAU_ASSERT_RETURN(dlssParams.inSharpness >= 0 && dlssParams.inSharpness <= 1, NVSDK_NGX_Result_FAIL_InvalidParameter,
    "Invalid inSharpness value in dlssParams. Value should be between in range [0, 1], but value is: %f", dlssParams.inSharpness);
  NAU_ASSERT_RETURN(abs(dlssParams.inJitterOffsetX) <= 0.5f && abs(dlssParams.inJitterOffsetY) <= 0.5f,
