...
struct FramebufferLayout
{
  ...
  void checkMsaaLevelsEqual() const
  {
    ...
    for (const auto index : nau::math::LsbVisitor{colorTargetMask})
    {
      NAU_ASSERT(!previous || *previous == getColorMsaaLevel(index));
      previous = getColorMsaaLevel(index);
    }
    NAU_ASSERT(!previous || !depthBitStates.hasDepth || *previous == depthBitStates.depthMsaaLevel);
  }
  ...
  {
    uint32_t mask = 0;
    for (auto i : nau::math::LsbVisitor{colorTargetMask})
    {
...
struct ComputePipelineSignature
{
    ...
    Bitset<dxil::MAX_B_REGISTERS> dirty_mask, bool descriptor_range_dirty)
  {
    ...
    {
      for (auto i : nau::math::LsbVisitor{def.registers.bRegisterUseMask & ~(1u << ROOT_CONSTANT_BUFFER_INDEX)})
      {
...
struct GraphicsPipelineSignature
{
    ...
    Bitset<dxil::MAX_B_REGISTERS> dirty_mask, bool descriptor_range_dirty)
  {
    if (def.psLayout.usesConstBufferRootDescriptors(def.vsLayout, def.gsLayout, def.hsLayout, def.dsLayout))
    {
      ...
      for (auto i : nau::math::LsbVisitor{def.vertexShaderRegisters.bRegisterUseMask & ~(1u << ROOT_CONSTANT_BUFFER_INDEX)})
      {
        if (dirty_mask.test(i))
        {
      ...
      for (auto i : nau::math::LsbVisitor{def.geometryShaderRegisters.bRegisterUseMask & ~(1u << ROOT_CONSTANT_BUFFER_INDEX)})
      {
        if (dirty_mask.test(i))
        {
          cmd.setGraphicsRootConstantBufferView(index, bcvs[i]);
      ...
      for (auto i : nau::math::LsbVisitor{def.hullShaderRegisters.bRegisterUseMask & ~(1u << ROOT_CONSTANT_BUFFER_INDEX)})
      {
        if (dirty_mask.test(i))
        {
          cmd.setGraphicsRootConstantBufferView(index, bcvs[i]);
      ...
      for (auto i : nau::math::LsbVisitor{def.domainShaderRegisters.bRegisterUseMask & ~(1u << ROOT_CONSTANT_BUFFER_INDEX)})
      {
        if (dirty_mask.test(i))
        {
          cmd.setGraphicsRootConstantBufferView(index, bcvs[i]);
    ...
    Bitset<dxil::MAX_B_REGISTERS> dirty_mask, bool descriptor_range_dirty)
  {
    ...
    {
      for (auto i : nau::math::LsbVisitor{def.pixelShaderRegisters.bRegisterUseMask & ~(1u << ROOT_CONSTANT_BUFFER_INDEX)})
      {
        if (dirty_mask.test(i))
        {
