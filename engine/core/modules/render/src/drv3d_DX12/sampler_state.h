...
BEGIN_BITFIELD_TYPE(SamplerState, uint32_t)
  enum
  {
    ...
    COORD_SHIFT = FILTER_SHIFT + FILTER_BITS*2,
    ANISO_BITS = 3,
  ...
  ADD_BITFIELD_MEMBER(filterMinMode, FILTER_SHIFT, FILTER_BITS)
  ADD_BITFIELD_MEMBER(filterMagMode, FILTER_SHIFT + FILTER_BITS, FILTER_BITS)
  ...
  D3D12_SAMPLER_DESC asDesc() const
  {
    ...
    if (result.MaxAnisotropy > 1)
      ...
      result.Filter = D3D12_ENCODE_BASIC_FILTER(getMinFilter(), getMagFilter(), getMip(), static_cast<uint32_t>(isCompare));
  ...
  void setMinFilter(D3D12_FILTER_TYPE filter) { filterMinMode = filter; }
  void setMagFilter(D3D12_FILTER_TYPE filter) { filterMagMode = filter; }
  D3D12_FILTER_TYPE getMinFilter() const { return static_cast<D3D12_FILTER_TYPE>(static_cast<uint32_t>(filterMinMode)); }
  D3D12_FILTER_TYPE getMagFilter() const { return static_cast<D3D12_FILTER_TYPE>(static_cast<uint32_t>(filterMagMode)); }
  ...
  {
      Vectormath::half hlf(b);
      biasBits = hlf.sh;
  }
  float getBias() const
  {
      Vectormath::half hlf;
      hlf.sh = static_cast<unsigned short>(biasBits);
      return (float)hlf;
  }
  void setAniso(float a)
  {
    ...
    // clamp from 1 to 16
    value = Vectormath::clamp(value, 0, 4);
  ...
  void setBorder(nau::math::E3DCOLOR color) { borderColorMode = ((color.r || color.g || color.b) ? 1 : 0) | (color.a ? 2 : 0); }
  nau::math::E3DCOLOR getBorder() const
  {
    nau::math::E3DCOLOR result;
...
{
  SamplerState state;
  state.isCompare = info.isCompare;
  state.setMinFilter(translate_filter_type_to_dx12(static_cast<int>(info.filter_min_mode)));
  state.setMagFilter(translate_filter_type_to_dx12(static_cast<int>(info.filter_mag_mode)));
