...
inline bool validate_resource_state(D3D12_RESOURCE_STATES state, uint32_t resource_flags, bool is_buffer)
{
  ...
  if (errors.any())
  {
    ...
    {
      NAU_LOG_ERROR("DX12: Invalid resource state, multiple write bits set, with {:#08x}", (int)state);
    }
    if (errors.test(ResourceStateValidationFailBits::READ_WRITE_BITS_CONFLICT))
    {
      NAU_LOG_ERROR("DX12: Invalid resource state, read and write bits set, with {:#08x}", (int)state);
    }
    if (errors.test(ResourceStateValidationFailBits::INVALID_READ_BITS_COMBINATION))
    {
      NAU_LOG_ERROR("DX12: Invalid resource state, invalid combination of read bits set, with {:#08x}", (int)state);
    }
    if (errors.test(ResourceStateValidationFailBits::UAV_ON_INCOMPATIBE_RESOURCE))
    {
      NAU_LOG_ERROR("DX12: Invalid resource state, UAV state without proper resource flags, with {:#08x}", (int)state);
    }
    if (errors.test(ResourceStateValidationFailBits::RENDER_TARGET_ON_INCOMPATIBLE_RESOURCE))
    {
      NAU_LOG_ERROR("DX12: Invalid resource state, RTV state without proper resource flags, with {:#08x}", (int)state);
    }
    if (errors.test(ResourceStateValidationFailBits::DEPTH_STENCIL_ON_INCOMPATIBLE_RESOURCE))
    {
      NAU_LOG_ERROR("DX12: Invalid resource state, DSV state without proper resource flags, with {:#08x}", (int)state);
    }
    if (errors.test(ResourceStateValidationFailBits::SRV_ON_INCOMPATILBE_RESOURCE))
    {
      NAU_LOG_ERROR("DX12: Invalid resource state, SRV state without proper resource flags, with {:#08x}", (int)state);
    }
  }
...
inline bool validate_transition_barrier(const D3D12_RESOURCE_TRANSITION_BARRIER &barrier)
{
  ...
  if (beforeErrors.any() || afterErrors.any() || (barrier.StateBefore == barrier.StateAfter) || !subResourceIndexIsValid)
  {
    ...
    {
      NAU_LOG_ERROR("DX12: Invalid transition barrier, subresource index {} is out of range, for <{}> "
             "({})",
    ...
    {
      NAU_LOG_ERROR("DX12: Invalid transition barrier, before and after state are identical, for <{}> "
             "({}) {} with StateBefore as {:#08x}",
        resnameBuffer, to_string(desc.Dimension), barrier.Subresource, (int)barrier.StateBefore);
    }

    if (beforeErrors.test(ResourceStateValidationFailBits::MULTPLE_WRITE_BITS_CONFLICT))
    {
      NAU_LOG_ERROR("DX12: Invalid transition barrier, multiple write bits set, for <{}> ({}) {} with "
             "StateBefore as {:#08x}",
        resnameBuffer, to_string(desc.Dimension), barrier.Subresource, (int)barrier.StateBefore);
    }
    if (beforeErrors.test(ResourceStateValidationFailBits::READ_WRITE_BITS_CONFLICT))
    {
      NAU_LOG_ERROR("DX12: Invalid transition barrier, read and write bits set, for <{}> ({}) {} with "
             "StateBefore as {:#08x}",
        resnameBuffer, to_string(desc.Dimension), barrier.Subresource, (int)barrier.StateBefore);
    }
    if (beforeErrors.test(ResourceStateValidationFailBits::INVALID_READ_BITS_COMBINATION))
    {
      NAU_LOG_ERROR("DX12: Invalid transition barrier, invalid combination of read bits set, for <{}> "
             "({}) {} with StateBefore as {:#08x}",
        resnameBuffer, to_string(desc.Dimension), barrier.Subresource, (int)barrier.StateBefore);
    }
    if (beforeErrors.test(ResourceStateValidationFailBits::UAV_ON_INCOMPATIBE_RESOURCE))
    {
      NAU_LOG_ERROR("DX12: Invalid transition barrier, UAV state without proper resource flags, for <{}> "
             "({}) {} with StateBefore as {:#08x}",
        resnameBuffer, to_string(desc.Dimension), barrier.Subresource, (int)barrier.StateBefore);
    }
    if (beforeErrors.test(ResourceStateValidationFailBits::RENDER_TARGET_ON_INCOMPATIBLE_RESOURCE))
    {
      NAU_LOG_ERROR("DX12: Invalid transition barrier, RTV state without proper resource flags, for <{}> "
             "({}) {} with StateBefore as {:#08x}",
        resnameBuffer, to_string(desc.Dimension), barrier.Subresource, (int)barrier.StateBefore);
    }
    if (beforeErrors.test(ResourceStateValidationFailBits::DEPTH_STENCIL_ON_INCOMPATIBLE_RESOURCE))
    {
      NAU_LOG_ERROR("DX12: Invalid transition barrier, DSV state without proper resource flags, for <{}> "
             "({}) {} with StateBefore as {:#08x}",
        resnameBuffer, to_string(desc.Dimension), barrier.Subresource, (int)barrier.StateBefore);
    }
    if (beforeErrors.test(ResourceStateValidationFailBits::SRV_ON_INCOMPATILBE_RESOURCE))
    {
      NAU_LOG_ERROR("DX12: Invalid transition barrier, SRV state without proper resource flags, for <{}> "
             "({}) {} with StateBefore as {:#08x}",
        resnameBuffer, to_string(desc.Dimension), barrier.Subresource, (int)barrier.StateBefore);
    }
    ...
    if (afterErrors.test(ResourceStateValidationFailBits::MULTPLE_WRITE_BITS_CONFLICT))
    {
      NAU_LOG_ERROR("DX12: Invalid transition barrier, multiple write bits set, for <{}> ({}) {} with "
             "StateAfter as {:#08x}",
        resnameBuffer, to_string(desc.Dimension), barrier.Subresource, (int)barrier.StateAfter);
    }
...
namespace detail
{
template <uint32_t Mask, uint32_t Index = 31, bool BitSet = bool(Mask & (1u << Index))>
...
public:
  D3D12_RESOURCE_STATES get() const
  {
    ...
    static_assert(detail::CountBits<AvailableBits>::value >= StatusBits);
  ...
  {
    NAU_ASSERT(user_bit_check(new_value));
...
{
  NAU_LOG_DEBUG(".Transition.pResource = {:p}", (void*)transition.pResource);
  NAU_LOG_DEBUG(".Transition.Subresource = {}", transition.Subresource);
  NAU_LOG_DEBUG(".Transition.StateBefore = {:#x}", (uint32_t)transition.StateBefore);
  NAU_LOG_DEBUG(".Transition.StateAfter = {:#x}", (uint32_t)transition.StateAfter);
}

inline void print_barrier(const D3D12_RESOURCE_ALIASING_BARRIER &aliasing)
{
  NAU_LOG_DEBUG(".Aliasing.pResourceBefore = {:p}", (uint32_t)aliasing.pResourceBefore);
  NAU_LOG_DEBUG(".Aliasing.pResourceAfter = {:p}", (uint32_t)aliasing.pResourceAfter);
}

inline void print_barrier(const D3D12_RESOURCE_UAV_BARRIER &uav) { NAU_LOG_DEBUG(".UAV.pResource = {:p}", (void*)uav.pResource); }

inline void print_barrier(const D3D12_RESOURCE_BARRIER &barrier)
{
  NAU_LOG_DEBUG(".Type = {}", (uint32_t)barrier.Type);
  NAU_LOG_DEBUG(".Flags = {:#x}", (uint32_t)barrier.Flags);
  switch (barrier.Type)
  {
...
{
  eastl::vector<D3D12_RESOURCE_BARRIER> dataSet;
...
public:
  ...
  void beginTransition(ID3D12Resource *res, SubresourceIndex sub_res, D3D12_RESOURCE_STATES from, D3D12_RESOURCE_STATES to)
  {
    ...
    NAU_ASSERT(validate_transition_barrier(newBarrier.Transition));
  ...
  bool tryUpdateTransition(ID3D12Resource *res, SubresourceIndex sub_res, D3D12_RESOURCE_STATES from, D3D12_RESOURCE_STATES to)
  {
    ...
    if (ref->Transition.StateAfter != from)
    {
      for (size_t i = 0; i < dataSet.size(); ++i)
      {
        ...
        NAU_LOG_DEBUG("Barrier[{}]:{}", i, postFix);
        print_barrier(b);
      }
      NAU_ASSERT(ref->Transition.StateAfter == from, "Unexpected barrier found, see listing in debug log for more detail!");
      return false;
    }
    ...
    NAU_ASSERT(validate_transition_barrier(ref->Transition));
    return true;
  }
  ...
  bool tryFuseBeginEndTransition(ID3D12Resource *res, SubresourceIndex sub_res, D3D12_RESOURCE_STATES from, D3D12_RESOURCE_STATES to)
  {
    ...
    G_UNUSED(from);
    G_UNUSED(to);
    NAU_ASSERT(ref->Transition.StateBefore == from);
    NAU_ASSERT(ref->Transition.StateAfter == to);
    return true;
  }
...
public:
  ...
  {
    NAU_ASSERT(res != nullptr, "DX12: Can not turn off null resource uav flush");
  ...
  void implicitFlushAll(const char *where, bool report_user_barriers)
  {
    ...
    if (report_user_barriers)
    {
      ...
      {
        NAU_LOG_WARNING("DX12: Dropping UAV barrier for resource {} - {:p}, during {}", get_resource_name(res, cbuf), res, where);
      }
    }
#else
    G_UNUSED(where);
    ...
    for (; inPos != inEnd && outPos != outEnd; ++inPos)
    {
      ...
      if (*inPos == *outPos)
      {
        if (end(userSkippedUavSync) == eastl::find(begin(userSkippedUavSync), end(userSkippedUavSync), *inPos))
        {
          ...
          {
            char cbuf[MAX_OBJECT_NAME_LENGTH];
            NAU_LOG_ERROR("DX12: Missing RB_FLUSH_UAV barrier for resource {} - {:p}, should "
...
private:
    ...
    SubresourceIndex sub_res_index, D3D12_RESOURCE_STATES from, D3D12_RESOURCE_STATES to, TransitionResult action)
  {
    ...
    G_UNUSED(action);
#if DAGOR_DBGLEVEL > 0
    NAU_ASSERT_RETURN(global_base.isValid(), );
    ...
    D3D12_RESOURCE_STATES from, D3D12_RESOURCE_STATES to, TransitionResult action)
  {
    ...
    {
      NAU_LOG_DEBUG("DX12: StateTrack: Transitioned {} {:p} <{}>[{}] {} from <{}> to <{}>", resTypeTable[is_texture], (void*)resource, resnameBuffer,
    ...
    {
      NAU_LOG_DEBUG("DX12: StateTrack: Auto promoted {} {:p} <{}>[{}] {} from <{}> to <{}>", resTypeTable[is_texture], (void*)resource,
    ...
    {
      NAU_LOG_DEBUG("DX12: StateTrack: Updated existing transition of {} {:p} <{}>[{}] {} from <{}> with <{}>", resTypeTable[is_texture],
        (void*)resource, resnameBuffer, sub_res_index, global_base + sub_res_index, fromTransitionMaskText, toTransitionMaskText);
    }
    else if (TransitionResult::Folded == action)
    {
      NAU_LOG_DEBUG("DX12: StateTrack: Transformed split barrier into updated regular barrier (Folded) for "
             "%s 0x%p <%s>[%u] %u from <%s> with <%s>",
        resTypeTable[is_texture], (void*)resource, resnameBuffer, sub_res_index, global_base + sub_res_index, fromTransitionMaskText,
        toTransitionMaskText);
    }
    else if (TransitionResult::Skipped == action)
    {
      NAU_LOG_DEBUG("DX12: StateTrack: Skipped transition for {} {:p} <{}>[{}] {} from <{}> to <{}>", resTypeTable[is_texture], (void*)resource,
        resnameBuffer, sub_res_index, global_base + sub_res_index, fromTransitionMaskText, toTransitionMaskText);
    }
    else if (TransitionResult::UserSkipped == action)
    {
      NAU_LOG_DEBUG("DX12: StateTrack: Skipped transition on user request for {} {:p} <{}>[{}] {} from <{}> "
             "to <{}>",
        resTypeTable[is_texture], (void*)resource, resnameBuffer, sub_res_index, global_base + sub_res_index, fromTransitionMaskText,
        toTransitionMaskText);
    }
    ...
    {
      NAU_LOG_DEBUG("DX12: StateTrack: Split barrier end was broader than the begin barrier of {} {:p} "
             "<{}>[{}] {} from <{}> to <{}>",
        resTypeTable[is_texture], (void*)resource, resnameBuffer, sub_res_index, global_base + sub_res_index, fromTransitionMaskText,
        toTransitionMaskText);
    }
    ...
    {
      NAU_LOG_DEBUG("DX12: StateTrack: Expansion {} {:p} <{}>[{}] {} from <{}> with additional <{}>", resTypeTable[is_texture], (void*)resource,
        resnameBuffer, sub_res_index, global_base + sub_res_index, fromTransitionMaskText, toTransitionMaskText);
    }
    else if (TransitionResult::Fused == action)
    {
      NAU_LOG_DEBUG("DX12: StateTrack: Fused back and forth barrier {} {:p} <{}>[{}] {} from <{}> to <{}>", resTypeTable[is_texture],
        (void*)resource, resnameBuffer, sub_res_index, global_base + sub_res_index, fromTransitionMaskText, toTransitionMaskText);
    }
  }
...
public:
  ...
  void decay()
  {
    ...
    if (reportDecay)
    {
      NAU_LOG_DEBUG("DX12: StateTrack: Decaying resource state of all regular buffers and auto promoted textures to <COMMON>");
      reportDecay = false;
    }
  }
#endif
...
// only report errors on dev builds
            ...
            if (!isFullState)
            {
              if (D3D12_RESOURCE_STATE_COPY_QUEUE_TARGET == currentState)
              {
                ...
                // dispatch call.
                NAU_LOG_ERROR("DX12: Resource tracker deduced a possible uninitialized subresource {} of "
                       "the static texture <{}>, this is probably an engine error, where a static "
      ...
      else
      {
        ...
        char cbuf[MAX_OBJECT_NAME_LENGTH];
        get_resource_name(res, cbuf);
        NAU_ASSERT(state == (D3D12_RESOURCE_STATES_STATIC_TEXTURE_READ_STATE & state), // -V616
  ...
  {
    NAU_ASSERT_RETURN(global_base.isValid(), true);
...
#if DAGOR_DBGLEVEL > 0
      char cbuf[MAX_OBJECT_NAME_LENGTH];
      NAU_ASSERT(endState == state, // -V547 always false
    ...
    ExtendedImageGlobalSubresouceId global_base, SubresourceIndex subresource, D3D12_RESOURCE_STATES state)
  {
    ...
    {
      char cbuf[MAX_OBJECT_NAME_LENGTH];
      NAU_ASSERT(state == (endState & state),
...
public:
  ...
  void useBufferAsCBV(BarrierBatcher &barriers, uint32_t stage, BufferResourceReference buffer)
  {
    ...
    G_UNUSED(stage);
    transitionBuffer(barriers, buffer.buffer, buffer.resourceId, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER|D3D12_RESOURCE_STATE_INDEX_BUFFER);
  }
  ...
  void useBufferAsIB(BarrierBatcher &barriers, BufferResourceReference buffer)
  {
    ...
    transitionBuffer(barriers, buffer.buffer, buffer.resourceId, D3D12_RESOURCE_STATE_INDEX_BUFFER|D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER);
  }
  ...
  void useTextureAsUAV(BarrierBatcher &barriers, SplitTransitionTracker &stt, uint32_t stage, Image *texture, ArrayLayerRange arrays,
    MipMapRange mips)
  {
    NAU_ASSERT(texture->hasTrackedState());
    G_UNUSED(stage);
  ...
  void useTextureToAliasToAndDiscard(BarrierBatcher &barriers, SplitTransitionTracker &stt, Image *texture, uint32_t tex_flags)
  {
    ...
    {
      NAU_LOG_ERROR("DX12: Trying to discard a texture that is neither a render target, a depth stencil "
...
private:
  ...
  nau::string_view segmentName(size_t index) const
  {
  ...
  nau::string_view resourceName(size_t index) const
  {
  ...
  size_t addResourceName(nau::string_view name)
  {
  ...
  size_t addResourceName(const char* name)
  {
      nau::string resName = {name};
      return addResourceName(resName);
  }
...
public:
  ...
  void beginEvent(eastl::span<const char> name)
  {
    if (segments.empty())
    {
        eastl::string tempStr = {begin(name), end(name) - 1};
      segments.emplace_back(tempStr.c_str());
    }
    else
    {
      auto place = segments.push_back_uninitialized();
      auto &last = *(segments.crbegin() + 1);
      auto &s = *::new (place) nau::string(last.size() + name.size());//nau::string(nau::string::CtorDoNotInitialize{}, last.size() + name.size());
      s.append(last);
      s.push_back('/');

      eastl::string tempUglyString = eastl::string(begin(name), end(name) - 1);

      s.append( tempUglyString.c_str() );
    }
  }
  ...
  void endEvent()
  {
    ...
    auto &last = *(segments.crbegin() + 1);
    //TODO: fix it
    //if(auto at = eastl::find_last_of(last.begin(), last.end(), '/'); at != nau::string::npos)
    //{
    //  ::new (place) nau::string(last, 0, at);
    //}
    //else
    //{
    //  ::new (place) nau::string();
    //}
    ::new (place) nau::string();
  }
  ...
  size_t memoryUse() const
  {
    ...
    for (auto &e : segments)
    {
      s += e.size() + 1; //e.capacity() + 1;
    }
    for (auto &n : resourceNames)
    {
      s += n.size() + 1; //n.capacity() + 1;
    }
...
class ResourceUsageHistoryDataSetDebugger
{
  ...
  void addAnalziedTextureInfo(D3D12_RESOURCE_STATES current_state, ResourceBarrier expected_state, nau::string_view segment,
    ResourceUsageHistoryDataSet::UsageEntryType what, bool is_automatic_transition, nau::string_view what_string,
    AnalyzedIssueType issue_type);
  void addUniqueAnalziedTextureInfo(D3D12_RESOURCE_STATES current_state, ResourceBarrier expected_state, nau::string_view segment,
    ResourceUsageHistoryDataSet::UsageEntryType what, bool is_automatic_transition, nau::string_view what_string,
    AnalyzedIssueType issue_type);
  void addAnalziedBufferInfo(D3D12_RESOURCE_STATES current_state, ResourceBarrier expected_state, nau::string_view segment,
    ResourceUsageHistoryDataSet::UsageEntryType what, bool is_automatic_transition, nau::string_view what_string,
    AnalyzedIssueType issue_type);
  void addUniqueAnalziedBufferInfo(D3D12_RESOURCE_STATES current_state, ResourceBarrier expected_state, nau::string_view segment,
    ResourceUsageHistoryDataSet::UsageEntryType what, bool is_automatic_transition, nau::string_view what_string,
    AnalyzedIssueType issue_type);
  void addTextureInfo(nau::string_view name, Image *texture, SubresourceIndex sub_res, FormatStore fmt, bool needs_transition);
  void addBufferInfo(nau::string_view name, BufferResourceReference buffer, bool needs_transition);
...
class BufferAccessTracker
{
  ...
  public:
    ...
    void deallocate(void *p, size_t) { ::operator delete[](p, std::align_val_t{std::hardware_constructive_interference_size}); }
