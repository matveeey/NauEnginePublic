...
inline DXGI_QUERY_VIDEO_MEMORY_INFO max(const DXGI_QUERY_VIDEO_MEMORY_INFO &l, const DXGI_QUERY_VIDEO_MEMORY_INFO &r)
{
  ...
  result.Budget = Vectormath::max(l.Budget, r.Budget);
  result.CurrentUsage = Vectormath::max(l.CurrentUsage, r.CurrentUsage);
  result.AvailableForReservation = Vectormath::max(l.AvailableForReservation, r.AvailableForReservation);
  result.CurrentReservation = Vectormath::max(l.CurrentReservation, r.CurrentReservation);
  return result;
}
#endif
...
{
  NAU_ASSERT(desc.Alignment != 0, "DX12: desc.Alignment should not be 0!");
