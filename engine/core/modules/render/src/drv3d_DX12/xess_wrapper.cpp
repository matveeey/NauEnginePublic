...
static xess_quality_settings_t toXeSSQuality(int quality)
{
  ...
  NAU_ASSERT(quality < eastl::extent<decltype(xessQualities)>::value);
...
public:
  bool loadLibrary()
  {
    ...
    if (!libxess)
    {
      NAU_LOG_DEBUG("DX12: XeSS: Failed to load libxess.dll, xess will be disabled");
      return false;
    }
...
#undef LOAD_FUNC
  ...
  xess_result_t evaluateXess(void *context, const void *params)
  {
    ...
    NAU_ASSERT_RETURN(xessParams.inColor != nullptr, XESS_RESULT_ERROR_INVALID_ARGUMENT);
    NAU_ASSERT_RETURN(xessParams.inDepth != nullptr, XESS_RESULT_ERROR_INVALID_ARGUMENT);
    NAU_ASSERT_RETURN(xessParams.inMotionVectors != nullptr, XESS_RESULT_ERROR_INVALID_ARGUMENT);
    NAU_ASSERT_RETURN(xessParams.outColor != nullptr, XESS_RESULT_ERROR_INVALID_ARGUMENT);
    NAU_ASSERT_RETURN(abs(xessParams.inJitterOffsetX) <= 0.5f && abs(xessParams.inJitterOffsetY) <= 0.5f,
  ...
  {
    NAU_ASSERT_RETURN(m_state == XessState::READY, false);

    xess_result_t result = evaluateXess(context, params);
    if (result != xess_result_t::XESS_RESULT_SUCCESS)
    {
      NAU_LOG_ERROR("DX12: XeSS: Failed to evaluate XeSS: {}", get_xess_result_as_string(result));
      return false;
    }
...
private:
  bool checkResult(xess_result_t result)
  {
    ...
    {
      NAU_LOG_WARNING("DX12: XeSS could not initialize: {}", get_xess_result_as_string(result));
    }
