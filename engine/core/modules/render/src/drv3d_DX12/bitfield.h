...
struct BitFieldMember
{
  ...
  inline T maximum() const
  {
    ...
    static_assert(Bits < (int)sizeof(T) * 8);
    return Maximum;
  }
  ...
  {
    NAU_ASSERT(v <= Maximum, "{} <= {}", v, maximum()); // v must fit inside the bitfield member
  ...
  inline BitFieldMember &operator+=(T v)
  {
    NAU_ASSERT(T(*this) + v <= Maximum); // result must fit inside the bitfield member
    value += v << Offset;
    return *this;
  }
  ...
  inline BitFieldMember &operator-=(T v)
  {
    NAU_ASSERT(T(*this) >= v); // result must not underflow
    value -= v << Offset;
    return *this;
  }
...
struct BitFieldArray
{
  ...
  public:
    ...
    {
      NAU_ASSERT(v <= Maximum); // v must fit inside the bitfield member
  ...
  {
    NAU_ASSERT(i >= 0 && i < NumItems); // array index must be in range
