...
public:
  ...
  static void patchTable(ID3D12Device *device, uint32_t vendor)
  {
    ...
    {
      NAU_LOG_DEBUG("DX12: Device vendor is AMD, assuming emulated D24S8, patching D24S8 to D32S8X24...");
    ...
    NAU_LOG_DEBUG("DX12: Patching planar format info table...");
    ...
    for (uint32_t i = 0; i < sizeof...(Formats); ++i)
    {
      ...
      NAU_LOG_DEBUG("DX12: {} PlantCount {}", dxgi_format_name(linearFormatArray[i]), fmtInfo.PlaneCount);
    }
  }
#endif
  static DXGI_FORMAT getFormat(uint32_t index, ColorSpace color_space)
  {
    NAU_ASSERT(index < sizeof...(Formats), "Invalid index {} into format table", index);
  ...
  static IndexInfo getIndex(DXGI_FORMAT format)
  {
    ...
    NAU_ASSERT(false, "Can not find index for format {}", (int)format);
...
FormatInfo<TEXFMT_V16U16,         DXGI_FORMAT_R16G16_TYPELESS,        DXGI_FORMAT_R16G16_SNORM,         DXGI_FORMAT_R16G16_SNORM>,
...
FormatInfo<TEXFMT_A4R4G4B4,       DXGI_FORMAT_B4G4R4A4_UNORM,        DXGI_FORMAT_B4G4R4A4_UNORM,       DXGI_FORMAT_B4G4R4A4_UNORM>,
FormatInfo<TEXFMT_R5G6B5,         DXGI_FORMAT_B5G6R5_UNORM,           DXGI_FORMAT_B5G6R5_UNORM,         DXGI_FORMAT_B5G6R5_UNORM>,
FormatInfo<TEXFMT_A8L8,           DXGI_FORMAT_R8G8_TYPELESS,          DXGI_FORMAT_R8G8_UNORM,           DXGI_FORMAT_R8G8_UNORM>,
...
void check()
{
  ...
  static_assert(T::LinearIndexCheck::Value);
  static_assert(T::FirstCheck::Value);
  static_assert(T::LastCheck::Value);
  static_assert(T::formatCount < (size_t(1) << (sizeof(FormatStore) * 8)));
}
...
FormatStore FormatStore::fromDXGIDepthFormat(DXGI_FORMAT fmt)
{
  ...
  switch (fmt)
  {
    ...
    default:
      NAU_FAILURE("A texture with D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL set has a format of {} which we can't map to TEXFMT_*!",
...
bool FormatStore::isColor() const
{
  switch (linearFormat)
  {
    ...
    case TEXFMT_BC6H >> CREATE_FLAGS_FORMAT_SHIFT:
    case TEXFMT_BC7 >> CREATE_FLAGS_FORMAT_SHIFT:
    case TEXFMT_R8UI >> CREATE_FLAGS_FORMAT_SHIFT: return true;
    case TEXFMT_DEPTH24 >> CREATE_FLAGS_FORMAT_SHIFT:
    case TEXFMT_DEPTH16 >> CREATE_FLAGS_FORMAT_SHIFT:
    case TEXFMT_DEPTH32 >> CREATE_FLAGS_FORMAT_SHIFT:
    case TEXFMT_DEPTH32_S8 >> CREATE_FLAGS_FORMAT_SHIFT: return false;
...
bool FormatStore::isDepth() const
{
  switch (linearFormat)
  {
    ...
    case TEXFMT_BC6H >> CREATE_FLAGS_FORMAT_SHIFT:
    case TEXFMT_BC7 >> CREATE_FLAGS_FORMAT_SHIFT:
    case TEXFMT_R8UI >> CREATE_FLAGS_FORMAT_SHIFT: return false;
    case TEXFMT_DEPTH24 >> CREATE_FLAGS_FORMAT_SHIFT:
    case TEXFMT_DEPTH16 >> CREATE_FLAGS_FORMAT_SHIFT:
    case TEXFMT_DEPTH32 >> CREATE_FLAGS_FORMAT_SHIFT:
