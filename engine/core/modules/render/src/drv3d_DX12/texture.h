...
public:
  ...
  void setResApiName(const char8_t *name) const override;
  ...
  void setParams(int w, int h, int d, int levels, const char8_t *stat_name);
  ...
  bool addDirtyRect(const nau::math::RectInt &) override { return true; }
  ...
  int texbordercolor(nau::math::E3DCOLOR c) override;
  ...
  eastl::vector<uint8_t> texCopy; //SmallTab<uint8_t, MidmemAlloc> texCopy; //< ddsx::Header + ddsx data; sysCopyQualityId stored in hdr.hqPartLevel
...
private:
  ...
  void setTexName(const char8_t *name);
