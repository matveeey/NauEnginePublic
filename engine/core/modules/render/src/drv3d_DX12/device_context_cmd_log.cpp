...
void append_arg(String &target, nau::math::E3DCOLOR const &arg, const char *)
{
...
void append_arg(String &target, ValueRange<ExtendedImageGlobalSubresouceId> const &arg, const char *)
{
  target.aprintf(64, "ValueRange<ExtendedImageGlobalSubresouceId>{begin: %d, end: %d, size: %d}", arg.begin().at, arg.end().at,
    arg.size());
}
...
void DeviceContext::dumpCommandLog()
{
  ...
  for (int32_t i = 0; i < NumFrameCommandLogs; i++)
  {
    ...
    NAU_LOG_DEBUG("Frame {} log begin", frameCmdLog.frameId);
    NAU_LOG_DEBUG("--------------------");
    frameCmdLog.log.visitAll([&buffer, this](const auto &value) { NAU_LOG_DEBUG(cmdToStr(buffer, back.sharedContextState, value)); });
    NAU_LOG_DEBUG("Frame {} log end", frameCmdLog.frameId);
  }
}
