...
#include "nau/util/dag_generationReferencedData.h"
...
bool destroy(OverrideStateId &override_id_)
{
  ...
  if (!s)
    return false;
  NAU_ASSERT_RETURN(s->refCount != 0, false);
  if (--s->refCount == 0)
  {
    //render_states::on_override_state_destroyed(override_id); // TODO: fix it
    NAU_VERIFY(overrideStates.destroyReference(override_id));
  }
  return true;
}
...
bool set(OverrideStateId override_id)
{
  ...
  {
    NAU_LOG_ERROR("override <{}> already destroyed", uint32_t(override_id));
    return false;
  }
  if (!oldState && !!current_override)
    NAU_LOG_ERROR("override <{}> already destroyed, without being reset", uint32_t(current_override));

  if (oldState && state)
    NAU_LOG_ERROR("override already set {} and not reset, resetting!, setting = {}", uint32_t(current_override), uint32_t(override_id));
  current_override = override_id;
  //ShaderElement::invalidate_cached_state_block(); // TODO: fix it!!!!
  return true;
}
