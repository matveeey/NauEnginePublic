...
{
  NAU_STATIC_ASSERT((initialCapacity & (initialCapacity - 1)) == 0); // pow2 required
  struct SubArray;
...
public:
  ...
  {
    NAU_ASSERT(i < (1 << (32 - SA_INDEX_BITS)));
    NAU_ASSERT(j < (1 << SA_INDEX_BITS));
  ...
  T &operator[](unsigned n)
  {
    ...
    NAU_ASSERT(i < arrays.size());
    NAU_ASSERT(j <= count(i)); // <= to allow use &data[cnt] as end()
#endif
    return arrays[i][j];
  }
  ...
  unsigned append(const T *p, uint32_t sz)
  {
    ...
    do
    {
      ...
      NAU_ASSERT(sz <= capacity(asize), "Attempt to append %d > bucket #%d pow2 capacity %d. Increase container's initialCapacity (%d)",
