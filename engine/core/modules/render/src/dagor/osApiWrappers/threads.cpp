...
static nau::threading::SpinLock threads_sl;
...
bool DaThread::applyThisThreadPriority(int priority, const char *name)
{
  ...
  {
    NAU_LOG_ERROR("failed to set thread sched params <{}>", name);
    return false;
  }
...
#error "DaThread is not ported on this platform yet"
  ...
  if (err != 0)//DAGOR_UNLIKELY(err != 0)) // start wasn't successfull, rollback
  {
    interlocked_release_store(threadState, DEAD);
    remove_thread_from_list(this);
    NAU_LOG_ERROR("Failed to start thread <{}> stackSize={}K with error {}", name, stackSize >> 10, err);
  }
...
#error "DaThread is not ported on this platform yet"
  ...
  remove_thread_from_list(this); //G_VERIFY(remove_thread_from_list(this));
  if (wait && timeout_ms < 0)
    NAU_ASSERT(threadState == ZOMBIE);
