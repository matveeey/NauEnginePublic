...
//
// That seem to be less portable, as we only rely on just partially documented Rtldag::CriticalSection content and undocumented pLoaderLock
...
// It would be 30x times slower than current code, but will make Rtldag::CriticalSection "opaque" to us.
//
...
#include "nau/osApiWrappers/masqueradePEB.h"
...
bool get_thread_handle_is_walking_stack(intptr_t thread_handle)
{
  ...
  {
    // this is a bit more risky/non-portable code, as we rely on Rtldag::CriticalSection meaning. It is known to be changed after WindowsXP
    ...
    // if (!RtlTryEnterdag::CriticalSection(pLoaderLock))
    //  return true;
    // RtlLeavedag::CriticalSection(pLoaderLock);
    // RtlTryEnterdag::CriticalSection/RtlLeavedag::CriticalSection has to be resolved from ntdll, same as NtQueryInformationThread, but thats
    // portable
