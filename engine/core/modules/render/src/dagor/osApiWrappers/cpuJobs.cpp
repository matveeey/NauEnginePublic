...
#include "nau/supp/dag_cpuControl.h"
...
struct JobMgrCtx //-V730
{
  ...
  void lock() { dag::enter_critical_section(critSec); }
  void unlock() { dag::leave_critical_section(critSec); }
  ...
  void init(int stk_sz, const char *name, int jmidx, uint64_t affinity_, int prio_, int idle_time_sec = 0)
  {
    ...
    dag::create_critical_section(critSec, "jobcrit");
    job.reset();
    doneJobs.reset();
    stackSz = stk_sz;
    affinity = affinity_;
  ...
  int startThread() // Returns 0 on success or errorcode otherwise
  {
    ...
    if (prio != THREAD_PRIORITY_NORMAL)
      ...
      {
        DWORD last_err = GetLastError(); // DAGOR_WITH_LOGS
        NAU_LOG_WARNING("{} SetThreadPriority failed with error {} ({:#x})", __FUNCTION__, last_err, last_err);
      }
#endif
    ...
    if (prio != DEFAULT_THREAD_PRIORITY)
    {
      ...
      if (pthread_getschedparam(handle, &policy, &param) == 0)
      {
        ...
        if (pthread_setschedparam(handle, policy, &param) != 0)
          NAU_LOG_ERROR("failed to set thread sched params ({}:{}) for jobMgr M{}>", policy, param.sched_priority, threadName);
      }
      else
        NAU_LOG_ERROR("failed to set thread sched params for jobMgr <{}>", threadName);
    }
#endif
  ...
  void term(IJob *&jout)
  {
    dag::destroy_critical_section(critSec);
  ...
  void addJob(IJob *j, bool prepend, bool need_release)
  {
    ...
    NAU_ASSERT(!job.isInQueue(j));
    NAU_ASSERT(!doneJobs.isInQueue(j));
    ...
    if (DAGOR_UNLIKELY(threadStarted))
      NAU_LOG_ERROR("Failed to start thread for jobMgr <{}> stackSize={}K with error {}", threadName, stackSz >> 10, threadStarted);
  }
  IJob *nextJob()
  {
    lock();
...
static struct CpuJobsData //-V730
{
  ...
  dag::CritSecStorage globCritSec;
#if CONSTEXPR_CORE_COUNT
  JobMgrCtx ctx[numLogicalCores + MAX_VIRT_JOB_MGR_COUNT];
#else
  JobMgrCtx *ctx; // array of ctxCount dim
#endif
  void lock() { dag::enter_critical_section(&globCritSec); }
  void unlock() { dag::leave_critical_section(&globCritSec); }
...
static void android_apply_thread_affinity(cpujobs::JobMgrCtx &ctx)
{
  ...
  if (!(ctx.affinity & validMask))
    NAU_LOG_WARNING("For thread<{}> wrong affinity mask {:08x} valid mask {:08x} max cores: {} shift mask", ctx.threadName, ctx.affinity,
  ...
  if (sched_setaffinity(gettid(), sizeof(cpuset), &cpuset) != 0)
    debug("job_mgr_thread failed to set affinity for {} mask: {:08x} online_cores: {} errno: {}", ctx.threadName, ctx.affinity,
      online_cores, errno);
}
#else
...
{
  NAU_ASSERT_RETURN(!cpujobs_data.isInited(), );
    ...
    for (int i = 0, n = bufLen / sizeof(SYSTEM_LOGICAL_PROCESSOR_INFORMATION); i < n; ++i)
    {
      ...
      switch (inf.Relationship)
      {
        case RelationCache:
        {
          ...
          NAU_ASSERT(cli >= 0 && cli < countof(maxCoresPerCacheL));
...
#error : unsupported target!
  ...
  dag::create_critical_section(cpujobs_data.globCritSec, "cpujobs");
  cpujobs_data.ctxCount = cpujobs_data.numLogicalCores + MAX_VIRT_JOB_MGR_COUNT;
#if CONSTEXPR_CORE_COUNT
  NAU_ASSERT(cpujobs_data.ctxCount == countof(cpujobs_data.ctx));
#else
...
void cpujobs::term(bool cancel_jobs, int timeout_msec)
{
  ...
  if (all_done)
  {
    dag::destroy_critical_section(cpujobs_data.globCritSec);
#ifndef CONSTEXPR_CORE_COUNT
...
{
#ifndef CONSTEXPR_CORE_COUNT
  NAU_ASSERT(cpujobs_data.isInited());
#endif
...
{
  NAU_ASSERT(cpujobs_data.isInited());
  NAU_ASSERT(core_id >= 0 && core_id < cpujobs_data.numLogicalCores);
  NAU_ASSERT(cpujobs_data.firstVirtJobMgrId > 0); // init wasn't called with reserve_jobmgr_cores = true?
  ...
  interlocked_release_store(cpujobs_data.maxVirtJobMgrId, std::max((int)cpujobs_data.maxVirtJobMgrId, core_id));
  ...
  if (DAGOR_UNLIKELY(threadStarted))
    NAU_LOG_ERROR("Failed to start thread for jobMgr <{}> stackSize={}K with error {}", threadName, stk_sz >> 10, threadStarted);
  ...
  int idle_time_sec)
{
  ...
  interlocked_release_store(cpujobs_data.maxVirtJobMgrId, std::max((int)cpujobs_data.maxVirtJobMgrId, vmgr_id));
  cpujobs_data.unlock();
  return vmgr_id;
}
...
{
  NAU_ASSERT(cpujobs_data.isInited());
  NAU_ASSERT(vmgr_id >= cpujobs_data.firstVirtJobMgrId && vmgr_id < cpujobs_data.ctxCount);
...
bool cpujobs::add_job(int core_or_vmgr_id, cpujobs::IJob *job, bool prepend, bool need_release)
{
  ...
  NAU_ASSERT(cpujobs_data.isInited());
  NAU_ASSERT(core_or_vmgr_id >= 0 && core_or_vmgr_id < cpujobs_data.ctxCount);
...
{
  NAU_ASSERT(cpujobs_data.isInited());
  NAU_ASSERT(vmgr_id >= 0 && vmgr_id < cpujobs_data.ctxCount);
