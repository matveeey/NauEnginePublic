...
{
  NAU_LOG_ERROR("{}:\n{} (device reset)", msg, d3d::get_last_error());
  G_UNUSED(msg);
}
...
bool check_and_restore_3d_device()
{
  ...
  NAU_LOG_DEBUG("==== resetting 3d device ====");
  if (!d3d::reset_device())
  {
    d3d::driver_command(DRV3D_COMMAND_RELEASE_OWNERSHIP, NULL, NULL, NULL);
    d3d::driver_command(DRV3D_COMMAND_RELEASE_LOADING, (void *)1, NULL, NULL);
    NAU_LOG_DEBUG("d3d::reset_device({}) failed with '{}'", reset_failed_count, d3d::get_last_error());
  ...
  if (d3d::device_lost(NULL))
    NAU_LOG_DEBUG("Device lost again during afterReset"); // It is OK for device to be lost again at this point.
...
void D3dResetQueue::perform_reset_queue(const D3dResetQueue *q, bool full_reset)
{
  ...
  for (; q; q = q->next)
    if (q->func)
    {
      NAU_LOG_DEBUG("perform reset func: {}", q->name);
      q->func(full_reset);
    }
}
