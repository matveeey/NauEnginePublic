...
void tql::initTexStubs()
{
    ...
    if (b.getBlock(i)->getBlockNameId() == nid)
    {
      ...
      if (texStub[idx])
      {
        if (!b2.getBool("alias", false))
          NAU_LOG_ERROR("stubTex tag={} tries to use already used idx={}, skipped", b2.getStr("tag", NULL), idx - 2);
        continue;
      }

      int flg = (gamma1 ? 0 : TEXCF_SRGBREAD | TEXCF_SRGBWRITE);
      nau::string name = nau::string::format("texStub{}:{}", idx - 2, ttype);
      ...
      if (gamma1)
        ...
        opd[idx].hdr.flags &= ~ddsx::Header::FLG_GAMMA_EQ_1;
      opd[idx].p[0].u = c.u;
      texStub[idx] = d3d::alloc_ddsx_tex(opd[idx].hdr, flg, 0, 1, name.c_str());
    }
  ...
  for (int i = 0; i < texStub.size(); i++)
    ...
    {
      if (i >= IMPLICIT_STUBS)
        NAU_LOG_ERROR("texStub {} is not defined, inited with default", i - IMPLICIT_STUBS);
      texStub[i] = texStub[0];
  ...
  if (gpu_mem_sz_kb == 0)
    ...
    tql::mem_quota_kb = gpu_mem_sz_kb - Vectormath::max(gpu_mem_sz_kb * mem_reserve_promille / 1000, mem_reserve_kb);
  ...
  NAU_LOG_DEBUG("initTexStubs: texStub.count={} (+{} implicit), mem quota={}K,  GPUmem={}K, reserve max({}K, {}K)",
...
static void reload_texstubs_from_mem(bool full_reset)
{
  ...
  {
    nau::iosys::InPlaceMemLoadCB mcrd(&opd[i].p, sizeof(opd[i].p));
