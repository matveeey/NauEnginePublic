...
static const TextureFormatDesc format_descs[] = {
  ...
  {TEXFMT_R8G8B8, 3, false, 1, 1, ChannelDType::UNORM, {},{8, 0, 0, 0, 1},{8, 8, 0, 0, 1},{8, 16, 0, 0, 1}, {}, {}, {}},
...
uint32_t parse_tex_format(const char *name, uint32_t default_fmt)
{
  ...
  NAU_ASSERT(0, "parse_tex_format: unknown format {}", name);
  return default_fmt;
}
...
const TextureFormatDesc &get_tex_format_desc(uint32_t fmt)
{
  ...
  NAU_ASSERT(0, "unknown format: {}", fmt);
...
float channel_bits_to_float(uint32_t bits, ChannelDType type, const TextureChannelFormatDesc &channel)
{
  ...
  switch (type)
  {
    ...
    case ChannelDType::SNORM:
    {
      ...
      return Vectormath::max(static_cast<float>(value) / ((1 << (channel.bits - 1)) - 1), -1.f);
    }
    ...
    case ChannelDType::SFLOAT:
      ...
      NAU_ASSERT_RETURN(false, NAN, "Encountered an unsupported signed float format!");
    ...
    case ChannelDType::UFLOAT:
     ...
     NAU_ASSERT_RETURN(false, NAN, "Encountered an unsupported unsigned float format!");

    default: NAU_ASSERT_RETURN(false, NAN, "The channel did not have a format convertible to float!");
  }
}
