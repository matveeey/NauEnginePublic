...
void RenderPass::execute(uint32_t idx, ClearAccumulator &clear_acm)
{
  ...
  if (bind.action & RP_TA_SUBPASS_READ)
  {
    ...
    {
      NAU_ASSERT(target.resource.mip_level == 0, "using mip level for depth bind is not supported (rp {})", getDebugName());
  ...
  else if (bind.action & RP_TA_SUBPASS_WRITE)
  {
    ...
    if (bind.action & (RP_TA_LOAD_CLEAR | RP_TA_LOAD_NO_CARE))
    {
      if (bind.slot != RenderPassExtraIndexes::RP_SLOT_DEPTH_STENCIL)
      {
        ...
        // nau::math::E3DCOLOR c(target.clearValue.asUint[0]);
...
RenderPass *render_pass_generic::create_render_pass(const RenderPassDesc &rp_desc)
{
  ...
  ++ret->subpassCnt;
  NAU_ASSERT(ret->subpassCnt > 0, "render pass desctiption lacking subpassCnt");
...
{
  NAU_ASSERT(rp_impl::activeRP != rp, "trying to delete active render pass {}", rp->getDebugName());
  delete rp;
}

void render_pass_generic::begin_render_pass(RenderPass *rp, const RenderPassArea area, const RenderPassTarget *targets)
{
  NAU_ASSERT(!rp_impl::activeRP, "render pass {} already started", rp_impl::activeRP->getDebugName());
  NAU_ASSERT(rp);
...
{
  NAU_ASSERT(rp_impl::activeRP);

  eastl::vector<uint32_t> &seq = rp_impl::activeRP->sequence;
  NAU_ASSERT(rp_impl::subpass + 1 < seq.size(), "trying to run non existent subpass {} of rp {}", rp_impl::subpass,
  ...
  if (ca.mask > 0)
  {
    nau::math::E3DCOLOR colorClearValue(0);
    if (ca.colorTarget >= 0)
    {
      ResourceClearValue &rcv = rp_impl::targets[ca.colorTarget].clearValue;
      colorClearValue = nau::math::E3DCOLOR(rcv.asFloat[0] * 255, rcv.asFloat[1] * 255, rcv.asFloat[2] * 255, rcv.asFloat[3] * 255);
    }
