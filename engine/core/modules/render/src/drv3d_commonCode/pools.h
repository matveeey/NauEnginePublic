...
struct PodPool
{
  ...
  {
    NAU_ASSERT(isIndexValid(index));
    return entries[index].elem;
  }

  bool reserve(int elements)
  {
    NAU_ASSERT(elements > 0);
...
public:
  ...
  public:
    AutoLock(PodPoolWithLock &op) : pCritSec(op.critSec) { dag::enter_critical_section(pCritSec); }
    ~AutoLock() { dag::leave_critical_section(pCritSec); }
  };
  ...
  ~PodPoolWithLock() { dag::destroy_critical_section(critSec); }
  inline void lock() { dag::enter_critical_section(critSec); }
  inline void unlock() { dag::leave_critical_section(critSec); }
  ...
  int safeAllocAndSet(T value)
  {
    lock();
    int index = alloc();
    NAU_ASSERT(index != BAD_HANDLE);
    entries[index].elem = value;
    NAU_ASSERT(entries[index].isUsed());
    unlock();
    return index;
  }
...
public:
  ...
  bool release(int handle)
  {
    ...
    lock();
    NAU_ASSERT(isIndexValid(handle));
