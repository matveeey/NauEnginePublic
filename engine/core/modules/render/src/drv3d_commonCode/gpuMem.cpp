...
static eastl::array<const char *, 7> nvidia_dll_names = {
...
static eastl::array<const char *, 4> ati_dll_names = {"aticfx32.dll", "aticfx64.dll", "atidxx32.dll", "atidxx64.dll"};

static eastl::array<const char *, 8> intel_dll_names = {"igd10iumd32.dll", "igd10iumd64.dll", "igd10umd32.dll", "igd10umd64.dll",
  "igdumd32.dll", "igdumd64.dll", "igdumdim32.dll", "igdumdim64.dll"};

static const eastl::array<eastl::span<const char *>, D3D_VENDOR_COUNT> vendor_dll_names{
  eastl::span<const char *>(), // D3D_VENDOR_NONE
  eastl::span<const char *>(), // D3D_VENDOR_MESA
  eastl::span<const char *>(), // D3D_VENDOR_IMGTEC
  eastl::span<const char *>(ati_dll_names),                  // D3D_VENDOR_ATI
  eastl::span<const char *>(nvidia_dll_names),               // D3D_VENDOR_NVIDIA
  eastl::span<const char *>(intel_dll_names),                // D3D_VENDOR_INTEL
  eastl::span<const char *>(), // D3D_VENDOR_APPLE
  eastl::span<const char *>(), // D3D_VENDOR_SHIM_DRIVER
  eastl::span<const char *>(), // D3D_VENDOR_ARM
  eastl::span<const char *>()  // D3D_VENDOR_QUALCOMM
};
...
static nau::hal::DagorDateTime cached_gpu_drv_date = {0};
...
static bool get_driver_info_from_dll(const char *dllName, uint32_t drv_ver[4], nau::hal::DagorDateTime &drv_date)
{
...
static bool get_driver_info_from_dlls(eastl::span<const char *> dlls, uint32_t drv_ver[4], nau::hal::DagorDateTime &drv_date)
{
  ...
  if (d3d11dll)
  {
    ...
    if (createDevice)
    {
      ...
      if (SUCCEEDED(hr))
      {
        ...
        NAU_LOG_DEBUG("D3D11CreateDevice failed.");
    }
    else
      NAU_LOG_DEBUG("D3D11CreateDevice not found.");
    FreeLibrary(d3d11dll);
  }
  else
    NAU_LOG_DEBUG("Can't load d3d11.dll");

  NAU_LOG_DEBUG("get_dx11_feature_level_supported={}", featureLevelSupported);

  return featureLevelSupported;
}

static int get_active_gpu_vendor_pc(nau::string &out_active_gpu_description, uint32_t &out_video_memory_mb,
  uint32_t &out_free_video_memory_mb, int &feature_level_supported, uint32_t drv_ver[4], nau::hal::DagorDateTime &drv_date, uint32_t &device_id)
{
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable : 4191) // 'type cast' : unsafe conversion from 'FARPROC'
  ...
  NAU_LOG_DEBUG("get_active_gpu_vendor...");
      ...
      if (SUCCEEDED(hr))
      {
        ...
        if (pDXGIDevice)
        {
          if (SUCCEEDED(pDXGIDevice->GetAdapter(&adapter)))
          {
            ...
            NAU_LOG_DEBUG("adapter=<{}>, vendor=0x{:X}, dev_id=0x{:X}, subsys=0x{:X}, vid_mem={}MB, sys_mem={}MB, shared_mem={}MB", deviceName,
            ...
            if (SUCCEEDED(adapter->QueryInterface(&adapter3)))
            {
              ...
              {
                NAU_LOG_DEBUG("Local budget={}MB", info.Budget >> 20);
              ...
              {
                NAU_LOG_DEBUG("Non-local budget={}MB", info.Budget >> 20);
                out_free_video_memory_mb = info.Budget >> 20;
              }
  ...
  if (vendor == D3D_VENDOR_NVIDIA)
  {
    ...
    if (nvDedicatedKb > 0)
    {
      NAU_LOG_DEBUG("Nvidia GPU memory dedicated: {}MB, free dedicated: {}MB, shared system: {}MB", nvDedicatedKb >> 10,
  ...
  {
    NAU_LOG_DEBUG("GPU with {}MB of dedicated memory. Use {}MB of dedicated and shared memory.", out_video_memory_mb,
  ...
  {
    NAU_LOG_DEBUG("Free GPU memory is unknown, fallback to total memory");
    out_free_video_memory_mb = out_video_memory_mb;
  }

  NAU_LOG_DEBUG("get_active_gpu_vendor={}, '{}', vid_mem_size={}MB, free={}MB", d3d_get_vendor_name(vendor), out_active_gpu_description,
...
extern void mac_get_vdevice_info(int &vram, int &vendor, nau::string &gpu_desc, bool &web_driver);
#endif


static int guess_gpu_vendor_internal(nau::string *out_gpu_desc, uint32_t *out_drv_ver, nau::hal::DagorDateTime *out_drv_date, uint32_t *device_id,
  uint32_t *out_gpu_video_memory_mb, int *out_feature_level_supported)
{
  if (cached_gpu_vendor == -1)
  {
    ...
    nau::string activeGpuDescription;
    ...
    NAU_LOG_DEBUG(cached_gpu_desc, sizeof(cached_gpu_desc), "{}", activeGpuDescription);
    ...
    if (d3d::is_inited())
    {
      ...
      NAU_LOG_DEBUG(cached_gpu_desc, sizeof(cached_gpu_desc), "{} [{}]", devName ? devName : "Unknown", ctxInfo ? ctxInfo : "None");
    ...
    NAU_LOG_DEBUG(cached_gpu_desc, sizeof(cached_gpu_desc), "{}", gpuDesc.str());
#else
...
int d3d::guess_gpu_vendor(nau::string *out_gpu_desc, uint32_t *out_drv_ver, nau::hal::DagorDateTime *out_drv_date, uint32_t *device_id)
{
  return guess_gpu_vendor_internal(out_gpu_desc, out_drv_ver, out_drv_date, device_id, nullptr, nullptr);
}

nau::hal::DagorDateTime d3d::get_gpu_driver_date(int vendor)
{
    nau::hal::DagorDateTime drv_date{};
#if _TARGET_PC_WIN
  uint32_t dummy[4];
...
{
  if (cached_gpu_vendor == -1)
      guess_gpu_vendor_internal(nullptr, nullptr, nullptr, nullptr, nullptr, nullptr);
...
#else
    NAU_LOG_DEBUG("d3d: GPU memory size before driver init is not available on platform");
#endif
  }
}
...
unsigned d3d::get_dedicated_gpu_memory_size_kb()
{
  ...
  if (estimated_android_gpu_memory_kb != retKB)
  {
    ...
    NAU_LOG_DEBUG("d3d: gpuMem: estimated {} Mb [{}{}{}{}{}{}] \n"
          // report both control flags and config/reported values
          "({} Mb reported, {}+{} Mb reserved, {} Mb fallback, {} Mb minimal, {} Mb safety margin)",
      retMB,
      // valid/invalid
      reportedIsValid ? "V" : "I",
      // device/gpu
...
#if _TARGET_PC_WIN
static nau::string video_vendor_info_str;

void d3d::get_video_vendor_str(nau::string &out_str)
{
  ...
  NAU_LOG_DEBUG("{}", video_vendor_info_str);
  out_str = video_vendor_info_str;
}

bool d3d::get_gpu_freq(nau::string &out_freq)
{
#if HAS_NVAPI
  if (get_nv_physical_gpu())
  {
    ...
    for (int clockType = 0; clockType < NV_GPU_CLOCK_FREQUENCIES_CLOCK_TYPE_NUM; clockType++)
    {
      ...
      out_freq += nau::string(0, "{}{}: ", clockType == 0 ? "" : ", ", clockTypeStr[clockType]);
      bool first = true;
      for (int domainNo = 0; domainNo < NVAPI_MAX_GPU_PUBLIC_CLOCKS; domainNo++)
      {
        if (freq.domain[domainNo].bIsPresent)
        {
          out_freq += nau::string(0, "{}{} MHz", first ? "" : " / ", freq.domain[domainNo].frequency / 1000);
          first = false;
        }
      }
    }
...
#else
void d3d::get_video_vendor_str(nau::string &) {}
bool d3d::get_gpu_freq(nau::string &) { return false; }
