...
void nvlowlatency::set_latency_mode(lowlatency::LatencyMode mode, float min_interval_ms)
{
  if (dx_device)
  {
    ...
    {
      NAU_LOG_ERROR("NVAPI: NvAPI_D3D_SetSleepMode has failed");
      firstError = false;
    }
  }
}
...
void nvlowlatency::sleep()
{
  if (dx_device)
  {
    ...
    if (status != NVAPI_OK && firstError)
    {
      NAU_LOG_ERROR("NVAPI: NvAPI_D3D_Sleep has failed");
      firstError = false;
    }
  }
}
...
static NV_LATENCY_MARKER_TYPE convertMarkerTypeToNV(lowlatency::LatencyMarkerType marker_type)
{
  ...
  NAU_ASSERT_RETURN(false, NV_LATENCY_MARKER_TYPE::SIMULATION_START);
}
...
void nvlowlatency::set_marker(uint32_t frame_id, lowlatency::LatencyMarkerType marker_type)
{
  if (dx_device)
  {
    ...
    if (status != NVAPI_OK && firstError)
    {
      NAU_LOG_ERROR("NVAPI: SetLatencyMarked has failed with frame_id: {}, and marker: {}", frame_id, static_cast<uint32_t>(marker_type));
      firstError = false;
    }
  }
}
...
lowlatency::LatencyData nvlowlatency::get_statistics_since(uint32_t frame_id, uint32_t max_count)
{
  ...
  if (dx_device)
  {
    ...
    static_assert(FRAME_COUNT == 64); // FRAME_COUNT should be 64 elements long. Check if the
