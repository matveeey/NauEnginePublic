...
//#include "nau/osApiWrappers/dag_progGlobals.h"
#include "nau/util/dag_parseResolution.h"
#include "gpuConfig.h"
...
bool get_settings_resolution(int &width, int &height, bool &is_retina, int def_width, int def_height, bool &out_is_auto)
{
  ...
  if (res_str && res_str[0] == 'x')
  {
    ...
    const int smallerDimension = Vectormath::min(def_width, def_height);
  ...
  if (res_max_str && get_resolution_from_str(res_max_str, max_w, max_h))
  {
    ...
    width = Vectormath::min(width, max_w);
    height = Vectormath::min(height, max_h);
    return isOk;
  }
...
void get_current_display_screen_mode(int &out_def_left, int &out_def_top, int &out_def_width, int &out_def_height)
{
  ...
  NAU_LOG_DEBUG("get_current_display_screen_mode - SPI_GETWORKAREA: {},{}-{},{}, EnumDisplaySettings: {}x{}", workAreaRect.left,
  ...
  {
    NAU_LOG_DEBUG("get_current_display_screen_mode - use SPI_GETWORKAREA");
  ...
  {
    NAU_LOG_DEBUG("get_current_display_screen_mode - use EnumDisplaySettings");
  ...
  {
    NAU_LOG_WARNING("get_current_display_screen_mode - fallback to default resolution");
...
void get_render_window_settings(RenderWindowSettings &p, Driver3dInitCallback *cb)
{
  ...
  NAU_LOG_DEBUG("base_scr = {}x{}", base_scr_wdt, base_scr_hgt);

  bool isAutoResolution = false, isRetina = false;
  get_settings_resolution(scr_wdt, scr_hgt, isRetina, base_scr_wdt, base_scr_hgt, isAutoResolution);  //G_VERIFY(get_settings_resolution(scr_wdt, scr_hgt, isRetina, base_scr_wdt, base_scr_hgt, isAutoResolution));
  ...
  if (windowMode == WindowMode::WINDOWED_FULLSCREEN)
  {
    ...
    scr_wdt = Vectormath::min(scr_hgt * base_scr_wdt / base_scr_hgt, base_scr_wdt);
  ...
  else if (windowMode != WindowMode::FULLSCREEN_EXCLUSIVE)
  {
    ...
    {
      const nau::math::IVector2 position = blk_video.getIPoint2(idx);
      wr.left = position.getX();
      wr.top = position.getY();
    }
    else
    {
    ...
    if (isAutoResolution)
    {
      wr.right = Vectormath::min(wr.right - wr.left, (LONG)base_scr_wdt);
      wr.bottom = Vectormath::min(wr.bottom - wr.top, (LONG)base_scr_hgt);
      wr.left = Vectormath::max(wr.left, (LONG)base_scr_left);
      wr.top = Vectormath::max(wr.top, (LONG)base_scr_top);
      wr.right += wr.left;
      wr.bottom += wr.top;
    ...
    {
      wr.right += Vectormath::max(-wr.left, 0L);
      wr.bottom += Vectormath::max(-wr.top, 0L);
      wr.left = Vectormath::max(wr.left, 0L);
      wr.top = Vectormath::max(wr.top, 0L);
    }
...
void *win32_init_empty_mouse_cursor()
{
    ...
    if (!win32_empty_mouse_cursor && win32_rdp_compatible_mode)
    {
        ...
        if (win32_empty_mouse_cursor)
        {
            NAU_LOG_DEBUG("initing win32_empty_mouse_cursor={:p} ({}x{}): supported size={}x{}, bitmap size={} (max {})", win32_empty_mouse_cursor, cursorX,
                          cursorY, sys_cursorX, sys_cursorY, (cursorX + 15) / 16 * 2 * cursorY, MAX_MASK_DATA_SZ);
        }
        else
        {
            NAU_LOG_ERROR("initing win32_empty_mouse_cursor={:p} ({}x{}): supported size={}x{}, bitmap size={} (max {})", win32_empty_mouse_cursor, cursorX,
                          cursorY, sys_cursorX, sys_cursorY, (cursorX + 15) / 16 * 2 * cursorY, MAX_MASK_DATA_SZ);
        }
...
bool set_render_window_params(RenderWindowParams &p, const RenderWindowSettings &s)
{
  if (!p.rwnd)
  {
    ...
    if (p.wcname && p.mainProc)
    {
      ...
      if (!RegisterClassW(&wc))
      {
        ...
        {
          NAU_LOG_ERROR("Can't register window class ({:08X})", err);
          return false;
        }
      }
    }

    NAU_LOG_DEBUG("window = {}x{} .. {}x{}", s.winRectLeft, s.winRectTop, s.winRectRight, s.winRectBottom);
    ...
    if (!p.hwnd)
    {
      NAU_LOG_ERROR("Can't create window ({:08X})", GetLastError());
      return false;
    }
    ...
    if (dgs_get_window_mode() != WindowMode::FULLSCREEN_EXCLUSIVE)
    {
      ...
      if (GetClientRect((HWND)p.hwnd, &clientRect))
      {
        ...
        {
          NAU_LOG_DEBUG("Incorrect window client area size: rect={},{}-{},{}, required={}x{}", clientRect.left, clientRect.top,
  ...
  {
    // At this time we not gonna do anything
  }
...
int set_display_device_mode(bool inwin, int res_x, int res_y, int scr_bpp, DEVMODE &out_devm)
{
  ...
  if (!mode_ok)
  {
    NAU_LOG_ERROR("No suitable mode found ({}x{}x{} requested)", res_x, res_y, scr_bpp);
    return 0;
  }
  ...
  if (res != DISP_CHANGE_SUCCESSFUL)
  {
    if (res == DISP_CHANGE_RESTART)
      NAU_LOG_ERROR("You should set display mode {}x{}x{} manually", mode_w, mode_h, mode_b);
    else
      NAU_LOG_ERROR("Error setting display mode {}x{}x{}", mode_w, mode_h, mode_b);
    return 0;
  }
    ...
    if (EnumDisplayDevices(device.DeviceName, 0, &deviceMonitor, 0))
    {
      if (strcmp(device.DeviceName, monitorName) == 0)
      {
        if (friendlyMonitorName)
          *friendlyMonitorName = deviceMonitor.DeviceString;//->setStr(deviceMonitor.DeviceString);
        if (uniqueIndex)
          *uniqueIndex = index;
        return true;
      }
...
static bool isHDRBlackListed(const nau::DataBlock &blk, const char *name)
{
  if (!name || name[0] == 0)
    return false;
  bool result = false;
  nau::dblk::iterate_params_by_name(blk, "hdrBlackList",
...
#endif
  const nau::DataBlock &blkDrv = *dgs_get_settings()->getBlockByNameEx(drvName);
