...
//#include "nau/perfMon/dag_graphStat.h"
...
{
public:
  static void errorDiscardingStageMemory(const char8_t *name)
  {
    NAU_LOG_ERROR("{}: Error while discarding staging memory of buffer '{}'", T::driverName(), name);
  }
  static void errorAllocatingTemporaryMemory(const char8_t *name)
  {
    NAU_LOG_ERROR("{}: Error while allocating temporary memory for buffer '{}'", T::driverName(), name);
  }
  static void errorUnknownStagingMemoryType(const char8_t *name)
  {
    NAU_LOG_ERROR("{}: Buffer '{}' has insufficient information to select proper staging memory type, "
           "falling back to read/write",
      T::driverName(), name);
  }
  static void errorLockWithoutPrevoiusUnlock(const char8_t *name)
  {
    NAU_LOG_ERROR("{}: Buffer '{}' unlocked without previous successful lock", T::driverName(), name);
  }
  static void errorLockOffsetIsTooLarge(const char8_t *name, uint32_t offset, uint32_t size)
  {
    NAU_LOG_ERROR("{}: Buffer '{}' lock with offset of {} is larger than buffer size {}", T::driverName(), name, offset, size);
  }
  static void errorLockSizeIsTooLarge(const char8_t* name, uint32_t offset, uint32_t range, uint32_t size)
  {
    NAU_LOG_ERROR("{}: Buffer '{}' lock with offset of {} and size of {} ({}) is larger than buffer size "
           "{}",
      T::driverName(), name, offset, range, offset + range, size);
  }
  static void errorLockWithInvalidLockingFlags(const char8_t *name, uint32_t flags)
  {
    NAU_LOG_ERROR("{}: Buffer '{}' locked with invalid combination of locking flags 0x{:X}", T::driverName(), name, flags);
  }
  static void errorLockOffsetIsNotZero(const char8_t* name, uint32_t offset, uint32_t flags)
  {
    NAU_LOG_ERROR("{}: Buffer '{}' lock with offset of {} (for 0x{:X} locking flags only 0 offset is supported)", T::driverName(), name, offset,
      flags);
  }
  static void errorDiscardingBuffer(const char8_t *name) { NAU_LOG_ERROR("{}: Error while discarding buffer '{}'", T::driverName(), name); }
  static void errorUnlockWithoutPreviousLock(const char8_t *name)
  {
    NAU_LOG_ERROR("{}: Buffer '{}' unlocked without previous lock", T::driverName(), name);
  }
  static void errorUnexpectedUseCase(const char8_t *name) { NAU_LOG_ERROR("{}: Unexpected use case for buffer '{}'", T::driverName(), name); }
  static void errorInvalidStructSizeForRawView(const char8_t *name, uint32_t struct_size)
  {
    NAU_LOG_ERROR("{}: Buffer '{}' uses invalid struct size of {} for buffer with RAW views", T::driverName(), name, struct_size);
  }
  static void errorAllocationOfBufferFailed(const char8_t *name, uint32_t size, uint32_t struct_size, uint32_t cflags)
  {
    NAU_LOG_ERROR("{}: Allocation of buffer '{}' failed, requested size '{}' bytes with a structure size "
           "of '{}' bytes and creation flags of '0x{:08x}'",
      T::driverName(), name, size, struct_size, cflags);
  }
  static void errorStructuredIndirect(const char8_t *name)
  {
    NAU_LOG_ERROR("indirect buffer can't be structured one in DX11, check <{}>", name);
  }

  static void errorDiscardWithoutPointer(const char8_t *name)
  {
    NAU_LOG_ERROR("{}: Discarded buffer '{}' without providing output pointer", name);
  }

  static void errorFormatUsedWithInvalidUsageFlags(const char8_t *name)
  {
    NAU_LOG_ERROR("{}: Can't create buffer '{}' with texture format which is Structured, Vertex, Index or "
           "Raw",
      T::driverName(), name);
  }
};
...
{
public:
  static void errorDiscardingStageMemory(const char8_t *) {}
  static void errorAllocatingTemporaryMemory(const char8_t *) {}
  static void errorUnknownStagingMemoryType(const char8_t *) {}
  static void errorLockWithoutPrevoiusUnlock(const char8_t *) {}
  static void errorLockOffsetIsTooLarge(const char8_t *, uint32_t, uint32_t) {}
  static void errorLockSizeIsTooLarge(const char8_t *, uint32_t, uint32_t, uint32_t) {}
  static void errorLockWithInvalidLockingFlags(const char8_t *, uint32_t) {}
  static void errorLockOffsetIsNotZero(const char8_t *, uint32_t, uint32_t) {}
  static void errorDiscardingBuffer(const char8_t *) {}
  static void errorUnlockWithoutPreviousLock(const char8_t *) {}
  static void errorUnexpectedUseCase(const char8_t *) {}
  static void errorInvalidStructSizeForRawView(const char8_t *, uint32_t) {}
  static void errorAllocationOfBufferFailed(const char8_t *, uint32_t, uint32_t, uint32_t) {}
  static void errorStructuredIndirect(const char8_t *) {}
  static void errorDiscardWithoutPointer(const char8_t *) {}
  static void errorFormatUsedWithInvalidUsageFlags(const char8_t *) {}
};
...
{
public:
  static void warnPerformanceBlockingBufferReadBack(const char8_t *name)
  {
    NAU_LOG_WARNING("{}: Performance: Blocking read back of buffer '{}'", T::driverName(), name);
  }
  static void warnPerformanceTemporaryMemoryForReadBack(const char8_t *name)
  {
    NAU_LOG_WARNING("{}: Performance: Using temporary memory as staging memory for read back of buffer "
            "'{}'",
      T::driverName(), name);
  }
  static void warnPerformanceNotConsumedAsyncReadBack(const char8_t *name)
  {
    NAU_LOG_WARNING("{}: Performance: Previous async read back was not consumed '{}'", T::driverName(), name);
  }
  static void warnPerformanceReadBackOfUnchangedResource(const char8_t *name)
  {
    NAU_LOG_WARNING("{}: Performance: Repeated read back of unchanged buffer '{}'", T::driverName(), name);
  }
  static void warnPerformanceHostCopyToBufferWithTemporary(const char8_t *name)
  {
    NAU_LOG_WARNING("{}: Performance: Host copy of buffer '{}' uploaded with temporary memory", T::driverName(), name);
  }
  static void warnBadStructureSize(const char8_t *name, uint32_t stuct_size)
  {
    NAU_LOG_WARNING("{}: Performance: The structure size of {} of buffer '{}' has a hardware unfriendly "
  ...
  static void warnUsageAsyncWithoutReadBackRequested(const char8_t *name)
  {
    NAU_LOG_WARNING("{}: Usage: Requested async access to buffer '{}' but without read request, read "
...
{
public:
  static void warnPerformanceBlockingBufferReadBack(const char8_t *) {}
  static void warnPerformanceTemporaryMemoryForReadBack(const char8_t *) {}
  static void warnPerformanceNotConsumedAsyncReadBack(const char8_t *) {}
  static void warnPerformanceReadBackOfUnchangedResource(const char8_t *) {}
  static void warnPerformanceHostCopyToBufferWithTemporary(const char8_t *) {}
  static void warnBadStructureSize(const char8_t *, uint32_t) {}
  static void warnUsageAsyncWithoutReadBackRequested(const char8_t *) {}
};
...
protected:
  ...
  bool allocateStagingMemory(uint32_t buf_flags, uint32_t size, const char8_t *name, GenericBufferErrorHandler<R, E> &reporter)
  {
  ...
  template <typename R, bool E>
  bool discardStagingMemory(uint32_t buf_flags, uint32_t size, const char8_t *name, GenericBufferErrorHandler<R, E> &reporter)
  {
    G_UNUSED(reporter); // for whatever reason VC throws a warning that reporter is unreferenced, which is obviously incorrect
    if (!T::isValidMemory(stagingMemory))
      return false;
...
protected:
  ...
  template <typename T>
  constexpr bool allocateStagingMemory(uint32_t, uint32_t, const char8_t *, T &) const
  {
    return false;
  }
  template <typename T>
  constexpr bool discardStagingMemory(uint32_t, uint32_t, const char8_t *, T &) const
  {
    return false;
  }
                                           ...
                                           public Sbuffer
{
  ...
  static uint32_t estimateSize(uint32_t buffer_size, uint32_t structure_size, uint32_t discard_count, uint32_t cflags)
  {
    ...
    return Vectormath::max<uint32_t>(estimatedSize * discard_count, T::minBufferSize(cflags));
  }

  void compose(const char8_t *buf_name)
  {
  ...
  void adopt(typename T::BufferType &&buf, const char8_t *buf_name)
  {
  ...
  bool bufferGpuTimelineUpdate(uint32_t lock_flags) const
  {
    ...
    // Removing requirement !isDynamicBuffer(). Reason: we want SBCF_DYNAMIC buffers updating during GPU Timeline because they are dynamic.
    // It doesn't make sense that static buffers are updating during GPU timeline sice they are static.
  ...
  bool validate_buffer_properties(uint32_t flags, uint32_t format, const char8_t *stat_name)
  {
    bool hadError = false;
...
public:
  ...
  GenericSbufferImplementation(uint32_t struct_size, uint32_t element_count, uint32_t flags, uint32_t format_flags,
    const char8_t *stat_name) :
    structSize(struct_size),
    bufSize(Vectormath::max<uint32_t>(struct_size, 1) * element_count),
    bufFlags(flags),
  ...
  GenericSbufferImplementation(typename T::BufferType &&buf, uint32_t struct_size, uint32_t element_count, uint32_t flags,
    uint32_t format_flags, const char8_t *stat_name) :
    structSize(struct_size),
    bufSize(Vectormath::max<uint32_t>(struct_size, 1) * element_count),
    bufFlags(flags),
    viewFormat(T::viewFormatFromFormatFlags(format_flags))
  ...
  int lock(unsigned ofs_bytes, unsigned size_bytes, void **ptr, int flags) override
  {
    ...
    {
      NAU_ASSERT(!T::isValidMemory(temporaryMemory));
