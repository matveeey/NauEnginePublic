...
{
  NAU_ASSERT_RETURN(buf != nullptr, , "Trying to activate a nullptr buffer!");
  ...
  switch (action)
  {
    ...
    case RAA::CLEAR_AS_RTV_DSV: NAU_ASSERT(false, "Tried to activate a buffer with a render target action!"); break;
...
{
  NAU_ASSERT_RETURN(tex != nullptr, , "Trying to activate a nullptr texture!");
  ...
  switch (action)
  {
    ...
    case RAA::CLEAR_AS_RTV_DSV:
    {
      ...
      if (!(d3d::get_texformat_usage(info.cflg) & d3d::USAGE_DEPTH))
      {
        ...
        auto color = nau::math::E3DCOLOR(ithToChar(0), ithToChar(1), ithToChar(2), ithToChar(3));
        ...
        for (uint32_t l = 0; l < info.a; ++l)
        {
          for (uint32_t m = 0; m < info.mipLevels; ++m)
          {
            ...
            d3d::set_render_target({}, DepthAccess::RW, {{tex, m, l}});
      ...
      else
      {
        for (uint32_t l = 0; l < info.a; ++l)
        {
          for (uint32_t m = 0; m < info.mipLevels; ++m)
          {
            d3d::set_render_target({tex, m, l}, DepthAccess::RW, {});
            d3d::clearview(CLEAR_ZBUFFER | CLEAR_STENCIL, nau::math::E3DCOLOR(), value.asDepth, value.asStencil);
          }
        }
      }
    }
...
{
  NAU_ASSERT(buf != nullptr, "Tried to deactivate a nullptr buffer!");
}

void res_act_generic::deactivate_texture(BaseTexture *tex, GpuPipeline)
{
  NAU_ASSERT(tex != nullptr, "Tried to deactivate a nullptr texture!");
}
