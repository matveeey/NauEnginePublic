...
class NameSpace
{
  ...
  NameSpace();
  NameSpace(dabfg::NameSpaceNameId nid);
...
public:
  ...
  void setNodeEnabled(const NodeHandle& uid, bool enabled) const
  {
    return detail::set_node_enabled(uid.uid, enabled);
  }
   ...
   */
  void setResolution(const char *type_name, nau::math::IVector2 value);
   ...
   */
  void setDynamicResolution(const char *type_name, nau::math::IVector2 value);
