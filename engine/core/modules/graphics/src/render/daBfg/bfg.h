...
inline void set_node_enabled(const NodeHandle& nodeHandle, bool enabled)
{
    return root().setNodeEnabled(nodeHandle, enabled);
}
 ...
 */
inline void set_resolution(const char *typeName, nau::math::IVector2 value) { dabfg::root().setResolution(typeName, value); }

/**
 * \brief Alias for dabfg::root().setDynamicResolution,
 * see \ref NameSpace::setDynamicResolution
 */
inline void set_dynamic_resolution(const char* typeName, nau::math::IVector2 value)
{
...
{
    dabfg::root().fillSlot(slot, dabfg::root(), res_name);
}
...
{
    dabfg::root().markResourceExternallyConsumed(res_name);
}
...
{
    dabfg::root().unmarkResourceExternallyConsumed(res_name);
}
