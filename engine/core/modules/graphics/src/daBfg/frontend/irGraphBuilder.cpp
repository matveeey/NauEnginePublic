...
#define NAU_ASSERT_DO_AND_LOG(expression, action, ...) \
  do                                                 \
  {                                                  \
...
intermediate::Graph IrGraphBuilder::build(multiplexing::Extents extents) const
{
  ...
  NAU_ASSERT_DO_AND_LOG(!sinks.empty(), dumpRawUserGraph(validityInfo),
...
void IrGraphBuilder::validateLifetimes(ValidityInfo &validity) const
{
  for (auto [resId, lifetime] : depData.resourceLifetimes.enumerate())
  {
    ...
    {
      NAU_LOG_ERROR("Resource '{}' was consumed by '{}' on it's frame but had it's "
    ...
    {
      NAU_LOG_ERROR("Resource '{}' was both created and consumed by node '{}'! "
             "This is impossible to satisfy, disabling the resource!",
    ...
    {
      NAU_LOG_ERROR("Resource '{}' was both read and introduced by node '{}'! "
             "This is impossible to satisfy, disabling the resource! ",
        registry.knownNames.getName(resId), registry.knownNames.getName(lifetime.introducedBy));
    ...
    {
      NAU_LOG_ERROR("Resource '{}' was both read and consumed by node '{}'! "
    ...
    if (!conflicts.empty())
    {
      ...
      NAU_LOG_ERROR("Found nodes that both modify and read resource '{}'! Offender(s): {}. "
...
bool IrGraphBuilder::validateResource(ResNameId resId) const
{
  ...
  if (isASlot && wasIntroduced)
    NAU_LOG_ERROR("The name '{}' was both used to create/register/rename "
           "a resource in node '{}' and was filled as a resource slot. "
  ...
  const auto checkResolution = [this](AutoResTypeNameId unresolvedAutoResId) {
    ...
    nau::math::IVector2 xy  = registry.autoResTypes[autoResId].staticResolution;
    nau::math::IVector2 dxy = registry.autoResTypes[autoResId].dynamicResolution;
    return xy.getX() > 0 && xy.getY() > 0 && dxy.getX() > 0 && dxy.getY() > 0;
  };
  ...
  if (!autoResValid)
    NAU_LOG_ERROR("Resource '{}' was created with auto-resolution '{}', "
...
bool IrGraphBuilder::validateNode(NodeNameId nodeId) const
{
  ...
  for (const auto &[resId, req] : nodeData.resourceRequests)
    ...
    {
      NAU_LOG_ERROR("Found a broken node '{}'! "
             "Reason: it requested the resource in slot '{}' as mandatory, "
  ...
  for (const auto &[bindId, req] : nodeData.bindings)
  {
    ...
    const auto svType = SHVT_BUFFER; // ShaderGlobal::get_var_type(bindId);
    NAU_FAILURE("ShaderGlobal::get_var_type - not implemented");
    ...
    NAU_ASSERT_CONTINUE(svType >= -1 && 1 + svType < SHVT_MESSAGES.size()); // Sanity check
    ...
    auto handleMismatch = [this, &anyBindingTypeMismatches, svType, nodeId = nodeId, svId = bindId, resId = req.resource](
      ...
      NAU_LOG_ERROR("Node '{}' requested {} '{}' to be set to"
             " shader variable '{}', but the variable {}!"
          ...
          "VARIABLE_NAME",//VariableMap::getVariableName(svId),
  ...
  const auto checkResIdRequest = [this, nodeId, &anyUsageConflictsAfterNameResolution](bool history, auto resolvedResId,
    ...
    NAU_ASSERT(!unresolvedResIds.empty());
    ...
    NAU_ASSERT_RETURN(firstUsage.has_value(), );

    for (const auto unresolvedResId : unresolvedResIds)
    {
      const auto usage = getUsage(unresolvedResId);
      NAU_ASSERT_CONTINUE(usage.has_value());

      if (firstUsage->access != usage->access || firstUsage->type != usage->type)
      {
        NAU_LOG_ERROR("Resource requests for names '{}' and '{}' both resolved to the same resource with name '{}' "
               "within the node '{}', but the requested usages were different! This would result in impossible"
...
auto IrGraphBuilder::findValidResourcesAndNodes() const -> ValidityInfo
{
  ...
  using ResToNodeMapping =
    IdIndexedMapping<ResNameId, eastl::fixed_vector<NodeNameId, 64, true, nau::EastlFrameAllocator>, nau::EastlFrameAllocator>;
  ...
  using ResToResMapping = IdIndexedMapping<ResNameId, eastl::optional<ResNameId>, nau::EastlFrameAllocator>;
  ...
  using Wave = eastl::fixed_vector<ResNameId, 64, true, nau::EastlFrameAllocator>;
  Wave wave;
  ...
  while (!wave.empty())
  {
    ...
    for (auto resId : wave)
    {
      ...
      for (auto nodeId : nodesDependentOnResource[resId])
      {
        ...
        NAU_LOG_ERROR("Found a broken node '{}'! "
               "Reason: the node requested '{}' as a mandatory resource, "
               "but {}. Disabling this node!",
          registry.knownNames.getName(nodeId), registry.knownNames.getName(resId),
  ...
  multiplexing::Extents extents) const
{
  ...
  // Group and multiplex resources
  {
    ...
    for (auto [nodeId, nodeData] : registry.nodes.enumerate())
    {
      ...
      for (auto resId : nodeData.createdResources)
      {
        ...
        const auto scanResourceUsages = [this, &validity](ResNameId res_id, const auto &process_readers,
          ...
          eastl::vector_set<ResNameId, eastl::less<ResNameId>, nau::EastlFrameAllocator> knownCandidates;
        ...
        if (auto resDesc = eastl::get_if<ResourceDescription>(&resData.creationInfo))
        {
          ...
          const auto processReaders = [this, type = resData.type, &firstUsage, &desc, &hasReaders](NodeNameId node_id,
            ...
            if (firstUsage.type == Usage::UNKNOWN)
              NAU_LOG_ERROR("Resource {} is read by {} before first modify usage", registry.knownNames.getName(res_id),
          ...
          {
            NAU_LOG_ERROR("Resource {} is not initialized and it's first modify usage is UNKNOWN.", registry.knownNames.getName(resId));
          }
          ...
          {
            NAU_LOG_ERROR("Could not infer an activation action for {} from the first usage of a"
        ...
        else if (auto externalProvider = eastl::get_if<ExternalResourceProvider>(&resData.creationInfo))
        {
          switch (resData.type)
          {
            ...
            case ResourceType::Blob:
            case ResourceType::Invalid: NAU_ASSERT_BREAK(false, "This should never ever happen!");
          }
        }
      }
    }
    ...
    for (auto [nodeId, nodeData] : registry.nodes.enumerate())
    {
      ...
      for (const auto &[toResId, unresolvedFromResId] : nodeData.renamedResources)
      {
        ...
        {
          const auto irMultiplexingIndex = multiplexing_index_to_ir(i, extents);
          NAU_ASSERT(mapping.wasResMapped(originalRes, irMultiplexingIndex));
  ...
  {
    eastl::vector<NodeNameId, nau::EastlFrameAllocator> validNodes;
    ...
    for (auto nodeId : validNodes)
    {
      ...
      for (multiplexing::Index i = {}; index_inside_extents(i, extents); i = next_index(i, extents))
      {
        ...
        eastl::array<eastl::vector_map<ResNameId, uint32_t, eastl::less<ResNameId>, nau::EastlFrameAllocator>, 2>
        ...
        auto processRequest = [&irNode = irNode, &mapping, &resourceValid, &nodeData, &validity, &graph,
          ...
          NAU_ASSERT_DO_AND_LOG(!less_multiplexed(nodeData.multiplexingMode, resMplexMode), dumpRawUserGraph(validity),
            "Node '{}' requested resource '{}', which is more"
          ...
          if (history && registry.resources[resId].history == History::No)
          {
            ...
            NAU_LOG_ERROR("Node '{}' requested history for resource '{}', but the"
          ...
          {
            dumpRawUserGraph(validity);
            NAU_LOG_ERROR("Node '{}' requested resource '{}' with a mismatched"
...
void IrGraphBuilder::fixupFalseHistoryFlags(intermediate::Graph &graph) const
{
  ...
  IdIndexedFlags<intermediate::ResourceIndex, nau::EastlFrameAllocator> historyWasRequested;
  ...
  const intermediate::Mapping &mapping) const
{
  ...
  for (auto [nodeId, nodeData] : registry.nodes.enumerate())
  {
    ...
    for (auto midx : IdRange<intermediate::MultiplexingIndex>(mapping.multiplexingExtent()))
    {
      ...
      for (auto next : nodeData.followingNodeIds)
      {
        ...
        if (index == intermediate::NODE_NOT_MAPPED)
        {
          NAU_LOG_WARNING("A follow dependency of '{}', '{}'"
                  " went missing during IR generation!",
  ...
  {
    dumpRawUserGraph(validity);
    NAU_LOG_ERROR("Some nodes had explicit dependencies on nodes that don't exist!"
  ...
  eastl::span<const ResNameId> sinkResources) const -> SinkSet
{
  ...
  const auto findIrNodes = [this, &mapping, &result](NodeNameId id) {
    for (auto midx : IdRange<intermediate::MultiplexingIndex>(mapping.multiplexingExtent()))
    {
      ...
      if (idx == intermediate::NODE_NOT_MAPPED)
      {
        ...
        NAU_LOG_ERROR("Node '{}' was requested as a sink but did not survive IR generation", name);
        continue;
      }
  ...
  for (const auto &resId : sinkResources)
  {
    ...
    {
      NAU_LOG_ERROR("Attempted to mark a non-existent resource '{}' as a sink!", registry.knownNames.getName(resId));
      continue;
    }
    ...
    {
      NAU_LOG_ERROR("Attempted to mark a FG-internal resource '{}' as an "
    ...
    if (anyUnmapped)
      NAU_LOG_ERROR("Resource '{}' was requested as a sink, but was removed from "
...
IdIndexedMapping<intermediate::NodeIndex, intermediate::NodeIndex, nau::EastlFrameAllocator> IrGraphBuilder::pruneGraph(
  ...
  const ValidityInfo &validity) const
{
  ...
  IdIndexedMapping<intermediate::NodeIndex, intermediate::NodeIndex, nau::EastlFrameAllocator> visited(graph.nodes.size(), NOT_VISITED);
  ...
  using Wave = eastl::fixed_vector<intermediate::NodeIndex, 64, true, nau::EastlFrameAllocator>;
  ...
  for (auto [nodeId, v] : visited.enumerate())
    ...
    {
      anyNodePruned = true;
      NAU_LOG_ERROR("FG: Node '{}' is pruned. See below for full user graph dump.",
...
struct StateFieldSetter
{
    ...
    const auto getAttachment = [this](VirtualSubresourceRef res) -> eastl::optional<intermediate::SubresourceRef> {
      if (res.nameId != ResNameId::Invalid)
      {
        ...
        if (resIndex == intermediate::RESOURCE_NOT_MAPPED)
        {
          ...
          NAU_ASSERT(registry.nodes[nodeId].resourceRequests.find(resNameId)->second.optional);
          return eastl::nullopt;
        }
  ...
  void set(intermediate::BindingsMap &to, const BindingsMap &from)
  {
    ...
    for (const auto &[bindIdx, bindInfo] : from)
    {
      ...
      NAU_ASSERT(bindInfo.type != BindingType::Invalid);
      ...
      {
        // Sanity check, node should've been skipped otherwise
        NAU_ASSERT((bindInfo.history ? registry.nodes[nodeId].historyResourceReadRequests : registry.nodes[nodeId].resourceRequests)
                   .find(resNameId)
                   ->second.optional);
