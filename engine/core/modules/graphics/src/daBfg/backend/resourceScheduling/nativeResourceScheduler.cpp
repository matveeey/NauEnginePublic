...
NativeResourceScheduler::ResourceKey::ResourceKey(const ResourceKey &key, const nau::math::Vector2 &resolution) :
  hash(key.rawHash), rawHash(key.rawHash), desc(key.desc)
{
  hashPack(hash, resolution.getX(), resolution.getY());
  desc.asTexRes.width = resolution.getX();
  desc.asTexRes.height = resolution.getY();
}
...
ManagedResView<typename ResType::BaseType> NativeResourceScheduler::getResource(int frame, intermediate::ResourceIndex res_id) const
{
  ...
  NAU_ASSERT_RETURN(it != placedResources.end(), {});

  ManagedResView<typename ResType::BaseType> resView = eastl::get<ResType>(it->second.resource);
  NAU_ASSERT_RETURN(resView, {});
  resView->setResName(reinterpret_cast<const char8_t*>(cachedIntermediateResourceNames[res_id].c_str()));
  return resView;
}
...
ResourceScheduler::DestroyedHeapSet NativeResourceScheduler::allocateHeaps(const HeapRequests &heap_requests)
{
  ...
  NAU_LOG_DEBUG("daBfg: attempting to reuse {} heaps and allocate {} new ones ({} total):", heaps.size(), heap_requests.size() - heaps.size(),
    heap_requests.size());
  ...
  for (auto [i, req] : heap_requests.enumerate())
  {
    ...
    {
      NAU_LOG_DEBUG("daBfg: Reusing heap {} of size {}KB instead of allocating a new {}KB heap", eastl::to_underlying(i),
    ...
    if (heaps[i] != nullptr)
    {
      ...
      NAU_LOG_DEBUG("daBfg: Destroyed heap {} because it was too small/big (size was {}KB, but {}KB are required)", eastl::to_underlying(i),
        allocatedHeaps[i].size >> 10, req.size >> 10);
    }
    if (req.size > 0)
    {
      heaps[i] = d3d::create_resource_heap(req.group, req.size, RHCF_REQUIRES_DEDICATED_HEAP);
      if (EASTL_UNLIKELY(!heaps[i]))
        NAU_FAILURE("daBfg: Failed to allocate resource heap {} of size {}KB", eastl::to_underlying(i), req.size >> 10);
      else
        NAU_LOG_DEBUG("daBfg: Successfully allocated a new heap {} of size {}KB", eastl::to_underlying(i), req.size >> 10);
    }
  ...
  const ResourceKey &key, const ResourceCreator &resourceCreator)
{
  ...
  if (placedResources.find(key) == placedResources.end())
  {
    ...
    placedResources.emplace(key, ResourceEntry{static_cast<uint32_t>(placedProperties.size() - 1), resourceCreator()});
  }
  NAU_ASSERT(placedProperties.size() > placedResources[key].collectionIdx);
...
static auto generate_name(uint32_t heap_idx, size_t hash_value)
{
  ...
  {
    NAU_LOG_ERROR("daBfg has allocated more than 255 resource heaps! This shouldn't be happening!");
    heap_idx &= 0xFF;
  }
  ...
  const ResourceDescription &desc, uint32_t offset, const ResourceAllocationProperties &properties)
{
  ...
  else
  {
    auto createTex = [this, &key, &heap_idx, &placedResName, &offset, &properties]() -> UniqueTex {
      return UniqueTex(
        dag::place_texture_in_resource_heap(
            heaps[heap_idx], 
            fix_tex_params(key.desc), 
            offset, properties, 
            placedResName.c_str()
        ));
    };
...
void NativeResourceScheduler::resizeTexture(intermediate::ResourceIndex res_idx, int frame, const nau::math::Vector2 &resolution)
{
...
void NativeResourceScheduler::resizeAutoResTextures(int frame, const DynamicResolutions &resolutions)
{
  for (auto idx : IdRange<intermediate::ResourceIndex>(cachedIntermediateResources.size()))
  {
    ...
    if (res)
      resizeTexture(idx, frame, nau::math::Vector2{mult * res->getX(), mult * res->getY()});
  }
}
