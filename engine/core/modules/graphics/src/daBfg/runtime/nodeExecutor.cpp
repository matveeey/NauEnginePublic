  ...
  const H &blobProvider)
{
  ...
  auto processRes = [&](ResNameId unresolvedResNameId, bool history, bool optional) {
    ...
    NAU_ASSERT(registry.resources.isMapped(resId)); // Sanity check, nodeTracker makes sure this doesn't happen
    ...
    switch (registry.resources[resId].type)
    {
      ...
      case ResourceType::Invalid: NAU_ASSERT(optional); break;
    }
  };
  ...
  const ResourceScheduler::FrameEventsRef &events, const sd::NodeStateDeltas &state_deltas)
{
    ...
    {
      {
        ...
        if (auto &exec = registry.nodes[irNode.frontendNode].execute)
          ...
          NAU_LOG_ERROR("Somehow, a node with an empty execution callback was "
  ...
  if(!events.empty()) // events could be empty if no graph nodes present
  {
      processEvents(events.back());
  }

  if(!state_deltas.empty()) // state_deltas could be empty if no graph nodes present
  {
  ...
  for (const auto &[resNameIdUnresolved, request] : registry.nodes[nameId].resourceRequests)
  {
    ...
    if (auto provider = eastl::get_if<ExternalResourceProvider>(&resData.creationInfo))
    {
      ...
      if (auto *tex = eastl::get_if<ManagedTexView>(&res))
        NAU_ASSERT(*tex, "External texture {} wasn't provided by node {}", registry.knownNames.getName(resNameId),
          registry.knownNames.getName(nameId));
      else if (auto *buf = eastl::get_if<ManagedBufView>(&res))
        NAU_ASSERT(*buf, "External buffer {} wasn't provided by node {}", registry.knownNames.getName(resNameId),
          registry.knownNames.getName(nameId));
    ...
    switch (resType)
    {
      case ResourceType::Texture:
      {
        ...
        else if (eastl::holds_alternative<ResourceScheduler::Event::Deactivation>(evt.data))
        {
          ...
          NAU_ASSERT(false, "Unexpected event type!");
      }
      break;
      ...
      case ResourceType::Invalid: NAU_ASSERT_BREAK(false, "Encountered an event for an invalid resource!")
    }
  }
}
...
void NodeExecutor::applyState(const sd::NodeStateDelta &state, int frame, int prev_frame) const
{
  ...
  if (state.pass)
  {
    if (auto pass = eastl::get_if<intermediate::RenderPass>(&*state.pass))
    {
      ...
      d3d::set_render_target(depthTarget, pass->depthReadOnly ? DepthAccess::SampledRO : DepthAccess::RW,
        nau::ConstSpan<RenderTarget>(colorTargets.data(), colorTargets.size()));
    }
    else
    {
  ...
  // TODO: fix state.shaderBlockLayers.sceneLayer
...
static void bindViewSetter(int, const nau::math::Matrix4& data) { d3d::settm(TM_VIEW, &data); };
static void bindProjSetter(int, const nau::math::Matrix4& data) { d3d::settm(TM_PROJ, &data); };
...
void NodeExecutor::applyBindings(const intermediate::BindingsMap &bindings, int frame, int prev_frame) const
{
  for (const auto &[bindIdx, bindInfo] : bindings)
  {
    ...
    switch (bindInfo.type)
    {
      ...
      {
        NAU_ASSERT_BREAK(bindInfo.projectedTag == tag_for<nau::math::Matrix4>(),
          "Binding a blob as VIEW whose projected type is not a matrix.");
        if (bindInfo.projectedTag == tag_for<nau::math::Matrix4>())
        {
          bindBlob<nau::math::Matrix4, static_cast<void (*)(int, const nau::math::Matrix4&)>(&bindViewSetter)>(bindIdx, bindInfo, frame);
        }
        break;
      }
      case BindingType::ProjMatrix:
      {
        ...
        bindBlob<nau::math::Matrix4, &bindProjSetter>(bindIdx, bindInfo, frame);
        break;
      }
      case BindingType::Invalid: NAU_ASSERT(false); break;
    };
  }
}
...
{
  const auto svType = SHVT_REAL; // static_cast<ShaderVarType>(ShaderGlobal::get_var_type(bind_idx));
...
#define SHV_CASE_END(shVarType)                                                                                  \
  NAU_ASSERT(!binding.resource.has_value(), "Binding a blob whose projected type do not match shader var type."); \
  break;
#define TAG_CASE(projType, shVarSetter)                        \
  ...
  {                                                            \
    ...
    //SHV_BIND_BLOB_LIST // TODO: fix it
...
#undef TAG_CASE
    case SHVT_TEXTURE:
    {
      ...
      NAU_FAILURE("ShaderGlobal::set_texture not implemented");
      break;
    }
    case SHVT_BUFFER:
    {
      ...
      NAU_FAILURE("ShaderGlobal::set_buffer not implemented");
      break;
    }
    case SHVT_SAMPLER: NAU_LOG_ERROR("set_sampler is not implemented yet"); break;
  }
}
