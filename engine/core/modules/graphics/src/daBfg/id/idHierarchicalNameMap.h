...
#include "dabfg/id/idNameMap.h"
#include "dabfg/id/idSparseNameMap.h"
...
{
  NAU_ASSERT(name != nullptr && *name != '\0', "Empty names are not allowed!");
  NAU_ASSERT(strchr(name, '/') == nullptr, "Names cannot contain slashes!");
...
const char *IdHierarchicalNameMap<Ts...>::getShortName(T id) const
{
  ...
  NAU_ASSERT_RETURN(lastSlash != nullptr, fullName); // sanity check
  return lastSlash + 1;
}
