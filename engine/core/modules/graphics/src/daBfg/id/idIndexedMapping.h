...
struct IdIndexedMapping : eastl::vector<ValueType, Allocator>//, true, eastl::underlying_type_t<EnumType>>
{
  using index_type = EnumType;
  using Container = eastl::vector<ValueType, Allocator>;// , true, eastl::underlying_type_t<EnumType >> ;
  ...
  {
    NAU_FAST_ASSERT(Container::size() < eastl::numeric_limits<eastl::underlying_type_t<EnumType>>::max());
