...
{
  NAU_ASSERT_RETURN(eventsScheme.size() > event_id, false);
...
{
  NAU_ASSERT_RETURN(eventsScheme.size() > event_id, invalid_event_scheme_hash);
...
{
  NAU_ASSERT_RETURN(eventsScheme.size() > event_id, 0);
...
{
  NAU_ASSERT_RETURN(eventsScheme.size() > event_id, "");
...
{
  NAU_ASSERT_RETURN(eventsScheme.size() > event_id, -1);
  const event_scheme_t *schemes = eventsScheme.get<event_scheme_t>();
  const event_scheme_t &scheme = schemes[event_id];
...
{
  NAU_ASSERT_RETURN(eventsScheme.size() > event_id, nullptr);
  const event_scheme_t *schemes = eventsScheme.get<event_scheme_t>();
  const event_scheme_t &scheme = schemes[event_id];
...
U EventsDB::getEventFieldValue(const ecs::Event &evt, event_id_t event_id, int idx) const
{
  ...
  NAU_UNUSED(type);
  NAU_FAST_ASSERT(type == ecs::ComponentTypeInfo<T>::type);
...
inline const char *EventsDB::getEventFieldValue<const char *, const char *>(const ecs::Event &evt, event_id_t event_id, int idx) const
{
  ...
  const component_type_t type = scheme.get<component_type_t>()[idx];
  NAU_UNUSED(type);
  NAU_FAST_ASSERT(type == ecs::ComponentTypeInfo<eastl::string>::type);
  return *(const char **)(((char *)&evt) + offset);
}

inline event_flags_t EventsDB::getEventFlags(event_id_t id) const
{
  NAU_ASSERT_RETURN(eventsInfo.size() > id, (event_flags_t)0);
