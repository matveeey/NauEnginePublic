...
enum EventCastType
{
  EVCAST_UNKNOWN =   NauFlag(0),
  EVCAST_UNICAST =   NauFlag(1),
  EVCAST_BROADCAST = NauFlag(2),
  EVCAST_BOTH =      NauFlag(1)|NauFlag(2),
};

enum EventFlags : event_flags_t
{
  EVFLG_CASTMASK = NauFlag(1)|NauFlag(2),   // EventCastType
  EVFLG_SERIALIZE = NauFlag(3),  // need (de)serialization for networking
  EVFLG_DESTROY = NauFlag(4),    // need destruction
  EVFLG_SCHEMELESS = NauFlag(5), // actual class is ecs::SchemelessEvent. This is temporarily, as we plan to get rid of all schemeless
                           // events, once we make inspection code
  EVFLG_CORE = NauFlag(6),       // this is not really required, that's for validation purpose
  EVFLG_PROFILE = NauFlag(7),    // ES for this event will be profiled by default
};
...
struct NAU_DAGORECS_EXPORT alignas(align_event_on_emplace ? 0 : EVENT_ALIGNMENT)  Event
{
protected:
  event_type_t type;
  event_size_t length;
...
#define ECS_INSIDE_EVENT_DECL(Klass, Flags)                                                       \
  ...
  {                                                                                               \
    static_assert(sizeof(Klass) <= ::ecs::Event::max_event_size);                               \
...
struct NAU_DAGORECS_EXPORT EventInfoLinkedList
{
...
protected:
  ...
  static EventInfoLinkedList *tail;
  static EventInfoLinkedList *registered_tail;
...
public:
  ...
  {
    static_assert(sizeof(T) < eastl::numeric_limits<event_size_t>::max());
  }
...
#define ECS_ON_EVENT(...)   NAU_FOR_EACH(ECS_ON_EVENT_ONE, __VA_ARGS__)
#else
