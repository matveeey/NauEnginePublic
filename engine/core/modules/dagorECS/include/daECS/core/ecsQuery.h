...
#include <daECS/utility/dag_generationRefId.h>
...
#define QUERY_FAST_ASSERT \
  NAU_FAST_ASSERT // there is no reason to check type correctness, as we generate our queries using codegen. do that only in
                // DAGOR_DBGLEVEL>1
...
{
  nau::ConstSpan<ComponentDesc> componentsRW; // descriptors of components that this query needs to write
  nau::ConstSpan<ComponentDesc> componentsRO; // descriptors of components that this query needs to read
  nau::ConstSpan<ComponentDesc> componentsRQ; // descriptors of components that this query needs to exist
  nau::ConstSpan<ComponentDesc> componentsNO; // descriptors of components that this query needs to not exist
  BaseQueryDesc(nau::ConstSpan<ComponentDesc> comps_rw, nau::ConstSpan<ComponentDesc> comps_ro, nau::ConstSpan<ComponentDesc> comps_rq,
    nau::ConstSpan<ComponentDesc> comps_no) :
    componentsRW(comps_rw), componentsRO(comps_ro), componentsRQ(comps_rq), componentsNO(comps_no)
  {}
  static inline bool equal_list(nau::ConstSpan<ComponentDesc> a, nau::ConstSpan<ComponentDesc> b)
  {
    if (a.size() != b.size())
      return false;
...
struct NamedQueryDesc : public BaseQueryDesc
{
  ...
  NamedQueryDesc(const char *n, nau::ConstSpan<ComponentDesc> comps_rw, nau::ConstSpan<ComponentDesc> comps_ro,
    nau::ConstSpan<ComponentDesc> comps_rq, nau::ConstSpan<ComponentDesc> comps_no) :
...
struct NAU_DAGORECS_EXPORT CompileTimeQueryDesc : public NamedQueryDesc
{
  CompileTimeQueryDesc(const char *n, nau::ConstSpan<ComponentDesc> comps_rw, nau::ConstSpan<ComponentDesc> comps_ro,
    nau::ConstSpan<ComponentDesc> comps_rq, nau::ConstSpan<ComponentDesc> comps_no, uint32_t def_quant = 0) :
...
#define ECS_REQUIRE(...)                   NAU_FOR_EACH(ECS_REQUIRE_ONE, __VA_ARGS__)
#define ECS_REQUIRE_NOT_ONE(a)             __attribute__((annotate("@require_not:" #a)))
#define ECS_REQUIRE_NOT(...)               NAU_FOR_EACH(ECS_REQUIRE_NOT_ONE, __VA_ARGS__)
...
inline nau::ConstSpan<ecs::ComponentDesc> empty_span(const ecs::ComponentDesc *) { return {}; }
inline nau::ConstSpan<ecs::ComponentDesc> empty_span() { return {}; }
