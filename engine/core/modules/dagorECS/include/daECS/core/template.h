...
#include <nau/utils/dag_nameHashers.h>
...
class NAU_DAGORECS_EXPORT Template
{
  Template();
...
public:
  ...
  typedef eastl::vector_set<component_t, eastl::less<component_t>, EASTLAllocatorType, eastl::vector<component_t, EASTLAllocatorType>>
    component_set;
  static constexpr size_t PARENTS_INPLACE_SIZE = 5; // 98% templates in Enlisted have <=4 parents
  typedef eastl::fixed_vector<template_t, PARENTS_INPLACE_SIZE> ParentsList;
...
#endif
  nau::ConstSpan<template_t> getParents() const { return parents; }
...
private:
  ...
  mutable dag::RelocatableFixedVector<component_index_t, 7, true, EASTLAllocatorType /*, MidmemAlloc TODO Allocators.*/, uint16_t> dependencies;  // cache for dependencies
...
struct DataBlock
{
    ...
    void addNewBlock(const DataBlock*, const char*) {
    };
    const DataBlock* getBlock(const int) const
    {
        return nullptr;
    };
};
...
struct TemplateComponentsDB
{
  ...
  dag::HashedKeyMap<component_t, template_component_index_t> componentToName;
  enum
  {
    COMPONENT_TYPE,
    COMPONENT_NAME
  ...
  dag::StringTableAllocator componentNames = {13, 13};

  component_type_t getComponentTypeById(template_component_index_t id) const
  {
    NAU_FAST_ASSERT(id < componentTypes.size());
  ...
  bool addComponent(component_t c, component_type_t type, const char *n)
  {
    if (auto ci = componentToName.findVal(c))
    {
      NAU_FAST_ASSERT(*ci < componentTypes.size());
      ...
      {
        logerr("hash collision for 0x{} on name {} and {}", c, n, componentNames.getDataRawUnsafe(oldNameAddr));
        return false;
      }
      if (type && oldType == 0)
        oldType = type;
      ...
      {
        logerr("different types 0x{} (was 0x{}) for component <{}|0x{}>", type, oldType, componentNames.getDataRawUnsafe(oldNameAddr),
          c);
        return false;
      }
      return true;
...
{
  typedef eastl::vector<Template> TemplateList;
  using NameMap = dag::OAHashNameMap<false, dag::FNV1OAHasher<false>>;
  ...
  dag::OAHashNameMap<false> pathNames;
#if NAU_DEBUG
#endif
...
public:
  ...
  AddResult addTemplate(Template &&templ, nau::ConstSpan<const char *> *pnames = NULL, uint32_t tag = 0);
  AddResult addTemplate(Template &&templ, nau::ConstSpan<template_t> parentIds = {}, uint32_t tag = 0);
...
private:
  ...
  eastl::vector<InstantiatedTemplateInfo> instantiatedTemplates; // parallel to templates
