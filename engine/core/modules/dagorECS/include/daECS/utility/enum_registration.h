...
namespace ecs
{
    ...
    ecs::ChildComponent parse_enum(const char* component_name, const char* enum_type_name, const char* enum_value)
    {
        ...
        {
            logerr("enum component {} : {} hasn't value, fallback to {}({})", enum_type_name, component_name, enum_to_str(enumValue),
                   enumValue);
            return ecs::ChildComponent(enumValue);
        }
        if(!str_to_enum(enum_value, enumValue))
        {
            logerr("enum component {} : {} value parse failed, unsupported {} value, fallback to {}({})", enum_type_name, component_name,
    ...
    typedef nau::ConstSpan<const char*> (*enum_names_t)();
    ...
    void ecs_enum_registration(const char* enum_name, enum_parse_t parse, enum_names_t get_names, update_enum_t update_value, find_enum_idx_t find_enum_idx, component_type_t component);
