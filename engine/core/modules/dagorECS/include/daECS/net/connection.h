...
public:
  ...
  void setEncryptionKey(nau::ConstSpan<uint8_t> ekey, EncryptionKeyBits ebits) override;
...
private:
  ...
  void onEntityReplicationLost(ObjectReplica &replica, nau::ConstSpan<CompRevision> comprevs);
...
private:
  ...
  eastl::vector<eastl::vector<ecs::component_index_t>> clientTemplatesComponents; // only on client. direct map of templates to server components
  ...
  eastl::bitvector<> componentsSynced; // if component_index is already relpicated to/from, it will be true here. Both client and
                                       // server
  eastl::vector<ecs::template_t> serverTemplatesIdx;          // only on server. indices of written templates
  eastl::vector<ecs::template_t> serverIdxToTemplates;        // reverse for serverTemplatesIdx
  ...
  struct DelayedAttrsUpdateRec
  {
    ...
    mutable eastl::fixed_vector<eastl::vector<uint8_t, MidmemAlloc>, 1> data;
  };
