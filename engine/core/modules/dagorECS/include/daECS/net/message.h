...
typedef nau::Span<net::IConnection *> (*recipient_filter_t)(Tab<net::IConnection *> &out_conns, ecs::EntityId to_eid,
  const net::IMessage &);
...
__declspec(noinline)
  ...
  nau::Span<net::IConnection *> broadcast_rcptf(Tab<net::IConnection *> &out_conns, ecs::EntityId, const IMessage &)
#ifdef __GNUC__ /* including clang */
    __attribute__((noinline))
#endif
    ;
nau::Span<net::IConnection *> direct_connection_rcptf(Tab<net::IConnection *> &out_conns, ecs::EntityId, const IMessage &);
...
{
public:
  static_assert(sizeof(T) <= eastl::numeric_limits<decltype(MessageClass::memSize)>::max());
