...
public:
  ...
  void setPeerKey(SystemIndex peer_id, nau::ConstSpan<uint8_t> key, net::EncryptionKeyBits ebits)
  {
    NAU_ASSERT(ebits == net::EncryptionKeyBits::None || key.size() >= KEY_LEN);
    WinAutoLock lock(mutex);
...
private:
    ...
    size_t outLimit) override
  {
    ...
    for (size_t i = 0; i < bufCount; ++i)
    {
      ...
      NAU_FAST_ASSERT(ptr + len <= (outData + outLimit));
      cipherCtx->update(ptr, &outlen, (const unsigned char *)buffers[i].data, len);
      NAU_FAST_ASSERT(len == outlen);
      ptr += outlen;
    }
    ...
    NAU_ASSERT(total == (int)inLimit, "%d != %d", total, (int)inLimit); // required for correct IV generation

    if (cipherCtx == gcmCipherCtx.get())
    {
      cipherCtx->getAuthTag(nau::Span<uint8_t>(outData - AUTH_TAG_LEN, AUTH_TAG_LEN)); // write final auth tag in first AUTH_TAG_LEN
                                                                                       // bytes
      total += AUTH_TAG_LEN;
    }
  ...
  size_t decode(SystemIndex peer_id, const uint8_t *inData, size_t inLimit, uint8_t *outData, size_t /*outLimit*/) override
  {
    ...
    if (gcmCipher)
    {
      cipherCtx->setAuthTag(nau::ConstSpan<uint8_t>(inData, AUTH_TAG_LEN));
      inData += AUTH_TAG_LEN;
    }

    int outlen;
    cipherCtx->update(outData, &outlen, inData, inLimit);
    NAU_FAST_ASSERT(outlen == inLimit);
