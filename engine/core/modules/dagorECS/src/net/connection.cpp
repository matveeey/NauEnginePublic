...
bool write_server_eid(ecs::entity_id_t eidVal, danet::BitStream &bs)
{
  ...
  NAU_ASSERT(generation <= UCHAR_MAX, "%u", eidVal);
  ...
  {
    NAU_ASSERT(index < (1 << 22), "%u", eidVal);
...
{
  NAU_ASSERT(id != net::INVALID_CONNECTION_ID);
  NAU_ASSERT(SCOPE_QUERYING_ENABLED || !scopeQuery);
}
...
#if _DEBUG
  NAU_FAST_ASSERT(repl->getEid() == eid);
#endif
  return repl;
}
...
ObjectReplica *Connection::addObjectInScope(Object &obj)
{
  ...
  NAU_ASSERT(MaxReplicas <= (USHRT_MAX + 1) || replIndex <= USHRT_MAX);
...
{
  NAU_ASSERT(isReplicatingFrom());
  NAU_ASSERT(replicas[repl->arrayIndex] == repl);
  NAU_ASSERT(repl->arrayIndex < numDirtyReplicas);
  NAU_ASSERT(!repl->isFree());
  numDirtyReplicas--;
...
void Connection::pushToDirty(ObjectReplica *repl)
{
  ...
  eastl::swap(replicas[repl->arrayIndex]->arrayIndex, replicas[numDirtyReplicas]->arrayIndex);
  numDirtyReplicas++;
  NAU_ASSERT(numDirtyReplicas <= numReplicas);
}
...
{
  NAU_ASSERT(!isReplicatingTo());
  if (isReplicatingFrom())
    return;

  replicatingType = REPLICATING_FROM;
  static_assert(MaxReplicas <= eastl::numeric_limits<decltype(ObjectReplica::arrayIndex)>::max());
...
int Connection::prepareWritePackets()
{
  ...
  {
    nau::Span<ObjectReplica *> dirtyReplicas(replicas.data(), numDirtyReplicas);
...
bool Connection::writeDestructionPacket(danet::BitStream &bs, danet::BitStream & /*tmpbs*/, int limit_bytes)
{
  ...
  {
    ObjectReplica *repl = replicas[i];
    NAU_FAST_ASSERT(!repl->isFree());
    if (!repl->isToKill())
    {
      // all to kill replicas should be in the end of 'replicas' (sorting invariant)
#if _DEBUG
      for (int j = i - 1; j >= 0; --j)
        NAU_ASSERT(!replicas[j]->isToKill(), "%d: 0x%x", j, replicas[j]->flags);
#endif
      break;
    }
...
void Connection::writeReplayKeyFrame(danet::BitStream &bs, danet::BitStream &tmpbs)
{
  ...
  eastl::vector<ecs::template_t> serverTemplatesIdxSaved;
  eastl::vector<ecs::template_t> serverIdxToTemplatesSaved;
...
bool Connection::readReplayKeyFrame(const danet::BitStream &bs, const on_object_constructed_cb_t &obj_constructed_cb)
{
  ...
  for (uint16_t i = 0; i < serializeBlockCount; ++i)
  {
    ...
    NAU_ASSERT_RETURN(success, false);
  }
  ...
  for (uint16_t i = 0; i < serializeBlockCount; ++i)
  {
    ...
    NAU_ASSERT_RETURN(tpl_deserialized, false);
  }
...
#if _DEBUG
      for (int j = i - 1; j >= 0; --j)
        NAU_ASSERT(!replicas[j]->isNeedInitialUpdate() && !replicas[j]->isToKill(), "%d: 0x%x", j, replicas[j]->flags);
#endif
      break;
    }
    ecs::EntityId eid = repl->getEid();
...
bool Connection::writeReplicationPacket(danet::BitStream &bs, danet::BitStream &tmpbs, int limit_bytes)
{
                                                                                          ...
                                                                                          // replicas without breaking sort order
  {
    ...
    else
    {
      ...
      for (auto &crev : make_span_const(&pendingPacketRepl.compRevisions[prevCompRevsSize], revcount))
      {
        ...
        NAU_ASSERT(!ins.second, "%d %u %p", crev.compIdx, pendingPacketRepl.pendingReplicas.back().first,
  ...
  NAU_ASSERT(pendingPacketRepl.compRevisions.empty() == (bs.GetWriteOffset() == afterHdrPos));
...
void Connection::applyDelayedAttrsUpdate(ecs::entity_id_t server_eid)
{
  ...
  if (it != delayedAttrsUpdate.end())
  {
    ...
    NAU_FAST_ASSERT(nobj);
    NAU_FAST_ASSERT(!it->data.empty());
...
#if _DEBUG // Note: don't do this check/logerr in release since in theory this might due to tampered server -> client
                           // network packet
      {
...
bool Connection::readReplicationPacket(const danet::BitStream &bs)
{
  ...
  do
  {
    ...
    if (nobj && !delayedAttrsUpdate.count(DelayedAttrsUpdateRec{serverEid}))
    {
      ...
      NAU_ASSERT(BITS_TO_BYTES(bs.GetReadOffset() - startPos) == blockSizeBytes);
    }
    else // attrs update for missing local object (assume that creation packet is not received yet, and save it for future apply)
    {
      ...
      eastl::vector<uint8_t, MidmemAlloc> &tab = it->data.emplace_back();
...
void Connection::onEntityReplicationLost(ObjectReplica &replica, nau::ConstSpan<CompRevision> comprevs)
{
  bool dirty = false;
...
void Connection::onReplicationAckPacketNotify(sequence_t seq, bool lost)
{
  ...
  for (auto &curPendInfo : pendingReplInfo)
    if (curPendInfo.waitSequence == seq)
    {
      if (EASTL_UNLIKELY(lost))
      {
        ...
        NAU_ASSERT(curOffs == curPendInfo.compRevisions.size());
      }
  ...
  NAU_ASSERT(0, "Failed to lookup pending packet replication info for seq %d", seq);
}

void Connection::update(int cur_time) { reliabilitySys.update(cur_time, onPacketDropped, this); }

void Connection::setEncryptionKey(nau::ConstSpan<uint8_t> ekey, EncryptionKeyBits ebits)
{
  if (encryptionCtx)
