...
static ecs::EventSet build_event_set_for_tx_routing(nau::ConstSpan<net::MessageRouting> tx_routings, net::Er er)
{
  ecs::EventSet evs;
...
static void update_esd_event_set(nau::ConstSpan<net::MessageRouting> tx_routings)
{
...
static void do_init_indexes(nau::ConstSpan<MessageRouting> tx_routings)
{
  ...
  auto collectIndexes = [&tx_routings](const EventRegRecord &err) {
    ...
    {
      NAU_ASSERT(err.msgClass.classId >= 0, "MessageClass::init() wasn't called?");
...
IMessage *create_message_by_event(const ecs::Event &evt, net::Er er, IMemAlloc *alloc)
{
  ...
  if (it == tx_events_index.end())
    return nullptr;
  NAU_ASSERT(it->second->er == er, "Message %s/%x was registered as broadcasted but sent as unicast (or vice versa)", evt.getName(),
    evt.getType());
  NAU_FAST_ASSERT(it->second->evt2msg);
