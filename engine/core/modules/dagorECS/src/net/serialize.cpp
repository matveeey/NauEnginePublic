...
{
  // assume Little endian
  NAU_ASSERT(str < uint32_t(1 << short_bits));
...
static const char *read_istring(const danet::BitStream &cb, InternedStrings *istrs, uint32_t short_bits)
{
  ...
  NAU_ASSERT(all.strings.size() <= uint32_t(1 << short_bits));
...
bool BitstreamDeserializer::read(void *to, size_t sz_in_bits, ecs::component_type_t user_type) const
{
  ...
  {
    NAU_ASSERT(sz_in_bits == CHAR_BIT);
...
bool BitstreamDeserializer::skip(ecs::component_index_t cidx, const ecs::DataComponent &compInfo)
{
  ...
  if (need_constructor(componentTypeInfo.flags))
  {
    ...
    NAU_ASSERT(ctm, "type manager for type 0x%X (%d) missing", compInfo.componentTypeName, compInfo.componentType);
  }
  if (ctm)
...
bool Connection::deserializeComponentReplication(ecs::EntityId eid, const danet::BitStream &bs)
{
  ...
  NAU_ASSERT(componentsSynced.test(serverCidx, false)); // should never happen, no need for sanity check in release
  ...
  bool error)
{
  ...
  if (clientCidx == ecs::INVALID_COMPONENT_INDEX)                                              // component is missing on client
  {
    ...
    if (typeIdx != ecs::INVALID_COMPONENT_TYPE_INDEX)
      clientCidx =
        g_entity_mgr->createComponent(ecs::HashedConstString{nullptr, name}, typeIdx, nau::Span<ecs::component_t>(), nullptr, 0);
  }
...
{
  NAU_ASSERT(templateId == serverTemplates.size()); // We rely on templateId being monotonically increased counter here
  ...
  for (uint16_t cid = 0; cid != componentsInTemplate; ++cid)
  {
    ...
    if (!read_component_index(serverCidx, bs))
      return false;
    NAU_ASSERT(serverCidx != ecs::INVALID_COMPONENT_INDEX);
    ...
    {
      NAU_ASSERT(componentsSynced.test(serverCidx, false));
...
const char *Connection::deserializeTemplate(const danet::BitStream &bs, ecs::template_t &templateId, bool &tpl_deserialized)
{
  ...
  NAU_ASSERT(serverTemplates.size() > templateId && serverTemplates[templateId].length());
...
void dump_initial_construction_stats()
{
  if (!templatesSize.size())
    return;
  static_assert(sizeof(ecs::template_t) + sizeof(ecs::component_index_t) == sizeof(uint32_t)); // if ever happen, can be replaced
                                                                                                 // with uint64_t
  NAU_CORE_DEBUG_LF("dumping_templates");
  ...
  {
    auto &t = templSizes[i];
    NAU_CORE_DEBUG_LF("template %d (%s) total kbytes =%.2f", t.first, g_entity_mgr->getTemplateName(t.first), t.second / (8. * 1024));
    ...
    NAU_CORE_DEBUG_LF("  template components total kb =%.2f, count = %d", totalCompSizes / (8. * 1024), templCompSizes.size());
    for (size_t j = 0; j < min(templCompSizes.size(), size_t(20)); ++j)
    {
      NAU_CORE_DEBUG_LF("    component %s bits =%d", g_entity_mgr->getDataComponents().getComponentNameById(templCompSizes[j].first),
...
void Connection::serializeConstruction(ecs::EntityId eid, danet::BitStream &bs, CanSkipInitial canSkipInitial)
{
  ...
  else                                    // not yet synced/known to client
  {
    ...
    NAU_FAST_ASSERT(serverWrittenIdx != ecs::INVALID_TEMPLATE_INDEX);
  ...
  NAU_ASSERT(lessThan256 == (componentsInTemplate < 256));
...
{
  NAU_ASSERT(serverId != ecs::ECS_INVALID_ENTITY_ID_VAL);
  NAU_UNUSED(sz);
  NAU_UNUSED(cratio);
...
#if _DEBUG
  NAU_ASSERT(g_entity_mgr->doesEntityExist(srvEid));
  NAU_ASSERT(!g_entity_mgr->getEntityTemplateName(srvEid),
  ...
  if (!templ->hasComponent(ECS_HASH("noECSDebug")))
    NAU_CORE_DEBUG_LF("create <%d> of server<%d> %stemplate <%s> %d bytes in %d/%d comps (cratio=%.3f,cpacket_seq=%u)", ecs::entity_id_t(srvEid),
