...
void dump_das_event_stat(DasEventStats &dasEventStats, const char *prefix)
{
  ...
  NAU_CORE_DEBUG_LF("das_net: dumping %s %d dasevents of total %d records, %d KBytes, %d packets:", prefix, limitLog, dasEventStats.size(),
    totalBytes >> 10, totalPackets);
  NAU_CORE_DEBUG_LF("  #N  %*s msg_size_bytes(avg/min/max) kilobytes(%%) packets(%%)", maxNameLen, "name");
  for (int i = 0; i < limitLog; ++i)
  {
    const DasEventStat &r = dasEventStats[i];
    const size_t ebytes = r.bytes;
    NAU_CORE_DEBUG_LF("  #%2d %*s %4u %4u %5u %7u(%5.2f%%) %7u(%5.2f%%)", i + 1, maxNameLen, g_entity_mgr->getEventsDb().findEventName(r.type),
...
{
  NAU_CORE_DEBUG_LF("das_net: invalidate das events now `%s`", reason);
...
{
  NAU_CORE_DEBUG_LF("das_net: invalidate das events for conn #%d", connId);
...
void send_dasevent(ecs::EntityManager *mgr, bool broadcast, const ecs::EntityId eid, bind_dascript::DasEvent *evt,
  const char *event_name, eastl::optional<nau::ConstSpan<net::IConnection *>> explicitConnections,
...
static void das_event_handshake_msg_handler(const net::IMessage *msg_)
{
  ...
  {
    NAU_CORE_DEBUG_LF("das_net: client #%d has correct dasevents version protocol 0x%x", (int)msg_->connection->getId(), serverVersion);
  }
  else
  {
...
static void invalidate_dasevents_cache_on_connect_to_server_es_event_handler(const EventOnConnectedToServer &)
{
  ...
  NAU_CORE_DEBUG_LF("das_net: we just connected to server. Lock dasevents registration. Send net version 0x%X", netVersion);
...
{
  NAU_CORE_DEBUG_LF("das_net: we just destroyed network. Invalidate cache, unlock dasevents registration");
...
static void invalidate_dasevents_cache_on_client_change_es_event_handler(const EventOnClientConnected &evt)
{
  ...
  {
    NAU_CORE_DEBUG_LF("das_net: new client #%d just connected to server", connId);
