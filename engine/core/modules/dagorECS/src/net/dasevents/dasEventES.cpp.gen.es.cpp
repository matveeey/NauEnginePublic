...
{
  NAU_FAST_ASSERT(evt.is<ecs::EventNetMessage>());
...
static void invalidate_dasevents_cache_on_connect_to_server_es_event_handler_all_events(const ecs::Event &__restrict evt, const ecs::QueryView &__restrict components)
{
  ...
  NAU_FAST_ASSERT(evt.is<EventOnConnectedToServer>());
...
{
  NAU_UNUSED(components);
  NAU_FAST_ASSERT(evt.is<EventOnNetworkDestroyed>());
...
{
  NAU_UNUSED(components);
  NAU_FAST_ASSERT(evt.is<EventOnClientConnected>());
