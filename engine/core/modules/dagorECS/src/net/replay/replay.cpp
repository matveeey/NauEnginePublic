...
struct ReplayWriter
{
  ...
  ~ReplayWriter()
  {
    ...
    {
      NAU_ASSERT(ftdata.size() >= sizeof(net::BaseReplayFooterData)); // shoud be at least BaseReplayFooterData bytes size
  ...
  {
    NAU_ASSERT(str_ends_with(write_fname, "XXXXXX")); // dd_mkstemp requirement
...
public:
  ...
  virtual void sendEcho(const char *, uint32_t) override { NAU_ASSERT(0, "Echo unsupported for ReplayWriterConnection"); }
  void saveKeyFrame(int cur_time)
  {
    NAU_ASSERT(dump_replay_key_frame_cb);
    ...
    NAU_CORE_DEBUG_LF("[Replay] dump key frame at %i", cur_time);
  }
  ...
  virtual void disconnect(DisconnectionCause) override { NAU_ASSERT(0, "Not supported!"); }
...
struct ReplayFileReader
{
    ...
    if (df_seek_to(fp, hdr.keyFrameTableOffset) != -1)
    {
      ...
      if (df_read(fp, &tableSize, sizeof(tableSize)) == sizeof(tableSize))
      {
        ...
        for (uint16_t i = 0; i < tableSize; ++i)
        {
          ...
          {
            NAU_CORE_FATAL("[Replay] Failed read key frame %i", i);
            keyFrameTable.clear();
            df_seek_to(fp, pos);
            return;
          }
  ...
  bool openForRead(const char *fname, uint16_t version, Tab<uint8_t> *out_footer_data)
  {
    ...
    if (!fp)
    {
      NAU_CORE_DEBUG_LF("[Replay] failed to open file: '%s'", fname);
      return false;
    }
    ...
    {
      NAU_CORE_DEBUG_LF("[Replay] Invalid replay header\n"
            "magic = %i vs %i\n"
            "version = %i vs %i\n"
            "hdrSize = %i vs %i\n"
            "for '%s'",
...
public:
  ...
  bool rewind(int rewind_time)
  {
    ...
    {
      NAU_ASSERT(hdr.isKeyFrame, "[Replay] Not a key frame at offset %i", it->offset);
      return false;
    }
    ...
    NAU_ASSERT_RETURN(keyFrameData, false, "[Replay] failed allocate memory for key frame %i", lenWithAlign);

    if (df_read(fp, keyFrameData, lenWithAlign) != lenWithAlign)
    {
      if (needAlloc)
        framemem_ptr()->free(keyFrameData);
      NAU_ASSERT(NULL, "[Replay] failed to read key frame at offset %i", it->offset);
      return false;
    }
  ...
  virtual Packet *receive(int cur_time_ms) override
  {
    ...
    NAU_ASSERT_RETURN(lenWithAlign <= BLOCK_SIZE, connectionLost(), "[Replay] packet is to big %i vs %i", lenWithAlign, BLOCK_SIZE);
...
public:
  ...
  {
    NAU_ASSERT(drv);
    NAU_ASSERT(ReplayWriter::wstream);
  }
