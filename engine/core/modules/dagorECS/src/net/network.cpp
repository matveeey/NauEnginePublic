...
static void dump_net_stat(eastl::vector_set<NetStatRecord> &netstat, const char *trans_type)
{
  ...
  NAU_CORE_DEBUG_LF("Dumping %s %d netstat records (message types) of total %d records, %d KBytes, %d packets:", trans_type, performNetStat,
    netstat.size(), totalBytes >> 10, totalPackets);
  NAU_CORE_DEBUG_LF("  #N compressed_flag name msg_size_bytes(avg/min/max) kilobytes(%%) packets(%%) compression_ratio"); // legend
  for (int i = 0; i < performNetStat && i < netstat.size(); ++i)
  {
    const NetStatRecord &r = netstat[i];
    size_t ebytes = r.effectiveBytes();
    NAU_CORE_DEBUG_LF("  #%2d %c %*s %4u %4u %5u %7u(%5.2f%%) %7u(%5.2f%%) %.3f", i + 1, r.isCompressed() ? 'C' : ' ', maxNameLen, r.name(),
...
#if _DEBUG
        NAU_ASSERT(robj, "%d", (ecs::entity_id_t)toEid);
#endif
...
void CNetwork::ObjMsg::apply(const net::Object &robj, net::Connection &from, CNetwork &cnet, const danet::BitStream &data) const
{
  ...
  NAU_ASSERT(!mgr.isLoadingEntity(toEid));
...
void CNetwork::syncStateUpdates(int cur_time, uint8_t replication_channel)
{
  ...
  for (auto &conn : clientConnections)
  {
    ...
    if (pwrRes & Connection::PWR_REPLICATION)
    {
      ...
      do
      {
        NAU_ASSERT(iterGuard++ < (16 << 10));
        NAU_UNUSED(iterGuard);
...
bool CNetwork::sendto(int cur_time, ecs::EntityId to_eid, const IMessage &msg, IConnection *conn_, const MessageNetDesc *msg_net_desc)
{
...
#if _DEBUG
  NAU_ASSERT(numClassIdBits >= 0, "MessageClass::init() wasn't called?");
  NAU_ASSERT(mcls.classId >= 0, "%s", mcls.debugClassName);
  NAU_ASSERT(mcls.classId < (1 << numClassIdBits));
#endif
  ...
  NAU_FAST_ASSERT(headerSize <= ID_ENTITY_MSG_HEADER_SIZE);
...
void CNetwork::destroyConnection(unsigned idx, DisconnectionCause cause)
{
  ...
  if (encryptionCtx)
  {
    ...
    encryptionCtx->setPeerKey(idx, nau::ConstSpan<uint8_t>(), EncryptionKeyBits::None);
  }
}
