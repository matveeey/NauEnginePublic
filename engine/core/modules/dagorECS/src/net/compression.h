  ...
  const uint32_t compression_threshold)
{
  ...
  NAU_ASSERT_RETURN(compressedSize > 0, bs, "lz4 compression of %d bytes failed (%d)!", payloadSize, compressedSize);
...
inline bool bitstream_decompress(const danet::BitStream &bs, danet::BitStream &outbs)
{
  if (bs.GetReadOffset() >= bs.GetNumberOfBitsUsed())
    return false;
  NAU_ASSERT((bs.GetReadOffset() & 7) == 0);
  NAU_ASSERT((bs.GetNumberOfBitsUsed() & 7) == 0);
