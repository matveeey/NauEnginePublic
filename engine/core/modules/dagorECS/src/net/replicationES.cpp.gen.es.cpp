...
{
  NAU_FAST_ASSERT(evt.is<ecs::EventEntityManagerEsOrderSet>());
...
{
  NAU_FAST_ASSERT(evt.is<ecs::EventEntityRecreated>());
  auto comp = components.begin(), compE = components.end(); NAU_ASSERT(comp!=compE); do
...
if (evt.is<ecs::EventEntityManagerAfterClear>()) {
  ...
  } else {NAU_ASSERT(0, "Unexpected event type <%s> in replication_destruction_es_event_handler", evt.getName());}
}
...
{
  NAU_FAST_ASSERT(evt.is<ecs::EventEntityDestroyed>());
  auto comp = components.begin(), compE = components.end(); NAU_ASSERT(comp!=compE); do
