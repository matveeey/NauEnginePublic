...
void ReliabilitySystem::packetReceived(sequence_t seq)
{
  ...
  for (int i = 1; i < receivedQueue.size(); ++i)
    NAU_ASSERT(is_seq_gt(receivedQueue[i], receivedQueue[i - 1]));
#endif
}
...
{
  NAU_FAST_ASSERT(packet_acked_cb != NULL && packet_lost_cb != NULL);
...
{
  NAU_ASSERT(!receivedQueue.empty()); // this method is expected to be called after 'packetReceived'
