...
public:
  ...
  void updateSrvOfs(int rdt_us)
  {
    ...
    // NAU_CORE_DEBUG_LF("%s skew=%d skd=%d offs=%d rdtus=%d", __FUNCTION__, timeSkewMs, skewDiff, (int)srvOfsUs, rdt_us);
  }
  ...
  void onReceiveTimeSyncResponse(const TimeSyncResponse *rmsg)
  {
    ...
    NAU_ASSERT(receivedServerTime >= 0);
    ...
    // NAU_CORE_DEBUG_LF("received rtt %d, server time %d, local time %d, queue %d, skew %d",
    ...
    if (memcmp(&bestTimeRec, &minTimeRec, sizeof(bestTimeRec)) != 0)
    {
      ...
      NAU_CORE_DEBUG_LF("applied rtt %d, skew %d ms", bestTimeRec.rtt, bestTimeRec.timeSkew);
    }
    float _1;
    advance(0.f, _1);
  }
...
void on_time_sync_response_received(const net::IMessage *msg) // non-static due to friendship
{
  ...
  if (t4cc == _MAKE4C('CLIT'))
    ...
    NAU_ASSERT(0, "Unknown/unsupported time impl %x", t4cc);
}
