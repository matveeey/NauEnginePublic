...
{
    eastl::vector<uint64_t, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> processList(process.size());
  uint32_t i = 0;
  for (auto p : process)
    processList[i++] = p;
  eastl::sort(processList.begin(), processList.end());
...
//template <> // TODO Uncomment or delete
...
{
#if NAU_DEBUG
  NAU_ASSERT_RETURN(archetypeId < archetypes.size(), false);
#endif
  ...
  if (podComparable)
  {
    switch (componentSize)
    {
      ...
      case 8: // int64_t
        ...
        break;
      //case 12: // Point3  // TODO Uncomment or delete
  ...
  else
  {
    switch (dataComponent.componentTypeName) // practicality beats purity. Specialize for most common slow types
    {
      ...
      case ecs::ComponentTypeInfo<ecs::FloatList>::type:
        static_assert(!ecs::ComponentTypeInfo<ecs::EidList>::is_boxed && !ecs::ComponentTypeInfo<ecs::IntList>::is_boxed &&
...
inline void EntityManager::trackChanged(EntityId eid, component_index_t cidx, TrackedChangesTemp &to_process)
{
  ...
  NAU_ASSERT_RETURN(componentInArchetypeIndex < INVALID_ARCHETYPE_COMPONENT_ID, );
...
inline unsigned EntityManager::trackScheduledChanges()
{
  ...
  // NAU_CORE_DEBUG_LF("archetypeTrackingQueue.size() = {}", archetypeTrackingQueue.size());
  ...
  }
  {
    ...
    {
      static_assert(sizeof(EntityId) + sizeof(component_index_t) <= sizeof(scheduled));
      trackChanged(get_eid_from_scheduled(scheduled), get_second_from_scheduled(scheduled), toProcess);
    }
  }
  if (std::max<size_t>(32, eidTrackingQueue.get_capacity(eidTrackingQueue.size()) * 2) < eidTrackingQueue.bucket_count()) // allow
                                                                                                                     // overallocation
                                                                                                                     // of size of 2,
                                                                                                                     // do not reduce
                                                                                                                     // to less than
  ...
  if (!toProcess.empty())
  {
    ...
    lastTrackedCount = nau::math::lerp((float)lastTrackedCount, (float)toProcess.size(), lastTrackedCount > toProcess.size() ? 0.2f : 0.8f);
  }
#if NAU_DEBUG
  DA_PROFILE_TAG(tracked_changes_amount, "archetypes {}, eids {}, to_process {}", aei, eei, toProcess.size());
#endif
  return tei;
}
...
void EntityManager::sheduleArchetypeTracking(const ArchetypesQuery &archDesc)
{
  ...
  ScopedMTMutexT<nau::threading::SpinLock> lock(isConstrainedMTMode(), archetypeTrackingMutex);
