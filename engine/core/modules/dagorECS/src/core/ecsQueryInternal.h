...
struct ScheduledArchetypeComponentTrack // sizeof == 4, can be effectively hashed
{
  ...
  {
    static_assert(sizeof(combined) == sizeof(cidx) + sizeof(archetype));
    return combined;
  }
};
...
{
  void operator()(void *p) { free(p); /* memfree_default TODO Allocators*/ }
};
...
struct alignas(16) ArchetypesQuery
{
  ...
  {
    static_assert(offsetof(ArchetypesQuery, rwCount) == offsetof(ArchetypesQuery, roCount) + 2);
    static_assert(sizeof(rwCount) == 2 && sizeof(roCount) == 2 && sizeof(roRW) == 4);
    return roRW != 0;
  }
  ...
  void reset()
  {
    // allComponentsId.resize(0);
      if(!isInplaceQueries())
      {
          free(queries); // memfree_default TODO Allocators
      }
    queries = nullptr;
      if(!isInplaceOffsets())
      {
          free(allComponentsArchOffsets);// memfree_default TODO Allocators
      }
...
struct ResolvedQueryDesc // resolved query won't change ever (if it was successfully resolved)
{
  ...
  typedef dag::RelocatableFixedVector<component_index_t, 7, true, /* MidmemAlloc TODO Allocators.*/ EASTLAllocatorType, uint16_t> components_vec_t;
