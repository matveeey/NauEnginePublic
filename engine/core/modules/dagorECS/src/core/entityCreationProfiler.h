...
#if NAU_DEBUG && TIME_PROFILER_ENABLED
#if _TARGET_64BIT
...
#undef _X
  ...
  void dump()
  {
    ...
    logmessage(ll, "Entity creation of template <{}> took too much time: {} us", manager.getTemplateName(templ), usecPassed);
    double pctK = 100. / double(usecPassed);
#define _X(x)                                         \
  x##D = (double)profile_usec_from_ticks_delta(x##D); \
  if (x##D > SKIP_TIME_THRESHOLD_US)                  \
    NAU_CORE_DEBUG_LF("  %24s took %6d us (%2.3f%%)", #x, x##D, double(x##D) * pctK);
    DECL_ENT_CREATE_MARKERS
#undef _X
    if ((constructorsD + tracked_constructorsD) > SKIP_TIME_THRESHOLD_US)
    {
      ...
      int i = 0;
      NAU_CORE_DEBUG_LF("  %2s %40s %55s %6s {}", "#N", "comp_name", "comp_type_name", "usec", "usec_percent");
      for (auto &c : compTimes)
      {
        ...
        NAU_CORE_DEBUG_LF("  %2d %40s %55s %6d %2.3f", ++i, cName, tpName, usec, usec * pctK);
        cur += c.second;
