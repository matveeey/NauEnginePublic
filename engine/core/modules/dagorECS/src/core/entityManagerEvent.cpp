...
inline bool has_component(nau::ConstSpan<ComponentDesc> components, component_t name)
{
...
__forceinline const EntitySystemDesc &EntityManager::getESDescForEvent(es_index_type esIndex, const Event &evt) const
{
  ...
  NAU_ASSERT(es.ops.onEvent && es.evSet.count(evt.getType()) > 0, "Broken ES events index while handling event '{}' in ES '{}'",
    evt.getName(), es.name);
#endif
  return es;
}
...
void EntityManager::notifyESEventHandlers(EntityId eid, const Event &evt)
{
  ...
  // notifyESEventHandlers is called only on alive entities by contract
  {
    ...
    NAU_ASSERT(idx < entDescs.allocated_size());
    ...
    NAU_ASSERT(entDesc.generation == eid.generation(), "{}: {} == {}", eid.index(), entDesc.generation, eid.generation());
    NAU_ASSERT(archetype < archetypes.size());
  }
  NAU_ASSERT(eventType != EventComponentChanged::staticType());
#endif
  ...
  do
  {
    ...
    if (!isQueryValid(queryId))
    {
      logerr("Currently supporting of 'empty' ES for unicast messages is not available."
             ...
             " Or, likely, you somehow unintentionally made empty ES and it is a bug {{}}",
        esList[esIndex]->name);
      // const EntitySystemDesc &es = getESDescForEvent(esIndex, evt);
...
void EntityManager::sendEventImmediate(EntityId eid, Event &evt)
{
  ...
  {
    logerr("Event {} sent with sendEventImmediate for loading entity {} ({}), and it will be skipped."
...
void EntityManager::validateEventRegistrationInternal(const Event &evt, const char *name)
{
  ...
  {
    logerr("{} event<0x{}> not registered. Flags=0x{} size={}", __FUNCTION__, evt.getType(), evt.getFlags(), evt.getLength());
...
{
  DAECS_EXT_ASSERTF(bool(evt.getFlags() & EVCAST_BROADCAST), "event {} has {} flags but sent as broadcast", evt.getName(),
...
void EntityManager::registerEsEvent(int j)
{
  ...
  NAU_UNUSED(logggedInvalidCoreEvtSub);
#if NAU_DEBUG
  enum
  {
    APPEAR = 1,
  ...
  for (auto evt : es->evSet)
  {
    ...
    {
      logerr("EntitySystem <{}> from <{}> is subscribed for event <0x{}>, which is not registered", es->name, es->getModuleName(),
        evt);
    }
#endif
    ...
    {
#if NAU_DEBUG
      logerr("EntitySystem <{}> from module <{}> with all optional components subscribed to broadcast event <0x{}|{}>", es->name,
  ...
  {
    logerr("EntitySystem <{}> from module <{}> signed for both ComponentsAppear (on_appear) and EntityRecreated events."
  ...
  if (es->getCompSet())
  {
    ...
    else // we explicitly iterate over listed components
    {
      ...
      tokenize_const_string(comps, ", ", [&](eastl::string_view comp_name) {
        ...
        {
          logerr("Entity System <{}> signs for tracking of changing component <%.*s> "
...
{
  NAU_ASSERT(ECS_HASH("name").hash == ecs_hash("name") && ECS_HASH("name").hash == ECS_HASH_SLOW("name").hash);
  esEvents.clear();
...
void EntityManager::validateArchetypeES(archetype_t archetype)
{
  ...
  {
    NAU_ASSERT(archetypesES[list].size() > archetype);
    NAU_UNUSED(list);
    ...
    for (es_index_type esIndex : esListIt->second)
    {
      ...
      if (!isQueryValid(queryId))
      {
        logerr("'empty' ES<{}> for unicast messages of (re)create, destroy doesn't make any sense.", esList[esIndex]->name);
        continue;
      }
      ...
      {
        NAU_ASSERT(!cached, "{}:", esList[esIndex]->name);
        continue;
      }
...
void EntityManager::updateArchetypeESLists(archetype_t archetype)
{
  ...
  for (int l = 0; l < ARCHETYPES_ES_LIST_COUNT; ++l)
    NAU_ASSERT(archListEvents[l].getType() == eventTypes[l]);
#endif
...
const EntityManager::RecreateEsSet *EntityManager::updateRecreatePair(archetype_t old_archetype, archetype_t new_archetype)
{
  ...
  NAU_ASSERT(archRecreateList.find(new_archetype) == archRecreateList.end());
  ...
  for (int l = 0; l < RECREATE_ES_LIST_COUNT; ++l)
  {
    ...
    NAU_ASSERT(shouldApplyToOld == !shouldApplyToNew);
    for (es_index_type esIndex : esListIt->second)
    {
...
uint32_t EntityManager::processEventsExhausted(uint32_t count, T &storage)
{
  ...
  for (; !storage.buffers.empty();)
  {
    ...
    if (!wasEmpty)
    {
      NAU_ASSERT(0, "buffer should be empty in the end of iteration");
      storage.buffers.erase(bi);
    }
  }
  NAU_ASSERT(storage.buffers.empty());
