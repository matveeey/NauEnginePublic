...
static_assert(sizeof(ArchetypesQuery) <= 64);
...
struct JobInfo //-V730
{
  ...
  int quant;
  eastl::fixed_vector<uint32_t, 32, true, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> starts;
...
#else
class alignas(128) ESJob                 // align on 2 cache lines to avoid false sharing on setting 'done' within
                                                      // threadpool
#endif
{
...
public:
  ...
  {
    // NAU_CORE_DEBUG_LF("start job {} us", get_time_usec(baset));
    ...
    for (;;)
    {
      ...
      int totalCount = std::min<int>(totalSize - totalStart, minQuant);
      ...
      // NAU_CORE_DEBUG_LF("{}:we start at {} do {} out of {}",threadId, totalStart, totalCount, query.totalSize);
      ...
      for (; totalStart >= starts[1]; currentChunk++, starts++) // we first need to find chunk where unfinished work starts
        ;
      // NAU_CORE_DEBUG_LF("{}:current chunk is {}, which starts at {} has size of {}",threadId, currentChunk, *starts, starts[1]-starts[0]);

      const Query &__restrict query = info->query;
      NAU_FAST_ASSERT(currentChunk < query.chunksCount());
      ...
      for (int workLeft = totalCount; workLeft > 0;)
      {
        ...
        const int chunkWork = std::min((int)(starts[1]) - totalStart, workLeft);
        // NAU_CORE_DEBUG_LF("{}:perform on chunk {} c_i {} c_size {}",threadId, currentChunk, chunkStart, chunkWork);
        ...
        {
          ++starts;
          ++currentChunk; // will be done anyway on next loop, only needed for NAU_FAST_ASSERT to be correct
  ...
  void doJob() { perform_fun(parent, parent->starts.data(), workerId); }
};

static constexpr int num_jobs_to_wake_up_all = 2; // it is faster to wake up 2 thread workers one-by-one, than wake up all

static __forceinline void parallel_for(int num_jobs, EntityManager &mgr, const query_cb_t &fun, const Query &pQuery, void *user_data, int min_quant /*, threadpool::JobPriority tpprio */)
{
  // NAU_CORE_DEBUG_LF("start working on {:p}[{}] == {}({})", &pQuery, chunk, cnt);
  ...
  num_jobs = std::min(num_jobs, (int)MAX_ES_JOBS);
  // NAU_CORE_DEBUG_LF("num_jobs = {}", num_jobs);
  uint32_t queue_pos = 0;

  auto jobFactory = [&](ESJob* j) -> nau::async::Task<>
  {
      co_await nau::async::Executor::getDefault();
      j->doJob();
  };

  eastl::vector<nau::async::Task<>> tasks;
 ...
 // auto wakeOnAdd = num_jobs <= num_jobs_to_wake_up_all ? threadpool::AddFlags::WakeOnAdd : threadpool::AddFlags::None;
  ...
  //}
  // NAU_CORE_DEBUG_LF("jobs added start is {} ({})",
  ...
  // NAU_CORE_DEBUG_LF("all work scheduled {} done = ({})",
  //   info.work_started.load(std::memory_order_acquire), cnt);
 ...
 //   threadpool::wait(&jobs[i]);
  auto awaiter = [&tasks]() -> nau::async::Task<>
  {
      co_await nau::async::Executor::getDefault();
      co_await nau::async::whenAll(tasks, nau::Expiration::never());
  }();

  nau::async::wait(awaiter);
}

void EntityManager::setMaxUpdateJobs(uint32_t num_jobs) { maxNumJobsSet = std::min(num_jobs, (uint32_t)MAX_ES_JOBS); }

void EntityManager::updateCurrentUpdateMaxJobs()
{
    maxNumJobs = std::min((uint32_t)std::max(int(/*threadpool::get_num_workers()) TODO Uncomment or delete */ 100), int(0)), maxNumJobsSet);
}

bool EntityManager::performMTQuery(const Query &pQuery, const query_cb_t &fun, void *user_data, int min_quant)
{
  const bool isMainThread = nau::getApplication().isMainThread();
  const int numJobs = std::min<int>(maxNumJobs - (isMainThread ? 0 : 1), // actually we want to check if we are running from thread pool
                                                                    // worker thread!
  ...
  ECS_LOG("start for {} jobs {} quant do: {}", numJobs, min_quant, pQuery.totalSize);
...
bool EntityManager::resolvePersistentQueryInternal(uint32_t index)
{
  ...
  DEBUG_VERY_VERBOSE_QUERY("set resolved query <{}> to {}", queryDescs[index].getName(), isResolved(index));
...
EntityManager::ResolvedStatus EntityManager::resolveQuery(uint32_t index, ResolvedStatus currentStatus, ResolvedQueryDesc &resDesc)
{
  ...
  FRAMEMEM_REGION;
  DEBUG_VERY_VERBOSE_QUERY("try resolve query <{}> {}, resolve = {}", name,
  ...
  auto makeRange = [&](nau::ConstSpan<ComponentDesc> components, ReqType req) -> bool {
    auto &dest = resDesc.getComponents();
    for (auto cdI = components.begin(), cdE = components.end(); cdI != cdE; ++cdI, ++componentsIterated)
    {
      DAECS_EXT_ASSERTF(componentsIterated < dest.size(), "{}: {} < {}", req, componentsIterated, dest.size());
      ...
      DEBUG_VERY_VERBOSE_QUERY("query <{}>, component 0x{}, type 0x{}", name, cd.name, cd.type);

      if (id == INVALID_COMPONENT_INDEX)
      {
          if((req == ReqType::REQUIRED_NOT) || (cd.flags & CDF_OPTIONAL))  // optional component can be even not registered at all
        {
          ret &= ~FULLY_RESOLVED;
          continue;
        }
      ...
      // verify type
      if(cd.type != ComponentTypeInfo<auto_type>::type && ((NAU_DEBUG != 0) || req == ReqType::DATA))  //-V560
      {
        ...
        {
          logerr("component<{}> type mismatch in query <{}>, type is {}(%0xX), required in query <{}>(0x{})",
...
#if NAU_DEBUG
      if(req == ReqType::DATA && !(cd.flags & CDF_OPTIONAL) &&
          componentTypes.getTypeInfo(dataComponents.getComponentById(id).componentType).size == 0)
      {
        logerr("component<{}> in query <{}>, is requested as data (ro/rw), but it has zero size",
  ...
  if (!wasResolved)
  {
    ...
    const uint32_t checkComponents = NAU_DEBUG != 0 ? totalComponents : dataComponentsCount;
  ...
  else
  {
    ...
    DAECS_EXT_ASSERTF(resDesc.getRwCnt() == desc.componentsRW.size() && resDesc.getRoCnt() == desc.componentsRO.size() &&
      ...
      "rw {}=={} ro {}=={} rq {}=={}, no {}=={}, req_set {}=={}", resDesc.getRwCnt(), desc.componentsRW.size(), resDesc.getRoCnt(),
  ...
  if(!makeRange(nau::ConstSpan<ComponentDesc>(copyDesc.components.data(), resDesc.getRwCnt() + resDesc.getRoCnt()), ReqType::DATA))
  ...
  {
      if(!makeRange(desc.componentsRQ, ReqType::REQUIRED))
      return (ResolvedStatus)ret;
  }
  if(!makeRange(desc.componentsNO, ReqType::REQUIRED_NOT))
    return (ResolvedStatus)ret;
  DEBUG_VERY_VERBOSE_QUERY("resolved query <{}>, resolve={}", name, currentStatus);
  return (ResolvedStatus)ret;
}
...
static T *add_to_fixed_container(T *ft, Cnt &__restrict cnt, const T *__restrict add, size_t size)
{
  ...
  if(!ft || (nullptr == nullptr) /* memresizeinplace_default(ft, nextSize * sizeof(T))  TODO Allocators Uncomment or delete*/)
  {
    T *next = (T *)realloc(ft, nextSize * sizeof(T));//memrealloc_default TODO Allocators
    if (EASTL_UNLIKELY(next == nullptr))
    {
      next = (T *)malloc(nextSize * sizeof(T)); // so no mem setting //memalloc_default TODO Allocators
      memcpy(next, ft, cnt * sizeof(T));                  //-V575
      free(ft);//memfree_default TODO Allocators
    }
    eastl::swap(ft, next);
  }
                                                                                                            ...
                                                                                                            // componentsCountChanged
{
  ...
  if (resDesc.isEmpty())
  {
    DEBUG_VERY_VERBOSE_QUERY("skipping empty query {}", index);
    return false;
  }
  ArchetypesQuery &query = archetypeQueries[index];
  ArchetypesEidQuery &queryEid = archetypeEidQueries[index];
  if (EASTL_UNLIKELY(!wasFullyResolved))
  {
    const uint32_t totalDataComponentsCount = resDesc.getRwCnt() + resDesc.getRoCnt();
    DEBUG_VERBOSE_QUERY("copying types and sizes to archetype query {} rw = {} ro ={}", index, resDesc.rw.cnt, resDesc.ro.cnt);
    ...
    if (!validCsz) // todo : only needed for eid queries. Can be deferred explicitly marked, if eid query is needed at all
    {
      if (totalDataComponentsCount)
      {
        queryEid.componentsSizesAt = archComponentsSizeContainers.size();
          archComponentsSizeContainers.resize(archComponentsSizeContainers.size() + totalDataComponentsCount, {});
      }
    }
    if (reResolve)
    {
  ...
  else
  {
    ...
    DAECS_EXT_ASSERTF(resDesc.getRW().start == 0 && query.rwCount == resDesc.getRW().cnt && query.roCount == resDesc.getRO().cnt,
      "q {} rw {} == {} ro {} == {}, tp = %p", index, query.rwCount, resDesc.getRW().cnt, query.roCount, resDesc.getRO().cnt);
    DAECS_EXT_ASSERT(
      totalDataComponentsCount == 0 || queryEid.componentsSizesAt + totalDataComponentsCount <= archComponentsSizeContainers.size());
    auto componentCsz = archComponentsSizeContainers.data() + queryEid.componentsSizesAt;
  ...
  FRAMEMEM_REGION;
  // NAU_ASSERT_RETURN(resDesc.components.size() == 0, false);//not sure if it is needed
  DAECS_EXT_ASSERTF_RETURN(resDesc.getRO().start == resDesc.getRW().start + resDesc.getRW().cnt, false, "{} {} {}",
  ...
  eastl::fixed_vector<archetype_t, 4, true, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> queries;
  eastl::fixed_vector<ArchetypesQuery::offset_type_t, 16, true, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> allComponentsArchOffsets;

  DEBUG_VERBOSE_QUERY("adding {} archetypes to query {}", archetypesCount - first_archetype,
  ...
  archetype_t firstArchetypeToAdd =
    !updateOneArchetype && query.queriesCount > 0 ? std::max(archetype_t(query.queriesEnd()[-1] + 1), first_archetype) : first_archetype;
  ...
  // NAU_ASSERT(query.componentsCount == totalDataComponentsCount || !query.componentsType);

  if (!queries.size())
    return query.getArchetypesRelated() != 0;
  DAECS_EXT_ASSERTF_RETURN(totalDataComponentsCount == resDesc.getRO().cnt + resDesc.getRW().cnt, false, "{}",
  ...
  NAU_ASSERT(query.getQueriesCount() < eastl::numeric_limits<archetype_t>::max());
  ...
  archSubQueriesContainer.resize(archSubQueriesContainer.size() + newSubQueriesCount, {});
  ...
  {
      eastl::fixed_vector<eastl::pair<component_index_t, component_index_t>, 8, true, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> trackedRw;
    ...
    if (trackedRw.size())
    {
        eastl::fixed_vector<ScheduledArchetypeComponentTrack, 8, true, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> trackedChanges;
...
bool EntityManager::makeArchetypesQueryInternal(archetype_t last_arch_count, uint32_t index, bool should_re_resolve)
{
  ...
  if (EASTL_UNLIKELY(!isFullyResolved(status)))
  {
    DEBUG_VERBOSE_QUERY("updating = {} {}, as not fully resolved", queryDescs[index].getName(), index);
    if (should_re_resolve)
    {
      if (!resolvePersistentQueryInternal(index))
      {
        DEBUG_VERBOSE_QUERY("update failed = {}", queryDescs[index].getName());
        return false;
      }
    }
...
inline EntityManager::query_components_hash EntityManager::query_components_hash_calc(const BaseQueryDesc &desc)
{
  ...
  auto hashComponents = [&componentsHash](nau::ConstSpan<ComponentDesc> list) {
...
inline void EntityManager::CopyQueryDesc::init(const EntityManager &mgr, const char *name_, const BaseQueryDesc &d)
{
  ...
  {
      logerr("query <{}> has too many components (rw={},ro={},rq={},no={}) max is {}", name_, rwCnt, roCnt, rqCnt, noCnt, max_cnt);
  }
...
#endif
  typedef eastl::fixed_vector<component_t, 64, true, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> fixed_vector_components_t;
  eastl::vector_set<component_t, eastl::less<component_t>, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType, fixed_vector_components_t> requiredComponentsSet;
  for (const auto &cd : d.componentsRQ)
  {
    if (cd.flags & CDF_OPTIONAL)
      {
          logerr("query <{}> doesn't make sense as component {}(0x{}) is both required and optional", name_, getNameStr(cd), cd.name);
      }
  ...
  for (const auto &cd : d.componentsNO)
  {
    if (cd.flags & CDF_OPTIONAL)
      {
          logerr("query <{}> doesn't make sense as component {}(0x{}) is both required_not and optional", name_, getNameStr(cd), cd.name);
      }
    if (requiredComponentsSet.find(cd.name) != requiredComponentsSet.end())
      {
          logerr("query <{}> doesn't make sense as component {}(0x{}) is both in required and required_not lists", name_, getNameStr(cd),
                 cd.name);
      }
  }
  ...
  {
      logerr("too many optional data components in query <{}>. To fix, increase bitset size or add first-optional-component-no", name_);
  }
...
{
  DAECS_EXT_ASSERT(!isConstrainedMTMode());
  NAU_ASSERT_RETURN(desc.componentsRW.size() + desc.componentsRO.size() < max_query_components_count, QueryId());
...
#if NAU_DEBUG
    auto debugComponent = [&](const char *m, nau::ConstSpan<ComponentDesc> cs) {
        for(auto& c : cs)
        {
            ECS_LOG("{} type (0x{}|{}) 0x{}|{}", m, c.type, componentTypes.findTypeName(c.type), c.name, c.nameStr);
        }
    };
...
#if NAU_DEBUG
    NAU_ASSERT(desc == queryDescs[index].getDesc(), "query {} hashed aliased with different query {}!", desc.name,
...
void EntityManager::destroyQuery(QueryId h)
{
  DAECS_EXT_ASSERT(!isConstrainedMTMode());
  convertArchetypeScheduledChanges();
  NAU_ASSERT_RETURN(isQueryValid(h), );
  auto idx = h.index();
  NAU_ASSERT_RETURN(queriesReferences[idx] > 0, );
  if (!(--queriesReferences[idx])) // we don't reset queries, so we can resurrect 'dead' queries (with new generation)
  {
    ...
    if (EASTL_UNLIKELY(idx == queriesReferences.size() - 1))
    {
...
QueryCbResult EntityManager::performQueryStoppable(QueryId h, const stoppable_query_cb_t &fun, void *user_data)
{
  ...
  {
    DEBUG_VERY_VERBOSE_QUERY("no fit <{}> out of {} archs. {} total", queryDescs[index].getName(), archDesc.getArchetypesRelated(),
      archetypes.size());
    return QueryCbResult::Continue;
  }
...
struct DataComponentsManagerAccess
{
    ...
    uint32_t idInChunk, const uint16_t *__restrict componentsSize)
  {
    ...
    {
      // NAU_CORE_DEBUG_LF("comp {} offs = {}", compI, archetypeOffsets[compI]);
  ...
  uint32_t trackedChangesCount, EntityId eid, uint32_t archetype)
{
  ...
  ScopedMTMutexT<nau::threading::SpinLock> lock(isConstrainedMTMode(), eidTrackingMutex);
  do
  {
...
void EntityManager::defragmentArchetypeSubQueries()
{
  ...
  DAECS_EXT_ASSERTF(archSubQueriesWasted <= archSubQueriesContainer.size(), "{} <= {}", archSubQueriesWasted,
...
void EntityManager::validateQuery(uint32_t index)
{
  ...
  {
    DEBUG_VERY_VERBOSE_QUERY("validate query <{}> {}", queryDescs[index].getName(), index);
...
#if NAU_DEBUG && VALIDATE_QUERY_THOROUGHLY // very expensive check
    {
      ...
      if (isResolved(status))
      {
        ...
        {
          NAU_ASSERT(archQuery.getQueriesCount() == 0, "{} queries={}", archQuery.getQueriesCount());
        }
        NAU_ASSERT(archQuery.getComponentsCount() == archDesc.getComponentsCount());
        NAU_ASSERT(archQuery.getQueriesCount() == archDesc.getQueriesCount());
        NAU_ASSERT(archQuery.getAllComponentsArchOffsetsCount() == archDesc.getAllComponentsArchOffsetsCount());
        if (archQuery.isInplaceOffsets())
        {
          NAU_ASSERT(memcmp(archQuery.getArchetypeOffsetsPtrInplace(), archDesc.getArchetypeOffsetsPtrInplace(),
                     archDesc.getAllComponentsArchOffsetsCount() * sizeof(ArchetypesQuery::offset_type_t)) == 0);
        }
        else
        {
          NAU_ASSERT(memcmp(archQuery.allComponentsArchOffsets, archDesc.allComponentsArchOffsets,
                     archDesc.getAllComponentsArchOffsetsCount() * sizeof(ArchetypesQuery::offset_type_t)) == 0);
        }
              ...
              sizeof(*archQuery.queriesBegin()) * (archQuery.queriesEnd() - archQuery.queriesBegin())) != 0)
        {
          ...
          {
            NAU_CORE_DEBUG_LF("{}: archetype {} | should be arch = {}", i, archDesc.queriesBegin()[i], archQuery.queriesBegin()[i]);
          }
          NAU_ASSERT(false, "broken {} queries={}", queryDescs[index].getName(), archQuery.getQueriesCount());
        }
        NAU_ASSERT(archQuery.trackedChangesCount == archDesc.trackedChangesCount);
              ...
              sizeof(*archDesc.trackedBegin()) * (archQuery.trackedEnd() - archQuery.trackedBegin())) != 0)
        {
          ...
          {
            NAU_CORE_DEBUG_LF("{}: archetype {}, cidx = {} | should be arch = {}, cidx = {}", i, archDesc.trackedBegin()[i].archetype,
              archDesc.trackedBegin()[i].cidx, archQuery.trackedBegin()[i].archetype, archQuery.trackedBegin()[i].cidx);
          }
          NAU_ASSERT(false, "broken {} changes={}", queryDescs[index].getName(), archQuery.trackedChangesCount);
        }
      }
      else
      {
        NAU_ASSERT(!archDesc.getQueriesCount(), "query = {} arch Resolved={}", queryDescs[index].getName(), (int)status);
      }
    }
#endif
  }
...
template <>
struct fmt::formatter<ecs::ReqType, char> : fmt::formatter<const char*, char>
{
    auto format(ecs::ReqType type, fmt::buffered_context<char>& ctx) const
    {
        switch(type)
        {
            case ecs::ReqType::DATA:
                return formatter<const char*, char>::format("DATA", ctx);
                break;
            case ecs::ReqType::REQUIRED:
                return formatter<const char*, char>::format("REQUIRED", ctx);
                break;
            case ecs::ReqType::REQUIRED_NOT:
                return formatter<const char*, char>::format("REQUIRED_NOT", ctx);
                break;
        };
        return formatter<const char*, char>::format("unknown ReqType", ctx);
    }
};