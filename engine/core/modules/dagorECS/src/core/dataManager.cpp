...
{
  static_assert(sizeof(DataComponentManager) <= 28);
#if NAU_DEBUG
  ...
  {
    logerr("chunks = {}, cached totalEntitiesUsed = {} != {} totalEntitiesCapacity = {} != {}", getChunksCount(), totalEntitiesUsed,
      countUsed, totalEntitiesCapacity, countCapacity);
    auto chunks = getChunksConst();
    for(int i = 0, e = chunks.size(); i < e; ++i)
    {
        logerr("{}:chunk = {} {}", i, chunks[i].getUsed(), chunks[i].getCapacity());
    }
  ...
  const uint32_t components_cnt, const uint32_t entity_size, uint32_t chunks_to_defragment)
{
  ...
  for (size_t ci = 0, ce = chunks.size(); ci < ce; ci++)
  {
    ...
    {
      const uint32_t capacityBits = eastl::clamp(nau::math::get_bigger_log2(chunk.getUsed()), (uint32_t)initialBits, (uint32_t)MAX_CAPACITY_BITS);
      if (capacityBits == chunk.getCapacityBits())
        continue;
      TIME_PROFILE(ecs_gc_shrink_to_fit);
      currentCapacityBits = eastl::clamp(nau::math::get_bigger_log2(totalEntitiesUsed), (uint32_t)initialBits, (uint32_t)MAX_CAPACITY_BITS);
  ...
  chunk_type_t &moved_to_chunk, uint32_t &moved_at_id, uint32_t &moved_cnt)
{
  ...
  {
    NAU_ASSERT(getChunk(i).getUsed() <= getChunk(i).getCapacity(), "chunk {} used {} cap {}<={}", i, getChunk(i).getUsed(),
  ...
  for (size_t e = getChunksCount() - 1; e > 0; --e)
    if (getChunkUsed(e) == 0)
    {
      ...
      break;
  currentCapacityBits =
    eastl::clamp(nau::math::get_log2i(getChunksConst().back().getCapacity()), (uint32_t)initialBits, (uint32_t)MAX_CAPACITY_BITS);
  verifyCachedData();
  workingChunk = 0;
  ...
  {
    moved_to_chunk = allocateChunk(entity_size, nau::math::get_bigger_log2(usedInNonMaxBlock));
    // NAU_CORE_DEBUG_FI_LF("moved_to_chunk = {}", moved_to_chunk);
  }
  ...
  {
    logmessage(NAU_DEBUG > 0 ? ::nau::diag::LogLevel::Warning : ::nau::diag::LogLevel::Error,
      "internal error moved_to_chunk == {} moved_from_chunk = {} minUsed={} maxFree={} minUsedChunk={}"
      " maxCapacityChunk={} (free = {}) usedInNonMaxBlock={} nonMaxBlocks={} maxCapacity={}",
  ...
  NAU_ASSERT(entity_size == entitySize);
#endif
  ...
  NAU_ASSERT(toChunk.getFree() >= srcEntitiesCnt, "{}:srcEntitiesCnt = {} moving to {}:free {}", moved_from_chunk, srcEntitiesCnt,
