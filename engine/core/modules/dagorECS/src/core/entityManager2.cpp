...
InitOnDemand<ecs::EntityManager, false> g_entity_mgr;
  ...
  const create_entity_async_cb_t &creation_cb)
{
  ...
  for (InitializerNode &init : initializer)
  {
    ...
    if (componentTypes.getTypeInfo(typeId).flags & COMPONENT_TYPE_NON_TRIVIAL_CREATE)
    {
      ...
      NAU_ASSERT_CONTINUE(typeManager);
  ...
  create_entity_async_cb_t &&creation_cb)
{
  ...
  NAU_FAST_ASSERT(templId != INVALID_TEMPLATE_INDEX); // couldn't happen
  EntityCreationProfiler entCreatProf(templId, *this);
  DAECS_EXT_ASSERTF(doesEntityExist(eid), "Attempt to use destroyed/not allocated eid handle {} {}", (ecs::entity_id_t)eid,
    entDescs.size());
  ...
  if (oldArchetype != INVALID_ARCHETYPE)
  {
    ...
    NAU_ASSERT(oldArchetype == entDescs[eid.index()].archetype,
      "we don't allow sync recreate entity from within ES on EventComponentsDisappear ES for eid {}", (ecs::entity_id_t)eid);
  }
  ...
  if (isRecreating && isQueryingArchetype(oldArchetype))
  {
    ...
    logerr("We don't support sync re-creating of entities within queries. Create <{}> was <{}>", getTemplateName(templId),
      getEntityTemplateName(eid));
  }
  // auto it = find_loading_entity(eid);
  // NAU_ASSERT(it==loadingEntities.end() || it->eid != eid,
  ...
  if (isRecreating)
  {
    ...
    {
      logerr("Attempt to recreate Entity on same archetype too often <{}>", getTemplateName(templId));
  ...
  *(EntityId *__restrict)(chunkData + idInChunk * sizeof(EntityId)) = eid;
  ...
  if (isRecreating) // migrate from old template
  {
    ...
    if (initializerEmpty)
    {
      ...
      {
        DAECS_EXT_ASSERTF(compStream >= compStreamBegin && oldCompStream >= oldCompStreamBegin, "compStream = {} oldCompStream = {}",
      ...
      for (auto &component : archetypes.getCreatableTrackeds(newArchetype))
      {
        ...
        else
        {
          ...
          if (!isRecreating) // sanity check
            {
                logerr("creating already initialized copy. Replication code is invalid for (copy = {}) component", component.originalCidx);
            }
#endif
  ...
  for (auto initIt = comp_init.end() - 1, initEnd = comp_init.begin() - 1; initIt != initEnd; --initIt)
  {
    if (initIt->cIndex == INVALID_COMPONENT_INDEX)
    {
      ...
      if (EASTL_UNLIKELY(initializerIndex == INVALID_COMPONENT_INDEX))
      {
        ...
        {
            logwarn("<0x{}> component is not known in template <{}>", initIt->name, getTemplateName(templId));
        }
#endif
        continue;
      }
      ...
      {
        logerr("initializer <{}|0x{}> type mismatch {} initializer, <{}> in template <{}>",
    ...
    else
    {
      ...
      DAECS_EXT_ASSERTF(initIt->cIndex == dataComponents.findComponentId(initIt->name),
        "componentIndex {} for <{}|0x{}> is incorrect, should be {}", initIt->cIndex,
    ...
    if (EASTL_UNLIKELY(archetypes.getArchetypeComponentIdUnsafe(archetype, initIt->cIndex) == INVALID_ARCHETYPE_COMPONENT_ID))
    {
      ...
      {
          logerr("<{}|0x{}> component is filtered out in template <{}>, misbevaior is possible on replication",
...
RequestResources EntityManager::requestResources(EntityId eid, archetype_t oldArchetype, template_t templId,
  const ComponentsInitializer &initializer, RequestResourcesType type)
{
  NAU_UNUSED(oldArchetype);
  ...
  if (!(archetypes.getArchetypeCombinedTypeFlags(archetype) & COMPONENT_TYPE_NEED_RESOURCES))
    return RequestResources::Loaded;
  ...
  ResourceRequestCb rcb(*this, eid);
  ...
  if (rcb.requestedResources.empty())
    ...
    if(!getECSResourceManager()->filter_out_loaded_gameres(rcb.requestedResources))
    {
        return RequestResources::Loaded;
    }
    else
    {
        ...
        {
            return getECSResourceManager()->load_gameres_list(eastl::move(rcb.requestedResources)) ? RequestResources::Loaded : RequestResources::Error;
        }
    }
}
...
EntityId EntityManager::createEntitySync(template_t templId, ComponentsInitializer &&initializer, ComponentsMap &&map)
{
  ...
  if (EASTL_UNLIKELY(result != RequestResources::Loaded))
  {
    ...
    if (result == RequestResources::Loaded)
    {
      ...
      // order)
      logmessage(getTemplateDB().info().filterTags.count(ECS_HASH(ASYNC_RESOURCES).hash) ? ::nau::diag::LogLevel::Error : ::nau::diag::LogLevel::Warning, 
          "sync creation of entity {} eid ({}) with resources", eid, getTemplateName(templId));
    }
#endif
    if (result == RequestResources::Error)
    {
      logerr("sync creation of entity {} eid ({}) failed, as some resources are missing", eid, getTemplateName(templId));
...
template_t EntityManager::instantiateTemplate(int id)
{
  ...
  {
    logerr("Entity of template <{}> can not be created as it has missing dependencies", templ.getName());
    templ.reportInvalidDependencies(getTemplateDB());
    return INVALID_TEMPLATE_INDEX;
  }
  DA_PROFILE_TAG(template, templ.getName());
  FRAMEMEM_REGION;
  eastl::vector_set<component_index_t, eastl::less<component_index_t>, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> templComponentsIndices;  // we better sort
                                                                                                                   // after that, but
                                                                                                                   // in pairs
  eastl::vector_set<component_index_t, eastl::less<component_index_t>, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> templReplicatedIndices;  // we better sort
                                                                                                                   // after that, but
                                                                                                                   // in pairs
  eastl::vector<const ChildComponent*, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> templComponentsData;
  ...
  templComponentsData.push_back(nullptr);
  {
    ...
    getTemplateDB().iterate_template_parents(templ, [&](const Template &p) {
      for (auto templateIt = p.getComponentsMap().begin(), e = p.getComponentsMap().end(); templateIt != e; ++templateIt)
      {
        ...
        if (EASTL_LIKELY(comp != INVALID_COMPONENT_INDEX))
        {
          ...
          {
            logerr("Component {}|0x{} in template {} has incorrect type of 0x{} (registered with <{}|0x{}>)",
        ...
        else
        {
          ...
          nau::Span<component_t> dependencies{};  // todo:actually we can create component dependencies on the fly
          ...
          comp = createComponent(hName, typeIdx, nau::Span<component_t>(), nullptr, flag);
        ...
        if (componentTypes.getTypeInfo(typeIdx).size != 0) // do not process tags
        {
          ...
          if (tempFlags)
          {
            ...
            component_index_t compCopy = createComponent(HashedConstString{nullptr, compHash}, typeIdx, nau::Span<component_t>(), nullptr,
  ...
  NAU_UNUSED(oldArchetypesCapacity); // only used in dev mode
...
#if NAU_DEBUG
  logwarn("template {} <{}> has archetype {}", ret, templ.getName(), templates.getTemplate(ret).archetype);
#endif
...
template_t EntityManager::templateByName(const char *templ_name, EntityId eid)
{
  ...
  if (EASTL_UNLIKELY(id == -1))
  {
    logerr("template <{}> is not in database", templ_name);
    return INVALID_TEMPLATE_INDEX;
  }
  auto templId = getTemplateDB().instantiatedTemplates[id];
  if (EASTL_UNLIKELY(templId.singleton))
  {
    const EntityId singletonEid = hasSingletoneEntity(templ_name);
    if (singletonEid != eid && singletonEid != INVALID_ENTITY_ID)
    {
      logerr("Singleton <{}> has been already created", templ_name);
      return INVALID_TEMPLATE_INDEX;
    }
  }
  if (EASTL_LIKELY(templId.t != INVALID_TEMPLATE_INDEX))
    return templId.t;
...
EntityId EntityManager::createEntitySync(const char *templ_name, ComponentsInitializer &&initializer, ComponentsMap &&map)
{
  ...
  {
#if NAU_DEBUG
      NAU_ENSURE_LOG(nau::diag::LogLevel::Error, u8"sync (re)creation is not valid in constrainedMt mode. Create <{}>", nau::string(templ_name));
#endif
...
EntityId EntityManager::createEntityAsync(template_t templ_id, ComponentsInitializer &&initializer, ComponentsMap &&map,
  create_entity_async_cb_t &&cb)
{
  ScopedMTMutex lock(isConstrainedMTMode(), creationMutex);
  EntityId eid = allocateOneEidDelayed(isConstrainedMTMode());
  emplaceCreate(eid, DelayedEntityCreation::Op::Create, templ_id, eastl::move(initializer), eastl::move(map), eastl::move(cb));
  return eid;
  // return createEntityAsync(templateByName(templ_name, EntityId()), eastl::move(initializer), eastl::move(map), eastl::move(cb));
}
...
void EntityManager::forceServerEidGeneration(EntityId from_eid)
{
  ...
  if (EASTL_UNLIKELY(entDescs[ei].archetype != INVALID_ARCHETYPE)) // already allocated (this is considered erroneous code path)
  {
    ...
    logerr("{} attempt to allocate eid {} while slot already occupied with {}<{}>. Did client created replicating entity?",
      ...
      if (EASTL_UNLIKELY(cr->eid == eid))
      {
        ...
        else if (!cr->isToDestroy())
        {
          ...
          // template names in re-creates might be too long (hence use id)
            ECS_LOG("{} {} templ {} -> {}", __FUNCTION__, eid, cr->templateName.c_str(), template_name);
#endif
...
EntityId EntityManager::allocateOneEid()
{
  ...
  if (findices.size() <= MINIMUM_FREE_INDICES)
  {
    ...
    {
      NAU_ENSURE_LOG(nau::diag::LogLevel::Error, u8"Exhausted reserved eids indexes range [1..{}]", MAX_RESERVED_EID_IDX_CONST);
  ...
  alloc_free_indices:
    ...
    NAU_FAST_ASSERT(idx < entDescs.size());
    findices.pop_front();
  }
  DAECS_EXT_ASSERTF(reserved ? (idx <= MAX_RESERVED_EID_IDX_CONST) : (idx > MAX_RESERVED_EID_IDX_CONST), "reserved {}, idx={}",
    reserved, idx);
  NAU_FAST_ASSERT(entDescs[idx].archetype == INVALID_ARCHETYPE);
...
EntityId EntityManager::allocateOneEidDelayed(bool delayed)
{
  ...
  alloc_free_indices:
    idx = findices.front() & ENTITY_INDEX_MASK;
    NAU_FAST_ASSERT(idx < entDescs.allocated_size());
    findices.pop_front();
  }
  DAECS_EXT_ASSERTF(reserved ? (idx <= MAX_RESERVED_EID_IDX_CONST) : (idx > MAX_RESERVED_EID_IDX_CONST), "reserved {}, idx={}",
    reserved, idx);
...
EntityManager::EntityManager()
{
  ...
  initializeCreationQueue();
  {
    ...
    NAU_UNUSED(q);
    NAU_ASSERT(ArchetypesQuery::offset_type_t(q + 1) == 0, "this assumption is used in fillQuery");
  }
  ...
  for (int i = 0; i < componentTypes.getTypeCount(); ++i)
    biggestType = std::max(biggestType, (uint32_t)componentTypes.getTypeInfo(i).size);
  zeroMem.reset(new uint8_t[(biggestType + 15) & ~15]);
  ECS_LOG("biggest type size is {}, zeroMem %p", biggestType, (void*)zeroMem.get());
  initCompileTimeQueries();
  resetEsOrder();
  setMaxUpdateJobs(100);
...
EntityManager::EntityManager(const EntityManager &from)
{
  ...
  eastl::for_each(dataComponents.componentToLT.begin(), dataComponents.componentToLT.end(),
                  [](auto& component)
                  {
                      component.second->info.cidx = INVALID_COMPONENT_INDEX;
                  });
...
void EntityManager::clear()
{
  ...
  if (clearAttempts == 0)
  {
    logerr("infinite recursion during destruction of ECS, spend {} attempts, {} deferred Events left", max_destroy_attempts,
      deferredEventsCount);
  ...
  if ((int)std::min(minGen, entDescs.globalGen) > 255 - (int)std::max(entDescs.globalGen, maxGen))
...
EntityManager::~EntityManager()
{
  ...
  clear();
  ECS_LOG("EntityManager destroyed");
}
...
inline void EntityManager::destroyEntityImmediate(EntityId eid)
{
  ...
  if (findices)
  {
    ...
    for (auto it = findices->begin(), ite = findices->end(); it != ite; ++it)
      NAU_ASSERT(((*it) & ENTITY_INDEX_MASK) != idx, "eid {}<{}> index {} is already in freeIndices (occupied with {})",
...
void EntityManager::flushGameResRequests()
{
  ...
  getECSResourceManager()->async_load_gameres_list(eastl::move(tmpResEntities), eastl::move(tmpReqRes));
}

void EntityManager::onEntitiesLoaded(nau::ConstSpan<EntityId> ents, bool all_ok)
{
  if (!ents.size())
    return;
  if (!all_ok)
                                                                                                               ...
                                                                                                               // loadingEntities
  {
    ...
    if (it == loadingEntities.end())
    {
      ...
      logwarn("{} eid <{}> not exist in loadingEntities", __FUNCTION__, (ecs::entity_id_t)eid);
      continue;
    }
    if (--it->second == 0)
...
void EntityManager::performDeferredEvents(bool flush_all)
{
  ...
  unsigned minimum_to_perform = std::max((int)average_tick_events_uint, (int)minEventsPerTick);
  ...
  const ComponentsInitializer &initializer)
{
  ...
  if (result == RequestResources::Error)
  {
    logerr("sync creation of entity {} eid to ({}) is not possible, as some resources are missing", eid, getTemplateName(templ));
    return false;
  }
  return true;
}
...
bool EntityManager::createQueuedEntitiesOOL()
{
  ...
  eastl::fixed_vector<Destroy, 32, true, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> destroyNow;  // use sorting queue, so we reduce amount of memory movements
  auto destroyQueued = [&destroyNow, this]() {
    if (destroyNow.empty())
      return;
    eastl::sort(destroyNow.begin(), destroyNow.end(), [](auto &a, auto &b) { return a.sortBy > b.sortBy; });
  ...
  do
  {
    ...
    for (auto it = chunkBegin; it != chunkEnd; ++it)
    {
      ...
      // NAU_CORE_DEBUG_LF("eid {} idx {}, isCreating {} {}",
      ...
      else
      {
        if (exist)
        {
          ...
          if (ce.templ == INVALID_TEMPLATE_INDEX)
          {
            ...
            {
              logerr("Entity %@ with template {}, could not be created, {}", ce.eid, ce.templateName.c_str(),
            ...
            {
              DAECS_EXT_ASSERT(result == RequestResources::Loaded);
          ...
          {
              if(find_loading_entity(eid) != loadingEntities.end())
              {
                  if(!firstLoading)
                      firstLoading = &ce;
                  continue;
        ...
        DAECS_EXT_ASSERTF(entDescs.isCurrentlyCreating(eid.index()), "{}", eid);
...
inline double profile_usec(auto& start)
{
    return (eastl::chrono::duration_cast<eastl::chrono::duration<double>>(eastl::chrono::system_clock::now() - start)).count();
}
inline bool profile_usec_passed(auto& start, double t)
{
    return profile_usec(start) > t;
}
void EntityManager::defragmentArchetypes()
{
  static_assert(sizeof(Archetype) <= 32);
  NAU_ASSERT_RETURN(!isConstrainedMTMode(), );
  const uint32_t archetypesCount = archetypes.size();
  if (!archetypesCount)
    return;
  TIME_PROFILE(ecs_gc);
  auto reft = eastl::chrono::system_clock::now();
  ...
  if (defrag == DataComponentManager::DATA_MOVED)
  {
    ...
    NAU_ASSERT(movedTo < archetypes.getArchetype(defragmentArchetypeId).manager.getChunksCount(), "{}<{}", movedTo,
    ...
    NAU_ASSERT(chunk.getUsed() == movedAt + movedCount, "chunk.getUsed() = {}, moved = {} + {}", chunk.getUsed(), movedAt, movedCount);
  ...
  else if (defrag == DataComponentManager::CHUNK_REMOVE)
  {
    ...
    for (uint32_t chunkI = movedFrom + 1, chunkE = archetype.manager.getChunksCount(); chunkI < chunkE; ++chunkI)
    {
      ...
      if (!chunkUsed)
        continue;
      NAU_ASSERT(chunk.data && chunk.getUsed() <= chunk.getCapacity(), "{:p} {} {}", (void*)chunk.data, chunk.getUsed(), chunk.getCapacity());
           ...
           eid != eidE; ++eid)
      {
        ...
        DAECS_EXT_ASSERTF(entDesc.idInChunk == eid - (EntityId *)chunk.getCompDataUnsafe(0) && entDesc.chunkId == chunkI,
          "eid {}: entDesc.idInChunk = {} entDesc.chunkId = {} should be id = {} chunk= {}, generation {} == {}", entity_id_t(*eid),
          entDesc.idInChunk, entDesc.chunkId, eid - (EntityId *)chunk.getCompDataUnsafe(0), chunkI, eid->generation(),
          entDesc.generation);
        DAECS_EXT_ASSERTF(eid->generation() == entDesc.generation, "eid {}: generation {} != {}", entity_id_t(*eid), eid->generation(),
          entDesc.generation);
        DAECS_EXT_ASSERTF(entDesc.archetype == defragmentArchetypeId, "archetype = {} should be {}", entDesc.archetype,
  ...
  {
      ECS_LOG("defrag of archetype = {} took {} s", defragmentArchetypeId, profile_usec(reft));
  }
#endif
}
#if DAGOR_DBGLEVEL == 0
...
void EntityManager::tick(bool flush_all)
{
  ...
  if (nestedQuery != 0)
  {
    logerr("tick within query/es isn't allowed, or nestedQuery={} leaked", nestedQuery);
    nestedQuery = 0;
  }
...
void EntityManager::set(EntityId eid, const HashedConstString name, const char* v)
{
    ...
    {
        NAU_ASSERT(0, "component {}|0x{} is not a string or missing", name.str, name.hash);
    }
}
