...
{
  DEBUG_VERY_VERBOSE_QUERY("updatePersistent = {}, index = {}", queryDescs[index].name.c_str(), index);
  DAECS_EXT_ASSERT(!isConstrainedMTMode());
  DEBUG_VERBOSE_QUERY("update = {}, current resolved= {}", queryDescs[index].name.c_str(), getQueryStatus(index));
