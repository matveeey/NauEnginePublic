...
{
  NAU_FAST_ASSERT(++rec_depth < 1024);
  NAU_UNUSED(rec_depth); // max recursion depth (sanity check)
...
void EntityManager::updateEntitiesWithTemplate(template_t oldT, template_t newTemp, bool update_templ_values)
{
  ...
  if (update_templ_values)
  {
    ...
    {
      changedComponents.push_back(EntityComponentRef());
      NAU_ASSERT(oldCid + 1 == changedComponents.size());
  ...
  if (oldArchetype != newArchetype || update_templ_values)
  {
    ...
    // NAU_CORE_DEBUG_LF("entities {}", archetypes.getArchetype(oldArchetype).manager.getTotalEntities());
        ...
        for (int ei = manager->getChunkUsed(ci) - 1; ei >= 0; --ei)
        {
          ...
          if (changedTemplValues)
          {
            for (uint32_t ai = 1, ac = archetypes.getComponentsCount(oldArchetype); ai < ac; ++ai)
            {
              ...
              init[HashedConstString{nullptr, dataComponents.getComponentTpById(cIndex)}] = ChildComponent(changedComponents[ai]);
...
TemplateDB::AddResult EntityManager::updateTemplate(Template &&update_templ, nau::ConstSpan<const char *> *pnames,
  bool update_templ_values)
{
  ...
  auto updateEntities = [&](size_t i) {
    ...
    {
      logerr("can't instantiate template {}??", templateDB.templates[i].getName());
...
void EntityManager::removeTemplateInternal(const uint32_t *to_remove, const uint32_t cnt)
{
  ...
  for (auto &p : templates.templateDbId)
  {
    ...
    NAU_FAST_ASSERT(it != (to_remove + cnt));
    if (p == *it)
      p = ~0u;
    else
      p -= it - to_remove;
...
EntityManager::RemoveTemplateResult EntityManager::removeTemplate(const char *name)
{
  ...
  eastl::vector<uint32_t, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> toRemove;
  for (size_t i = 0, e = templateDB.templates.size(); i < e; ++i)
  {
  ...
  eastl::function<void(const char *, EntityManager::UpdateTemplateResult)> cb)
{
  ...
  if (!trefs.isTopoSorted())
  {
    trefs.reportLoops([&](const char *t, const char *p) {
      NAU_UNUSED(p);
      logerr("template <{}> has loop at parent <{}>", t, p);
      cb(t, UpdateTemplateResult::InvalidParents);
      errors = true;
    });
  }
  ...
  trefs.finalize(tag);
  eastl::vector_set<uint32_t, eastl::less<uint32_t>, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> templatesToRemove;  // Note: this also handle dups so vector
                                                                                            // can't be used here
  ...
  eastl::bitvector</* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> noChange(trefs.size());
  ...
  eastl::vector_set<uint32_t, eastl::less<uint32_t>, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> templatesToUpdate;
  for (size_t tid = 0, tidE = trefs.size(); tid != tidE; ++tid)
  {
    ...
    for (int pi = 0, pe = updateTempl.parents.size(); pi != pe; ++pi)
    {
      ...
      if (pid < 0)
      {
        logerr("this was not supposed to happen. add template <{}> failed, due to broken ref to {}", updateTempl.getName(),
          parentName);
    ...
    if (existingId < 0)
    {
      ...
      {
        logerr("this was not supposed to happen. add template <{}> failed, due to {}", updateTempl.getName(), (int)r);
  ...
  eastl::vector_set<uint32_t, eastl::less<uint32_t>, /* framemem_allocator TODO Allocators.*/ EASTLAllocatorType> allTemplatesToUpdate = templatesToUpdate;
