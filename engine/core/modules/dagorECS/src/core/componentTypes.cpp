...
#define DECL_LIST_TYPE(lt, t) static_assert(ecs::ComponentTypeInfo<ecs::lt>::can_be_tracked);
ECS_DECL_LIST_TYPES
...
ECS_REGISTER_TYPE(nau::math::Point2, nullptr);
ECS_REGISTER_TYPE(nau::math::Point3, nullptr);
ECS_REGISTER_TYPE(nau::math::Vector4d, nullptr);//Point4
//ECS_REGISTER_TYPE(IPoint2, nullptr);
ECS_REGISTER_TYPE(nau::math::IVector2, nullptr);  // IPoint2
ECS_REGISTER_TYPE(nau::math::IVector3, nullptr);  // IPoint3
ECS_REGISTER_TYPE(nau::math::IVector4, nullptr);          // IPoint4
//ECS_REGISTER_TYPE(Vectormath::SSE::, nullptr);          // DPoint3
ECS_REGISTER_TYPE(nau::math::Matrix4, nullptr);           // TMatrix
ECS_REGISTER_TYPE(nau::math::Vector3, nullptr);
ECS_REGISTER_TYPE(nau::math::Vector4, nullptr);    //vec4f,
ECS_REGISTER_TYPE(nau::math::Quat, nullptr);
//ECS_REGISTER_TYPE(bbox3f, nullptr);   //bbox3f,
ECS_REGISTER_TYPE(nau::math::Matrix4, nullptr);  // mat44f,
ECS_REGISTER_TYPE(nau::math::Matrix4d, nullptr);
ECS_REGISTER_TYPE(nau::math::Transform, nullptr);
...
ECS_REGISTER_TYPE(nau::math::E3DCOLOR, nullptr);
...
public:
    ...
    {
        NAU_ASSERT(hint == ecs::ComponentTypeInfo<ecs::string>::type);
        NAU_UNUSED(hint);
        NAU_UNUSED(sz);
...
public:
    ...
    void serialize(ecs::SerializerCb& cb, const void* data, size_t sz, ecs::component_type_t hint) override
    {
        NAU_ASSERT(hint == ecs::ComponentTypeInfo<SerializedType>::type);
...
{
    NAU_ASSERT(is<ecs::string>() && !isNull(), "0x{} != 0x{}", +ComponentTypeInfo<ecs::string>::type, componentType);
    return is<ecs::string>() ? get<ecs::string>().c_str() : "";
}
// ecs20 compatibility
// todo: remove or optimize
const char* ecs::ChildComponent::getOr(const char* v) const
{
    return is<ecs::string>() ? get<ecs::string>().c_str() : v;
}
...
{
    NAU_ASSERT(!noCollisions && a.size() == size() && !a.noCollisions);
...
{
    NAU_ASSERT(hashContainer.size() == container.size());
    if(!size())
        return;
    auto hi = hashContainer.begin();
    for(auto i = begin(), e = end(); i != e; ++i, ++hi)
        NAU_ASSERT(*hi == ECS_HASH_SLOW(i->first.c_str()).hash);
    ...
    else
    {
        ...
        for(auto i = begin(), e = end() - 1; i != e; ++i, ++hi)
        {
            NAU_ASSERT(*hi <= *(hi + 1));
            if(*hi == *(hi + 1))
                NAU_ASSERT(strcmp(i->first.c_str(), (i + 1)->first.c_str()) < 0);
        }
    }
}
...
{
    NAU_ASSERT(!noCollisions);
    NAU_ASSERT_RETURN(str.str, end(), "find_as can't be called without string on Object with hash collisions");
