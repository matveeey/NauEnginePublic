...
struct QueryContainer
{
  ...
  {
    if (EASTL_UNLIKELY(entitiesCnt >= entitiesCntEnd))
  ...
  {
    if (EASTL_UNLIKELY(componentsData + c > componentsDataEnd))
...
__forceinline uint32_t EntityManager::fillQuery(const ArchetypesQuery &__restrict archDesc, QueryContainer &__restrict ctx)
{
  ...
  do
  {
    ...
    // NAU_CORE_DEBUG_LF("{}: ofs ro = {}", query.ro.start-1, archetypeOffsets[query.ro.start-1]);
  ...
  const stoppable_query_cb_t &__restrict fun, void *__restrict user_data)
{
  ...
  do
  {
    ...
    if (EASTL_UNLIKELY(fun(qv) == QueryCbResult::Stop))
...
inline void EntityManager::performQueryES(QueryId h, ESFuncType fun, const ESPayLoad &__restrict evt, void *user_data, int min_quant)
{
  ...
  {
    DEBUG_VERY_VERBOSE_QUERY("no fit <{}> out of {} archs. {} in q, {} total", queryDescs[index].getName(),
  ...
  RaiiOptionalCounter nested(!isConstrainedMTMode(), nestedQuery);
  if (EASTL_UNLIKELY(min_quant && maxNumJobs && querySize > min_quant * 2)) // doesn't make sense to run multithreading if there is
